"""Secret-safe diagnostics for the H2OS companion runtime."""
...
def run_doctor(home: Path) -> DoctorReport:
    """Inspect only the selected H2OS home and return bounded diagnostics."""
    ...
    dispatch_ok = Path(dispatch[0]).is_absolute() and "hermes" not in dispatch

    checks = (
        DoctorCheck("data-home", resolved.name == ".h2os" or resolved.exists(), str(resolved)),
        DoctorCheck(
            "runtime-isolated",
            runtime_ok,
