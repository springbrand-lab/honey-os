...
from honeyos.tools.environments.local import _find_shell, _resolve_safe_cwd, _sanitize_subprocess_env
...
CHECKPOINT_PATH = get_honeyos_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        try:
            from honeyos.tools.async_delegation import restore_undelivered_completions
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from honeyos.runtime.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        from honeyos.tools.terminal_tool import _rewrite_compound_background as _rewrite_bg
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/honeyos_bg_{session.id}.log"
        pid_path = f"{temp_dir}/honeyos_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/honeyos_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``honeyos.runtime.goals``) to support
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `honeyos update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the HoneyOS desktop app.",
            }
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'HoneyOS'}"
    elif _reason == "lost":
...
def _redact_process_result(result: dict) -> dict:
    ...
    if not isinstance(result, dict):
        return result
    from honeyos.agent.redact import redact_sensitive_text, redact_terminal_output
