...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or HoneyOS exit
...
# ---------------------------------------------------------------------------
from honeyos.tools.interrupt import is_interrupted, _interrupt_event  # noqa: F401 — re-exported
from honeyos.tools.registry import tool_error
# display_honeyos_home imported lazily at call site (stale-module safety during honeyos update)
...
from honeyos.tools.environments.singularity import _get_scratch_dir
from honeyos.tools.tool_backend_helpers import (
    coerce_modal_mode,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of honeyos directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "honeyos-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("HONEYOS_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("HONEYOS_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("HONEYOS_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (HONEYOS_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (HONEYOS_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive HoneyOS password prompt or the sudo -S password-pipe path.
...
from honeyos.tools.environments.local import LocalEnvironment as _LocalEnvironment
from honeyos.tools.environments.singularity import SingularityEnvironment as _SingularityEnvironment
from honeyos.tools.environments.ssh import SSHEnvironment as _SSHEnvironment
from honeyos.tools.environments.docker import DockerEnvironment as _DockerEnvironment
from honeyos.tools.environments.modal import ModalEnvironment as _ModalEnvironment
from honeyos.tools.environments.managed_modal import ManagedModalEnvironment as _ManagedModalEnvironment
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``honeyos-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by HoneyOS processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      HoneyOS processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling HoneyOS processes a generous grace
    ...
    try:
        from honeyos.tools.environments.docker import (
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, HoneyOSSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.honeyos/.env or environment variables."
        )
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the HoneyOS host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    HoneyOS host HOME (often ``/opt/data`` under Docker) and inject a
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``honeyos
    ...
    be stale from ``honeyos setup``). Environment values for omitted terminal
    ...
    try:
        from honeyos.runtime.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for honeyos-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior HoneyOS processes that hit SIGKILL / OOM / a closed terminal
        ...
        from honeyos.tools.environments.daytona import DaytonaEnvironment as _DaytonaEnvironment
        ...
        from honeyos.tools.environments.vercel_sandbox import (
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from honeyos.tools.file_tools import clear_file_ops_cache
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so HoneyOS tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/honeyos
        # restart|stop targeting honeyos-gateway) must never run inside the
        ...
        # never restart. This mirrors the `honeyos gateway restart` guard in
        # honeyos.runtime/gateway.py and the cron-path guard in honeyos.runtime/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_HONEYOS_GATEWAY") == "1":
            from honeyos.scheduler.lifecycle_guard import (
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use HoneyOS cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `honeyos gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # honeyos-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in honeyos-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `honeyos -z`, a "
                        ...
                        _gw_platform = _gse("HONEYOS_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("HONEYOS_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("HONEYOS_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("HONEYOS_SESSION_USER_ID", "")
                            _gw_user_name = _gse("HONEYOS_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("HONEYOS_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("HONEYOS_INTERACTIVE"):
                    output += (
            ...
            from honeyos.tools.tool_output_limits import get_max_bytes
            ...
            from honeyos.agent.redact import redact_terminal_output
            ...
            if returncode != 0 and not exit_note:
                try:
                    from honeyos.tools.terminal_hints import annotate_failure
            ...
            if spill_file_path:
                try:
                    ...
                    try:
                        ...
                        pass
            try:
                from honeyos.agent.verification_evidence import record_terminal_result
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from honeyos.tools.environments.docker import find_docker
            docker = find_docker()
            if not docker:
