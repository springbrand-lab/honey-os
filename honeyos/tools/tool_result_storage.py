...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/honeyos-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/honeyos-results/{tool_use_id}.txt on Termux)
...
from honeyos.tools.budget_config import (
...
STORAGE_DIR = "/tmp/honeyos-results"
HEREDOC_MARKER = "HONEYOS_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/honeyos-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"HONEYOS_PERSIST_{uuid.uuid4().hex[:8]}"
