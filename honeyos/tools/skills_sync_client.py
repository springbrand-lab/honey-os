...
409), and is driven by:
  ...
  * the ``honeyos sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``honeyos.runtime/``) so the
...
is toggled via ``honeyos sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.honeyos/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``honeyos sync enable`` — it is
...
# namespace from honeyos-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the HONEYOS_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: HONEYOS_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("HONEYOS_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a HoneyOS Cloud instance can be set
...
# precedence as base_url: the HONEYOS_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   HONEYOS_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   HONEYOS_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   HONEYOS_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``HONEYOS_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a HoneyOS Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("HONEYOS_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``HONEYOS_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``honeyos sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "HONEYOS_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``HONEYOS_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``honeyos sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    HoneyOS Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("HONEYOS_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.honeyos/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.honeyos/skills/, NOT bundled, NOT
    ...
    """
    try:
        from honeyos.tools.skill_usage import is_bundled, is_hub_installed, _find_skill_dir
        from honeyos.agent.skill_utils import is_external_skill_path
    except Exception:
        return False
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (HoneyOS Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``HONEYOS_SYNC_DEFAULT_OPT_IN``) so a user's skills
      follow them with no per-skill setup.

    Sorted, deduped.
    """
    try:
        from honeyos.tools.skill_usage import load_usage
    except Exception:
        return []
    usage = load_usage() or {}
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.honeyos/skills/ containing
    ...
    try:
        ...
        for skill_md in root.rglob("SKILL.md"):
            ...
            try:
                from honeyos.tools.skill_usage import _read_skill_name
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``honeyos sync device --name`` to set an explicit
    label."""
    ...
    # HoneyOS Cloud (and any templated deployment) can seed the label
    # declaratively via HONEYOS_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `honeyos sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("HONEYOS_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.honeyos/skills/.sync_device_id, overwriting
...
# ~/.honeyos/skills/.sync_state as JSON.
#
...
# path under ~/.honeyos/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.honeyos/skills/ (posix), or None."""
    try:
        from honeyos.tools.skill_usage import _find_skill_dir
    except Exception:
        return None
    skill_dir = _find_skill_dir(skill_name)
    if skill_dir is None:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this HoneyOS speaks "
            f"{WIRE_VERSION} — update HoneyOS to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "honeyos skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (honeyos sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.honeyos/skills/. Only paths the user has opted into (``sync: true``) are
    ...
    if remote_manifest:
        try:
            from honeyos.tools.skill_usage import set_sync, is_curation_eligible, is_sync_enabled
...
def maybe_push_skills(*, message: str = "honeyos skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (HONEYOS_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``honeyos sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
def list_org_skill_names() -> List[str]:
    ...
    names: List[str] = []
    try:
        from honeyos.agent.skill_utils import read_active_org_id
...
#
# Org skills live under a DISTINCT local namespace, ~/.honeyos/skills/_org/
...
#
# `honeyos sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.honeyos/skills/_org/<org_id>/``.
...
def _org_baseline_path(org_id: str) -> Path:
    ...
    from honeyos.agent.skill_utils import ORG_BASELINE_FILE
...
def _write_active_org_marker(org_id: str) -> None:
    ...
    try:
        from honeyos.agent.skill_utils import ORG_ACTIVE_MARKER
...
def _write_org_provenance(org_id: str, data: Dict[str, Any]) -> None:
    ...
    try:
        from honeyos.agent.skill_utils import ORG_PROVENANCE_FILE
