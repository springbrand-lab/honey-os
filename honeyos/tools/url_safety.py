...
For local proxies that synthesize public DNS answers from the benchmark range,
``security.allow_proxy_fake_ips: true`` provides a narrower compatibility mode:
only DNS hostnames resolving to ``198.18.0.0/15`` are accepted. Literal IP URLs,
other private ranges, and metadata endpoints remain blocked.

Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    can return a public IP for the check, then a private IP for the actual
    connection. HoneyOS-owned direct httpx request paths should use
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for HoneyOS-owned URL tools.
...
_PROXY_FAKE_IPV6_NETWORK = ipaddress.ip_network("::ffff:0:0:0/96")
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``HONEYOS_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("HONEYOS_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def _global_allow_proxy_fake_ips() -> bool:
    """Return whether the active profile permits proxy-synthesized Fake-IPs."""
    try:
        from honeyos.runtime.config import read_raw_config
        ...
        return isinstance(security, dict) and is_truthy_value(
            security.get("allow_proxy_fake_ips"), default=False
        )
    except Exception:
        return False


def _allows_proxy_fake_ip(
    hostname: str, ip: ipaddress.IPv4Address | ipaddress.IPv6Address
) -> bool:
    """Allow only a DNS hostname mapped by a proxy into 198.18.0.0/15."""
    if not _global_allow_proxy_fake_ips():
        return False
    try:
    ...
    if isinstance(ip, ipaddress.IPv4Address):
        return ip in _PROXY_FAKE_IP_NETWORK
    if ip in _PROXY_FAKE_IPV6_NETWORK:
        embedded = ipaddress.IPv4Address(int(ip) & 0xFFFFFFFF)
        return embedded in _PROXY_FAKE_IP_NETWORK
    return False
...
def is_safe_url(url: str) -> bool:
    ...
    ``HONEYOS_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
    ...
    try:
        ...
        for family, _, _, _, sockaddr in addr_info:
            ...
            if (
                ...
                and not _allows_proxy_fake_ip(hostname, ip)
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("honeyos_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("honeyos_ssrf_origin_schemes")
...
def _install_ssrf_guard_on_async_transport(transport: Any, schemes_by_origin_var: Any) -> None:
    ...
    if transport is None or state.get("_honeyos_ssrf_guarded", False):
        return
    ...
    transport._honeyos_ssrf_guarded = True
