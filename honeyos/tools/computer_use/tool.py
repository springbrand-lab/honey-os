...
`health_report` (surfaced by `honeyos computer-use doctor`) reports the
...
# Per-HoneyOS-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/honeyos-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map HoneyOS's explicit approval bypass onto Cua's immutable mode.

    HoneyOS has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "HONEYOS_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    from honeyos.tools.computer_use.cua_backend import CuaDriverBackend
                    ...
                    raise RuntimeError(
                        f"Unknown HONEYOS_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    HoneyOS YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated HoneyOS sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the HoneyOS process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `honeyos computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/honeyos-agent#67052).
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across HoneyOS sessions.
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/honeyos-agent#47072 — trycua/cua#1961
...
def _should_route_through_aux_vision() -> bool:
    ...
    """
    try:
        from honeyos.agent.auxiliary_client import _read_main_model, _read_main_provider
        from honeyos.runtime.config import load_config
        from honeyos.tools.computer_use.vision_routing import (
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$HONEYOS_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_honeyos_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `honeyos computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
    if sys.platform not in ("darwin", "win32", "linux"):
        return False
    from honeyos.tools.computer_use.cua_backend import cua_driver_binary_available
    return cua_driver_binary_available()


def get_computer_use_schema() -> Dict[str, Any]:
    from honeyos.tools.computer_use.schema import COMPUTER_USE_SCHEMA
