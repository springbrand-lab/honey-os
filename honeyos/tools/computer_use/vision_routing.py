...
def _lookup_user_declared_supports_vision(
    ...
    try:
        from honeyos.agent.image_routing import _supports_vision_override
...
def _lookup_supports_vision(
    ...
    if not provider or not model:
        return None
    try:
        from honeyos.agent.image_routing import _lookup_supports_vision as _lookup_image_supports
    except Exception:
...
def _provider_accepts_multimodal_tool_result(provider: str, model: str) -> Optional[bool]:
    ...
    if not provider:
        return None
    try:
        from honeyos.tools.vision_tools import _supports_media_in_tool_results
    except Exception as exc:  # pragma: no cover - defensive
        logger.debug(
