...
One ``CDPSupervisor`` runs per HoneyOS ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach HoneyOS logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "honeyos-dialog-bridge.invalid"
...
(() => {
  if (window.__honeyosDialogBridgeInstalled) return;
  window.__honeyosDialogBridgeInstalled = true;
  const ENDPOINT = "http://honeyos-dialog-bridge.invalid/";
