#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the HoneyOS Skills Hub.
...
Used by honeyos.runtime/skills_hub.py for CLI commands and the /skills slash command.
"""
...
from honeyos.tools.skills_guard import (
...
# so external `from honeyos.tools.skills_hub import SKILLS_DIR` callers still work.
...
def _honeyos_home() -> Path:
    return get_honeyos_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _honeyos_home() / "skills"
...
def _ssrf_safe_http_get(url: str, *, timeout: int = 20) -> httpx.Response:
    ...
    from honeyos.tools.url_safety import create_ssrf_safe_client
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> Optional[httpx.Response]:
    ...
    from honeyos.tools.url_safety import SSRFConnectionBlocked
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            honeyos_meta = metadata.get("honeyos", {})
            if isinstance(honeyos_meta, dict):
                tags = honeyos_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            honeyos_meta = metadata.get("honeyos", {})
            if isinstance(honeyos_meta, dict):
                raw_tags = honeyos_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.honeyos/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/honeyos-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                honeyos_meta = meta_block.get("honeyos", {})
                if isinstance(honeyos_meta, dict):
                    tags = honeyos_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``honeyos.runtime.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def install_from_quarantine(
    ...
    try:
        from honeyos.tools.skill_usage import record_installed
...
# ---------------------------------------------------------------------------
# HoneyOS centralized index source
# ---------------------------------------------------------------------------

HONEYOS_INDEX_URL = "https://honeyos-agent.nousresearch.com/docs/api/skills-index.json"
HONEYOS_INDEX_TTL = 6 * 3600  # 6 hours


def _honeyos_index_cache_file() -> Path:
    return _index_cache_dir() / "honeyos-index.json"


def _load_honeyos_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for HONEYOS_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    honeyos_index_cache_file = _honeyos_index_cache_file()
    if honeyos_index_cache_file.exists():
        try:
            age = time.time() - honeyos_index_cache_file.stat().st_mtime
            if age < HONEYOS_INDEX_TTL:
                return json.loads(honeyos_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("HoneyOS index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "HoneyOS index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("HoneyOS index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        honeyos_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        honeyos_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class HoneyOSIndexSource(SkillSource):
    """Skill source backed by the centralized HoneyOS Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_honeyos_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "honeyos-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "honeyos-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        HoneyOSIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "honeyos-index"
