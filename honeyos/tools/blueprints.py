...
`honeyos skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.honeyos.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.honeyos.blueprint`` is a mapping containing
    ...
    honeyos = meta.get("honeyos") if isinstance(meta, dict) else None
    blueprint = honeyos.get("blueprint") if isinstance(honeyos, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.honeyos.blueprint must be a mapping")
...
def create_blueprint_job(
    ...
    *,
    ...
    """
    from honeyos.scheduler.jobs import create_job
...
def register_blueprint_suggestion(spec: BlueprintSpec) -> Optional[Dict[str, Any]]:
    ...
    if not spec.skill_name:
        return None
    try:
        from honeyos.scheduler.suggestions import add_suggestion
    except Exception:  # pragma: no cover - import guard
        return None
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.honeyos.blueprint`` block) they can hand
    to ``honeyos skills publish`` to share. ``body`` is the plain-language
