...
(see ``honeyos.runtime`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<HONEYOS_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from honeyos.runtime.config import load_config, cfg_get
        cfg = load_config()
...
def _pending_dir(subsystem: str) -> Path:
    return get_honeyos_home() / "pending" / subsystem
...
def current_origin() -> str:
    ...
    """
    try:
        from honeyos.tools.skill_provenance import get_current_write_origin
