...
from honeyos.tools.managed_tool_gateway import (
...
_SIGN_IN_MESSAGE = (
    ...
    "Ask the user to run `honeyos model` and sign in to Nous, then retry."
)
...
async def _deliver_media(value, permitted: tuple, task_id: Optional[str]):
    ...
    from honeyos.tools.image_source import ImageResolutionError, ResolveContext, resolve_image_source
...
def _delivers_as_an_attachment() -> bool:
    ...
    """
    try:
        from honeyos.gateway.session_context import session_is_messaging_surface
...
def _default_directory():
    ...
    is not: an operator running HONEYOS_MEDIA_DELIVERY_STRICT=1 delivers only
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return get_honeyos_dir("cache/videos", "video_cache")
        except Exception:
