...
from honeyos.agent.file_safety import get_read_block_error
from honeyos.tools.binary_extensions import has_binary_extension
from honeyos.tools.file_operations import (
    ShellFileOperations,
    normalize_read_pagination,
    normalize_search_pagination,
)
from honeyos.tools import file_state
...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``honeyos.core.constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from honeyos.core.constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where honeyos previously hard-rejected an oversized read
...
def _uses_container_paths(task_id: str = "default") -> bool:
    try:
        from honeyos.tools.terminal_tool import _CONTAINER_BACKENDS
...
def _registered_task_cwd_override(task_id: str = "default") -> str | None:
    ...
    """
    try:
        from honeyos.tools.terminal_tool import resolve_task_overrides
...
def _resolve_base_dir(
    ...
    *,
    ...
    from honeyos.tools.environments.local import _msys_to_windows_path
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as HoneyOS.
    ...
    if env is None:
        return True
    try:
        from honeyos.tools.environments.local import LocalEnvironment
    except ImportError:
        return True
...
_honeyos_config_resolved: str | None = None
_honeyos_config_resolved_loaded = False


def _get_honeyos_config_resolved() -> str | None:
    """Return the resolved absolute path of the HoneyOS config file (cached)."""
    global _honeyos_config_resolved, _honeyos_config_resolved_loaded
    if _honeyos_config_resolved_loaded:
        return _honeyos_config_resolved
    _honeyos_config_resolved_loaded = True
    try:
        from honeyos.runtime.config import get_config_path
        _honeyos_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _honeyos_config_resolved = str(Path(_expand_tilde("~/.honeyos/config.yaml")).resolve())
        except Exception:
            _honeyos_config_resolved = None
    return _honeyos_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the HoneyOS config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    honeyos_config = _get_honeyos_config_resolved()
    if honeyos_config and (resolved == honeyos_config or normalized == honeyos_config):
        return (
            f"Refusing to write to HoneyOS config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.honeyos/config.yaml directly or use 'honeyos config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side HoneyOS mirror prefix for Docker file tools."""
    try:
        from honeyos.tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another HoneyOS
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of HoneyOS state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.honeyos/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      HoneyOS process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.honeyos/…`` path.

    Returns ``None`` when the write is in-scope or outside HoneyOS scope.
    ...
    """
    try:
        from honeyos.agent.file_safety import (
    ...
    # in a session that cd'd into ``~/.honeyos/profiles/other/`` is
    ...
    if cached is not None:
        with _env_lock:
            if task_id in _active_environments:
                ...
                if old_cwd:
                    try:
                        from honeyos.tools.terminal_tool import record_session_cwd
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        from honeyos.tools.read_extract import ExtractionError, extract_document_text, is_extractable_document
        ...
        # ── HoneyOS internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under HONEYOS_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == HONEYOS_HOME (e.g. "auth.json") still hits the
...
def _mark_verification_stale(
    ...
    if not paths:
        return
    try:
        from honeyos.agent.coding_context import project_facts_for
        from honeyos.agent.verification_evidence import mark_workspace_edited
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-HoneyOS-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-HoneyOS-profile guard for
...
def _check_file_reqs():
    ...
    from honeyos.tools import check_file_requirements
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another HoneyOS profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another HoneyOS profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with honeyos_tools.write_file() for very "
            "large files."
        )
