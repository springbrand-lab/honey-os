...
Usage:
    from honeyos.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from honeyos.agent.redact import redact_cdp_url
from honeyos.core.constants import (
    agent_browser_runnable,
    get_honeyos_home,
    get_honeyos_home_override,
)
from honeyos.utils import env_int, is_truthy_value
from honeyos.runtime.config import DEFAULT_CONFIG, cfg_get
...
def __getattr__(name: str):
    ...
    if name == "requests":
        ...
        return _requests
    if name == "call_llm":
        from honeyos.agent.auxiliary_client import call_llm as _call_llm
...
# means a compromised transitive dependency could read every HoneyOS secret
...
def _build_browser_env() -> dict:
    ...
    Strips HoneyOS-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``honeyos_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    """
    from honeyos.tools.environments.local import honeyos_subprocess_env

    env = honeyos_subprocess_env(inherit_credentials=False)
...
try:
    from honeyos.tools.website_policy import check_website_access
except Exception:
...
from honeyos.agent.browser_provider import BrowserProvider as CloudBrowserProvider  # noqa: F401  (legacy alias)
from honeyos.agent.browser_registry import (  # noqa: F401  (test-patchable surface)
    get_provider as _registry_get_browser_provider,
)
from honeyos.plugins.browser.browserbase.provider import (  # noqa: F401  (legacy import surface)
    BrowserbaseBrowserProvider as BrowserbaseProvider,
)
from honeyos.plugins.browser.browser_use.provider import (  # noqa: F401
    BrowserUseBrowserProvider as BrowserUseProvider,
)
from honeyos.plugins.browser.firecrawl.provider import (  # noqa: F401
    FirecrawlBrowserProvider as FirecrawlProvider,
)
from honeyos.tools.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from honeyos.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    honeyos_node_bin = str(honeyos_home / "node" / "bin")
    honeyos_node_root = str(honeyos_home / "node")
    honeyos_nm_bin = str(honeyos_home / "node_modules" / ".bin")
    return [honeyos_node_bin, honeyos_node_root, honeyos_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``honeyos doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from honeyos.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from honeyos.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
# plugins drop in under ``~/.honeyos/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.honeyos/plugins/browser/<vendor>/``) participate
...
from honeyos.core.constants import is_termux as _is_termux_environment
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when HoneyOS silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the HoneyOS CLI where sys.stdout and sys.stderr both report
...
def _allow_private_urls() -> bool:
    ...
    if get_honeyos_home_override() is not None:
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-honeyos_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed honeyos processes — this way every clean honeyos exit sweeps
    ...
    # Sweep orphans from other crashed honeyos processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live honeyos processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current honeyos PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live honeyos process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning honeyos process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced honeyos PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent honeyos instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-honeyos_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live honeyos process.
            continue
        ...
        try:
            from honeyos.tools.process_registry import ProcessRegistry
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, HoneyOS-managed
    ...
    # node_modules binary, which disappears on the next ``honeyos update`` and
    ...
    # Build an extended search PATH including HoneyOS-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_honeyos_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_honeyos_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_honeyos_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this honeyos PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_honeyos_dir("cache/web", "web_cache")
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    import urllib.parse
    from honeyos.agent.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_type
    ...
    from honeyos.agent.display import (
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_console
...
def _camofox_current_page_private_url(tab_id: str, user_id: str) -> Optional[str]:
    ...
    """
    try:
        from honeyos.tools.browser_camofox import _post
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from honeyos.tools.browser_camofox import _ensure_tab, _post
    try:
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = honeyos_home / "browser_recordings"
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise HoneyOS
    ...
    """
    if _is_camofox_mode():
        from honeyos.tools.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from honeyos.core.constants import get_honeyos_dir
    screenshots_dir = get_honeyos_dir("cache/screenshots", "browser_screenshots")
    ...
    try:
        ...
        from honeyos.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from honeyos.tools.browser_camofox import camofox_close, camofox_soft_cleanup
...
def _maybe_autoinstall_chromium() -> bool:
    ...
    from honeyos.tools.lazy_deps import _allow_lazy_installs
...
def check_browser_vision_requirements() -> bool:
    ...
    if not check_browser_requirements():
        return False
    try:
        from honeyos.tools.vision_tools import check_vision_requirements
    except ImportError:
        return False
...
if __name__ == "__main__":
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/honeyos-agent:latest")
                else:
    ...
    print("\n💡 Usage:")
    print("  from honeyos.tools.browser_tool import browser_navigate, browser_snapshot")
...
# Registry
# ---------------------------------------------------------------------------
from honeyos.tools.registry import registry, tool_error
