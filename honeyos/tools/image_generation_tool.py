...
selectable via ``honeyos tools`` → Image Generation; the active model is
...
def _load_fal_client() -> Any:
    ...
    if fal_client is not None:
        return fal_client
    from honeyos.tools.fal_common import import_fal_client
    fal_client = import_fal_client()
    return fal_client


from honeyos.tools.debug_helpers import DebugSession
from honeyos.tools.fal_common import (
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `honeyos tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _active_terminal_env(task_id: str | None):
    try:
        from honeyos.tools.terminal_tool import get_active_env
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.honeyos"
    ...
    # HoneyOS cache roots can be translated without side effects. SSH can still
...
def _agent_visible_cache_path(host_path: str, env: Any) -> str | None:
    ...
    try:
        from honeyos.tools.credential_files import map_cache_path_to_container
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `honeyos tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`honeyos setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `honeyos tools` "
        "→ Image Generation (run `honeyos plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `honeyos plugins list` to see "
    ...
    try:
        ...
        if reference_image_urls is not None:
            from honeyos.agent.image_gen_provider import normalize_reference_images
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `honeyos tools` → Image Generation."
                ),
...
def _maybe_route_managed_krea(
    ...
    try:
        from honeyos.plugins.image_gen.krea import _resolve_managed_krea_gateway
...
# model/provider via `honeyos tools` or `/skills`.
