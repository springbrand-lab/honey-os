...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.honeyos/.env``.
...
from honeyos.runtime.config import cfg_get, load_config, read_raw_config
from honeyos.tools.browser_camofox_state import get_camofox_identity
...
def _managed_persistence_enabled() -> bool:
    """Return whether HoneyOS-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so HoneyOS operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether HoneyOS should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for HoneyOS talking to the
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside HoneyOS. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the HoneyOS profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if identity_override:
            ...
            session = {
                "user_id": f"honeyos_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        from honeyos.agent.display import redact_browser_typed_text_for_display
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_honeyos_home() / "browser_screenshots"
