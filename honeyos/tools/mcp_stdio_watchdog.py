...
<url>``) is spawned as a direct child of the HoneyOS process. HoneyOS's own
...
HoneyOS process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next HoneyOS startup's opt-in
