...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.honeyos/config.yaml``. HoneyOS
...
Configuration is loaded from ~/.honeyos/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``honeyos.runtime.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from honeyos.runtime.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``honeyos auth add <provider_id>``).
    Resolved at call time so tests that reload the helpers module see the
    live function.
    """
    try:
        from honeyos.tools.tool_backend_helpers import resolve_provider_secret
...
from honeyos.tools.managed_tool_gateway import resolve_managed_tool_gateway
...
from honeyos.tools.xai_http import honeyos_xai_user_agent
...
def _import_elevenlabs():
    ...
    """
    try:
        from honeyos.tools.lazy_deps import FeatureUnavailable, ensure
...
def _import_mistral_client():
    ...
    """
    try:
        from honeyos.tools.lazy_deps import ensure
...
# Base URL now resolved via honeyos.runtime.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_honeyos_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.honeyos/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.honeyos/config.yaml.
    ...
    try:
        ...
        logger.debug("honeyos.runtime.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``honeyos tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into HoneyOS without any Python code
...
#           output_format: wav
#
# HoneyOS writes the input text to a temp UTF-8 file, runs the command with
...
def _dispatch_to_plugin_provider(
    ...
    if _is_command_provider_config(_get_named_provider_config(tts_config, key)):
        return None
    try:
        from honeyos.agent.tts_registry import get_provider
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__HONEYOS_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of HoneyOS secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of HoneyOS secrets (salvage of #56332) while still
    ...
    scrubbed = honeyos_subprocess_env(inherit_credentials=False)
...
def _sniff_audio_container(path: str) -> str:
    ...
    """
    from honeyos.tools.audio_container import sniff_container
...
# file, so retired models disappear from honeyos the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``honeyos.runtime.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `honeyos setup` to configure, "
    ...
    from honeyos.runtime.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from honeyos.tools.xai_http import resolve_xai_http_credentials

    creds = resolve_xai_http_credentials()
    api_key = str(creds.get("api_key") or "").strip()
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `honeyos model` or set XAI_API_KEY.")
    ...
    # output_format when HoneyOS actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": honeyos_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import honeyos.runtime as _honeyos_runtime

            _honeyos_version = str(_honeyos_runtime.__version__)
        except Exception:
            _honeyos_version = "0.0.0"
        # Include HoneyOS client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where HoneyOS caches Piper voice models.

    Resolves to ``~/.honeyos/cache/piper-voices/`` under the active
    HONEYOS_HOME so voice downloads follow profile boundaries.
    """
    from honeyos.core.constants import get_honeyos_dir
    root = Path(get_honeyos_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.honeyos/config.yaml (tts: section).
    ...
    # Detect platform from honeyos.gateway env var to choose the best output format.
    ...
    platform = get_session_env("HONEYOS_SESSION_PLATFORM", "").lower()
    ...
    if output_path:
        ...
        from honeyos.tools.path_security import has_traversal_component
        ...
        from honeyos.agent.file_safety import is_write_denied
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `honeyos setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run honeyos setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'honeyos setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'honeyos tools' and select Piper under TTS, or install manually: "
        ...
        while True:
            ...
            try:
                ...
                if (tmp_path and not self._stop.is_set()
                    ...
                    from honeyos.tools.voice_mode import play_audio_file
...
def stream_tts_to_speaker(
    ...
    try:
        ...
        from honeyos.tools.tts_streaming import SentenceChunker, resolve_streaming_provider
        ...
        def _playback_worker() -> None:
            ...
            if output_stream is not None:
                ...
                try:
                    from honeyos.tools.voice_mode import mark_audio_output_active
                except Exception:
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_honeyos_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
