...
it is automatically downloaded from GitHub releases to $HONEYOS_HOME/bin/tirith.
...
def _get_honeyos_home() -> str:
    """Return the HoneyOS home directory, respecting HONEYOS_HOME env var."""
    return str(get_honeyos_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_honeyos_home(), ".tirith-install-failed")
...
def _honeyos_bin_dir() -> str:
    """Return $HONEYOS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_honeyos_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HONEYOS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_honeyos_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HONEYOS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HONEYOS_HOME/bin/tirith
    ...
    honeyos_bin = os.path.join(_honeyos_bin_dir(), "tirith")
    if os.path.isfile(honeyos_bin) and os.access(honeyos_bin, os.X_OK):
        _resolved_path = honeyos_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return honeyos_bin
