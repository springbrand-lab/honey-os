...
Usage:
    from honeyos.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
# Cache validation (mirrors honeyos.runtime/profiles.py::_count_skills, d5eee133e):
...
def _skills_scan_signature(dirs_to_scan, disabled) -> tuple:
    ...
    """
    from honeyos.agent import skill_utils as _skill_utils
...
# All skills live in ~/.honeyos/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
HONEYOS_HOME = get_honeyos_home()
SKILLS_DIR = HONEYOS_HOME / "skills"
...
def _skills_dir() -> Path:
    ...
    set HONEYOS_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped HONEYOS_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from HONEYOS_HOME/.env."""
    env_path = get_honeyos_home() / ".env"
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from honeyos.agent.skill_utils import skill_matches_platform as _impl
...
def skill_matches_environment(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from honeyos.agent.skill_utils import skill_matches_environment as _impl
    return _impl(frontmatter)
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set HONEYOS_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (HONEYOS_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("HONEYOS_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("HONEYOS_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        from honeyos.gateway.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE

        return GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
    except Exception:
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_honeyos_home()}/.env manually."
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from honeyos.agent.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.honeyos/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    """Resolve the current platform from honeyos.gateway session context.

    Mirrors the platform-resolution logic in
    ``agent.skill_utils.get_disabled_skill_names`` so that
    ``_is_skill_disabled`` respects ``HONEYOS_SESSION_PLATFORM``.
    """
    try:
        from honeyos.gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``HONEYOS_PLATFORM`` environment variable
    3. ``HONEYOS_SESSION_PLATFORM`` from honeyos.gateway session context
    """
    try:
        from honeyos.runtime.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("HONEYOS_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.honeyos/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``honeyos skills`` config UI). Default False
    ...
    """
    from honeyos.agent.skill_utils import get_external_skills_dirs, iter_skill_index_files
    ...
    # (_skills_dir() resolves the LIVE profile HONEYOS_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from honeyos.runtime.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: honeyos plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    if preprocess:
        try:
            from honeyos.agent.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from honeyos.agent.skill_utils import is_valid_namespace, parse_qualified_name
        ...
        from honeyos.agent.skill_utils import iter_skill_index_files
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.honeyos/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `honeyos skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if file_path and skill_dir:
            from honeyos.tools.path_security import validate_within_dir, has_traversal_component
            ...
            try:
                from honeyos.tools.skill_manager_tool import mark_background_review_skill_read
        ...
        # Check metadata.honeyos.* first (agentskills.io convention), fall back to top-level
        honeyos_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            honeyos_meta = metadata.get("honeyos", {}) or {}

        tags = _parse_tags(honeyos_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            honeyos_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
        ...
        if available_env_names:
            try:
                from honeyos.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from honeyos.tools.credential_files import register_credential_files
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `honeyos sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from honeyos.tools.skill_usage import bump_use, bump_view
