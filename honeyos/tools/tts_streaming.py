...
The keystone of HoneyOS' conversational voice UX. `stream_tts_to_speaker`
...
from honeyos.tools.tool_backend_helpers import resolve_openai_audio_api_key
from honeyos.tools.tts_tool import _get_provider, _load_tts_config, get_env_value
...
def _resolve_key(env_var: str, provider_id: str) -> str:
    ...
    """
    try:
        from honeyos.tools.tts_tool import _resolve_provider_key
...
class ElevenLabsStreamer(StreamingTTSProvider):
    ...
    def stream(self, text: str) -> Iterator[bytes]:
        from honeyos.tools.tts_tool import (
...
class XAIStreamer(StreamingTTSProvider):
    ...
    async def _async_frames(self, text: str):
        ...
        from honeyos.tools.tts_tool import DEFAULT_XAI_VOICE_ID
        from honeyos.tools.xai_http import resolve_xai_http_credentials
