...
~/.honeyos/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from honeyos.utils import atomic_write_text, is_truthy_value
...
def mark_background_review_skill_read(path: Path) -> None:
    ...
    """
    try:
        from honeyos.tools.skill_provenance import is_background_review
...
try:
    from honeyos.tools.skills_guard import scan_skill, should_allow_install, format_scan_report
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `honeyos config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.honeyos/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch HONEYOS_HOME and
    ...
    live profile-scoped HONEYOS_HOME on every call.
    """
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from honeyos.agent.skill_utils import get_all_skills_dirs
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`honeyos curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `honeyos curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `honeyos curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.honeyos/skills/) first, then any
    external dirs configured via skills.external_dirs.  Returns
    {"path": Path} or None.
    """
    from honeyos.agent.skill_utils import get_all_skills_dirs, is_excluded_skill_path
...
def _maybe_auto_propose_org_edit(name: str, skill_path: Path) -> Optional[str]:
    ...
    """
    try:
        from honeyos.agent.skill_utils import is_org_mirror_path
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `honeyos sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `honeyos sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `honeyos sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `honeyos sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER HoneyOS profiles.
    ...
    # Default profile (~/.honeyos/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.honeyos/profiles/*/skills)
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    """
    from honeyos.agent.file_safety import _resolve_active_profile_name
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`honeyos -p "
            ...
            base += (
                ...
                f"Switch profiles (`honeyos -p <name>`) to edit there, or "
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from honeyos.tools.path_security import validate_within_dir
...
def _patch_skill(
    ...
    from honeyos.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from honeyos.tools.fuzzy_match import format_no_match_hint
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.honeyos/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `honeyos curator restore` promises the skill can be brought back. Route
    ...
    if curator_pass:
        try:
            from honeyos.tools.skill_usage import archive_skill
...
def _maybe_debounced_sync_push(skill_name: str) -> None:
    ...
    try:
        from honeyos.tools.skill_usage import is_sync_enabled
    ...
    def _fire():
        try:
            from honeyos.tools.skills_sync_client import maybe_push_skills
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from honeyos.tools.skill_usage import bump_patch, forget, record_created
            from honeyos.tools.skill_provenance import is_background_review
            if action == "create":
                record_created(
                    name,
                ...
                # keeps its usage record as STATE_ARCHIVED so `honeyos curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_honeyos_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `honeyos curator unpin <name>`. "
