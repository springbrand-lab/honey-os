...
from honeyos.utils import env_var_enabled, is_truthy_value
...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("HONEYOS_YOLO_MODE", ""))
...
# os.environ["HONEYOS_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export HONEYOS_INTERACTIVE.
_honeyos_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "honeyos_interactive",
    default=None,
)


def set_honeyos_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["HONEYOS_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``HONEYOS_INTERACTIVE`` env var for legacy callers.
    """
    return _honeyos_interactive_ctx.set("1" if interactive else "")


def reset_honeyos_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_honeyos_interactive_context`."""
    _honeyos_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``HONEYOS_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _honeyos_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("HONEYOS_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HONEYOS_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("HONEYOS_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("HONEYOS_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("HONEYOS_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("HONEYOS_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set HONEYOS_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HONEYOS_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HONEYOS_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("HONEYOS_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $HONEYOS_HOME, or by the resolved absolute
# active profile home path such as /home/honeyos/.honeyos/config.yaml. The
# resolved-absolute form is folded into the ~/.honeyos/ patterns at detection
...
# go stale when HONEYOS_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HONEYOS_ENV_PATH = (
    r'(?:~\/\.honeyos/|'
    r'(?:\$home|\$\{home\})/\.honeyos/|'
    r'(?:\$honeyos_home|\$\{honeyos_home\})/)'
    r'\.env\b'
)
# ~/.honeyos/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HONEYOS_ENV_PATH; matches the HONEYOS_HOME override form as
# well as ~/.honeyos/.
_HONEYOS_CONFIG_PATH = (
    r'(?:~\/\.honeyos/|'
    r'(?:\$home|\$\{home\})/\.honeyos/|'
    r'(?:\$honeyos_home|\$\{honeyos_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_HONEYOS_ENV_PATH}|'
    rf'{_HONEYOS_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_honeyos_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by HoneyOS: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # `honeyos` and `gateway` (e.g. `honeyos -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhoneyos\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart honeyos gateway (kills running agents)"),
    (r'\bhoneyos\s+update\b', "honeyos update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `honeyos gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `honeyos ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart honeyos-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart honeyos-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(honeyos|gateway|cli\.py)\b', "kill honeyos/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill honeyos` but not
    # `kill -9 $(pgrep -f honeyos)` because the substitution is opaque
    ...
    # the `honeyos gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.honeyos.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(honeyos|ai\.honeyos)\b', "stop/restart honeyos launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/HoneyOS file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.honeyos/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a HoneyOS-managed security file (~/.honeyos/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HONEYOS_CONFIG_PATH}|{_HONEYOS_ENV_PATH})', "in-place edit of HoneyOS config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HONEYOS_CONFIG_PATH}|{_HONEYOS_ENV_PATH})', "in-place edit of HoneyOS config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HONEYOS_CONFIG_PATH}|{_HONEYOS_ENV_PATH})', "in-place edit of HoneyOS config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    """
    from honeyos.tools.ansi_strip import strip_ansi
    ...
    # ~/ and ~/.honeyos/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / HONEYOS_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) HoneyOS home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\honeyos), so folding the user home
    # first would eat the prefix the HoneyOS-home fold needs.
    command = _rewrite_resolved_honeyos_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / HONEYOS_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.honeyos``); the matched
...
def _rewrite_resolved_honeyos_home(command: str) -> str:
    """Rewrite the resolved absolute HoneyOS home prefix to ``~/.honeyos/``.

    Resolves the active ``HONEYOS_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.honeyos/`` so the static ``_HONEYOS_CONFIG_PATH`` / ``_HONEYOS_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/honeyos/.honeyos/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$HONEYOS_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from honeyos.core.constants import get_honeyos_home
        home = get_honeyos_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.honeyos")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``honeyos send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one HoneyOS ad-hoc temp script."""
    try:
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from HoneyOS YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["HONEYOS_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "HONEYOS_SPINNER_PAUSE" in os.environ:
            del os.environ["HONEYOS_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses HoneyOS approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``HONEYOS_YOLO_MODE`` (frozen at import time
...
def _smart_approve(command: str, description: str) -> str:
    ...
    """
    try:
        from honeyos.agent.auxiliary_client import call_llm
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            HONEYOS_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if approval_callback is None:
        try:
            from honeyos.tools.terminal_tool import _get_approval_callback
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set HONEYOS_INTERACTIVE or "
                "HONEYOS_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set HONEYOS_INTERACTIVE or "
            "HONEYOS_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("HONEYOS_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``HONEYOS_INTERACTIVE``) fails CLOSED — a plugin-
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    try:
        from honeyos.tools.environments.base import touch_activity_if_due
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("HONEYOS_EXEC_ASK")
    ...
    if not is_cli and not is_gateway and not is_ask:
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    from honeyos.tools.tirith_security import check_command_security
                    ...
                    try:
                        from honeyos.runtime.config import load_config_readonly as _load_cfg
