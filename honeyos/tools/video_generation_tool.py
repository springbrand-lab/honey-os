...
provider** — turn on a backend by enabling a plugin (``honeyos plugins
enable video_gen/<name>``) and selecting it in ``honeyos tools`` → Video
Generation.
...
from honeyos.agent.video_gen_provider import (
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/honeyos-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `honeyos tools` "
...
def _resolve_active_provider():
    ...
    """
    try:
        from honeyos.agent.video_gen_registry import get_active_provider
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `honeyos tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `honeyos tools` → "
...
# `honeyos tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`honeyos tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `honeyos tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
    ...
    if provider.name == "xai":
        ...
        try:
            from honeyos.tools.xai_http import xai_storage_notice_text
