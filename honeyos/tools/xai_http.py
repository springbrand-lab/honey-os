...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``honeyos tools`` repaint, tool-registration scans,
    ...
    2. ``~/.honeyos/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``honeyos auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_honeyos_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.honeyos/.env`` first, then ``os.environ``.

    Wraps :func:`honeyos.runtime.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from honeyos.runtime.config import get_env_value as _honeyos_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _honeyos_get_env_value(name)
    return value if value is not None else default


def honeyos_xai_user_agent() -> str:
    """Return a stable HoneyOS-specific User-Agent for xAI HTTP calls."""
    try:
        from honeyos.runtime import __version__
    except Exception:
        __version__ = "unknown"
    return f"HoneyOS-Agent/{__version__}"


def honeyos_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as HoneyOS,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": honeyos_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level HoneyOS config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per HoneyOS home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_honeyos_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers HoneyOS-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``honeyos.runtime.config.get_env_value`` so keys
    stored in ``~/.honeyos/.env`` (the standard HoneyOS location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("HONEYOS_XAI_BASE_URL")
