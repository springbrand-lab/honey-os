...
Lets the LLM write a Python script that calls HoneyOS tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `honeyos_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `honeyos_tools.py` with file-based RPC stubs
...
# available on every platform HoneyOS itself runs on.
...
# match a safe prefix, the operational HONEYOS_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports HoneyOS
# code can lose the DB-layer Kanban mutation guard while still inheriting
# HONEYOS_HOME.
#
# NB: the broad "HONEYOS_" prefix was deliberately removed (#27303) — it leaked
# HONEYOS_*-named config that lacks a secret substring (e.g. HONEYOS_BASE_URL,
# HONEYOS_KANBAN_DB, HONEYOS_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HONEYOS_CHILD_ALLOWED below; HONEYOS_RPC_SOCKET /
# HONEYOS_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational HONEYOS_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set honeyos.runtime treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_HONEYOS_CHILD_ALLOWED = frozenset({
    "HONEYOS_HOME",
    "HONEYOS_PROFILE",
    "HONEYOS_CONFIG",
    "HONEYOS_ENV",
    "HONEYOS_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational HONEYOS_* vars (_HONEYOS_CHILD_ALLOWED) pass by exact name.
    ...
    if is_passthrough is None:
        ...
        try:
            from honeyos.tools.env_passthrough import resolve_passthrough_value
        except Exception:
            resolve_passthrough_value = lambda _name, _fallback: None  # noqa: E731
    ...
    scrubbed = {}
    # Non-secret HONEYOS_* vars dropped by the tightened allowlist (#27303). The
    # broad "HONEYOS_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like HONEYOS_KANBAN_DB / HONEYOS_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _HONEYOS_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("HONEYOS_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped HONEYOS_* var.
            _dropped_honeyos.append(k)
    if _dropped_honeyos:
        logger.debug(
            "execute_code: dropped %d non-allowlisted HONEYOS_* var(s) from the "
            ...
            len(_dropped_honeyos),
            ", ".join(sorted(_dropped_honeyos)),
        )
...
def check_sandbox_requirements() -> bool:
    ...
    try:
        from honeyos.tools.terminal_tool import (
...
# ---------------------------------------------------------------------------
# honeyos_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-HoneyOS-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-HoneyOS-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    honeyos_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'honeyos_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_honeyos_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the honeyos_tools.py stub module.
...
"""Auto-generated HoneyOS tools RPC stubs."""
...
def _connect():
    ...
    HONEYOS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["HONEYOS_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("HONEYOS_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated HoneyOS tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HONEYOS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "honeyos_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated honeyos_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/honeyos_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_honeyos_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/honeyos_tools.py", tools_src)
        ...
        env_prefix = (
            f"HONEYOS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"HONEYOS_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("HONEYOS_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    if _guard.get("user_approved"):
        from honeyos.tools.interrupt import clear_current_thread_interrupt
    ...
    from honeyos.tools.interrupt import is_interrupted as _is_interrupted
    ...
    # --- Set up temp directory with honeyos_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="honeyos_sandbox_")
    ...
    # selector from HONEYOS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"honeyos_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated honeyos_tools module.
        ...
        tools_src = generate_honeyos_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "honeyos_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  HONEYOS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["HONEYOS_RPC_SOCKET"] = rpc_endpoint
        child_env["HONEYOS_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the honeyos-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from honeyos_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _honeyos_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _honeyos_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # HONEYOS_TIMEZONE is an internal HoneyOS setting and must not leak
        # into child processes.
        _tz_name = os.getenv("HONEYOS_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("HONEYOS_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.honeyos/.env')).
...
def _load_config() -> dict:
    ...
    ``honeyos --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (honeyos-agent's python). Reproducible and the
...
def _is_usable_python(python_path: str) -> bool:
    ...
    """
    try:
        from honeyos.agent.delegation_context import delegated_child_subprocess_env
...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    if task_id:
        ...
        try:
            from honeyos.tools.terminal_tool import get_session_cwd
        ...
        try:
            from honeyos.tools.file_tools import _registered_task_cwd_override
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``honeyos tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and honeyos-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.honeyos/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls HoneyOS tools programmatically. "
        ...
        f"Available via `from honeyos_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from honeyos_tools import {import_str}` "
