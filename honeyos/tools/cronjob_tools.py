"""
Cron job management tools for HoneyOS.
...
# uses 30s) — comfortably below the 1800s default HONEYOS_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (HONEYOS_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with HONEYOS_CRON_TIMEOUT=0
...
# Import from honeyos.scheduler module (will be available when properly installed)
...
#      saying "cat ~/.honeyos/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `honeyos-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.honeyos/.env`, which tripped `read_secrets` and silently
...
from honeyos.tools.threat_patterns import INVISIBLE_CHARS as _CRON_INVISIBLE_CHARS
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("HONEYOS_SESSION_PLATFORM")
    origin_chat_id = get_session_env("HONEYOS_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("HONEYOS_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes HONEYOS_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``HONEYOS_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
    ...
    if (user_deliver or "").strip().lower() == "local":
        return None
    try:
        from honeyos.scheduler.scheduler import _resolve_delivery_targets
...
def _validate_cron_base_url(
    ...
    try:
        ...
        from honeyos.utils import base_url_host_matches, base_url_hostname
    except Exception:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within HONEYOS_HOME/scripts/.
    ...
    # Only relative paths within ~/.honeyos/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.honeyos/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.honeyos/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from honeyos.tools.path_security import validate_within_dir

    scripts_dir = get_honeyos_home() / "scripts"
...
def _execute_job_now(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    job_id = job["id"]
    try:
        from honeyos.scheduler.scheduler import run_one_job
        ...
        try:
            from honeyos.tools.environments.base import get_activity_callback
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            ...
            if context_from:
                from honeyos.scheduler.jobs import get_job as _get_job
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_honeyos_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("HONEYOS_INTERACTIVE")
        or env_var_enabled("HONEYOS_GATEWAY_SESSION")
        or env_var_enabled("HONEYOS_EXEC_ASK")
    )
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `honeyos cron create/edit --model`, or hand-edited jobs). The agent
