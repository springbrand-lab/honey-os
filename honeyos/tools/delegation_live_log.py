...
per child under::
    ...
    <honeyos_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.honeyos)."""
    from honeyos.core.constants import get_honeyos_dir

    return get_honeyos_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        try:
            ...
            header = [
                "=== HoneyOS subagent live transcript ===",
