...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``honeyos send`` (and the send_message
    ...
    ``honeyos send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
...
def _handle_list():
    ...
    try:
        from honeyos.gateway.channel_directory import format_directory_for_display
...
def _handle_react(args, remove=False):
    ...
    try:
        from honeyos.model_tools import _run_async
        if remove:
            result = _run_async(
...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.honeyos/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.honeyos/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from honeyos.gateway.config import HomeChannel
        ...
        if home:
            ...
            return tool_error(
                ...
                f"or set a home channel via: honeyos config set {home_env} <channel_id>"
            )
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("HONEYOS_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from honeyos.gateway.session_context import get_session_env
    platform = get_session_env("HONEYOS_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("HONEYOS_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("HONEYOS_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    from honeyos.gateway.platforms.base import BasePlatformAdapter, utf16_len

    # Telegram adapter import is optional (requires python-telegram-bot)
    try:
        from honeyos.plugins.platforms.telegram.adapter import TelegramAdapter
    ...
    if platform == Platform.FEISHU and media_files:
        from honeyos.gateway.platform_registry import platform_registry as _pr_feishu
        from honeyos.runtime.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
    ...
    if platform == Platform.SLACK and media_files:
        from honeyos.gateway.platform_registry import platform_registry as _pr_slack
        from honeyos.runtime.plugins import discover_plugins as _dp_slack
        _dp_slack()
    ...
    if platform == Platform.WHATSAPP and media_files:
        from honeyos.gateway.platform_registry import platform_registry as _pr_wa
        from honeyos.runtime.plugins import discover_plugins as _dp_wa
        _dp_wa()
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    try:
        ...
        try:
            from honeyos.gateway.platforms.base import resolve_proxy_url
        ...
        _tg_caption = None
        from honeyos.gateway.platforms.base import utf16_len as _utf16_len
            ...
            for chunk in text_chunks:
                try:
                    ...
                    if _is_telegram_thread_not_found(md_error) and text_kwargs.get("message_thread_id") is not None:
                        ...
                        if not _has_html:
                            try:
                                from honeyos.plugins.platforms.telegram.adapter import _strip_mdv2
            ...
            try:
                with open(media_path, "rb") as f:
                    ...
                    if (ext in _VOICE_EXTS and is_voice) or ext in _TELEGRAM_SEND_AUDIO_EXTS:
                        try:
                            from honeyos.plugins.platforms.telegram.adapter import _probe_voice_duration_seconds
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    from honeyos.gateway.platforms.signal_rate_limit import (
    ...
    from honeyos.gateway.platforms.signal_format import markdown_to_signal
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``honeyos send`` CLI).
    """
    media_files = media_files or []
    ...
    try:
        from honeyos.plugins.platforms.matrix.adapter import MatrixAdapter
    except ImportError:
...
async def _send_weixin(pconfig, chat_id, message, media_files=None):
    ...
    try:
        from honeyos.gateway.platforms.weixin import check_weixin_requirements, send_weixin_direct
...
async def _send_bluebubbles(extra, chat_id, message):
    ...
    try:
        from honeyos.gateway.platforms.bluebubbles import BlueBubblesAdapter, check_bluebubbles_requirements
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``HONEYOS_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``HONEYOS_HOME`` which has no ``gateway.pid``, so the gateway-running check
    ...
    if platform and platform != "local":
        return True
    try:
        from honeyos.gateway.status import is_gateway_running
...
async def _send_qqbot(pconfig, chat_id, message):
    ...
    from honeyos.gateway.config import _getenv
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    """
    try:
        from honeyos.gateway.platforms.yuanbao import get_active_adapter, send_yuanbao_direct
    except ImportError:
...
#   - the ``honeyos send`` CLI command (honeyos.runtime/send_cmd.py)
