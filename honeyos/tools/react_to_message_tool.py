#!/usr/bin/env python3
"""Let the agent react to a message with an emoji in the HoneyOS desktop app.
...
Gated on ``HONEYOS_DESKTOP`` (like the other GUI affordances) so it costs nothing
...
def react_to_message_tool(emoji: str, message_row_id=None, messages_back=None) -> str:
    ...
    session_key = get_session_env("HONEYOS_SESSION_KEY", "") or get_session_env(
        "HONEYOS_SESSION_ID", ""
    )
...
def check_react_requirements() -> bool:
    ...
    HONEYOS_DESKTOP is set on the gateway the app spawns; the feature itself is
    off by default and enabled from Settings → Appearance (the desktop mirrors
    the toggle into ``display.message_reactions``).
    """
    if not env_var_enabled("HONEYOS_DESKTOP"):
        return False
    try:
