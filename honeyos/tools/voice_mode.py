...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled HoneyOS venv at
    # ~/.honeyos/profiles/<name>/honeyos-agent/venv/), `pip install` on the
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "honeyos_voice")
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.honeyos/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``HoneyOSCLI._voice_start_recording``). A value <= 0
...
def transcribe_recording(wav_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from honeyos.tools.transcription_tools import MAX_FILE_SIZE, transcribe_audio
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent HoneyOS TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"honeyos-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("HONEYOS_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    HONEYOS_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
...
def check_voice_requirements() -> Dict[str, Any]:
    ...
    from honeyos.tools.transcription_tools import (
        _get_provider,
        _load_stt_config,
