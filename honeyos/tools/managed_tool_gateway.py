...
def auth_json_path():
    """Return the HoneyOS auth store path, respecting HONEYOS_HOME overrides."""
    return get_honeyos_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`honeyos tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a HoneyOS release, and
