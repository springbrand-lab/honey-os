...
LOCAL_STT_COMMAND_ENV = "HONEYOS_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "HONEYOS_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via honeyos.runtime.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``HONEYOS_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
#
# The single-env-var ``HONEYOS_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from HoneyOS's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__HONEYOS_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"honeyos-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set HONEYOS_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `honeyos plugins list` to see "
...
def _dispatch_to_plugin_provider(
    ...
    *,
    ...
    if stt_config is not None and _is_command_stt_provider_config(
        ...
        return None
    try:
        from honeyos.agent.transcription_registry import get_provider
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="honeyos-silk-")
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="honeyos-local-stt-") as output_dir:
            ...
            # Scrub HoneyOS secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from honeyos.tools.environments.local import honeyos_subprocess_env

            child_env = honeyos_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``HONEYOS_LOCAL_STT_LANGUAGE`` (env). When none is set, Groq
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="honeyos-stt-") as work_dir:
                try:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `honeyos model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``honeyos.runtime.models`` helpers so every DeepInfra surface resolves the
