...
async def _download_to_bytes(url: str) -> bytes:
    ...
    from honeyos.tools.vision_tools import _download_image
...
def _media_cache_roots() -> list:
    """Agent-managed media cache directories under HONEYOS_HOME (host side).
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Optional[Path]:
    ...
    - Non-local backend: permitted only if the path resolves inside a media
      cache root. A container-visible cache path (e.g. ``/root/.honeyos/cache/
    ...
    from honeyos.tools.credential_files import from_agent_visible_cache_path
...
def _finalize(
    ...
    """
    from honeyos.tools.vision_tools import _detect_image_mime_type_from_bytes
...
def _detect_video_mime(data: bytes, src: str) -> Optional[str]:
    ...
    from honeyos.tools.vision_tools import _detect_video_mime_type
