...
Backend is selected during ``honeyos tools`` setup (web.backend in config.yaml).
When available, HoneyOS can route Firecrawl calls through a Nous-hosted tool-gateway
...
from honeyos.plugins.web.firecrawl.provider import (
    Firecrawl,  # noqa: F401  # re-exported for tests that mock.patch("tools.web_tools.Firecrawl")
    _firecrawl_backend_help_suffix,
    _get_firecrawl_client,  # noqa: F401  # re-exported for tests that `from honeyos.tools.web_tools import _get_firecrawl_client`
...
from honeyos.plugins.web.tavily.provider import (  # noqa: F401 — backward-compat names
...
from honeyos.plugins.web.parallel.provider import (  # noqa: F401 — backward-compat names
    _get_async_parallel_client,
    _get_parallel_client,
)
from honeyos.plugins.web.exa.provider import _get_exa_client  # noqa: F401
...
from honeyos.tools.debug_helpers import DebugSession
...
from honeyos.tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    build_vendor_gateway_url,
    peek_nous_access_token as _peek_nous_access_token,
    read_nous_access_token as _read_nous_access_token,
    resolve_managed_tool_gateway,
)
from honeyos.tools.tool_backend_helpers import (  # noqa: F401
    managed_nous_tools_enabled,
    nous_tool_gateway_unavailable_message,
    prefers_gateway,
)
from honeyos.tools.url_safety import async_is_safe_url, normalize_url_for_request, sensitive_query_param_name
import sys
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via HoneyOS config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through HoneyOS' config/.env layer (``honeyos config set``, ``honeyos tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.honeyos/config.yaml."""
    try:
...
def _list_registered_web_providers():
    ...
    try:
        from honeyos.agent.web_search_registry import list_providers
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``honeyos tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `honeyos tools` repaint.
        try:
            from honeyos.tools.xai_http import has_xai_credentials
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        from honeyos.agent.web_search_registry import (
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `honeyos plugins enable {disabled_key}` "
                        "(or remove it from honeyos.plugins.disabled)."
                    ),
                }
            else:
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `honeyos tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`honeyos plugins enable {disabled_key}` "
                                    "(or remove it from honeyos.plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from honeyos.runtime.config import get_env_value as _gev
