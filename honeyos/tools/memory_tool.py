...
# (HONEYOS_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_honeyos_home() / "memories"
...
from honeyos.tools.threat_patterns import first_threat_message as _first_threat_message
...
class MemoryStore:
    ...
    def _sanitize_entries_for_snapshot(entries: List[str], filename: str) -> List[str]:
        ...
        """
        from honeyos.tools.threat_patterns import scan_for_threats
...
def _companion_write_origin() -> str:
    try:
        from honeyos.tools.write_approval import current_origin
...
def _apply_companion_write_policy(action: str) -> Optional[str]:
    """Return a structured error when companion policy rejects a mutation."""

    try:
        from honeyos.companion.memory_policy import check_companion_write

        decision = check_companion_write(
            origin=_companion_write_origin(),
            action=action,
        )
    except Exception:
        return tool_error(
            "Companion memory policy could not be evaluated; the write was refused.",
            success=False,
        )
    if decision.allowed:
        return None
    return tool_error(decision.reason, success=False)


def _audit_companion_write(
    *, target: str, action: str, content: str, result: Dict[str, Any]
) -> None:
    if not result.get("success"):
        return
    try:
        from honeyos.companion.memory_policy import MemoryAuditEvent, audit_memory_write
        ...
        logging.getLogger(__name__).exception("Failed to audit companion memory write")
...
def memory_tool(
    ...
    if operations:
        ...
        companion_result = _apply_companion_write_policy("batch")
    ...
    companion_result = _apply_companion_write_policy(action or "")
    if companion_result is not None:
        return companion_result
    ...
    audit_content = content if action in {"add", "replace"} else old_text
    _audit_companion_write(
        target=target,
        action=action,
        content=audit_content or "",
        result=result,
    )
