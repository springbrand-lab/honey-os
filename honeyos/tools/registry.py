"""Central registry for all honeyos-agent tools.
...
Import chain (circular-import safe):
    tools/registry.py  (no imports from honeyos.model_tools or tool files)
           ^
    tools/*.py  (import from honeyos.tools.registry at module level)
           ^
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (honeyos.core.constants itself is stdlib-only, so no cycle).
        from honeyos.core.constants import get_honeyos_home

        return Path(get_honeyos_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
def _save_discovery_cache(cache: Dict[str, list]) -> None:
    ...
    if path is None:
        return
    try:
        from honeyos.utils import atomic_json_write  # stdlib+yaml only; no cycle
...
# timescales. Cache results for ~30 s so env-var flips via ``honeyos tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a HoneyOS-home override for every
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``honeyos tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("honeyos_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``honeyos_plugins.{name}``), not the exact module string.
                # A handler defined in ``honeyos_plugins.pkg.handlers`` is
                # still owned by the ``honeyos_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("honeyos_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``honeyos tools enable foo``)
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str | dict:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                from honeyos.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from honeyos.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from honeyos.tools.registry import registry, tool_error, tool_result
#
