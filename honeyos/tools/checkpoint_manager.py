...
-----------------------------------------------------------------------------
    ...
    ~/.honeyos/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/honeyos/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_honeyos_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/honeyos"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (HoneyOS convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("HONEYOS_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal HoneyOS infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``honeyos checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `honeyos checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "honeyos@local"], store, cfg_wd)
    _run_git(["config", "user.name", "HoneyOS Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "HONEYOS_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``HONEYOS_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at HONEYOS_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "HONEYOS_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has HoneyOS changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        HoneyOS changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `honeyos checkpoints` CLI
# ---------------------------------------------------------------------------
