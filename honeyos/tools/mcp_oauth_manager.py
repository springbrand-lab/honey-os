...
`mcp_tool.py`, and `honeyos.runtime/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# HoneyOSMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_honeyos_provider_class() -> Optional[type]:
    ...
    class HoneyOSMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._honeyos_server_name = server_name
            self._honeyos_home = ""
            ...
            self._honeyos_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`HoneyOSTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, HoneyOSTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._honeyos_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from honeyos.tools.mcp_oauth import HoneyOSTokenStorage
                        if isinstance(storage, HoneyOSTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._honeyos_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from honeyos.tools.mcp_oauth import HoneyOSTokenStorage
            if not isinstance(storage, HoneyOSTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``honeyos mcp reauth``.
            ...
            back to ``honeyos mcp reauth``.
            """
            try:
                if self._honeyos_preregistered:
                    return
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._honeyos_server_name,
                    honeyos_home=self._honeyos_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return HoneyOSMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_HONEYOS_PROVIDER_CLS: Optional[type] = _make_honeyos_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._honeyos_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`HoneyOSMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _HONEYOS_PROVIDER_CLS is None:
            logger.warning(
        ...
        from honeyos.tools.mcp_oauth import (
            HoneyOSTokenStorage,
        ...
        from honeyos.tools.mcp_oauth import apply_oauth_provider_defaults
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `honeyos mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``honeyos mcp remove <name>`` and (indirectly) by
        ``honeyos mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, honeyos_home), None)

        from honeyos.tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, honeyos_home=honeyos_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, honeyos_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, honeyos_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        """
        from honeyos.tools.mcp_oauth import _get_token_dir, _safe_filename

        entry = self._entries.get(self._key(server_name, honeyos_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(honeyos_home) / f"{_safe_filename(server_name)}.json"
            try:
