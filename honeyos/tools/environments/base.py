"""Base class for all HoneyOS execution environment backends.
...
# HONEYOS_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("HONEYOS_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {HONEYOS_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_honeyos_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__HONEYOS_CWD_{session_id}__"
...
# the FIRST session's HONEYOS_SESSION_ID into the snapshot makes every LATER
...
# HONEYOS_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not HoneyOS' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is HONEYOS_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (HONEYOS_SESSION_|HONEYOS_UI_SESSION_ID|HONEYOS_CRON_AUTO_DELIVER_|HONEYOS_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``HONEYOS_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!HONEYOS_SESSION_*} ${!HONEYOS_CRON_AUTO_DELIVER_*} "
        f"HONEYOS_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all HoneyOS backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/honeyos-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/honeyos-cwd-{self._session_id}.txt"
    ...
    def _snapshot_excluded_passthrough_names(self) -> tuple[str, ...]:
        ...
        try:
            ...
            if is_multiplex_active():
                from honeyos.tools.env_passthrough import get_all_passthrough
                names = (
                    *get_all_passthrough(),
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__honeyos_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__honeyos_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__honeyos_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__honeyos_fns\" ] && declare -f $__honeyos_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_HONEYOS_RUNTIME_PASSTHROUGH_{name}"
        ...
        parts.append("__honeyos_ec=$?")
        # Restrict HoneyOS metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__honeyos_snap_tmp=$(mktemp {_snap_tmp_template}) && "
        ...
        parts.append("exit $__honeyos_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"HONEYOS_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_honeyos_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via HONEYOS_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __HONEYOS_CWD_{session}__ marker from stdout output.
    ...
    def execute(
        ...
        *,
        ...
        if rewrite_compound_background:
            from honeyos.tools.terminal_tool import _rewrite_compound_background
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from honeyos.tools.terminal_tool import _transform_sudo_command
