...
from honeyos.tools.environments.base import BaseEnvironment, _pipe_stdin
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory HoneyOS was already launched from (for example ``honeyos-agent``
    while the process cwd is ``~/.honeyos/honeyos-agent``), passing it through
    unchanged makes the wrapper run ``cd honeyos-agent`` *inside* the project
    ...
    # Common recovery for config values like ``honeyos-agent`` when HoneyOS was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./honeyos-agent``; use the current directory instead.
...
# HoneyOS-internal env vars that should NOT leak into terminal subprocesses.
_HONEYOS_PROVIDER_ENV_FORCE_PREFIX = "_HONEYOS_FORCE_"

# HoneyOS-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a HoneyOS inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from honeyos.runtime.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "HONEYOS_DASHBOARD_SESSION_TOKEN",
    ...
    # HoneyOS-managed inference credential — Claude subscription auth is not a
    # working HoneyOS provider path.  Stripping it broke agent-spawned
...
_HONEYOS_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the HoneyOS venv path instead of the project's own
# ``.venv`` — silently clobbering the HoneyOS environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# HoneyOS venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_honeyos_internal_secret(key: str) -> bool:
    """Return True for HoneyOS-internal secrets injected under *dynamic* names.

    ``_HONEYOS_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "HoneyOS-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`honeyos_subprocess_env` helper all call it, so the
...
def _inject_context_honeyos_home(env: dict) -> None:
    """Bridge the context-local HoneyOS home override into subprocess env."""
    try:
        from honeyos.core.constants import get_honeyos_home_override

        value = get_honeyos_home_override()
        if value:
            env["HONEYOS_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (HONEYOS_SESSION_*) are bridged onto the child env here.
    ...
    """
    try:
        from honeyos.gateway.session_context import (
            _UNSET,
            _VAR_MAP,
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter HoneyOS-managed secrets from a subprocess environment."""
    try:
        from honeyos.tools.env_passthrough import (
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_HONEYOS_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_honeyos_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _HONEYOS_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_HONEYOS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_HONEYOS_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_honeyos_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_honeyos_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _HONEYOS_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_honeyos_home(sanitized)

    from honeyos.core.constants import apply_subprocess_home_env
...
def _scrub_delegated_child_kanban_env(env: dict[str, str]) -> dict[str, str]:
    ...
    try:
        from honeyos.agent.delegation_context import (
...
# legitimate child HoneyOS spawns needs them, and they are the highest-value
...
# narrow subset of _HONEYOS_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in honeyos_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_honeyos_internal_secret, but _ID has no secret suffix, so it must be
...
def honeyos_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_HONEYOS_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child HoneyOS spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HONEYOS_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and HoneyOS-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_honeyos_internal_secret`.
    for key in list(env):
        if key.startswith(_HONEYOS_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_honeyos_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _HONEYOS_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_honeyos_home(env)
    from honeyos.core.constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose HONEYOS_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's HONEYOS_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`honeyos_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``HONEYOS_HOME`` / subprocess ``HOME`` contract) and the
    HoneyOS secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_honeyos_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``HONEYOS_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local HoneyOS home override into ``HONEYOS_HOME`` and apply the
      subprocess HOME contract (``honeyos.core.constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``HONEYOS_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs HONEYOS_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("HONEYOS_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale HONEYOS_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\honeyos\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\honeyos\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _honeyos_portable_git = os.path.join(_local_appdata, "honeyos", "git") if _local_appdata else ""
    if _honeyos_portable_git:
        for candidate in (
            os.path.join(_honeyos_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_honeyos_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # HONEYOS_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\honeyos\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "HONEYOS_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. HoneyOS requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set HONEYOS_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart HoneyOS. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``honeyos`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_HONEYOS_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_honeyos_bin_dir() -> str | None:
    """Return the directory holding the ``honeyos`` console-script, or None.
    ...
    honeyos install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``honeyos``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``honeyos`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``honeyos`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("honeyos")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``honeyos`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _HONEYOS_BIN_DIR
    if _HONEYOS_BIN_DIR is not _SENTINEL:
        return _HONEYOS_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("honeyos")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "honeyos" or base.startswith("honeyos."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "honeyos.exe" if _IS_WINDOWS else "honeyos"
    ...
    _HONEYOS_BIN_DIR = candidate
    return candidate


def _prepend_honeyos_bin_dir(existing_path: str) -> str:
    """Prepend the honeyos install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_honeyos_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing HoneyOS-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes HoneyOS installs for
    itself, so on a machine where HoneyOS provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$HONEYOS_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$HONEYOS_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_honeyos_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_honeyos_managed_node``, a first browser install).
    """
    try:
        from honeyos.core.constants import get_honeyos_home, iter_honeyos_node_dirs

        candidates = [*iter_honeyos_node_dirs(), get_honeyos_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    HoneyOS-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  HoneyOS
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_HONEYOS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_HONEYOS_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_honeyos_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_honeyos_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _HONEYOS_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the honeyos install dir is reachable so plugins can shell out
        # to bare ``honeyos`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_honeyos_bin_dir(new_path)

    _inject_context_honeyos_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, HoneyOS trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``HONEYOS_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under HONEYOS_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_honeyos_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "honeyos_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._honeyos_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_honeyos_pgid", None)
                    if pgid is None:
                        raise
