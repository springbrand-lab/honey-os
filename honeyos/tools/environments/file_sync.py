...
from honeyos.tools.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "HONEYOS_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.honeyos") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.honeyos to *container_base* because the remote
...
def _credential_host_paths() -> set[str]:
    ...
    try:
        from honeyos.tools.credential_files import get_credential_file_mounts
    except Exception:
        return set()
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``HONEYOS_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, honeyos_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.honeyos/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(honeyos_home=honeyos_home)

    def _sync_back_transaction(self, honeyos_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="honeyos-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.honeyos/skills/a.md`` →
        ``~/.honeyos/skills/a.md``, a new remote file at
        ``/root/.honeyos/skills/b.md`` maps to ``~/.honeyos/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
