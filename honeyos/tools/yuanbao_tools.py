...
提供以下工具函数，供 honeyos-agent 的 "honeyos-yuanbao" toolset 使用：
...
def _get_active_adapter():
    ...
    try:
        from honeyos.gateway.platforms.yuanbao import get_active_adapter
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    ...
    """
    from honeyos.gateway.platforms.yuanbao_sticker import search_stickers
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（HONEYOS_SESSION_CHAT_ID）。
    ...
    target = (chat_id or "").strip() or get_session_env("HONEYOS_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
from honeyos.tools.registry import registry, tool_result  # noqa: E402
...
def _check_yuanbao():
    ...
    try:
        from honeyos.gateway.session_context import get_session_env
        if get_session_env("HONEYOS_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
