...
transport, discovers their tools, and registers them into the honeyos-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.honeyos/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.honeyos/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach HoneyOS' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    HoneyOS explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so HoneyOS and
    its subprocesses can consume those credentials without duplicating them
    in every MCP server's ``env:`` block.
    """
    try:
        from honeyos.runtime.env_loader import get_secret_source
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            honeyos_home = os.path.expanduser(
                os.getenv(
                    "HONEYOS_HOME", os.path.join(os.path.expanduser("~"), ".honeyos")
                )
            )
            candidates = [
                os.path.join(honeyos_home, "node", "bin", resolved_command),
                ...
                # image (which the HoneyOS Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → HoneyOS MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that HoneyOS gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the HoneyOS document cache; returns a marker with
    ...
    try:
        from honeyos.gateway.platforms.base import cache_document_from_bytes
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`honeyos.runtime.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through HoneyOS' existing approval
    ...
    async def __call__(self, context, params):
        ...
        try:
            from honeyos.tools.approval import request_elicitation_consent
        ...
        # the agent's contextvars (HONEYOS_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit HONEYOS_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into HoneyOS' logging (agent.log via honeyos.core.logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `honeyos setup` to install MCP support, "
                "then retry."
            )
        ...
        from honeyos.tools.osv_check import check_package_for_malware
        try:
        ...
        # ungraceful exit of this HoneyOS process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through HoneyOS' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until HoneyOS is fully
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from honeyos.tools.mcp_oauth import OAuthNonInteractiveError
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `honeyos mcp login {server_name}` (or delete the tokens "
        f"file under ~/.honeyos/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent HoneyOS processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_honeyos_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local HONEYOS_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_honeyos_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_honeyos_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local HONEYOS_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_honeyos_home()
...
def _interpolate_env_vars(value):
    ...
    """
    from honeyos.agent.secret_scope import get_secret as _get_secret
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the HoneyOS config file.
    ...
    ``os.environ`` (which includes ``~/.honeyos/.env`` loaded at startup).
    """
    try:
        from honeyos.runtime.config import load_config
        from honeyos.utils import env_var_enabled as _env_enabled

        if _env_enabled("HONEYOS_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through HoneyOS' MEDIA: tag convention and out to messaging
            ...
            # HoneyOS' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        """
        from honeyos.tools.schema_sanitizer import strip_nullable_unions
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves HoneyOS's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. HoneyOS uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the HoneyOS registry schema format.
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    if registered_names:
        ...
        try:
            from honeyos.tools.mcp_schema_cache import config_fingerprint, write_cache_entry
...
def _register_from_cache_sync(name: str, config: dict, entry: dict) -> List[str]:
    ...
    """
    from honeyos.tools.registry import registry
    from honeyos.tools.mcp_schema_cache import (
        config_fingerprint,
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    lazy_server_count = 0
    try:
        from honeyos.tools.mcp_schema_cache import config_fingerprint, get_cached_entry
    ...
    from honeyos.tools.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``honeyos tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the HoneyOS registry.
...
def refresh_agent_mcp_tools(
    ...
    *after* that snapshot — a slow HTTP/OAuth server that misses the bounded
    ...
    """
    from honeyos.model_tools import get_tool_definitions
    from honeyos.tools.registry import registry
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    try:
        ...
        if callable(get_mem_schemas):
            ...
            from honeyos.agent.memory_manager import memory_provider_tools_enabled
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple honeyos
