...
# Must match honeyos.runtime.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
_RUNTIME_PROVIDER_CUSTOM = "custom"
from honeyos.tools import file_state
from honeyos.tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``honeyos-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``honeyos-cli`` because those also contain
...
def _build_child_agent(
    ...
    if parent_enabled is not None:
        ...
        from honeyos import model_tools
        parent_toolsets = {
            ts
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. honeyos-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (honeyos-cli,
    # honeyos-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child HoneyOS/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    from honeyos.agent.delegation_context import delegated_child_context
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.honeyos/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_honeyos_dir("cache/delegation", "delegation_cache")
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        try:
            from honeyos.tools.terminal_tool import get_session_cwd, record_session_cwd
        ...
        from honeyos.tools.daemon_pool import DaemonThreadPoolExecutor
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from honeyos.runtime.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    from honeyos.tools.delegation_live_log import (
    ...
    # HONEYOS_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-HoneyOS-Session-Id on
    ...
    if background:
        from honeyos.tools.async_delegation import dispatch_async_delegation_batch
        ...
        try:
            from honeyos.gateway.session_context import async_delivery_supported
        ...
        if not _async_ok:
            ...
            # with that id in X-HoneyOS-Session-Id once the batch completes.
            ...
            # HONEYOS_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `honeyos -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("HONEYOS_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("HONEYOS_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and HONEYOS_SESSION_KEY is not in the CLI
...
def _resolve_child_credential_pool(
    ...
    if effective_provider == "custom":
        try:
            from honeyos.agent.credential_pool import get_custom_provider_pool_key, load_pool
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'honeyos auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active HoneyOS config.

    Prefer the shared persistent loader because it follows the active
    HONEYOS_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``HONEYOS_IGNORE_USER_CONFIG=1`` (``honeyos chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("HONEYOS_IGNORE_USER_CONFIG") == "1"
