...
Copies bundled skills from the repo's skills/ directory into ~/.honeyos/skills/
...
The manifest lives at ~/.honeyos/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from honeyos.core.constants import get_bundled_skills_dir, get_honeyos_home, get_optional_skills_dir
from honeyos.agent.skill_utils import is_excluded_skill_path
from typing import Dict, List, Optional, Set, Tuple
from honeyos.utils import atomic_replace
...
# Marker file written by `honeyos profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.honeyos profile).
# When present in HONEYOS_HOME, sync_skills() is a no-op so neither the
# installer, `honeyos update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# honeyos.runtime.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks HONEYOS_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _build_external_skill_index() -> Set[str]:
    ...
    """
    try:
        from honeyos.agent.skill_utils import get_external_skills_dirs, _external_dirs_cache_clear
    except ImportError:
        return set()
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.honeyos/skills/.curator_suppressed`` directly if that import
    is unavailable in a packaged/update context.
    """
    try:
        from honeyos.tools.skill_usage import read_suppressed_names
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.honeyos/skills/mlops/axolotl
    """
...
def _content_hash(directory: Path) -> str:
    ...
    try:
        from honeyos.tools.skills_guard import content_hash
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``honeyos skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `honeyos skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.honeyos/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.honeyos) that wrote the
    ...
    if (HONEYOS_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.honeyos/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `honeyos update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `honeyos skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``HONEYOS_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.honeyos/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``HONEYOS_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `honeyos skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `honeyos update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``honeyos update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``honeyos update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``honeyos skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `honeyos skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes HONEYOS_HOME/.no-bundled-skills so the
    installer, ``honeyos update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``honeyos skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = HONEYOS_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `honeyos update` (or `honeyos skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (HONEYOS_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.honeyos/skills/ ...")
