...
Usage:
    from honeyos.tools.file_operations import ShellFileOperations
    from honeyos.tools.terminal_tool import _active_environments
...
from honeyos.tools.binary_extensions import BINARY_EXTENSIONS
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__HONEYOS_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__HONEYOS_FENCE_" in line or "\x1b]" in line
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from honeyos.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from honeyos.tools.tool_output_limits import get_max_line_length
    ...
    def _escape_shell_arg(self, arg: str) -> str:
        ...
        """
        from honeyos.tools.environments.local import _bash_safe_path
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.honeyos-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".honeyos-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.honeyos-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.honeyos-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str,
        ...
        # clean the temp up on any failure so we never leak a ``.honeyos-tmp``
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        if error or match_count == 0:
            ...
            from honeyos.tools.fuzzy_match import is_already_applied
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from honeyos.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from honeyos.agent.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        if not self._lsp_local_only():
            return False
        try:
            from honeyos.agent.lsp import get_service
        except Exception:  # noqa: BLE001
            return False
        try:
            svc = get_service()
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if pre_content is not None and post_content is not None and pre_content != post_content:
            try:
                from honeyos.agent.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from honeyos.agent.lsp.reporter import report_for_file, truncate
