...
# ``agent.auxiliary_client`` pulls credential_pool → honeyos.runtime.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("HONEYOS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: HONEYOS_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("HONEYOS_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
async def _validate_image_url_async(url: str) -> bool:
    ...
    if not _image_url_shape_ok(url):
        return False
    from honeyos.tools.url_safety import async_is_safe_url
...
def _normalize_to_supported_image(
    ...
    out_dir = get_honeyos_dir("cache/vision", "temp_vision_images")
...
async def _download_image(image_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    async def _ssrf_redirect_guard(response):
        ...
        """
        from honeyos.tools.url_safety import async_is_safe_url, redirect_target_from_response
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    try:
        ...
        try:
            from honeyos.tools.lazy_deps import ensure as _ensure_dep
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    """
    try:
        from honeyos.agent.auxiliary_client import _read_main_provider, _read_main_model
        from honeyos.agent.image_routing import decide_image_input_mode, _lookup_supports_vision
...
async def _vision_analyze_native(
    ...
    try:
        ...
        from honeyos.tools.image_source import (
        ...
        temp_dir = get_honeyos_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $HONEYOS_HOME/cache/vision/ and cleaned up
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_honeyos_dir("cache/video", "temp_video_files")
