...
from honeyos.tools.registry import (
...
from honeyos.toolsets import resolve_toolset, validate_toolset
...
def _is_dispatcher_owned_worker() -> bool:
    """False when HONEYOS_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_honeyos_home() under the active profile.
...
#   - cli.py, honeyos.runtime/*      -> inline on startup (no event loop)
...
def _companion_mode_enabled() -> bool:
    try:
        ...
        return (
            ...
            and str(section.get("mode", "assistant") or "assistant").strip().lower()
            == "companion"
        )
    except Exception:
        return False
...
_COMPANION_MEMORY_TOOL_DESCRIPTION = (
    "Save a durable fact only when the user explicitly stated, selected, confirmed, "
    "or asked you to remember it. Allowed facts include confirmed names, preferences, "
    "boundaries, relationship terms, real shared experiences, and long-term agreements. "
    "Never save inferred feelings or relationships, temporary moods, unconfirmed personality "
    "labels, casual details, or fabricated history. Use target='user' for facts about the "
    "user and target='memory' for confirmed facts about the companion or relationship. "
    "Use operations for an atomic batch; otherwise use add, replace, or remove."
)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("HONEYOS_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by HONEYOS_KANBAN_TASK and
        ...
        from honeyos.toolsets import get_all_toolsets
        for ts_name in get_all_toolsets():
            tools_to_include.update(resolve_toolset(ts_name))

    # Always apply disabled toolsets as a subtraction step at the end.
    # This ensures that even if a composite toolset (like honeyos-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                from honeyos.toolsets import bundle_non_core_tools, get_toolset
                if toolset_name.startswith("honeyos-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (honeyos-*) include _HONEYOS_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("honeyos-")
    ...
    # HONEYOS companions keep the same memory implementation but receive a
    # confirmation-only schema. Clone the schema so the registry's shared
    # assistant definition is never mutated.
    if _companion_mode_enabled():
            ...
            if function.get("name") == "memory":
                filtered_tools[index] = {
                    **definition,
                    "function": {
                        **function,
                        "description": _COMPANION_MEMORY_TOOL_DESCRIPTION,
                    },
                }
                break
    ...
    if "execute_code" in available_tool_names:
        from honeyos.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from honeyos.tools import discord_tool as _dt
    ...
    try:
        from honeyos.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    # window). Core HoneyOS tools (toolsets._HONEYOS_CORE_TOOLS) are NEVER
    ...
    try:
        from honeyos.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from honeyos.runtime.config import load_config as _load
        cfg = _load() or {}
...
def coerce_tool_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        from honeyos.tools.schema_sanitizer import unrename_tool_args
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to honeyos-agent's coercion layer.
...
def _tool_result_observer_fields(
    ...
    try:
        ...
        pass
    try:
        from honeyos.agent.display import _detect_tool_failure
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from honeyos.runtime.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    _ts_mod = None
    try:
        from honeyos.tools import tool_search as _ts_mod  # noqa: F401
    except Exception:
        _ts_mod = None
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from honeyos.runtime.plugins import resolve_pre_tool_block
        ...
        try:
            ...
            # ACP is an optional editor integration and is intentionally not
            # shipped in the standalone HoneyOS runtime.  Its absence means
            # there is no ACP approval requester for this session; normal
            # HoneyOS file tools must continue through their own boundary
            # checks.  Missing dependencies *inside* an installed adapter are
            # still treated as guard failures below.
            missing_name = _edit_approval_import_err.name or ""
            if not (
                missing_name == "acp_adapter"
                or missing_name.startswith("acp_adapter.")
            ):
                raise
        try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from honeyos.tools.file_tools import notify_other_tool_call
        ...
        try:
            ...
            if skip_tool_execution_middleware:
                ...
                from honeyos.runtime.middleware import run_tool_execution_middleware
