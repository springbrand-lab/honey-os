...
Usage:
    from honeyos.run_agent import AIAgent

    agent = AIAgent(base_url="http://localhost:30000/v1", model="claude-opus-4-20250514")
    response = agent.run_conversation("Tell me about the latest Python updates")
"""

# IMPORTANT: honeyos.core.bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See honeyos.core.bootstrap.py for full rationale.
try:
    import honeyos.core.bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when honeyos.core.bootstrap isn't registered in the venv
    # yet — happens during partial ``honeyos update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``honeyos -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("HONEYOS_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("HONEYOS_SESSION_SOURCE", "")
...
# `mock.patch("run_agent.<X>")`, `from honeyos.run_agent import <X>` in production
# siblings, or the `_ra().<X>` indirection in agent/system_prompt.py — none
# of which ruff's in-module usage scan can see.
from honeyos.agent.process_bootstrap import (
    OpenAI,  # noqa: F401  # re-exported for tests that mock.patch("run_agent.OpenAI")
    _SafeWriter,  # noqa: F401  # re-exported for tests that `from honeyos.run_agent import _SafeWriter`
    _get_proxy_for_base_url,
)
from honeyos.agent.iteration_budget import IterationBudget
from honeyos.agent.interrupt_compat import request_hard_interrupt


from honeyos.runtime.env_loader import load_honeyos_dotenv
from honeyos.runtime.timeouts import (
    get_provider_request_timeout,
    get_provider_stale_timeout,
)

_honeyos_home = get_honeyos_home()
_project_env = Path(__file__).parent / '.env'
_loaded_env_paths = load_honeyos_dotenv(honeyos_home=_honeyos_home, project_env=_project_env)
...
from honeyos.model_tools import (
...
from honeyos.tools.terminal_tool import cleanup_vm, get_active_env
from honeyos.tools.interrupt import set_interrupt as _set_interrupt
from honeyos.tools.browser_tool import cleanup_browser


# Agent internals extracted to agent/ package for modularity
from honeyos.agent.memory_manager import sanitize_context
from honeyos.agent.memory_provider import is_trivial_prompt
from honeyos.agent.error_classifier import FailoverReason
from honeyos.agent.redact import redact_sensitive_text
from honeyos.agent.message_content import flatten_message_text
from honeyos.agent.session_activity import ActivityProvenance
from honeyos.agent.model_metadata import (
    estimate_request_tokens_rough,  # noqa: F401  # re-exported for tests that mock.patch("run_agent.estimate_request_tokens_rough")
    is_local_endpoint,
)
from honeyos.agent.usage_pricing import normalize_usage
# Re-exported for tests that monkeypatch these symbols on run_agent.
from honeyos.agent.context_compressor import (  # noqa: F401
    COMPRESSED_SUMMARY_METADATA_KEY,
    ContextCompressor,
)
from honeyos.agent.retry_utils import jittered_backoff  # noqa: F401
from honeyos.agent.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("run_agent.<name>") / from honeyos.run_agent import <name>
...
from honeyos.agent.process_bootstrap import _get_proxy_from_env  # noqa: F401
from honeyos.agent.message_sanitization import (  # noqa: F401
    _SURROGATE_RE,
...
from honeyos.agent.codex_responses_adapter import (
...
from honeyos.agent.tool_guardrails import (
    ToolGuardrailDecision,
    append_toolguard_guidance,
    toolguard_synthetic_result,
)
from honeyos.agent.tool_result_classification import (
    FILE_MUTATING_TOOL_NAMES as _FILE_MUTATING_TOOLS,
    file_mutation_result_landed,
)
from honeyos.agent.trajectory import (
    convert_scratchpad_to_think,
    save_trajectory as _save_trajectory_to_file,
)
from honeyos.agent.tool_dispatch_helpers import (
    _should_parallelize_tool_batch,  # noqa: F401  # re-exported for tests that `from honeyos.run_agent import _should_parallelize_tool_batch`
    _is_destructive_command,  # noqa: F401  # re-exported for tests that access `run_agent._is_destructive_command`
    _extract_parallel_scope_path,  # noqa: F401  # re-exported for tests that `from honeyos.run_agent import _extract_parallel_scope_path`
    _paths_overlap,  # noqa: F401  # re-exported for tests that `from honeyos.run_agent import _paths_overlap`
    _is_multimodal_tool_result,
    _multimodal_text_summary,
    _append_subdir_hint_to_multimodal,  # noqa: F401  # re-exported for tests that `from honeyos.run_agent import _append_subdir_hint_to_multimodal`
    _extract_file_mutation_targets,
    _extract_landed_file_mutation_paths,
    _extract_error_preview,
    _trajectory_normalize_msg,  # noqa: F401  # re-exported for tests that `from honeyos.run_agent import _trajectory_normalize_msg`
)
from honeyos.utils import atomic_json_write, base_url_host_matches, base_url_hostname, env_float, is_truthy_value, model_forces_max_completion_tokens
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"HoneyOSAgent/{_HONEYOS_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-HoneyOS-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.honeyos/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[honeyos-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        from honeyos.agent.agent_init import init_agent
        init_agent(
            self,
            base_url=base_url,
            api_key=api_key,
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `honeyos --resume`. set_session_yolo() no-ops on a missing row, so
            # this is the only chance to record a pre-first-turn toggle.
            _init_model_config = self._session_init_model_config
            try:
                from honeyos.tools.approval import is_session_yolo_enabled
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from honeyos.runtime.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        from honeyos.agent.agent_runtime_helpers import switch_model
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``honeyos chat -q``. In that mode,
    ...
    def _warn_context_overflow_blocked(
        ...
        if getattr(self, "_last_ctx_overflow_warn", None) != _warn_key:
            ...
            from honeyos.agent.conversation_compression import (
    ...
    from honeyos.agent.stream_diag import STREAM_DIAG_HEADERS as _STREAM_DIAG_HEADERS  # noqa: E402

    @staticmethod
    def _stream_diag_init() -> Dict[str, Any]:
        """Forwarder — see ``agent.stream_diag.stream_diag_init``."""
        from honeyos.agent.stream_diag import stream_diag_init
    ...
    def _stream_diag_capture_response(
        ...
        from honeyos.agent.stream_diag import stream_diag_capture_response
    ...
    def _flatten_exception_chain(error: BaseException) -> str:
        ...
        from honeyos.agent.stream_diag import flatten_exception_chain
    ...
    def _log_stream_retry(
        ...
        *,
        ...
        from honeyos.agent.stream_diag import log_stream_retry
        log_stream_retry(
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        from honeyos.agent.stream_diag import emit_stream_drop
        emit_stream_drop(
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        from honeyos.agent.conversation_compression import check_compression_model_feasibility
    ...
    def _replay_compression_warning(self) -> None:
        ...
        from honeyos.agent.conversation_compression import replay_compression_warning
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``HONEYOS_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``HONEYOS_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("HONEYOS_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``HONEYOS_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("HONEYOS_API_CALL_STALE_TIMEOUT")
        ...
        # local NIM endpoint.
        from honeyos.agent.reasoning_timeouts import get_reasoning_stale_timeout_floor
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        from honeyos.agent.chat_completion_helpers import estimate_request_context_tokens
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        honeyos-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See honeyos-agent#21444 for symptom history."
        )
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        from honeyos.agent.agent_runtime_helpers import anthropic_prompt_cache_policy
    ...
    def _direct_native_anthropic_tool_cache_capability(
        ...
        *,
        ...
        from honeyos.agent.agent_runtime_helpers import _direct_native_anthropic_tool_cache_capability
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from honeyos.runtime.models import _should_use_copilot_responses_api
    ...
    def _strip_think_blocks(self, content: str) -> str:
        ...
        from honeyos.agent.agent_runtime_helpers import strip_think_blocks
    ...
    def _looks_like_codex_intermediate_ack(
        ...
        from honeyos.agent.agent_runtime_helpers import looks_like_codex_intermediate_ack
    ...
    def _extract_reasoning(self, assistant_message) -> Optional[str]:
        ...
        from honeyos.agent.agent_runtime_helpers import extract_reasoning
    ...
    def _cleanup_task_resources(self, task_id: str) -> None:
        ...
        from honeyos.agent.chat_completion_helpers import cleanup_task_resources
        return cleanup_task_resources(self, task_id)

    # ------------------------------------------------------------------
    # Background memory/skill review — prompts live in agent.background_review
    # ------------------------------------------------------------------
    from honeyos.agent.background_review import (
    ...
    def _summarize_background_review_actions(
        ...
        from honeyos.agent.background_review import summarize_background_review_actions
    ...
    def _spawn_background_review(
        ...
        """
        from honeyos.agent.background_review import spawn_background_review_thread
        from honeyos.tools.thread_context import propagate_context_to_thread
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        from honeyos.agent.background_review import build_memory_write_metadata
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        from honeyos.agent.agent_runtime_helpers import note_turn_persisted
    ...
    def _repair_message_sequence(self, messages: List[Dict]) -> int:
        ...
        from honeyos.agent.agent_runtime_helpers import repair_message_sequence
    ...
    def _format_tools_for_system_message(self) -> str:
        ...
        from honeyos.agent.system_prompt import format_tools_for_system_message
    ...
    def _convert_to_trajectory_format(self, messages: List[Dict[str, Any]], user_query: str, completed: bool) -> List[Dict[str, Any]]:
        ...
        from honeyos.agent.agent_runtime_helpers import convert_to_trajectory_format
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use HoneyOS against xai-oauth.  Lots of
    ...
    def _extract_api_error_context(error: Exception) -> Dict[str, Any]:
        ...
        from honeyos.agent.agent_runtime_helpers import extract_api_error_context
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("HONEYOS_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from honeyos.runtime import lifecycle as _lifecycle
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        from honeyos.agent.agent_runtime_helpers import dump_api_request_debug
    ...
    def _redact_message_content(content):
        ...
        Respects ``HONEYOS_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.honeyos/sessions/session_{sid}.json``
        ...
        # untrusted input (X-HoneyOS-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects HONEYOS_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than HoneyOS' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal HoneyOS model request this cancels only that request;
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.honeyos/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``HONEYOS_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("HONEYOS_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _apply_pending_steer_to_tool_results(self, messages: list, num_tool_msgs: int) -> None:
        ...
        from honeyos.agent.agent_runtime_helpers import apply_pending_steer_to_tool_results
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (HONEYOS_KANBAN_TASK set),
        ...
        """
        from honeyos.agent.session_activity import (
        ...
        if os.environ.get("HONEYOS_KANBAN_TASK"):
            try:
                from honeyos.tools.kanban_tools import (
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from honeyos.agent.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (HONEYOS_DEV_CREDITS_FIXTURE): inject a chosen notice state
        # each turn for repeatable testing, bypassing real headers. Throwaway scaffolding.
        try:
            from honeyos.agent.credits_tracker import dev_fixture_credits_state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset HONEYOS_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("HONEYOS_DEV_CREDITS"))

        # ── Parse (fail-open → miss; never overwrite good state with None) ──
        try:
            from honeyos.agent.credits_tracker import parse_credits_headers
        ...
        if _dev:
            # HONEYOS_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `honeyos logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _emit_credits_notices(self) -> None:
        ...
        if state is None:
            return
        try:
            from honeyos.agent.credits_tracker import evaluate_credits_notices, is_free_tier_model, new_credits_latch
    ...
    def close(self) -> None:
        ...
        try:
            from honeyos.tools.process_registry import process_registry
        ...
        # a private embedded daemon when HoneyOS YOLO selected unrestricted
        # mode.  The import is lazy so sessions without computer_use retain
        # the narrow core footprint.
        try:
            from honeyos.tools.computer_use import release_computer_use_session
        ...
        try:
            from honeyos.runtime.mem_trim import trim_memory
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        (GHSA-5g4g-6jrg-mw3g).
        """
        from honeyos.tools.todo_tool import MAX_TODO_RESULT_CHARS
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        from honeyos.agent.system_prompt import build_system_prompt
    ...
    def _sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        from honeyos.agent.agent_runtime_helpers import sanitize_api_messages
    ...
    def _drop_thinking_only_and_merge_users(
        ...
        *,
        ...
        from honeyos.agent.agent_runtime_helpers import drop_thinking_only_and_merge_users
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from honeyos.tools.delegate_tool import _get_max_concurrent_children
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        ...
        from honeyos.agent.agent_runtime_helpers import repair_tool_call
        return repair_tool_call(self, tool_name)

    def _invalidate_system_prompt(self):
        """Forwarder — see ``agent.system_prompt.invalidate_system_prompt``."""
        from honeyos.agent.system_prompt import invalidate_system_prompt
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``HONEYOS_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        from honeyos.agent.agent_runtime_helpers import create_openai_client
    ...
    def _force_close_tcp_sockets(client: Any) -> int:
        ...
        from honeyos.agent.agent_runtime_helpers import force_close_tcp_sockets
    ...
    def _cleanup_dead_connections(self) -> bool:
        ...
        from honeyos.agent.agent_runtime_helpers import cleanup_dead_connections
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from honeyos.runtime.copilot_auth import copilot_request_headers
    ...
    def _create_request_anthropic_client(self, *, reason: str) -> Any:
        ...
        if getattr(self, "provider", None) == "bedrock":
            from honeyos.agent.anthropic_adapter import build_anthropic_bedrock_client
            region = getattr(self, "_bedrock_region", "us-east-1") or "us-east-1"
            client = build_anthropic_bedrock_client(region)
        else:
            from honeyos.agent.anthropic_adapter import build_anthropic_client
    ...
    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        ...
        from honeyos.agent.codex_runtime import run_codex_stream
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        from honeyos.agent.codex_runtime import run_codex_create_stream_fallback
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``honeyos auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                from honeyos.runtime.auth import resolve_codex_runtime_credentials

                singleton_now = resolve_codex_runtime_credentials(
                    refresh_if_expiring=False,
                )
            else:
                from honeyos.runtime.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(
        ...
        *,
        ...
        try:
            from honeyos.runtime.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("HONEYOS_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.honeyos/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``honeyos setup``) updates
        ...
        if getattr(self, "_fallback_activated", False):
            return False
        try:
            from honeyos.agent.credential_pool import get_env_prefer_dotenv
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from honeyos.runtime.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from honeyos.runtime.auth import _resolve_zai_base_url
            ...
            try:
                from honeyos.runtime.runtime_provider import _get_named_custom_provider
            except ImportError:
                return False
    ...
    def _try_refresh_vertex_client_credentials(self) -> bool:
        ...
        try:
            from honeyos.agent.vertex_adapter import get_vertex_config
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from honeyos.runtime.copilot_auth import (
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from honeyos.agent.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
        ...
        from honeyos.agent.anthropic_adapter import _is_oauth_token
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        from honeyos.agent.auxiliary_client import (
            _AI_GATEWAY_HEADERS,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from honeyos.runtime.models import copilot_default_headers
            ...
            from honeyos.agent.auxiliary_client import _codex_cloudflare_headers
            self._client_kwargs["default_headers"] = _codex_cloudflare_headers(
                self._client_kwargs.get("api_key", "")
            )
        elif base_url_host_matches(base_url, "x.ai"):
            # Cover both provider=xai and provider=xai-oauth (api.x.ai).
            from honeyos.tools.xai_http import honeyos_xai_default_headers

            self._client_kwargs["default_headers"] = honeyos_xai_default_headers()
        else:
            # No URL-specific headers — check profile.default_headers before clearing.
            _ph_headers = None
            try:
                from honeyos.providers import get_provider_profile as _gpf2
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from honeyos.agent.anthropic_adapter import build_anthropic_client, _is_oauth_token
    ...
    def _recover_with_credential_pool(
        ...
        *,
        ...
        from honeyos.agent.agent_runtime_helpers import recover_with_credential_pool
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        from honeyos.agent.anthropic_adapter import create_anthropic_message
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        from honeyos.agent.chat_completion_helpers import interruptible_api_call
    ...
    def _interruptible_streaming_api_call(
        ...
        from honeyos.agent.chat_completion_helpers import interruptible_streaming_api_call
        return interruptible_streaming_api_call(self, api_kwargs, on_first_delta=on_first_delta)

    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        """Forwarder — see ``agent.chat_completion_helpers.try_activate_fallback``."""
        from honeyos.agent.chat_completion_helpers import try_activate_fallback
    ...
    def _restore_primary_runtime(self) -> bool:
        ...
        from honeyos.agent.agent_runtime_helpers import restore_primary_runtime
    ...
    def _try_recover_primary_transport(
        ...
        from honeyos.agent.agent_runtime_helpers import try_recover_primary_transport
    ...
    def _describe_image_for_anthropic_fallback(self, image_url: str, role: str) -> str:
        ...
        description = ""
        try:
            from honeyos.tools.vision_tools import vision_analyze_tool
    ...
    def _model_supports_vision(self) -> bool:
        ...
        try:
            ...
            from honeyos.agent.image_routing import _lookup_supports_vision
            cfg = load_config()
    ...
    def _get_transport(self, api_mode: str = None):
        ...
        t = cache.get(mode)
        if t is None:
            from honeyos.agent.transports import get_transport
    ...
    def _try_shrink_image_parts_in_messages(
        ...
        *,
        ...
        from honeyos.agent.conversation_compression import try_shrink_image_parts_in_messages
    ...
    def _build_api_kwargs(self, api_messages: list, tools_for_api: Optional[list] = None) -> dict:
        ...
        from honeyos.agent.chat_completion_helpers import build_api_kwargs
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from honeyos.runtime.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from honeyos.runtime.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from honeyos.runtime.models import ollama_model_supports_thinking
    ...
    def _resolve_lmstudio_summary_reasoning_effort(self) -> Optional[str]:
        ...
        """
        from honeyos.agent.lmstudio_reasoning import resolve_lmstudio_effort
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        from honeyos.agent.chat_completion_helpers import build_assistant_message
    ...
    def _needs_kimi_tool_reasoning(self) -> bool:
        ...
        """
        from honeyos.agent.message_sanitization import matches_reasoning_echo_family
    ...
    def _copy_reasoning_content_for_api(self, source_msg: dict, api_msg: dict) -> None:
        ...
        from honeyos.agent.agent_runtime_helpers import copy_reasoning_content_for_api
    ...
    def _reapply_reasoning_echo_for_provider(self, api_messages: list) -> int:
        ...
        from honeyos.agent.agent_runtime_helpers import reapply_reasoning_echo_for_provider
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        ...
        if not isinstance(tool_calls, list):
            return api_msg
        from honeyos.agent.transports.chat_completions import _model_consumes_thought_signature
    ...
    def _sanitize_tool_call_arguments(
        ...
        *,
        ...
        from honeyos.agent.agent_runtime_helpers import sanitize_tool_call_arguments
    ...
    def _compress_context(
        ...
        *,
        ...
        from honeyos.agent.portal_tags import (
        ...
        try:
            ...
            # id rotation updated honeyos.core.logging._session_context (a
            ...
            try:
                from honeyos.core.logging import set_session_context
            ...
            # HONEYOS_SESSION_ID to the child id after an out-of-place
            # rotation (idempotent when no rotation happened).
            try:
                from honeyos.gateway.session_context import set_current_session_id
                if self.session_id:
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        try:
            ...
            from honeyos.agent.tool_dispatch_helpers import _plan_tool_batch_segments
            ...
            from honeyos.agent.tool_executor import execute_tool_calls_segmented
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from honeyos.tools.delegate_tool import (
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        from honeyos.agent.agent_runtime_helpers import invoke_tool
        return invoke_tool(
            self,
            function_name,
            function_args,
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from honeyos.agent.tool_executor import execute_tool_calls_concurrent
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from honeyos.agent.tool_executor import execute_tool_calls_sequential
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        from honeyos.agent.chat_completion_helpers import handle_max_iterations
    ...
    def run_conversation(
        ...
        from honeyos.agent.aux_accounting import (
            reset_accounting_context,
            set_accounting_context,
        )
        from honeyos.agent import relay_runtime
        from honeyos.agent.conversation_loop import run_conversation
        from honeyos.agent.portal_tags import (
            reset_conversation_context,
            set_conversation_context,
        )
        from honeyos.runtime.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        from honeyos.agent.subagent_lifecycle import bind_subagent_parent
        ...
        try:
            ...
            from honeyos.agent.auxiliary_client import scoped_runtime_main
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        from honeyos.agent.codex_runtime import run_codex_app_server_turn
...
def main(
    ...
    # Handle tool listing
    if list_tools:
        from honeyos.model_tools import get_all_tool_names, get_available_toolsets
        from honeyos.toolsets import get_all_toolsets, get_toolset_info
