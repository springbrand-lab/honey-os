"""Short-lived, source-backed continuity across HONEYOS session resets.
...
the HoneyOS gateway only needs two best-effort calls at its existing seams.
"""
...
def _active_honeyos_home(chat_type: str) -> Path | None:
    """Resolve the active private-companion home, never generic HoneyOS state."""
...
def record_reset_handoff(
    *,
    ...
    """Best-effort gateway adapter for an explicit HONEYOS session reset."""

    home = _active_honeyos_home(chat_type)
    if home is None:
        return False
