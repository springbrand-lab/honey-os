...
Ledger path resolution (first wins):
  --ledger PATH
  $HONEYOS_CITATION_LEDGER
  $HONEYOS_HOME/cache/citations/ledger.json
"""
...
from _honeyos_home import get_honeyos_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("HONEYOS_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_honeyos_home() / "cache" / "citations" / "ledger.json"
