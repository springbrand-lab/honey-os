...
Storage
-------
Bundles live in ``~/.honeyos/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``HONEYOS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``honeyos bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under HONEYOS_HOME.

    Honors ``HONEYOS_BUNDLES_DIR`` for tests; falls back to
    ``<HONEYOS_HOME>/skill-bundles``.
    """
    override = os.environ.get("HONEYOS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_honeyos_home() / "skill-bundles"
...
def build_bundle_invocation_message(
    ...
    from honeyos.agent.skill_commands import _load_skill_payload, _build_skill_message
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `honeyos bundles` CLI subcommand.
# ---------------------------------------------------------------------------
