...
from honeyos.agent.lsp.workspace import nearest_root
...
def _spawn_pyright(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from honeyos.agent.lsp.install import try_install
...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<HONEYOS_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<HONEYOS_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        honeyos_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(honeyos_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName HoneyOS -HostProfileId honeyos -HostVersion 1.0.0 "
...
def honeyos_lsp_session_dir() -> str:
