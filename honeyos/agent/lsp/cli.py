...
``honeyos.runtime/main.py``) so the LSP module ships self-contained.
"""
...
def register_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Wire the ``honeyos lsp`` subcommand tree into the main argparse."""
...
def run_lsp_command(args: argparse.Namespace) -> int:
    """Top-level dispatcher for ``honeyos lsp <subcommand>``."""
...
def _cmd_status(emit_json: bool) -> int:
    ...
    from honeyos.agent.lsp.install import detect_status
...
def _cmd_install(server_id: str) -> int:
    from honeyos.agent.lsp.install import try_install, INSTALL_RECIPES, detect_status
...
def _cmd_restart() -> int:
    from honeyos.agent.lsp import shutdown_service
...
def _cmd_which(server_id: str) -> int:
    from honeyos.agent.lsp.install import INSTALL_RECIPES, _existing_binary
...
def _backend_warnings() -> list:
    ...
    from honeyos.agent.lsp.install import _existing_binary
    notes: list = []
