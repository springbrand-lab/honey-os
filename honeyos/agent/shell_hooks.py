...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`honeyos.runtime.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.honeyos/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``HONEYOS_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``honeyos.runtime/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # HoneyOS-canonical
...
from honeyos.runtime._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``honeyos.runtime.config.load_config``
    ...
    setting.  ``HONEYOS_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / HONEYOS_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("HONEYOS_SAFE_MODE"):
        logger.info("HONEYOS_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / HONEYOS_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``honeyos hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a HoneyOS wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical HoneyOS
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`honeyos.runtime.plugins.get_pre_tool_call_block_message`.  This is
    ...
    if event == "pre_verify":
        # "continue" (HoneyOS) / "block" (Claude-Code Stop: block the stop) both
...
def allowlist_path() -> Path:
    ...
    return get_honeyos_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / HONEYOS_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ HoneyOS is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``HONEYOS_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("HONEYOS_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `honeyos hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``honeyos hooks test`` and ``honeyos hooks doctor``.

    ``kwargs`` is the same dict that :func:`honeyos.runtime.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``honeyos hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical HoneyOS-wire-shape response."""
