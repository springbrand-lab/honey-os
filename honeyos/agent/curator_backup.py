...
A pre-run snapshot of ``~/.honeyos/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.honeyos/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.honeyos/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_honeyos_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.honeyos/cron/jobs.json``)."""
    return get_honeyos_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.honeyos/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.honeyos/skills/ directory — nothing to back up")
        return None
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    ...
    try:
        from honeyos.scheduler.jobs import load_jobs, save_jobs, _jobs_lock
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.honeyos/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.honeyos/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `honeyos curator rollback --list` to see available snapshots)",
            None,
        )
