...
MoA-enabled; the normal HoneyOS agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from honeyos.runtime.moa_config import coerce_privacy_filter
...
def _maybe_apply_moa_cache_control(
    ...
    *,
    ...
    try:
        ...
        from honeyos.agent.prompt_caching import apply_anthropic_cache_control
...
def _run_reference(
    ...
    *,
    ...
    """
    from honeyos.agent.usage_pricing import CanonicalUsage, estimate_usage_cost, normalize_usage
    ...
    try:
        ...
        from honeyos.agent.auxiliary_client import _normalize_aux_provider
...
def _run_references_parallel(
    ...
    *,
    ...
    """
    from honeyos.agent.usage_pricing import CanonicalUsage
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of HoneyOS boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model HoneyOS agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "HoneyOS agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal HoneyOS agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
class MoAChatCompletions:
    ...
    def consume_and_save_trace(
        ...
        if not pending or "aggregator_input_messages" not in pending:
            return
        try:
            from honeyos.agent.moa_trace import save_moa_turn
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        from honeyos.runtime.config import get_config_path, load_config
