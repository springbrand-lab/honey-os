...
from honeyos.core.constants import get_honeyos_home, get_skills_dir, is_wsl
from typing import Optional

from honeyos.agent.runtime_cwd import resolve_agent_cwd
from honeyos.agent.skill_utils import (
    EXCLUDED_SKILL_DIRS,
    ORG_ACTIVE_MARKER,
    ORG_MIRROR_DIR_NAME,
    ORG_PROVENANCE_FILE,
...
from honeyos.tools.threat_patterns import scan_for_threats as _scan_for_threats
...
_HONEYOS_MD_NAMES = (".honeyos.md", "HONEYOS.md")


def _find_honeyos_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.honeyos.md`` or ``HONEYOS.md``.
    ...
    # could pick up a .honeyos.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _HONEYOS_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are HoneyOS, a private AI companion. "
...
COMPANION_MEMORY_GUIDANCE = (
    ...
    "真实发生的重要共同经历和长期约定。不要根据语气推断用户的感情、关系或边界；"
...
COMPANION_STRUCTURED_MEMORY_GUIDANCE = (
    ...
    "短期状态不写入长期 memory。用户明确说出今天、本周、某天或某事件前后时，必须通过 "
    "expires_at 按该语义设置有效期；只有没有时间线索时才使用默认有效期。事项完成时 resolve；"
    "用户纠正时 update；用户要求忘记时 forget。"
...
HONEYOS_AGENT_HELP_GUIDANCE = (
    "You run on HoneyOS. When the user needs help with "
    "HoneyOS itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://honeyos-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `honeyos-agent` skill with skill_view(name='honeyos-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.honeyos/kanban.db`. Your task id is in "
    "`$HONEYOS_KANBAN_TASK`; your workspace is `$HONEYOS_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $HONEYOS_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $HONEYOS_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${HONEYOS_KANBAN_BRANCH:-wt/$HONEYOS_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$HONEYOS_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`honeyos profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `honeyos kanban <verb>` for board operations. Use "
...
#
# The honeyos-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to honeyos-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when HoneyOS has none; explicit HoneyOS YOLO uses a private unrestricted "
        ...
        "run `honeyos computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that HoneyOS "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the HoneyOS terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the HoneyOS desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the HoneyOS WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where HoneyOS itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host HoneyOS runs on.
    """
    ...
    try:
        ...
        from honeyos.tools.terminal_tool import _create_environment, _get_env_config  # type: ignore
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where HoneyOS itself is running. The host OS, home, and cwd "
                f"of the HoneyOS process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where HoneyOS "
    ...
    # Embedder-supplied environment description. Lets a host that wraps HoneyOS
    ...
    extra = (os.getenv("HONEYOS_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_honeyos_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("HONEYOS_PLATFORM") or os.environ.get("HONEYOS_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.honeyos/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        if companion_mode:
            ...
            guidance = (
                ...
                "or troubleshoot HoneyOS itself — its CLI, config, models, providers, tools, "
                "skills, voice, gateway, plugins, or any feature — load the `honeyos-agent` skill "
                "first. It has the actual commands (e.g. `honeyos config set …`, `honeyos tools`, "
                "`honeyos setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: honeyos setup, honeyos setup tools, honeyos setup terminal, honeyos status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from HONEYOS_HOME and return its content, or None.
    ...
    """
    try:
        from honeyos.runtime.config import ensure_honeyos_home
        ensure_honeyos_home()
    except Exception as e:
        logger.debug("Could not ensure HONEYOS_HOME before loading SOUL.md: %s", e)

    soul_path = get_honeyos_home() / "SOUL.md"
...
def _load_honeyos_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.honeyos.md / HONEYOS.md — walk to git root."""
    honeyos_md_path = _find_honeyos_md(cwd_path)
    if not honeyos_md_path:
        return ""
    try:
        content = honeyos_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = honeyos_md_path.name
        try:
            rel = str(honeyos_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".honeyos.md", context_length=context_length,
            read_path=str(honeyos_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", honeyos_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .honeyos.md / HONEYOS.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from HONEYOS_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the HoneyOS install/source
    ...
    # explicitly configured cwd is honored verbatim — the HoneyOS tree is a
    ...
    # their launch dir IS the user's shell cwd (developing HoneyOS in-tree).
    from honeyos.agent.runtime_cwd import _is_install_tree
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the HoneyOS install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_honeyos_md(cwd_path, context_length)
    ...
    # SOUL.md from HONEYOS_HOME only — skip when already loaded as identity
    if not skip_soul:
