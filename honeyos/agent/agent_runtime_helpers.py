...
from honeyos.agent.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from honeyos.agent.trajectory import convert_scratchpad_to_think
from honeyos.agent.credential_pool import STATUS_EXHAUSTED, credential_pool_matches_provider
from honeyos.agent.error_classifier import FailoverReason
from honeyos.agent.turn_context import drop_stale_api_content
from honeyos.utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
    ...
    *,
    ...
    if pool_provider and current_provider != pool_provider:
        ...
        if current_provider == "custom" and pool_provider.startswith("custom:"):
            try:
                from honeyos.agent.credential_pool import get_custom_provider_pool_key
    ...
    try:
        ...
        if pool is not None and pool.has_available():
            ...
            if entry is not None:
                ...
                from honeyos.agent.credential_pool import CUSTOM_POOL_PREFIX
                if (
                    primary_provider == "custom"
        ...
        from honeyos.agent.chat_completion_helpers import _reset_stale_streak
        _reset_stale_streak(agent)

        # Undo the fallback's identity rewrite so the prompt is
        # byte-identical to the stored copy again (prefix cache match).
        from honeyos.agent.chat_completion_helpers import rewrite_prompt_model_identity
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-HoneyOS-Session-Id header), and an
        ...
        if env_var_enabled("HONEYOS_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from honeyos.runtime.config import load_config as _load_moa_cfg
    ...
    from honeyos.agent.anthropic_adapter import _model_name_is_kimi_family
    is_kimi = (
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from honeyos.agent.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    ...
    # Delegate all rate-limit / 5xx retry to honeyos's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from honeyos.runtime.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        if _sm_custom_providers is None:
            try:
                from honeyos.runtime.config import get_compatible_custom_providers, load_config
    ...
    try:
        ...
        from honeyos.runtime.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if block_message is not None:
        ...
        try:
            from honeyos.model_tools import _emit_post_tool_call_hook
    ...
    if function_name == "todo":
        def _execute(next_args: dict) -> Any:
            from honeyos.tools.todo_tool import todo_tool as _todo_tool
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from honeyos.tools.session_search_tool import session_search as _session_search
            return _finish_agent_tool(
                _session_search(
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from honeyos.tools.memory_tool import memory_tool as _memory_tool
            result = _memory_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from honeyos.tools.clarify_tool import clarify_tool as _clarify_tool
            return _finish_agent_tool(
                _clarify_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from honeyos.tools.read_terminal_tool import read_terminal_tool as _read_terminal_tool
            return _finish_agent_tool(
                _read_terminal_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from honeyos.tools.read_preview_tool import read_preview_tool as _read_preview_tool
            return _finish_agent_tool(
                _read_preview_tool(
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: honeyos' own dispatch loop intentionally keeps an
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    """
    from honeyos.agent.message_sanitization import apply_reasoning_content_policy
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a strict provider that rejects the field (Mistral,
    ...
    removed.
    """
    from honeyos.agent.message_sanitization import reapply_reasoning_echo
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    HoneyOS' keepalive client (#10324 / ``_build_keepalive_http_client``) and
