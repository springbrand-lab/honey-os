...
``kind: backend``) or ``~/.honeyos/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`honeyos.runtime.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.honeyos/.env``) so credentials set
    through HoneyOS' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``honeyos tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``honeyos tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``honeyos tools`` picker.

        Used by ``honeyos.runtime/tools_config.py`` to inject this provider as a
