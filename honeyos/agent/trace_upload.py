"""Upload a HoneyOS session transcript to Hugging Face as an agent trace.

HoneyOS stores sessions in its own SQLite store (``honeyos.core.state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``honeyos trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through HoneyOS' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "honeyos-traces"
_HONEYOS_VERSION = "honeyos-agent"
...
# ---------------------------------------------------------------------------
# Conversion: HoneyOS OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses HoneyOS' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render HoneyOS conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.honeyos/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `honeyos trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/honeyos-traces`` dataset. Returns a
