...
upstream prefix cache warm.  See ``honeyos-agent-dev``'s
...
from honeyos.agent.prompt_builder import (
...
from honeyos.agent.runtime_cwd import resolve_context_cwd
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``HONEYOS_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("HONEYOS_DESKTOP_TERMINAL")):
        return hint
    return hint + _TUI_EMBEDDED_PANE_CLARIFIER


def _build_companion_system_prompt_parts(
    agent: Any,
    runtime_helpers: Any,
    ...
    for filename, heading in (
        ...
        try:
            content = (
                get_honeyos_home() / "memories" / filename
    ...
    return {
        "stable": "\n\n".join(
            part.strip() for part in stable_parts if part and part.strip()
        ),
        "context": "\n\n".join(
            part.strip() for part in context_parts if part and part.strip()
        ),
        "volatile": "\n\n".join(
            part.strip() for part in volatile_parts if part and part.strip()
        ),
    }
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  HoneyOS never re-renders parts of this string mid-
    ...
    if getattr(agent, "_agent_mode", "assistant") == "companion":
        return _build_companion_system_prompt_parts(
            agent,
            _r,
            _ctx_len,
            system_message,
    ...
    # Some execution modes (cron) still want HONEYOS_HOME persona while keeping
    ...
    # Pointer to the honeyos-agent skill + docs for user questions about HoneyOS itself.
    stable_parts.append(HONEYOS_AGENT_HELP_GUIDANCE)
    ...
    # HONEYOS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if "computer_use" in agent.valid_tool_names:
        from honeyos.agent.prompt_builder import computer_use_guidance
    ...
    if has_skills_tools:
        ...
        try:
            from honeyos.agent.coding_context import coding_compact_skill_categories
    ...
    # Coding posture (base HoneyOS, any interactive coding surface in a code
    ...
    if agent.valid_tool_names:
        try:
            from honeyos.agent.coding_context import coding_system_prompt_parts
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            from honeyos.tools.env_probe import get_environment_probe_line
    ...
    # Active-profile hint — names the HoneyOS profile the agent is running
    # under so it doesn't conflate ~/.honeyos/skills/ (default profile) with
    # ~/.honeyos/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active HoneyOS profile: default. Other profiles (if any) live "
            "under " + str(get_honeyos_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active HoneyOS profile: {active_profile}. This session reads "
            f"and writes {get_honeyos_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_honeyos_home()}/skills/, {get_honeyos_home()}/plugins/, "
            f"{get_honeyos_home()}/cron/, {get_honeyos_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing HoneyOS). Every other surface (desktop chat panel,
