...
from honeyos.agent.skill_preprocessing import (
...
#
# When a user invokes a /skill (or /bundle), HoneyOS expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_honeyos_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``HONEYOS_PLATFORM`` env var and
    ``HONEYOS_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("HONEYOS_PLATFORM")
            or get_session_env("HONEYOS_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("HONEYOS_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        from honeyos.tools.skills_tool import SKILLS_DIR, skill_view
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.honeyos.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_honeyos_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.honeyos/skills/ and return a mapping of /command -> skill info.
    ...
    _skill_commands = {}
    try:
        from honeyos.tools.skills_tool import SKILLS_DIR, _parse_frontmatter, skill_matches_platform, skill_matches_environment, _get_disabled_skill_names
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core HoneyOS slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core HoneyOS command; skipping "
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.honeyos/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``honeyos -s <skill>`` or
    a deployment's ``HONEYOS_TUI_SKILLS`` env var could force-load a skill an
