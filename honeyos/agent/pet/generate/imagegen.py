...
def _forced_provider_from_env() -> str | None:
    ...
    `HONEYOS_PET_IMAGE_PROVIDER=<name>` (e.g. `openrouter`) bypasses the normal
    active/default provider resolution for pet generation only. Unknown values are
    ignored so existing users are unaffected.
    """
    forced = os.environ.get("HONEYOS_PET_IMAGE_PROVIDER", "").strip().lower()
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    """
    _discover()
    from honeyos.agent.image_gen_registry import get_active_provider, get_provider
    ...
    raise GenerationError(
        ...
        "Open `honeyos tools` → Image Generation and configure Nous Portal, "
...
def _save_local(image_ref: str, *, prefix: str) -> Path:
    ...
    if image_ref.startswith(("http://", "https://")):
        from honeyos.agent.image_gen_provider import save_url_image
