...
Pets live under ``get_honeyos_home()/pets/<slug>/`` so every profile gets its
...
def install_pet(slug: str, *, force: bool = False, timeout: float = _DOWNLOAD_TIMEOUT) -> InstalledPet:
    ...
    failure.
    """
    from honeyos.agent.pet.manifest import find_entry
    ...
    if sheet_bytes is None and source_url and _is_petdex_host(source_url):
        try:
            ...
            resp = httpx.get(
                ...
                headers={"User-Agent": "honeyos-agent-petdex"},
            )
