...
row taxonomy is inferred from the atlas shape so HoneyOS can render both legacy
...
# rows (1536x1872), while older HoneyOS/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are HoneyOS' activity state names. They are not always identical to the
...
# Legacy HoneyOS/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical HoneyOS activity names -> accepted row-name aliases in descending
