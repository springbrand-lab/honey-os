...
an 8-column × 9-row atlas; older HoneyOS/petdex sheets used an 8-row atlas.
HoneyOS infers the row taxonomy from the sheet and maps agent activity onto
...
- :mod:`agent.pet.store`     — install / list / resolve pets on disk
                               (profile-aware via ``get_honeyos_home()``).
...
from honeyos.agent.pet.constants import (
    DEFAULT_SCALE,
    FRAME_H,
    FRAME_W,
    FRAMES_PER_STATE,
...
from honeyos.agent.pet.state import derive_pet_state
