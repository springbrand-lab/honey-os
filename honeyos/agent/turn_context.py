...
from honeyos.agent.memory_manager import build_memory_context_block
...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``honeyos logs``.
    ...
    try:
        from honeyos.agent.auxiliary_client import set_runtime_main
        set_runtime_main(
    ...
    from honeyos.agent.agent_runtime_helpers import note_turn_start
    ...
    if reaction_callback is not None:
        try:
            from honeyos.agent.reactions import detect_reaction
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # HoneyOS only initiates compaction in "honeyos" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping HoneyOS preflight compression for codex app-server "
                "(mode=%s); HoneyOS will not start thread compaction here.",
    ...
    try:
        ...
        try:
            from honeyos.tools.hook_output_spill import (
