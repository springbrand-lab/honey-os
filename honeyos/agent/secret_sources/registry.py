...
``honeyos.runtime.env_loader._apply_external_secret_sources()``.
...
from honeyos.agent.secret_sources.base import (
...
def _ensure_builtin_sources() -> None:
    ...
    try:
        from honeyos.agent.secret_sources.bitwarden import BitwardenSource
        ...
        logger.warning("Failed to register bundled Bitwarden secret source",
                       exc_info=True)
    try:
        from honeyos.agent.secret_sources.onepassword import OnePasswordSource
        ...
        logger.warning("Failed to register bundled 1Password secret source",
                       exc_info=True)
    try:
        from honeyos.agent.secret_sources.command import CommandSource
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``honeyos`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's HONEYOS_HOME is ``~/.honeyos/profiles/<name>``; the
    default profile (``~/.honeyos``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("HONEYOS_PROFILE_NAME", "HONEYOS_PROFILE"):
