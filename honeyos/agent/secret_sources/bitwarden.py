...
HoneyOS pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.honeyos/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<honeyos_home>/bin/bws`` on
  first use.  HoneyOS pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.honeyos/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``honeyos`` invocations don't
  hammer the API.
* Failures NEVER block HoneyOS startup.  Missing binary, no network,
...
from honeyos.agent.secret_sources._cache import (
    CachedFetch as _CachedFetch,
    DiskCache,
    FetchResult,
    is_valid_env_name as _is_valid_env_name,
)
from honeyos.agent.secret_sources.base import ErrorKind, SecretSource
from honeyos.agent.secret_sources.base import get_source_environment
...
# In-process cache so repeated load_honeyos_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `honeyos chat -q ...`
...
# <honeyos_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.honeyos/.env (which
...
_ENCRYPTED_CACHE_INFO = b"honeyos-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under honeyos_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$HONEYOS_HOME` / `~/.honeyos` when home is None.
    """
    return _DISK_CACHE.path(home_path)


def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the encrypted disk cache path under honeyos_home/cache/."""
    from honeyos.agent.secret_sources._cache import resolve_cache_home
...
def _honeyos_bin_dir() -> Path:
    """Where HoneyOS stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<honeyos_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``honeyos secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="honeyos-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a HoneyOS
...
# ---------------------------------------------------------------------------
# Public entry point — called from honeyos.runtime.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_honeyos_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `honeyos secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `honeyos secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `honeyos secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `honeyos secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`honeyos secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`honeyos secrets bitwarden token`) so the
