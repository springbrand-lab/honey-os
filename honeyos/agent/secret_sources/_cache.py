...
cache problem must never block HoneyOS startup.
"""
...
from honeyos.agent.secret_sources.base import (  # noqa: E402
    FetchResult,
    is_valid_env_name,
)
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the HoneyOS home used for cache paths.

    ``home_path`` is whatever ``load_honeyos_dotenv()`` already resolved;
    falling back to ``$HONEYOS_HOME`` / ``~/.honeyos`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from honeyos.core.constants import get_honeyos_home

        home_path = get_honeyos_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<honeyos_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if HONEYOS_HOME is ever made traversable.
            try:
