...
credentials at process startup, _after_ ~/.honeyos/.env has loaded.
...
Currently bundled:
  ...
  - ``bitwarden`` — Bitwarden Secrets Manager (`bws` CLI).  See
    ...
    ``honeyos.runtime.secrets_cli`` for the user-facing setup wizard.
  - ``onepassword`` — 1Password ``op://`` secret references (`op` CLI).
    See ``agent.secret_sources.onepassword`` for the integration and
    ``honeyos.runtime.onepassword_secrets_cli`` for the user-facing commands.
...
from honeyos.agent.secret_sources.base import (  # noqa: F401
