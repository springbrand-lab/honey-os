...
Owns one Codex thread per HoneyOS session. Drives `turn/start`, consumes
...
from honeyos.agent.codex_responses_adapter import _format_responses_error
from honeyos.agent.redact import redact_sensitive_text
from honeyos.agent.transports.codex_app_server import (
    CodexAppServerClient,
    CodexAppServerError,
)
from honeyos.agent.transports.codex_event_projector import CodexEventProjector
...
# HoneyOS' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_HONEYOS_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse HoneyOS/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per HoneyOS session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _HONEYOS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("HONEYOS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="honeyos",
            client_title="HoneyOS",
            client_version=_get_honeyos_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but HoneyOS' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into HoneyOS' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own honeyos-tools callback we
            # auto-accept — the user already approved HoneyOS' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO HoneyOS
        approval-mode/timeout logic. The HoneyOS-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; HoneyOS approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map HoneyOS approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    HoneyOS returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the HoneyOS approval mode/timeout resolution
    ...
    # "prompt expired"; the HoneyOS-side messaging already distinguishes them.
    return "decline"
...
def _get_honeyos_version() -> str:
    """Best-effort HoneyOS version string for codex's userAgent line."""
    try:
