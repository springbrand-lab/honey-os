...
projection into HoneyOS' display, approval bridging, transcript projection into
...
"codex_app_server"`. HoneyOS' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 HoneyOS secret — gateway
        ...
        spawn_env = honeyos_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("HONEYOS_KANBAN_TASK"):
            kanban_db = spawn_env.get("HONEYOS_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "HONEYOS_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("HONEYOS_HOME", os.path.expanduser("~/.honeyos")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "honeyos",
        client_title: str = "HoneyOS",
