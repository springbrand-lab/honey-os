...
from honeyos.agent.transports.base import ProviderTransport
from honeyos.agent.transports.types import NormalizedResponse, ToolCall
...
# Wire-name used when HoneyOS keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through HoneyOS's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "honeyos_web_search"
...
def _xai_prefers_native_web_search() -> bool:
    ...
    """
    try:
        from honeyos.agent.web_search_registry import get_active_search_provider

        provider = get_active_search_provider()
        if provider is not None:
            return getattr(provider, "name", None) == "xai"

        from honeyos.tools.web_tools import _get_search_backend
...
def _default_prompt_cache_retention_for_request(
    ...
    from honeyos.utils import base_url_hostname
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def _resolve_issuer_kind(self, params: Dict[str, Any]) -> str:
        ...
        from honeyos.agent.codex_responses_adapter import _classify_responses_issuer
    ...
    def convert_tools(self, tools: List[Dict[str, Any]]) -> Any:
        ...
        from honeyos.agent.codex_responses_adapter import _responses_tools
    ...
    def build_kwargs(
        ...
        **params,
        ...
        from honeyos.run_agent import DEFAULT_AGENT_IDENTITY
        ...
        if (params.get("provider") or "").strip().lower() == "actual":
            ...
            # line 1 column 1"). Clamp HoneyOS' wider set to the supported one.
            _effort_clamp.update({"xhigh": "high", "ultra": "max"})
        reasoning_effort = _effort_clamp.get(reasoning_effort, reasoning_effort)

        response_tools = _responses_tools(tools)

        # xAI server-side web search vs HoneyOS web providers.
        #
        ...
        #    keep HoneyOS dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``honeyos_web_search`` so Grok cannot hijack the name. The alias
        ...
        if reasoning_enabled and is_xai_responses:
            from honeyos.agent.model_metadata import grok_supports_reasoning_effort
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so HoneyOS dispatches
    ...
    def preflight_kwargs(
        ...
        *,
        ...
        """
        from honeyos.agent.codex_responses_adapter import _preflight_codex_api_kwargs
...
from honeyos.agent.transports import register_transport  # noqa: E402
