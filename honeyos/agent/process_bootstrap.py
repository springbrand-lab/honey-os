...
``from honeyos.run_agent import _get_proxy_from_env`` imports keep working
unchanged.
"""
...
from honeyos.utils import base_url_hostname, normalize_proxy_url
...
class _SafeWriter:
    ...
    When honeyos-agent runs as a systemd service, Docker container, or headless
...
def build_keepalive_http_client(
    ...
    *,
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` and ``HONEYOS_CA_BUNDLE`` settings the main
...
# ``from honeyos.agent.process_bootstrap import OpenAI`` (or re-exported via
