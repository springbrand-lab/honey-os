...
``auth.json``.  They intentionally have no dependency on ``honeyos.runtime.auth`` so
...
# Sources HoneyOS owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and HoneyOS-owned OAuth/device-code state)
