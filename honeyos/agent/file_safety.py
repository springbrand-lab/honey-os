...
def _honeyos_home_path() -> Path:
    """Resolve the active HONEYOS_HOME (profile-aware) without circular imports."""
    try:
        from honeyos.core.constants import get_honeyos_home  # local import to avoid cycles
        return get_honeyos_home()
    except Exception:
        return Path(os.path.expanduser("~/.honeyos"))


def _honeyos_root_path() -> Path:
    """Resolve the HoneyOS root dir (always the parent of any profile, never per-profile)."""
    try:
        from honeyos.core.constants import get_default_honeyos_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    honeyos_home = _honeyos_home_path()
    honeyos_root = _honeyos_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(honeyos_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(honeyos_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(honeyos_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(honeyos_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(honeyos_home / "cache" / "bws_cache.enc.json"),
            str(honeyos_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved HONEYOS_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("HONEYOS_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    honeyos_dirs = []
    for base in (_honeyos_home_path(), _honeyos_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in honeyos_dirs:
                honeyos_dirs.append(real)
        except Exception:
            continue

    for base_real in honeyos_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside HONEYOS_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied HoneyOS path.

    Three categories are blocked:

      * Internal HoneyOS cache files under ``HONEYOS_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HONEYOS_HOME and the global HoneyOS
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.honeyos/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active HONEYOS_HOME (profile-aware) AND the global
    # HoneyOS root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (HONEYOS_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    honeyos_dirs: list[Path] = []
    for base in (_honeyos_home_path(), _honeyos_root_path()):
        try:
            real = base.resolve()
            if real not in honeyos_dirs:
    ...
    for hd in honeyos_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a HoneyOS credential store "
    ...
    for hd in honeyos_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the HoneyOS MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a HoneyOS MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied HoneyOS read (see
...
#
# HoneyOS profiles are separate HONEYOS_HOME dirs under
...
#
# Reference: May 2026 incident where a honeyos-security profile session
# edited skills under both ``~/.honeyos/profiles/honeyos-security/skills/``
# AND ``~/.honeyos/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under HONEYOS_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from HONEYOS_HOME.

    ``~/.honeyos``              -> ``"default"``
    ``~/.honeyos/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _honeyos_home_path().resolve()
        root_real = _honeyos_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside HoneyOS scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    HoneyOS entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to HoneyOS profile {info['target_profile']!r}, but the "
...
#
#   <HONEYOS_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.honeyos/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.honeyos/…`` segment and warns
# regardless of which HoneyOS profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.honeyos/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.honeyos`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.honeyos/…`` and returns the
    index where the inner HoneyOS-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .honeyos / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".honeyos":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative HoneyOS state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.honeyos``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.honeyos`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any HoneyOS resolver to
    succeed, so it works correctly even when called from contexts where
    HONEYOS_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host HoneyOS process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real HONEYOS_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.honeyos/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.honeyos/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host HONEYOS_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative HoneyOS state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host HoneyOS "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real HONEYOS_HOME. Use the "
