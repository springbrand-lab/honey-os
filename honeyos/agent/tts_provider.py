...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into HoneyOS with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.honeyos/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of HoneyOS
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``honeyos tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``honeyos setup`` for
