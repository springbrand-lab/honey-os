"""Vertex AI (Google Cloud) adapter for HoneyOS.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows HoneyOS to use Gemini models via Google Cloud with
...
from honeyos.agent.secret_scope import get_secret as _get_secret, is_multiplex_active
...
# who installed plain `honeyos-agent` and only later selected a Gemini model.
try:
