...
from honeyos.agent.auxiliary_client import AuxiliaryExplicitCancellation
from honeyos.agent.context_engine import (
    automatic_compaction_status_message,
    sanitize_memory_context,
)
from honeyos.agent.model_metadata import estimate_request_tokens_rough
from honeyos.agent.session_activity import ActivityProvenance, normalize_activity_provenance
...
def _cached_prompt_reflects_builtin_memory(agent: Any, cached_prompt: str) -> bool:
    ...
    if snapshot is None:
        return False
    try:
        from honeyos.tools.memory_tool import MEMORY_BLOCK_HEADERS
    except Exception:
        return False
...
# Mirror honeyos.runtime.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``honeyos.core.state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["HONEYOS_SESSION_ID"] = child_session_id
    try:
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `honeyos setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by HoneyOS "
        ...
        if aux_context < threshold:
            ...
            from honeyos.agent.context_compressor import ContextCompressor as _CC
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"HoneyOS's small-context floor and output reservation "
...
def _strip_stale_todo_snapshot(content: Any) -> Any:
    ...
    """
    from honeyos.tools.todo_tool import TODO_INJECTION_HEADER
...
def _ensure_compressed_has_user_turn(original_messages: list, compressed: list) -> None:
    ...
    if any(_is_real_user_message(message) for message in compressed):
        return
    from honeyos.agent.context_compressor import (
...
def compress_context(
    ...
    *,
    ...
    # HoneyOS' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|honeyos|off).
    # The memory-provider context handoff below is intentionally HoneyOS-only:
    ...
    try:
        ...
        if todo_snapshot:
            ...
            from honeyos.agent.context_compressor import _append_text_to_content
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the HONEYOS_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("HONEYOS_SESSION_SOURCE", "cli"),
                        model=agent.model,
                    ...
                    try:
                        from honeyos.runtime.goals import migrate_goal_to_session
                        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
                    except Exception as _goal_err:
                        logger.debug("Could not migrate goal on compression: %s", _goal_err)
                    # Same boundary hazard for /heartbeat state — carry it too.
                    try:
                        from honeyos.runtime.heartbeat import migrate_heartbeat_to_session
        ...
        # engines (e.g. honeyos-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See honeyos-lcm#68. Built-in ContextCompressor
        ...
        try:
            from honeyos.tools.file_tools import reset_file_dedup
        ...
        try:
            from honeyos.tools.skills_tool import reset_skill_view_dedup
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    HoneyOS' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep HoneyOS' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "honeyos", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "honeyos":
        logger.info(
    ...
    try:
        from honeyos.agent.codex_runtime import (
...
def try_shrink_image_parts_in_messages(
    ...
    *,
    ...
    try:
        from honeyos.tools.vision_tools import _resize_image_for_vision
    except Exception as exc:
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="honeyos_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
