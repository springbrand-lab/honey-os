...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into HoneyOS accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    HoneyOS' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, HoneyOS should still count that turn
    ...
    from honeyos.agent.usage_pricing import CanonicalUsage, estimate_usage_cost
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in HoneyOS state.

    The app-server owns the compacted thread context, so HoneyOS should not
    ...
    if not force:
        try:
            from honeyos.agent.conversation_compression import COMPACTION_STATUS
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior HoneyOS deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → HoneyOS UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal HoneyOS tool loop. Without this bridge gateway
...
# Codex item types that map to a HoneyOS tool_call in the projector (and
...
# Internal MCP server that wraps HoneyOS' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# honeyos-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.honeyos-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as HoneyOS tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "honeyos-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic HoneyOS tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (HoneyOS' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into HoneyOS' gateway UI callbacks.
    ...
    def _stable_call_id(item: dict, name: str) -> str:
        ...
        from honeyos.agent.transports.codex_event_projector import _deterministic_call_id
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into HoneyOS'
    messages list so memory/skill review keep working.

    Called from run_conversation() when agent.api_mode == "codex_app_server".
    Returns the same dict shape as the chat_completions path.
    """
    from honeyos.agent.transports.codex_app_server_session import (
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to HoneyOS' standard prompt flow if a
        ...
        # explicitly opted out of HoneyOS approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / HONEYOS_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing HoneyOS UI. Defaults (manual/smart/unset) preserve the
        # current fail-closed behavior — this is a no-op for those users.
        auto_approve_requests = False
        try:
            from honeyos.tools.approval import is_approval_bypass_active
        ...
        # item/agentMessage/delta, ...) into HoneyOS' gateway UI callbacks
...
def _raise_stream_error(event: Any) -> None:
    ...
    """
    from honeyos.run_agent import _StreamErrorEvent
