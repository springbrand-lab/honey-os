...
from honeyos.agent.monitoring.events import GatewayDiagnosticEvent, GatewayHealthEvent
...
def _derive_busy(gateway_running: bool, gateway_state: Any, active_agents: Any) -> bool:
    try:
        from honeyos.gateway.status import derive_gateway_busy
...
def _derive_drainable(gateway_running: bool, gateway_state: Any) -> bool:
    try:
        from honeyos.gateway.status import derive_gateway_drainable
...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "honeyos.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("honeyos.gateway.up", 1 if gateway_running else 0, base),
        _metric("honeyos.gateway.active_agents", active_agents, base),
        _metric("honeyos.gateway.busy", 1 if busy else 0, base),
        _metric("honeyos.gateway.drainable", 1 if drainable else 0, base),
        _metric("honeyos.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("honeyos.gateway.state", 1, base, **{"honeyos.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"honeyos.platform": str(platform), "honeyos.platform.state": state},
        ))
        metrics.append(_metric(
            "honeyos.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"honeyos.platform": str(platform), "honeyos.platform.state": state, "honeyos.error_code": error_code},
        ))
        if is_degraded:
