...
from honeyos.agent.monitoring.events import CronExecutionEvent
from honeyos.agent.monitoring.gateway_health import GatewayHealthSnapshot, GatewayMetric
...
def build_cron_health_snapshot() -> CronHealthSnapshot:
    metrics: list[GatewayMetric] = []
    for name, reader in (
        ("honeyos.cron.scheduler.heartbeat_age_seconds", get_ticker_heartbeat_age),
        ("honeyos.cron.scheduler.last_success_age_seconds", get_ticker_success_age),
    ):
        try:
            value = reader()
    ...
    try:
        ...
        metrics.append(GatewayMetric("honeyos.cron.jobs.enabled", len(enabled), {}))
        metrics.append(
            GatewayMetric(
    ...
    try:
        metrics.append(
            GatewayMetric("honeyos.cron.jobs.running", len(get_running_job_ids()), {})
        )
    except Exception:
