...
from honeyos.runtime.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from honeyos.core.constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
from honeyos.agent.error_classifier import FailoverReason
from honeyos.agent.errors import EmptyStreamError
from honeyos.agent.turn_context import substitute_api_content
from honeyos.agent.gemini_native_adapter import is_native_gemini_base_url
from honeyos.agent.model_metadata import is_local_endpoint
from honeyos.agent.message_content import flatten_message_text
from honeyos.agent.message_sanitization import (
    _sanitize_surrogates,
    _repair_tool_call_arguments,
)
from honeyos.agent.stream_single_writer import claim_stream_writer, stream_writer_is_current
from honeyos.tools.terminal_tool import is_persistent_env
from honeyos.utils import base_url_host_matches, base_url_hostname, env_float, env_int
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("HONEYOS_STREAM_STALE_GIVEUP", 5)
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("HONEYOS_STREAM_STALE_TIMEOUT", 180.0)
...
def direct_api_call(agent, api_kwargs: dict):
    ...
    timeout (provider ``request_timeout_seconds`` / ``HONEYOS_API_TIMEOUT``) bounds
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    request_client_lock = threading.Lock()
    # Request-local cancellation flag. Distinct from honeyos.agent._interrupt_requested
    ...
    # activity. Operators can tune via HONEYOS_CODEX_TTFB_TIMEOUT_SECONDS and
    # HONEYOS_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via HONEYOS_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("HONEYOS_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    # HONEYOS_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("HONEYOS_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("HONEYOS_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("HONEYOS_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set HONEYOS_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("HONEYOS_CODEX_TTFB_MAX_SECONDS", 120.0)
        if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
            logger.info(
                "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set HONEYOS_CODEX_TTFB_MAX_SECONDS to tune.",
                _ttfb_timeout,
                _ttfb_cap,
                f"{_est_tokens_for_codex_watchdog:,}",
            )
    ...
    _codex_idle_timeout = _env_float(
        "HONEYOS_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    try:
        from honeyos.agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "honeyos",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects HONEYOS_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    from honeyos.agent.backend_identity import BackendIdentity, should_skip_candidate
    ...
    try:
        ...
        if fb_api_mode == "anthropic_messages":
            ...
            from honeyos.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
            ...
            # causing 403s from honeyos.providers like Kimi Coding that
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every HoneyOS-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (HoneyOS bookkeeping,
        ...
        summary_extra_body = {}
        try:
            from honeyos.agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
        except Exception:
        ...
        if _is_nous:
            from honeyos.agent.portal_tags import nous_portal_tags as _portal_tags
...
def cleanup_task_resources(agent, task_id: str) -> None:
    ...
    try:
        headed = False
        try:
            from honeyos.tools.browser_tool import _is_headed_mode
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    if agent.api_mode == "bedrock_converse":
        ...
        while t.is_alive():
            ...
            if _stale_elapsed > _bedrock_stale_timeout:
                ...
                try:
                    from honeyos.agent.bedrock_adapter import invalidate_runtime_client
                ...
                # crosses HONEYOS_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the HONEYOS_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("HONEYOS_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # HONEYOS_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("HONEYOS_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode HONEYOS_STREAM_READ_TIMEOUT.
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to HoneyOS. Preserve only
        ...
        if agent.provider == "moa":
            # HoneyOS interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while HoneyOS preserves
    ...
    def _call_anthropic(request_client):
        ...
        from honeyos.agent import relay_llm
        from honeyos.agent.anthropic_adapter import sanitize_anthropic_kwargs
    ...
    def _call():
        ...
        _max_stream_retries = env_int("HONEYOS_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("HONEYOS_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set HONEYOS_STREAM_STALE_TIMEOUT; override the
    # local ceiling with HONEYOS_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``HONEYOS_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("HONEYOS_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
    ...
    if result["error"] is not None:
        if deltas_were_sent["yes"]:
            ...
            try:
                from honeyos.agent.error_classifier import classify_api_error, FailoverReason
