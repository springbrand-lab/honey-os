...
from honeyos.tools.tool_result_storage import (
    maybe_persist_tool_result,
    enforce_turn_budget,
)
from honeyos.tools.budget_config import BudgetConfig, DEFAULT_BUDGET, budget_for_context_window
...
def _ensure_file_checkpoint(
    ...
    # which can differ from the HoneyOS process cwd (notably in Docker).  Resolve
    # through that same path pipeline before asking the checkpoint manager to
    # discover the project root.
    from honeyos.tools.file_tools import _resolve_path_for_task
...
def _authorization_gate_lock_timeout() -> float:
    ...
    """
    try:
        from honeyos.tools.approval import human_wait_ceiling
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("HONEYOS_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid HONEYOS_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current HoneyOS process before the normal turn-end persistence path runs.
...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    try:
        ...
        from honeyos.tools import tool_search as _ts
...
class _ConcurrentToolAuthorizationGate:
    ...
    def _human_wait_seconds(self) -> float:
        try:
            from honeyos.tools.approval import human_wait_seconds
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before HoneyOS policy and dispatch exactly once."""
    from honeyos.agent import relay_tools
    from honeyos.runtime.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "HoneyOS tool execution callback invoked more than once"
                )
    ...
    def _honeyos_pipeline(relay_args: dict[str, Any]) -> Any:
...
def _begin_tool_execution(
    ...
    *,
    ...
    try:
        from honeyos.tools.environments.base import set_activity_callback
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        from honeyos.agent.agent_runtime_helpers import agent_runtime_owns_post_tool_hook
