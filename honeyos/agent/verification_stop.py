...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``HONEYOS_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("HONEYOS_VERIFY_ON_STOP")
    if env is not None:
...
def _verification_snapshot(
    *,
    ...
    try:
        ...
        from honeyos.agent.verification_evidence import verification_status
    except Exception:
        return None
...
def build_verify_on_stop_nudge(
    *,
    ...
    try:
        from honeyos.agent.verify_hooks import coding_verify_guidance
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `honeyos-verify-` filename prefix, run it "
