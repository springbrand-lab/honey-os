...
def _supports_vision_override(
    ...
    *,
    ...
    # (honeyos.runtime/runtime_provider.py:_resolve_named_custom_runtime), so the
...
def _resolve_inference_base_url(
    ...
    try:
        from honeyos.agent.auxiliary_client import _runtime_main_value
...
def _should_probe_ollama_vision(provider: str, base_url: str) -> bool:
    ...
    if not base_url:
        return False
    try:
        from honeyos.agent.model_metadata import detect_local_server_type
...
def _lookup_supports_vision(
    ...
    *,
    ...
    if _should_probe_ollama_vision(provider, base_url):
        try:
            from honeyos.agent.model_metadata import query_ollama_supports_vision
