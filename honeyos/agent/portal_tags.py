...
Every HoneyOS request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to HoneyOS and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=honeyos-agent",
        "client=honeyos-client-v<__version__>",
    ]

The version is sourced live from ``honeyos.runtime.__version__`` so it auto-aligns
...
``honeyos.runtime.__version__`` is the canonical source of truth.
"""
...
def _honeyos_version() -> str:
    """Return the current HoneyOS release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``honeyos.runtime`` cannot be imported (should
...
def honeyos_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=honeyos-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=honeyos-client-v{_honeyos_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a HoneyOS session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    HoneyOS conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.honeyos/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific HoneyOS
    ...
    """
    tags = ["product=honeyos-agent", honeyos_client_tag()]
