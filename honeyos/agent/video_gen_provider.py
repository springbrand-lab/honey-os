...
as ``kind: backend``) or ``~/.honeyos/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``honeyos tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$HONEYOS_HOME/cache/videos/``, creating parents as needed."""
    from honeyos.core.constants import get_honeyos_home

    path = get_honeyos_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$HONEYOS_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$HONEYOS_HOME/cache/videos/``.
