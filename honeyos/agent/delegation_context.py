...
The parent HoneyOS process may itself be a Kanban dispatcher worker with
HONEYOS_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's HONEYOS_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "honeyos_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "HONEYOS_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "HONEYOS_KANBAN_TASK",
    "HONEYOS_KANBAN_RUN_ID",
    "HONEYOS_KANBAN_WORKSPACE",
    "HONEYOS_KANBAN_WORKSPACES_ROOT",
    "HONEYOS_KANBAN_CLAIM_LOCK",
    "HONEYOS_KANBAN_BOARD",
    "HONEYOS_KANBAN_DB",
)
...
def delegated_child_context(session_id: str | None = None) -> Iterator[None]:
    ...
    try:
        ...
        from honeyos.gateway.session_context import scoped_current_session_id
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``HONEYOS_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$HONEYOS_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``HONEYOS_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``HONEYOS_KANBAN_*`` vars while losing the ContextVar in
