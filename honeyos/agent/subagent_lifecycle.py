"""Public, plugin-safe lifecycle API for delegated HoneyOS subagents.
...
from honeyos.tools.daemon_pool import DaemonThreadPoolExecutor as _DaemonExecutor

_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="honeyos-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar(
    "honeyos_subagent_lifecycle_parent", default=None
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active HoneyOS parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("HoneyOS failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _run(self, record: _Record, goal: str, parent: Any) -> None:
        ...
        try:
            from honeyos.tools.delegate_tool import _run_child_lifecycle
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because HoneyOS delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. HoneyOS always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
