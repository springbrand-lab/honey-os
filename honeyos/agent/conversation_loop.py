...
from honeyos.agent.codex_responses_adapter import _summarize_user_message_for_log
...
from honeyos.agent.context_engine import automatic_compaction_status_message
from honeyos.agent.display import KawaiiSpinner
from honeyos.agent.error_classifier import FailoverReason, classify_api_error
...
from honeyos.agent.turn_retry_state import TurnRetryState
from honeyos.agent.runtime_cwd import resolve_agent_cwd
from honeyos.agent.message_sanitization import (
...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in honeyos.core.state.py — kept local
# to avoid importing honeyos.core.state at module load time (its module-level
# DEFAULT_DB_PATH = get_honeyos_home() / "state.db" breaks tests that
# monkeypatch get_honeyos_home to return a str).
...
from honeyos.agent.prompt_caching import (
    build_prompt_cache_plan,
    strip_anthropic_cache_control,
    strip_anthropic_tool_cache_control,
)
from honeyos.agent.retry_utils import (
    adaptive_rate_limit_backoff,
    is_zai_coding_overload_error,
    jittered_backoff,
    zai_coding_overload_retry_ceiling,
)
from honeyos.agent.trajectory import has_incomplete_scratchpad
from honeyos.agent.usage_pricing import estimate_usage_cost, normalize_usage
from honeyos.core.constants import PARTIAL_STREAM_STUB_ID
from honeyos.core.logging import set_session_context
from honeyos.tools.skill_provenance import set_current_write_origin
from honeyos.utils import base_url_host_matches, env_var_enabled
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.honeyos/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for HoneyOS tool use: "
    ...
    return (
        ...
        f"context, but HoneyOS needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In HoneyOS config, set `model.ollama_num_ctx: 65536` "
...
def _billing_or_entitlement_message(
    *,
    ...
    try:
        from honeyos.agent.billing_links import build_billing_block
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        ...
        from honeyos.agent.system_prompt import reconstruct_static_prefix
    ...
    try:
        from honeyos.agent.credits_tracker import seed_credits_at_session_start
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only HoneyOS' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `honeyos fallback add`."
)
...
def _peel_moa_guidance(
    ...
    """
    from honeyos.agent.moa_loop import peel_reference_guidance
...
def _apply_context_engine_selection(
    ...
    *,
    ...
    try:
        from honeyos.agent.context_engine import ContextEngine as _CE
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from honeyos.runtime.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.honeyos/.env credential/base-url edits made since the last
    ...
    # all run inside Codex). Default HoneyOS path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if _pre_api_steer:
            ...
            for _si in range(len(messages) - 1, -1, -1):
                ...
                if isinstance(_sm, dict) and _sm.get("role") == "tool":
                    from honeyos.agent.prompt_builder import format_steer_marker
        ...
        from honeyos.agent.agent_runtime_helpers import repair_message_sequence_with_cursor
        ...
        # cache prefix.  The system prompt is reserved for HoneyOS internals.
        #
        # HoneyOS invariant: the system prompt is built ONCE per session
        ...
        if moa_config:
            try:
                from honeyos.agent.message_content import flatten_message_text as _flatten_mt
                from honeyos.agent.moa_loop import _preset_temperature, aggregate_moa_context
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for HoneyOS tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    from honeyos.agent.nous_rate_guard import (
            ...
            try:
                ...
                try:
                    from honeyos.runtime.middleware import apply_llm_request_middleware
                ...
                try:
                    from honeyos.runtime.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("HONEYOS_DUMP_REQUESTS"):
                ...
                from honeyos.runtime.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a HoneyOS/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if agent.provider == "nous":
                    try:
                        from honeyos.agent.nous_rate_guard import clear_nous_rate_limit
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: honeyos auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `honeyos doctor` for credential-chain diagnostics, or")
                    ...
                    from honeyos.core.constants import display_honeyos_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for HoneyOS-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: honeyos config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: honeyos config set ANTHROPIC_API_KEY \"\"")
                ...
                if (
                    ...
                    try:
                        from honeyos.tools.schema_sanitizer import strip_pattern_and_format
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from honeyos.runtime.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `honeyos model`."
                        )
                ...
                # providers (DeepSeek, Kimi, MiMo, HoneyOS).  A 429 can
                ...
                # which HoneyOS' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. HoneyOS' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`honeyos",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # HoneyOS may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See honeyos-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `honeyos auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `honeyos model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: honeyos portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: honeyos setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        honeyos fallback add   (interactive picker — same as `honeyos model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    from honeyos.agent.thinking_timeout_guidance import (
                        is_thinking_timeout,
                    )
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.honeyos/config.yaml` to extend the per-call "
                            f"timeout. (HoneyOS's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates HoneyOS mid-turn, resume logic still sees the
                ...
                # timeout (HONEYOS_AGENT_TIMEOUT, default 1800s) and the
                ...
                from honeyos.agent.agent_runtime_helpers import (
                ...
                try:
                    from honeyos.agent.verification_stop import (
                ...
                try:
                    from honeyos.agent.verify_hooks import max_verify_nudges
                    from honeyos.runtime.lifecycle import has_hook
                    from honeyos.runtime.plugins import get_pre_verify_continue_message

                    if _edited and has_hook("pre_verify") and _attempt < max_verify_nudges():
                        # Posture is fixed for the session — resolve once + cache.
                        coding = getattr(agent, "_resolved_is_coding", None)
                        if coding is None:
                            from honeyos.agent.coding_context import is_coding_context
                ...
                try:
                    from honeyos.agent.kanban_stop import build_kanban_stop_nudge
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("HONEYOS_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
    ...
    from honeyos.agent.turn_finalizer import finalize_turn
