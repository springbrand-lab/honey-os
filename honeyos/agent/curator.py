...
def _state_file() -> Path:
    return get_honeyos_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.honeyos/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `honeyos update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``honeyos curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``honeyos update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``honeyos curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `honeyos curator run --dry-run` to preview now"
            )
            save_state(state)
...
def _cron_referenced_skills() -> Set[str]:
    ...
    """
    try:
        from honeyos.scheduler.jobs import referenced_skill_names as _refs
        return _refs()
    except Exception as e:
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    """
    from honeyos.tools import skill_usage as _u
...
CURATOR_REVIEW_PROMPT = (
    "You are running as HoneyOS' background skill CURATOR. This is an "
    ...
    "into ~/.honeyos/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: honeyos-config-*, honeyos-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.honeyos/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.honeyos/logs/curator/``)
    ...
    authored skill data in ``~/.honeyos/skills/``.

    ``ensure_honeyos_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_honeyos_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every honeyos install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: honeyos curator status
        keep an umbrella stable: honeyos curator pin document-tools
    ...
    lines.append("full report: honeyos curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: honeyos curator pin {example}"
            )
    return "\n".join(lines)
    ...
    *,
    ...
    try:
        ...
        if consolidated_map or pruned_names:
            from honeyos.scheduler.jobs import rewrite_skill_refs as _rewrite_cron_refs
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.honeyos/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.honeyos/skills/.archive/` for "
            "safety and can be restored via `honeyos curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.honeyos/skills/.archive/`. "
            "Restore any via `honeyos curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `honeyos curator restore <name>`")
    lines.append("- All archives live under `~/.honeyos/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``honeyos curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    if dry_run:
        ...
        try:
            from honeyos.agent import curator_backup
    ...
    # real pass out. We still record a summary so `honeyos curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `honeyos update` only if "
        ...
        # recorded in state so `honeyos curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``honeyos model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `honeyos model` → auxiliary
