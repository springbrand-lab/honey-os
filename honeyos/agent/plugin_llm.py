...
Plugins built on HoneyOS often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing HoneyOS
...
:class:`~honeyos.runtime.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors HoneyOS' main config
...
handles every provider, fallback chain, and per-task override HoneyOS
supports.
"""
...
def _resolve_attribution(
    *,
    ...
    if provider_override:
        ...
        try:
            from honeyos.agent.auxiliary_client import _read_main_provider
    ...
    if isinstance(response_model, str) and response_model.strip():
        ...
        try:
            from honeyos.agent.auxiliary_client import _read_main_model
...
class PluginLlm:
    ...
    Instances are constructed by :class:`honeyos.runtime.plugins.PluginContext`
