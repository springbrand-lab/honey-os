...
(``gateway/platforms/webhook.py``): inbound wakes HoneyOS when the world
changes; outbound tells the world when HoneyOS does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-HoneyOS-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``HONEYOS_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.honeyos/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/honeyos-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: HONEYOS_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `honeyos hooks list`
...
Headers::
    ...
    User-Agent:              HoneyOS-Agent-Outbound-Webhook
    X-HoneyOS-Event:          <hook event name>
    X-HoneyOS-Delivery:       <delivery_id>
    X-HoneyOS-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("HONEYOS_SAFE_MODE"):
        logger.info("HONEYOS_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-HoneyOS-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-HoneyOS-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
