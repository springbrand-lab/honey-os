...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from honeyos.toolsets import _HONEYOS_CORE_TOOLS

        _core_tool_names = set(_HONEYOS_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, HoneyOS expands the turn into
    ...
    def on_pre_compress(self, messages: List[Dict[str, Any]]) -> str:
        ...
        Returns combined text from honeyos.providers to include in the compression
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``honeyos_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_honeyos_home()`` themselves.
        """
        if "honeyos_home" not in kwargs:
            from honeyos.core.constants import get_honeyos_home
            kwargs["honeyos_home"] = str(get_honeyos_home())
        for provider in self._providers:
            try:
