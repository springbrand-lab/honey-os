...
- **skills** → the skill name (e.g. ``"debugging-honeyos-desktop"``)
...
shared by the CLI (``honeyos journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``honeyos curator restore``); deleting a memory rewrites its
...
def _memory_local_index(source: str, global_index: int) -> int:
    ...
    """
    from honeyos.agent.learning_graph import _memory_cards
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (honeyos curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: honeyos curator restore {name}" if ok else message}
...
def _edit_skill(name: str, content: str) -> dict[str, Any]:
    from honeyos.tools.skill_manager_tool import _edit_skill as _do_edit
