...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.honeyos/auth.json active provider)
...
from honeyos.agent.model_metadata import MINIMUM_CONTEXT_LENGTH, get_model_context_length
from honeyos.runtime.config import get_honeyos_home
from honeyos.core.constants import OPENROUTER_BASE_URL
from honeyos.utils import base_url_host_matches, base_url_hostname, env_float, model_forces_max_completion_tokens, normalize_proxy_env_vars
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``HONEYOS_CA_BUNDLE`` /
    ``SSL_CERT_FILE`` env conventions. Best-effort: any failure falls back to
    the httpx/certifi default (``True``).
    """
    try:
        from honeyos.agent.ssl_verify import resolve_httpx_verify
...
def _openai_http_client_kwargs(
    ...
    *,
    ...
    try:
        from honeyos.agent.process_bootstrap import build_keepalive_http_client
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `honeyos update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # HoneyOS owns auxiliary retry + provider/model fallback policy (the
    ...
    # HoneyOS sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let HoneyOS control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="honeyos-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before HoneyOS triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://honeyos-agent.nousresearch.com",
    "X-Title": "HoneyOS",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from honeyos.runtime.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``HONEYOS_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``HONEYOS_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("HONEYOS_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("HONEYOS_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "HoneyOSAgent",
}
...
#
# The tags are computed from honeyos.agent.portal_tags so the client= marker stays
# in lockstep with honeyos.runtime.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from honeyos.agent.portal_tags import nous_portal_tags as _nous_portal_tags


def _nous_extra_body() -> dict:
    """Return a fresh Nous Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``honeyos.runtime.__version__`` is
...
_AUTH_JSON_PATH = get_honeyos_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (HoneyOS)",
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from honeyos.runtime.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("HONEYOS_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        from honeyos.agent.codex_responses_adapter import _chat_messages_to_responses_input
        ...
        if tools:
            ...
            try:
                ...
                from honeyos.tools.schema_sanitizer import (
        ...
        try:
            from honeyos.agent.transports.codex import (
        ...
        def _check_cancelled() -> None:
            ...
            try:
                from honeyos.tools.interrupt import is_interrupted
        ...
        try:
            ...
            from honeyos.agent.codex_runtime import _consume_codex_event_stream
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from honeyos.agent.anthropic_adapter import _is_nous_portal_endpoint
    ...
    def create(self, **kwargs) -> Any:
        from honeyos.agent.anthropic_adapter import build_anthropic_kwargs, create_anthropic_message
        ...
        if temperature is not None:
            from honeyos.agent.anthropic_adapter import _forbids_sampling_params
        ...
        #   - ``_``-prefixed keys: private HoneyOS plumbing (_reasoning_config
...
class _BedrockCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from honeyos.agent.bedrock_adapter import call_converse
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``honeyos.runtime.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from honeyos.agent.gemini_native_adapter import GeminiNativeClient
        if _safe_isinstance(client_obj, GeminiNativeClient):
            return client_obj
    except ImportError:
        pass
    try:
        from honeyos.agent.copilot_acp_client import CopilotACPClient
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.honeyos/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from honeyos.runtime.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from honeyos.runtime.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``honeyos auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``honeyos.runtime.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from HoneyOS auth store.
    ...
    try:
        from honeyos.runtime.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from honeyos.runtime.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from honeyos.runtime.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from honeyos.agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                try:
                    from honeyos.providers import get_provider_profile as _gpf_aux
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from honeyos.providers import get_provider_profile as _gpf_aux2
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from honeyos.runtime.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from honeyos.agent.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from honeyos.runtime.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from honeyos.runtime.config import load_config
        from honeyos.runtime.moa_config import resolve_moa_preset
...
def _relay_sync_completion(
    ...
    *,
    ...
    from honeyos.agent import relay_llm
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `honeyos setup` or `honeyos model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=honeyos_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`honeyos.runtime.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from honeyos.runtime.runtime_provider import _resolve_azure_foundry_runtime
        from honeyos.runtime.auth import AuthError
        from honeyos.runtime.config import load_config_readonly
    except ImportError:
        return None, None
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    try:
        from honeyos.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second honeyos
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``honeyos config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from honeyos.runtime.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from honeyos.agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_honeyos_fallback_destination", None)
...
def _replan_synchronous_cache_sections(
    ...
    *,
    ...
    from honeyos.agent.agent_runtime_helpers import plan_cache_sections_for_destination
...
def _try_main_agent_model_fallback(
    ...
    from honeyos.agent.backend_identity import (
        BackendIdentity,
        FailureScope,
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``honeyos.runtime.fallback_config.resolve_entry_api_key``).
    """
    from honeyos.runtime.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._honeyos_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into HoneyOS' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `honeyos model` but the
    #    old OPENAI_BASE_URL lingers in ~/.honeyos/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: honeyos model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.honeyos/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from honeyos.agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = honeyos_xai_default_headers()
    else:
        # Fall back to profile.default_headers for providers that declare
        # client-level headers on their ProviderProfile (e.g. attribution
        # User-Agent strings). Provider is inferred from the hostname.
        try:
            from honeyos.agent.model_metadata import _infer_provider_from_url
            from honeyos.providers import get_provider_profile as _gpf_async
    ...
    # See _create_openai_client: disable SDK-internal retries so HoneyOS owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: honeyos auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        from honeyos.runtime.providers import nous_api_mode
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: honeyos model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: honeyos model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "custom":
        ...
        if custom_base and custom_key:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                ...
                try:
                    from honeyos.providers import get_provider_profile as _gpf_custom
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: honeyos doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("honeyos.runtime.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(honeyos_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
            ...
            try:
                from honeyos.providers import get_provider_profile as _gpf_main
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from honeyos.agent.vertex_adapter import get_vertex_config, has_vertex_credentials
        except ImportError:
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`honeyos.runtime.plugins.PluginContext.register_auxiliary_task`) the
    ...
    if "reasoning" not in result:
        ...
        if effort is not None and effort != "":
            if task in ("moa_reference", "moa_aggregator"):
                ...
                return result
            from honeyos.core.constants import parse_reasoning_effort
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                from honeyos.agent.gemini_native_adapter import is_native_gemini_base_url
    ...
    if _provider_for_portal in {"nous", "nous-portal", "nousresearch"}:
        ...
        if "session_id" not in merged_extra:
            try:
                from honeyos.agent.portal_tags import get_conversation_context
    ...
    # Anthropic Messages adapters translate HoneyOS reasoning into native
...
def _validate_llm_response(
    ...
    from honeyos.agent.aux_accounting import record_aux_usage
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional HoneyOS reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `honeyos model`."
                    )
