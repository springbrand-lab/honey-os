...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `honeyos backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - honeyos_home (str): The active HONEYOS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.honeyos``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "honeyos").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'honeyos memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], honeyos_home: str) -> None:
        ...
        Called by 'honeyos memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``honeyos_home`` is the active HONEYOS_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE HONEYOS_HOME.

        ``honeyos backup`` only walks HONEYOS_HOME, so any provider state kept
        ...
        of the archive, and ``honeyos import`` restores them to their original
