...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. HoneyOS does
...
def _require_azure_identity():
    ...
    try:
        ...
        try:
            from honeyos.tools.lazy_deps import ensure, FeatureUnavailable
...
class EntraIdentityConfig:
    ...
    Captures the HoneyOS-managed Entra knobs we need outside Azure SDK
    ...
    ``honeyos.runtime/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only HoneyOS-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.honeyos/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    HoneyOS processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``honeyos doctor`` /
    ``honeyos auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``honeyos doctor`` and the wizard preflight — never
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``honeyos.runtime/azure_detect.py``).
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `honeyos doctor` or `az login` to recover.",
                exc,
            )
