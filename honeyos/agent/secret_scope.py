...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the HONEYOS_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # HoneyOS runtime / deployment
    "HONEYOS_HOME", "HONEYOS_PROFILE", "HONEYOS_GATEWAY_LOCK_DIR",
    "HONEYOS_MAX_ITERATIONS", "HONEYOS_MAX_TOKENS", "HONEYOS_API_TIMEOUT",
    "HONEYOS_REDACT_SECRETS", "HONEYOS_NOUS_TIMEOUT_SECONDS",
    "_HONEYOS_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "HONEYOS_KANBAN_DB", "HONEYOS_KANBAN_WORKSPACES_ROOT", "HONEYOS_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "HONEYOS_KANBAN_",
    "HONEYOS_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset HoneyOS writes
    ...
    ``honeyos.runtime.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical HoneyOS parser: save_env_value
    ...
    from honeyos.runtime.config import _parse_env_value
...
def build_profile_secret_scope(honeyos_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(honeyos_home)
    secrets = load_env_file(home / ".env")

    try:
        from honeyos.runtime.env_loader import get_secret_source_values
