...
from honeyos.utils import safe_json_loads
from honeyos.agent.redact import redact_sensitive_text
from honeyos.agent.tool_result_classification import file_mutation_result_landed
...
def build_status_phrase(tool_name: str, args: dict | None, max_len: int = 49) -> str | None:
    ...
    name ("HoneyOS is running …"), so it starts lowercase with "is".
...
def _resolve_skill_manage_paths(args: dict) -> list[Path]:
    ...
    from honeyos.tools.skill_manager_tool import _find_skill, _resolve_skill_dir
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in HoneyOS' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("HONEYOS_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
