...
from honeyos.agent.context_compressor import ContextCompressor
from honeyos.agent.iteration_budget import IterationBudget
from honeyos.agent.memory_manager import StreamingContextScrubber
...
from honeyos.agent.process_bootstrap import _install_safe_stdio
from honeyos.agent.subdirectory_hints import SubdirectoryHintTracker
from honeyos.agent.think_scrubber import StreamingThinkScrubber
...
from honeyos.runtime.timeouts import get_provider_request_timeout
from honeyos.core.constants import get_honeyos_home
from honeyos.utils import base_url_host_matches, is_truthy_value
...
def _resolve_agent_mode(config: Any) -> str:
    """Resolve the immutable runtime posture while preserving upstream defaults."""
    ...
    if not isinstance(section, dict):
        return "assistant"
    value = str(section.get("mode", "assistant") or "assistant").strip().lower()
    return value if value in {"assistant", "companion"} else "assistant"
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from honeyos.runtime.providers import HONEYOS_OVERLAYS, get_provider

        overlay = HONEYOS_OVERLAYS.get(provider)
    ...
    try:
        ...
        from honeyos.runtime.models import normalize_provider as normalize_model_provider
        from honeyos.runtime.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: honeyos config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$HONEYOS_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_honeyos_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$HONEYOS_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .honeyos.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HONEYOS_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.honeyos/SOUL.md as the primary
    ...
    if credential_pool is not None:
        try:
            from honeyos.agent.credential_pool import credential_pool_matches_provider
    ...
    agent._cache_ttl = "5m"
    try:
        from honeyos.runtime.config import load_config_readonly as _load_pc_cfg

        from honeyos.agent.agent_runtime_helpers import cache_ttl_means_disabled
    ...
    # report cumulative micros spent.  Surfaced behind HONEYOS_DEV_CREDITS.
    agent._credits_state = None
    agent._credits_session_start_micros = None
    # Threshold-notice latch (L4): active sticky-notice keys + the crossing gates.
    from honeyos.agent.credits_tracker import new_credits_latch
    ...
    # both live under ~/.honeyos/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from honeyos.core.logging import setup_logging, setup_verbose_logging
    setup_logging(honeyos_home=_ra()._honeyos_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that honeyos.core.logging.setup_logging() does not
        ...
        # handler level inside honeyos.core.logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from honeyos.runtime.auth import build_minimax_oauth_token_provider
            ...
            from honeyos.agent.anthropic_adapter import _is_oauth_token as _is_oat
            ...
            if not agent.quiet_mode:
                ...
                from honeyos.agent.azure_identity_adapter import is_token_provider
        ...
        from honeyos.agent.moa_loop import build_moa_facade
        agent.api_mode = "chat_completions"
        ...
        try:
            from honeyos.runtime.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                from honeyos.agent.auxiliary_client import build_or_headers
                client_kwargs["default_headers"] = build_or_headers()
            elif base_url_host_matches(effective_base, "integrate.api.nvidia.com"):
                from honeyos.agent.auxiliary_client import build_nvidia_nim_headers
                ...
                client_kwargs["default_headers"] = honeyos_xai_default_headers()
            elif "default_headers" not in client_kwargs:
                # Fall back to profile.default_headers for providers that
                # declare custom headers (e.g. Vercel AI Gateway attribution,
                # Kimi User-Agent on non-kimi.com endpoints).
                try:
                    from honeyos.providers import get_provider_profile as _gpf
            ...
            from honeyos.agent.auxiliary_client import resolve_provider_client
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `honeyos model` to "
                        "select a provider, or run `honeyos setup` for first-time "
                        "configuration."
                    )
    ...
    from honeyos.agent.agent_runtime_helpers import sync_credential_pool_entry_id
    ...
    try:
        from honeyos.tools.registry import registry as _snapshot_registry
    ...
    # worker (kanban_show tool is present iff HONEYOS_KANBAN_TASK is set).
    # Resolving the ~835-token block once here avoids re-running the
    # membership test + reference on every system-prompt rebuild
    # (init + each context compression).
    from honeyos.agent.prompt_builder import KANBAN_GUIDANCE
    ...
    try:
        ...
        try:
            from honeyos.agent.delegation_context import is_delegated_child_context

            delegated_child = is_delegated_child_context()
        except Exception:
            delegated_child = False
        if not delegated_child:
            os.environ["HONEYOS_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.honeyos/sessions/ alongside gateway sessions
    honeyos_home = get_honeyos_home()
    agent.logs_dir = honeyos_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.honeyos/sessions/session_{sid}.json)
    ...
    try:
        from honeyos.runtime.config import load_config_readonly as _load_sess_cfg
    ...
    # `honeyos --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from honeyos.tools.approval import _YOLO_MODE_FROZEN
    ...
    from honeyos.tools.todo_tool import TodoStore
    agent._todo_store = TodoStore()

    # Load config once for memory, skills, and compression sections
    try:
        from honeyos.runtime.config import load_config_readonly as _load_agent_config
        _agent_cfg = _load_agent_config()
    except Exception:
        _agent_cfg = {}

    # Runtime posture is resolved once per agent so the prompt remains byte-stable
    # for the life of the conversation. Unknown values preserve upstream behavior.
    agent._agent_mode = _resolve_agent_mode(_agent_cfg)
    try:
        _raw_agent_section = _agent_cfg.get("agent", {})
        _raw_agent_mode = (
            _raw_agent_section.get("mode", "assistant")
            if isinstance(_raw_agent_section, dict)
            else "assistant"
        )
        if str(_raw_agent_mode or "assistant").strip().lower() not in {
            "assistant",
            "companion",
        }:
            logger.warning(
                "Invalid agent.mode=%r; expected assistant or companion. Using assistant.",
                _raw_agent_mode,
            )
    except Exception:
        pass
    ...
    # management API (the historical HoneyOS behavior) or left to LM Studio's
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from honeyos.agent.memory_manager import MemoryManager as _MemoryManager
                from honeyos.plugins.memory import load_memory_provider as _load_mem
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "honeyos"
                    except Exception:
                        pass
    ...
    from honeyos.agent.memory_manager import inject_memory_provider_tools as _inject_memory_provider_tools
    ...
    if agent._environment_probe:
        try:
            from honeyos.tools.env_probe import warm_environment_probe_async
            warm_environment_probe_async()
        except Exception:
            pass

    # Per-platform prompt-hint overrides (config.yaml → platform_hints).
    # Lets an enterprise admin append to or replace HoneyOS' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "honeyos", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, honeyos, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from honeyos.runtime.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from honeyos.runtime.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        try:
            from honeyos.plugins.context_engine import load_context_engine
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from honeyos.runtime.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by HoneyOS.  Choose a model with at least "
    ...
    # Nous HoneyOS 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from honeyos.runtime.model_switch import _check_honeyos_model_warning

            _honeyos_warn = _check_honeyos_model_warning(agent.model or "")
            if _honeyos_warn:
                _user_msg = (
                    "⚠ Nous Research HoneyOS 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_honeyos_warn)
        except Exception:
            pass
    ...
    if (
        ...
        from honeyos.agent.memory_manager import normalize_tool_schema as _normalize_tool_schema
