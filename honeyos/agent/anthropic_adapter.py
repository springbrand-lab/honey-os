"""Anthropic Messages API adapter for HoneyOS.

Translates between HoneyOS's internal OpenAI-style message format and
...
from honeyos.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in honeyos.runtime/runtime_provider.py.
    """
...
# HoneyOS effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    from honeyos.agent.azure_identity_adapter import build_bearer_http_client
    ...
    kwargs = {
        ...
        # Delegate retry to honeyos's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to honeyos's outer conversation
    ...
    # from ``ANTHROPIC_API_KEY`` (HoneyOS loads that into the process env from
    # ``~/.honeyos/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    HoneyOS notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    HoneyOS historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``honeyos models``) never mutates ``~/.honeyos/auth.json``
    or makes a network refresh call. Refresh-on-expiry is owned by the API call
    path's pool recovery, not the resolver.
    """
    try:
        from honeyos.agent.credential_pool import AUTH_TYPE_OAUTH, load_pool
    except Exception:
        return None
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by HoneyOS)
      ...
      5. Anthropic credential_pool OAuth entry (~/.honeyos/auth.json)
    ...
    # 1. HoneyOS-managed OAuth/setup token env var
    ...
    # 5. HoneyOS credential_pool OAuth entry.
...
# ── HoneyOS-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.honeyos/.anthropic_oauth.json (our own file).
...
def _get_honeyos_oauth_file() -> Path:
    return get_honeyos_home() / ".anthropic_oauth.json"
...
def run_honeyos_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run HoneyOS-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize HoneyOS with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from honeyos.runtime.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_honeyos_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read HoneyOS-managed OAuth credentials from ~/.honeyos/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read HoneyOS OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See honeyos-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See honeyos-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See honeyos-agent#13848 (Kimi) and
    honeyos-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("HoneyOS", "Claude Code")
                text = text.replace("HoneyOS agent", "Claude Code")
                text = text.replace("honeyos-agent", "claude-code")
        ...
        #      a) bare HoneyOS-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that HoneyOS surfaces in its CLI. We
...
def _is_stream_unavailable_error(exc: Exception) -> bool:
    ...
    if "invokemodelwithresponsestream" in err_lower:
        from honeyos.agent.bedrock_adapter import is_streaming_access_denied_error
