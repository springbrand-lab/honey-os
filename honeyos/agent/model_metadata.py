...
from honeyos.utils import atomic_json_write, base_url_host_matches, base_url_hostname
...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. HoneyOS also honours HONEYOS_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("HONEYOS_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_honeyos_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from honeyos.core.constants import get_honeyos_home
    return get_honeyos_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run HoneyOS.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent HoneyOS from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for HoneyOS' system prompt, but mapping
...
try:
    from honeyos.providers import list_providers as _list_providers
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets HoneyOS minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from honeyos.core.constants import get_honeyos_home
    return get_honeyos_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # HoneyOS grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # entries are dropped — never a cached value above the table.
                ...
                try:
                    from honeyos.agent.bedrock_adapter import get_bedrock_context_length
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from honeyos.runtime.models import get_copilot_model_context
    ...
    if effective_provider:
        from honeyos.agent.models_dev import lookup_models_dev_context
    ...
    # ``HoneyOS-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets HoneyOS sends to providers:
