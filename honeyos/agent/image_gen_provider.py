...
as ``kind: backend``) or ``~/.honeyos/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``honeyos tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``honeyos tools`` model picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``honeyos tools``
...
def _images_cache_dir() -> Path:
    """Return ``$HONEYOS_HOME/cache/images/``, creating parents as needed."""
    from honeyos.core.constants import get_honeyos_home

    path = get_honeyos_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$HONEYOS_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$HONEYOS_HOME/cache/images/``.
