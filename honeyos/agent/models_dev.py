...
Data resolution order:
  ...
  2. Disk cache (~/.honeyos/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: HoneyOS ↔ models.dev
# ---------------------------------------------------------------------------

# HoneyOS provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → HoneyOS (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_honeyos_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``honeyos config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a HoneyOS provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from honeyos.runtime.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal HoneyOS agent traffic.
...
def get_provider_info(
    ...
    Accepts either a HoneyOS provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve HoneyOS ID → models.dev ID
...
def get_model_info(
    ...
    Accepts HoneyOS or models.dev provider ID.  Tries exact match then
