...
from honeyos.core.constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.honeyos/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    HONEYOS_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``HONEYOS_KANBAN_TASK`` / ``HONEYOS_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("HONEYOS_KANBAN_TASK") or os.getenv("HONEYOS_KANBAN_BOARD"):
        ...
        if _owns_dispatcher_task:
            ...
            try:
                from honeyos.tools.kanban_tools import _profile_has_kanban_toolset
        ...
        # The HoneyOS Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``honeyos.runtime.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``HONEYOS_PLATFORM`` or
            ``HONEYOS_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("HONEYOS_PLATFORM")
        or get_session_env("HONEYOS_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``honeyos`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.honeyos/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``honeyos`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against HONEYOS_HOME, not cwd
        if not p.is_absolute():
            p = (honeyos_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.honeyos/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.honeyos/skills/`` (including via symlinks) or configured
    ...
    # ~/.honeyos/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: HoneyOS can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    honeyos = metadata.get("honeyos") or {}
    if not isinstance(honeyos, dict):
        honeyos = {}
    return {
        "fallback_for_toolsets": honeyos.get("fallback_for_toolsets", []),
        "requires_toolsets": honeyos.get("requires_toolsets", []),
        "fallback_for_tools": honeyos.get("fallback_for_tools", []),
        "requires_tools": honeyos.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    honeyos = metadata.get("honeyos")
    if not isinstance(honeyos, dict):
        return []
    raw = honeyos.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes HoneyOS metadata, VCS, virtualenv/dependency, cache, and skill
