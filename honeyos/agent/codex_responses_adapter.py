...
from honeyos.agent.message_sanitization import deterministic_call_id
from honeyos.agent.prompt_builder import DEFAULT_AGENT_IDENTITY
...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``honeyos-tools`` server already overflows 64 chars,
...
# output items.  HoneyOS injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while HoneyOS-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on HoneyOS threading encrypted reasoning back across
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a HoneyOS-side metadata key and not part
...
def _preflight_codex_api_kwargs(
    ...
    *,
    ...
    if is_xai_model and normalized.get("tools"):
        try:
            from honeyos.tools.schema_sanitizer import strip_slash_enum
...
def _normalize_codex_response(
    ...
    *,
    ...
    if tool_calls:
        ...
        # See https://github.com/NousResearch/honeyos-agent/issues/64434
