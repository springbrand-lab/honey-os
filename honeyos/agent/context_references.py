...
from honeyos.agent.model_metadata import estimate_tokens_rough
...
_SENSITIVE_HONEYOS_DIRS = (Path("skills") / ".hub",)
...
async def _default_url_fetcher(url: str) -> str:
    from honeyos.tools.web_tools import web_extract_tool
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    honeyos_home = get_honeyos_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(honeyos_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(honeyos_home / rel for rel in _SENSITIVE_HONEYOS_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal HoneyOS path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.honeyos/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal HoneyOS path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
