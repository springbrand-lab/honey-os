...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with honeyos-agent)
2. User plugins: ``$HONEYOS_HOME/plugins/model-providers/<name>/``
...
from honeyos.providers.base import OMIT_TEMPERATURE, ProviderProfile  # noqa: F401
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HONEYOS_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HONEYOS_HOME/plugins/model-providers/`` if it exists."""
    try:
        from honeyos.core.constants import get_honeyos_home

        d = get_honeyos_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HONEYOS_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_honeyos_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HONEYOS_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with honeyos-agent.
    ...
    # 2. User plugins — under $HONEYOS_HOME/plugins/model-providers/<name>/.
    ...
    try:
        ...
        import honeyos.providers as _pkg
