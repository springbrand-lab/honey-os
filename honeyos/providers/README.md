...
Registry and ABC for every inference provider HoneyOS knows about.
...
`$HONEYOS_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `honeyos.runtime/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `honeyos.runtime/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `honeyos.runtime/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `honeyos.runtime/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `honeyos.runtime/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$HONEYOS_HOME/plugins/model-providers/` for a private plugin).
