...
``{title, url, description, position}`` rows every other HoneyOS web
provider produces.
...
prefers HoneyOS-managed xAI Grok OAuth (via ``honeyos auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.honeyos/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``honeyos tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `honeyos auth` to sign in with "
