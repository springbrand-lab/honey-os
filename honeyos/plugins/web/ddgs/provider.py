...
``honeyos tools`` can prompt the user to install it.
...
GIL — so the whole HoneyOS process freezes through Ctrl+C/SIGTERM. Each search
...
def _plugins_path_entry() -> str:
    ...
    """
    try:
        import honeyos.plugins as plugins_pkg
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``honeyos tools`` probes.
    ...
    if _test_hook:
        env["HONEYOS_DDGS_ALLOW_TEST_HOOKS"] = "1"

    # Running the worker as a script puts ``plugins/web/ddgs/`` on ``sys.path[0]``,
    # which breaks ``import honeyos.plugins...``. Prepend the path entry that makes the
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        ``primp`` call cannot freeze the HoneyOS process (#36776, #68096).
        """
        try:
