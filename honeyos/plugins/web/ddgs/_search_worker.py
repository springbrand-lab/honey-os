...
Optional test hooks (only when ``HONEYOS_DDGS_ALLOW_TEST_HOOKS=1``)::
...
def main() -> int:
    ...
    if hook:
        if os.environ.get("HONEYOS_DDGS_ALLOW_TEST_HOOKS") != "1":
            _write_envelope(
    ...
    try:
        ...
        from honeyos.plugins.web.ddgs.provider import _run_ddgs_search
