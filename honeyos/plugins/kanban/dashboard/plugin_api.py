...
``honeyos.runtime.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__HONEYOS_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``honeyos.runtime/web_server.py``.

This means ``honeyos dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from honeyos.runtime import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``honeyos.runtime.web_server._ws_auth_ok``).
    ...
    """
    try:
        from honeyos.runtime import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``honeyos.runtime.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from honeyos.runtime import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from honeyos.runtime.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``HONEYOS_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from honeyos.runtime.kanban import _check_dispatcher_presence
                from honeyos.core.constants import get_honeyos_home

                # Scope the probe to the request's active home. The dashboard
                # backend can run under a different HONEYOS_HOME than the
...
from honeyos.runtime.kanban_db import (  # noqa: E402
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``honeyos kanban
...
# spawn failures, stuck-blocked). See honeyos.runtime.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``honeyos kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``honeyos kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``honeyos kanban specify <task_id>``.
    ...
    # HONEYOS_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from honeyos.runtime import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``honeyos kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.honeyos/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current HoneyOS profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.honeyos/profiles/*`` on disk and every
...
def model_options():
    ...
    Thin wrapper around ``honeyos.runtime.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of HoneyOS
...
def _resolve_project(ref: Optional[str]) -> tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    try:
        from honeyos.runtime import projects_db as pdb
...
def list_kanban_projects():
    """List first-class HoneyOS projects for board scoping.
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from honeyos.core.constants import get_honeyos_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_honeyos_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``honeyos profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from honeyos.runtime import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``honeyos kanban decompose <task_id>``.
    ...
    # HONEYOS_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from honeyos.runtime import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.honeyos/config.yaml.
