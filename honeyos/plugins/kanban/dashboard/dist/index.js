/**
 * HoneyOS Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.honeyos/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__HONEYOS_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__HONEYOS_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "honeyos-kanban-dot-triage",
    todo: "honeyos-kanban-dot-todo",
    ready: "honeyos-kanban-dot-ready",
    running: "honeyos-kanban-dot-running",
    blocked: "honeyos-kanban-dot-blocked",
    done: "honeyos-kanban-dot-done",
    archived: "honeyos-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-honeyos-task";
  ...
  const DOCS_URL = "https://honeyos-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://honeyos-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "honeyos.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="honeyos-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])honeyos-kanban-md-code\1/i.test(attrs)) {
      return ' class="honeyos-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "honeyos-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "honeyos-kanban-md",
  ...
  // standard `drop` event and our `honeyos-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("honeyos-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("honeyos-kanban-column--drop");
          if (target) target.classList.add("honeyos-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("honeyos-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("honeyos-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("honeyos-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__HONEYOS_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "honeyos-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "honeyos-kanban-attention",
        "honeyos-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "honeyos-kanban-attention-bar" },
        h("span", { className: "honeyos-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "honeyos-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "honeyos-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "honeyos-kanban-attention-row",
                  "honeyos-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "honeyos-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "honeyos-kanban-attention-row-id" }, task.id),
                h("span", { className: "honeyos-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "honeyos-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "honeyos-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "honeyos-kanban-diag-action-btn",
      action.suggested ? "honeyos-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " honeyos-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".honeyos-kanban-drawer-comment-row input, .honeyos-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "honeyos-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("honeyos-kanban-diag", sevClass) },
      h("div", { className: "honeyos-kanban-diag-header" },
        h("span", { className: "honeyos-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "honeyos-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "honeyos-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "honeyos-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "honeyos-kanban-diag-data-row" },
                  h("span", { className: "honeyos-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "honeyos-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "honeyos-kanban-diag-data-row" },
                h("span", { className: "honeyos-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "honeyos-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "honeyos-kanban-diag-reassign-row" },
            h("span", { className: "honeyos-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "honeyos-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "honeyos-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "honeyos-kanban-diag-msg",
              msg.ok ? "honeyos-kanban-diag-msg--ok" : "honeyos-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "honeyos-kanban-section" },
      h("div", { className: "honeyos-kanban-section-head-row" },
        h("span", { className: "honeyos-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "honeyos-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "honeyos-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "honeyos-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "honeyos-kanban-docs-link",
      title: "Open HoneyOS Kanban docs in a new tab",
      "aria-label": "HoneyOS Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `honeyos kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "honeyos-kanban-msg-ok" : "honeyos-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "honeyos-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "honeyos-kanban-boardswitcher" },
      h("div", { className: "honeyos-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "honeyos-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "honeyos-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "honeyos-kanban-dialog-title" },
        ...
        h("div", { className: "honeyos-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned HoneyOS profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "honeyos-kanban-bulk" },
      h("span", { className: "honeyos-kanban-bulk-count" },
      ...
      h("div", { className: "honeyos-kanban-bulk-priority",
      ...
      h("div", { className: "honeyos-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different HoneyOS profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "honeyos-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("honeyos-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("honeyos-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "honeyos-kanban-trash",
        dragOver ? "honeyos-kanban-trash--drop" : "",
        props.draggingTaskId ? "honeyos-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "honeyos-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "honeyos-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".honeyos-kanban-card")) return true;
      if (target.closest && target.closest(".honeyos-kanban-column-add")) return true;
      if (target.closest && target.closest(".honeyos-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("honeyos-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("honeyos-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".honeyos-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "honeyos-kanban-columns",
        isScrollable ? "honeyos-kanban-columns--scrollable" : "",
        isPanning ? "honeyos-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("honeyos-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("honeyos-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "honeyos-kanban-column",
        dragOver ? "honeyos-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "honeyos-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "honeyos-kanban-col-check",
        ...
        h("span", { className: cn("honeyos-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "honeyos-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "honeyos-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "honeyos-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "honeyos-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "honeyos-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "honeyos-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "honeyos-kanban-lane" },
                  h("div", { className: "honeyos-kanban-lane-head" },
                    h("span", { className: "honeyos-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "honeyos-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "honeyos-kanban-card--stale-red";
    if (age >= tier.amber) return "honeyos-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".honeyos-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "honeyos-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "honeyos-kanban-card",
        props.selected ? "honeyos-kanban-card--selected" : "",
        props.failed ? "honeyos-kanban-card--failed" : "",
        props.draggingSource ? "honeyos-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "honeyos-kanban-card-content" },
          h("div", { className: "honeyos-kanban-card-row" },
            h("label", {
              className: "honeyos-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "honeyos-kanban-card-check",
            ...
            h("span", { className: "honeyos-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or honeyos kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "honeyos-kanban-warning-badge",
                    "honeyos-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "honeyos-kanban-needs-assignee",
          ...
          h("div", { className: "honeyos-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "honeyos-kanban-card-row honeyos-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "honeyos-kanban-assignee",
                            title: `Assigned to HoneyOS profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "honeyos-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "honeyos-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "honeyos-kanban-ago",
  ...
  // centered modal reusing the honeyos-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "honeyos-kanban-dialog honeyos-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "HoneyOS profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "HoneyOS profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "honeyos-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "honeyos-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "honeyos-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "honeyos-kanban-drawer-close",
        ...
        data ? h("div", { className: "honeyos-kanban-drawer-comment-foot" },
          h("div", {
            className: "honeyos-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "honeyos-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "honeyos-kanban-section" },
      h("div", { className: "honeyos-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "honeyos-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "honeyos-kanban-drawer-body" },
      h("div", { className: "honeyos-kanban-drawer-title" },
        h("span", { className: cn("honeyos-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "honeyos-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "honeyos-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "honeyos-kanban-section" },
            h("div", { className: "honeyos-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "honeyos-kanban-section" },
            h("div", { className: "honeyos-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "honeyos-kanban-done-no-result honeyos-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "honeyos-kanban-section" },
            h("div", { className: "honeyos-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "honeyos-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "honeyos-kanban-section" },
        h("div", { className: "honeyos-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "honeyos-kanban-comment" },
            h("div", { className: "honeyos-kanban-comment-head" },
              h("span", { className: "honeyos-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "honeyos-kanban-diag-action-btn",
      ...
      h("div", { className: "honeyos-kanban-section" },
        h("div", { className: "honeyos-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "honeyos-kanban-comment" },
                h("div", { className: "honeyos-kanban-comment-head" },
                  h("span", { className: "honeyos-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "honeyos-kanban-comment-ago" },
      ...
      h("div", { className: "honeyos-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "honeyos-kanban-event",
              isDiag ? "honeyos-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "honeyos-kanban-event-header" },
                  h("span", { className: "honeyos-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "honeyos-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "honeyos-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "honeyos-kanban-event-header-plain" },
                  h("span", { className: "honeyos-kanban-event-kind" }, e.kind),
                  h("span", { className: "honeyos-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "honeyos-kanban-event-phantom-row" },
                  h("span", { className: "honeyos-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "honeyos-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "honeyos-kanban-event-payload" },
    ...
    return h("div", { className: "honeyos-kanban-section" },
      h("div", { className: "honeyos-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "honeyos-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `honeyos-kanban-run--${r.outcome || r.status || "ended"}`
          : "honeyos-kanban-run--active";
        return h("div", { key: r.id, className: cn("honeyos-kanban-run", outcomeClass) },
          h("div", { className: "honeyos-kanban-run-head" },
            h("span", { className: "honeyos-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "honeyos-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "honeyos-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "honeyos-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "honeyos-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "honeyos-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "honeyos-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "honeyos-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "honeyos-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "honeyos-kanban-pre honeyos-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "honeyos-kanban-meta-row" },
      h("span", { className: "honeyos-kanban-meta-label" }, props.label),
      h("span", { className: "honeyos-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "honeyos-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "honeyos-kanban-meta-row" },
        h("span", { className: "honeyos-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "honeyos-kanban-meta-value honeyos-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "honeyos-kanban-meta-row" },
        h("span", { className: "honeyos-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "honeyos-kanban-meta-value honeyos-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "honeyos-kanban-meta-row" },
        h("span", { className: "honeyos-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "honeyos-kanban-meta-value honeyos-kanban-editable",
    ...
    return h("div", { className: "honeyos-kanban-meta-row" },
      h("span", { className: "honeyos-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "honeyos-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "honeyos-kanban-section" },
      h("div", { className: "honeyos-kanban-section-head-row" },
        h("span", { className: "honeyos-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "honeyos-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "honeyos-kanban-section" },
      h("div", { className: "honeyos-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "honeyos-kanban-deps-row" },
        h("span", { className: "honeyos-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "honeyos-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "honeyos-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "honeyos-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "honeyos-kanban-dep-chip-x",
      ...
      h("div", { className: "honeyos-kanban-deps-row" },
        h("span", { className: "honeyos-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "honeyos-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "honeyos-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "honeyos-kanban-msg-ok"
          : "honeyos-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "honeyos-kanban-section" },
      ...
      h("div", { className: "honeyos-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "honeyos-kanban-home-sub honeyos-kanban-home-sub--on"
              : "honeyos-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__HONEYOS_PLUGINS__ && typeof window.__HONEYOS_PLUGINS__.register === "function") {
    window.__HONEYOS_PLUGINS__.register("kanban", KanbanPage);
  }
})();
