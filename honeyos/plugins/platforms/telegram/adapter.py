...
from honeyos.gateway.authz_mixin import _coerce_allow_set
...
from honeyos.plugins.platforms.telegram.telegram_ids import (
    normalize_telegram_chat_id,
)
from honeyos.plugins.platforms.telegram.telegram_network import (
    TelegramFallbackTransport,
    discover_fallback_ips,
    parse_fallback_ip_env,
)
from honeyos.utils import atomic_replace, env_float, env_int
...
# trigger its own recovery path. Refs: NousResearch/honeyos-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: NousResearch/honeyos-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("HONEYOS_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "HONEYOS_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "HONEYOS_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. HoneyOS-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on HoneyOS' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        HoneyOS must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _try_send_rich(
        ...
        if message_id is not None:
            ...
            try:
                from honeyos.gateway import rich_sent_store
        ...
        try:
            if app and app.updater and app.updater.running:
                try:
                    ...
                    # Refs: NousResearch/honeyos-agent#58270
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `honeyos update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other HoneyOS or OpenClaw instance is running "
            "with this token, then restart the gateway with 'honeyos gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                from honeyos.runtime.commands import telegram_menu_commands, telegram_menu_max_commands
                if not self._bot:
                    return
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  HoneyOS defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the HoneyOS process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("HONEYOS_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("HONEYOS_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("HONEYOS_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("HONEYOS_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("HONEYOS_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            from honeyos.gateway.platforms._http_client_limits import platform_httpx_limits
            ...
            disable_fallback = (os.getenv("HONEYOS_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("HONEYOS_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set HONEYOS_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"HONEYOS_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set HONEYOS_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``honeyos model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from honeyos.runtime.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from honeyos.runtime.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.honeyos/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".honeyos" / "scripts" / "gmail-triage" / script_name
    ...
    async def send_image(
        ...
        try:
            ...
            try:
                ...
                from honeyos.tools.url_safety import create_ssrf_safe_async_client
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@honeyos_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several HoneyOS bot profiles. A message like
    ...
    async def _cache_observed_media(self, msg: Message, event: MessageEvent) -> None:
        ...
        """
        from honeyos.gateway.platforms.base import cache_media_bytes
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the HoneyOS inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the HoneyOS inbox (#52363).
        #
    ...
    async def _handle_sticker(self, msg: Message, event: "MessageEvent") -> None:
        ...
        """
        from honeyos.gateway.sticker_cache import (
...
# honeyos.runtime/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("HONEYOS_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
            from honeyos.gateway.config import load_gateway_config
            from honeyos.gateway.run import cfg_get
...
async def _standalone_send(
    ...
    *,
    ...
    from honeyos.tools.send_message_tool import _send_telegram
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in honeyos.runtime/gateway.py.
    """
    from honeyos.runtime import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `honeyos setup` to install Telegram support.",
