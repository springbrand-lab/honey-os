"""
Microsoft Teams platform adapter for HoneyOS.
...
class TeamsSummaryWriter:
    ...
    def _build_graph_client(self, config: dict[str, Any]) -> Any:
        ...
        from honeyos.tools.microsoft_graph_auth import MicrosoftGraphTokenProvider
        from honeyos.tools.microsoft_graph_client import MicrosoftGraphClient
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    HoneyOS owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "HoneyOS"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; HoneyOSAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        from honeyos.tools.approval import resolve_gateway_approval, has_blocking_approval

        action = ctx.activity.value.action
        data = action.data or {}
        honeyos_action = data.get("honeyos_action", "")
        session_key = data.get("session_key", "")

        if not honeyos_action or not session_key:
        ...
        choice = choice_map.get(honeyos_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="honeyos_approve",
            data={**btn_data_base, "honeyos_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="honeyos_approve",
                data={**btn_data_base, "honeyos_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="honeyos_approve",
                    data={**btn_data_base, "honeyos_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="honeyos_approve",
            data={**btn_data_base, "honeyos_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"HoneyOS\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.honeyos/.env")
