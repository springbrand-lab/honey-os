...
from honeyos.utils import env_float
...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("HONEYOS_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("HONEYOS_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
...
_QR_CODE_PAGE = "https://work.weixin.qq.com/ai/qc/gen?source=honeyos&scode="
...
def qr_scan_for_bot_info(
    *,
    ...
    generate_url = f"{_QR_GENERATE_URL}?source=honeyos"

    # ── Step 1: Fetch QR code ──
    print("  Connecting to WeCom...", end="", flush=True)
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "HoneyOSAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "HoneyOSAgent/1.0"})
...
# wizard + _PLATFORMS["wecom"] static dict in honeyos.runtime/gateway.py, and the
...
def interactive_setup() -> None:
    ...
    Replaces honeyos.runtime/gateway.py::_setup_wecom and the static
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("Approve with: honeyos pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("Direct messages disabled.")
        else:
            print_info("Skipped — configure later with 'honeyos gateway setup'")
...
def _build_callback_adapter(config):
    ...
    from honeyos.plugins.platforms.wecom.callback_adapter import WecomCallbackAdapter
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `honeyos setup` to install WeCom support.",
    ...
    from honeyos.plugins.platforms.wecom.callback_adapter import check_wecom_callback_requirements
    ctx.register_platform(
        name="wecom_callback",
