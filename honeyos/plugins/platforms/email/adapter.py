"""
Email platform adapter for the HoneyOS gateway.

Allows users to interact with HoneyOS by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "honeyos-agent" "version" "{_honeyos_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `honeyos config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `honeyos gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "HoneyOS")
        ...
        msg_id = f"<honeyos-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# honeyos.runtime/gateway.py, and the _send_email dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        try:
            from honeyos.tools.send_message_tool import _error as _e
