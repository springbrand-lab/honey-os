# A2A — Agent-to-Agent protocol for HoneyOS
...
peer (another HoneyOS, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
honeyos gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, HoneyOS serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.honeyos/a2a_audit.jsonl`.
- Conversations persist to `~/.honeyos/a2a_conversations/` — they survive context
