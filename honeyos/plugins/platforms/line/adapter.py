"""
LINE Messaging API platform adapter for HoneyOS.
...
to HoneyOS. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``HONEYOS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            honeyos_home = Path(get_honeyos_home()).resolve()
        except Exception:
            honeyos_home = Path.home().joinpath(".honeyos").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``honeyos status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``honeyos status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``honeyos setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.honeyos/.env`` via ``honeyos.runtime.config``.
    """
    print()
    ...
    try:
        from honeyos.runtime.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("honeyos.runtime.config not available; set LINE_* vars manually in ~/.honeyos/.env")
        return
