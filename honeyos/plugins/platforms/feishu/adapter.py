...
In single-bot mode (what HoneyOS currently supports), open_id works as a
...
from honeyos.gateway.status import acquire_scoped_lock, release_scoped_lock
...
def _strip_markdown_to_plain_text(text: str) -> str:
    ...
    horizontal rules, \\r\\n normalisation).
    """
    from honeyos.gateway.platforms.helpers import strip_markdown
    plain = text.replace("\r\n", "\n")
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_honeyos_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("HONEYOS_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "HONEYOS_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "HONEYOS_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("HONEYOS_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("HONEYOS_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "HONEYOS_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="honeyos-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local HoneyOS gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``honeyos_action`` in their value dict so that
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "honeyos_update_prompt_action": answer,
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_honeyos_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that HoneyOS does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        ...
        """
        from honeyos.plugins.platforms.feishu.feishu_comment import handle_drive_comment_event
    ...
    def _on_meeting_invited_event(self, data: Any) -> None:
        ...
        from honeyos.plugins.platforms.feishu.feishu_meeting_invite import handle_meeting_invited_event
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        honeyos_action = action_value.get("honeyos_action") if isinstance(action_value, dict) else None
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("honeyos_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("honeyos_update_prompt_action", "") or "").strip().lower()
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by HoneyOS webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto HoneyOS' SessionSource fields.
    ...
    async def _connect_websocket(self) -> None:
        ...
        self._ws_client = FeishuWSClient(
            ...
            # See https://github.com/NousResearch/honeyos-agent/issues/50656
...
# Called by `honeyos gateway setup` via _setup_feishu() in honeyos.runtime/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=honeyos&tp=honeyos"
    else:
        qr_url += "?from=honeyos&tp=honeyos"
    return {
...
# dict in honeyos.runtime/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `honeyos setup` to install Feishu support."}
...
def interactive_setup(*, private_only: bool = False) -> None:
    ...
    Replaces the central _setup_feishu in honeyos.runtime/gateway.py and the static
    ...
    from honeyos.runtime.setup import prompt_choice
    ...
    if private_only:
        ...
        if access_idx == 0:
            ...
            save_env_value("FEISHU_ALLOWED_USERS", "")
            print_success("DM pairing enabled.")
            print_info("Unknown users can request access; approve with `honeyos pairing approve`.")
        elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `honeyos setup` to install Feishu support.",
