"""ntfy platform adapter (HoneyOS plugin).
...
a HoneyOS dependency.

This adapter ships as a HoneyOS platform plugin under
``plugins/platforms/ntfy/``. The HoneyOS plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "honeyos-in"              # subscribe topic (incoming)
          publish_topic: "honeyos-out"     # optional — defaults to topic
...
_ECHO_TAG = "honeyos-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``honeyos cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a HoneyOS dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `honeyos gateway status` without
