...
# ``model_tools`` import time, so ``honeyos status``, ``honeyos chat``, etc.
...
# ``honeyos_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "HoneyOS is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a honeyos MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "honeyos-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${HONEYOS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from honeyos.core.constants import get_honeyos_home as _get_honeyos_home
            _honeyos_home = _get_honeyos_home()
        except (ModuleNotFoundError, ImportError):
            _honeyos_home = _Path.home() / ".honeyos"
        self._thread_count_store = _ThreadCountStore(
            _honeyos_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "HoneyOS is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HONEYOS_HOME", str(_Path.home() / ".honeyos"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${HONEYOS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a honeyos MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'HoneyOS is thinking…' marker message.
        ...
        "HoneyOS is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "HoneyOS is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "HoneyOS is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "HoneyOS is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``honeyos setup``.

    The setup wizard at ``honeyos.runtime/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.honeyos/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. honeyos-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.honeyos/.env")
    print_info("Restart the gateway: honeyos gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``honeyos cron`` running as a
    separate process from ``honeyos gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the HoneyOS plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `honeyos setup` to install Google Chat support.",
        ...
        # the "HoneyOS is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'HoneyOS is thinking…' marker message "
