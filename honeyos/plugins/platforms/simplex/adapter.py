"""SimpleX Chat platform adapter (HoneyOS plugin).
...
This adapter ships as a HoneyOS platform plugin under
``plugins/platforms/simplex/``. The HoneyOS plugin loader scans the
...
Optional environment variables:
    ...
    HONEYOS_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``honeyos setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside HoneyOS — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("HONEYOS_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``honeyos setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.honeyos/.env`` via
    ...
    try:
        ...
        print(
            "honeyos.runtime.config not available; set SIMPLEX_* vars manually in "
            "~/.honeyos/.env"
        )
        return
