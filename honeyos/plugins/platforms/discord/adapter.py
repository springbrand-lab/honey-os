...
from honeyos.agent.async_utils import (
    consume_detached_task_result as _consume_background_task_result,
)
from honeyos.agent.display import ToolPreview
...
from honeyos.gateway.platforms.helpers import (
    MessageDeduplicator,
    ThreadParticipationTracker,
    convert_table_to_bullets,
)
from honeyos.utils import atomic_json_write, env_float, env_int
...
from honeyos.tools.url_safety import is_safe_url
...
def _scoped_gate_env(name: str, default: str = "") -> str:
    ...
    try:
        from honeyos.gateway.authz_mixin import _platform_gate_env
...
def _profile_scoped_config_load() -> bool:
    ...
    """
    try:
        from honeyos.agent.secret_scope import current_secret_scope, is_multiplex_active
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("HONEYOS_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid HONEYOS_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.honeyos/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("HONEYOS_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("HONEYOS_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="HONEYOS_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="HONEYOS_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        from honeyos.core.constants import get_honeyos_home
        from honeyos.plugins.platforms.discord.recovery import DiscordRecoveryStore
        self._discord_recovery_store = DiscordRecoveryStore(get_honeyos_home())
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the HoneyOS gateway
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            from honeyos.gateway.platforms.base import resolve_proxy_url, proxy_kwargs_for_bot
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``HONEYOS_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("HONEYOS_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_honeyos_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage HoneyOS response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs HoneyOS work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit HoneyOS/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:honeyos|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields HoneyOS manages."""
                ...
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        try:
                            ...
                            from honeyos.gateway.platforms.base import resolve_proxy_url, proxy_kwargs_for_aiohttp
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "honeyos_voice",
    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        from honeyos.tools.voice_mode import is_whisper_hallucination
        ...
        try:
            ...
            from honeyos.tools.transcription_tools import transcribe_audio
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit HoneyOS pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your HoneyOS session")
        ...
        @tree.command(name="status", description="Show HoneyOS session status")
        ...
        @tree.command(name="stop", description="Stop the running HoneyOS agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.honeyos/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update HoneyOS to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the HoneyOS gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a HoneyOS session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to HoneyOS in the thread",
        ...
        # honeyos.runtime/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from honeyos.runtime.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from honeyos.runtime.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a HoneyOS skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from honeyos.runtime.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new HoneyOS session in the thread
    ...
    def _resolve_channel_skills(self, channel_id: str, parent_id: str | None = None) -> list[str] | None:
        ...
        """
        from honeyos.gateway.platforms.base import resolve_channel_skills
        return resolve_channel_skills(self.config.extra, channel_id, parent_id)

    def _resolve_channel_prompt(self, channel_id: str, parent_id: str | None = None) -> str | None:
        """Resolve a Discord per-channel prompt, preferring the exact channel over its parent."""
        from honeyos.gateway.platforms.base import resolve_channel_prompt
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by HoneyOS: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        HoneyOS has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "HoneyOS"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_honeyos_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by HoneyOS: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        from honeyos.gateway.platforms.base import utf16_len, _prefix_within_utf16_limit
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="HoneyOS semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "HoneyOS session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 HoneyOS handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want HoneyOS to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ HoneyOS needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **HoneyOS needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``honeyos update --gateway``
        ...
        *,
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ HoneyOS could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_honeyos_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``honeyos pairing approve``).
    ...
    # so users approved via ``honeyos pairing approve`` can interact with
...
def _resolve_exec_approval_admin_gate(
    ...
    if not require_admin:
        return (False, set())
    try:
        from honeyos.gateway.slash_access import _coerce_id_list
...
def _define_discord_view_classes() -> None:
    ...
    class SlashConfirmView(discord.ui.View):
        ...
        async def _resolve(
            ...
            try:
                from honeyos.tools import slash_confirm as _slash_confirm_mod
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``honeyos update`` prompts.
    ...
    class ClarifyChoiceView(discord.ui.View):
        ...
        async def _resolve_choice(
            ...
            try:
                from honeyos.tools.clarify_gateway import _entries as _clarify_entries  # type: ignore
            ...
            try:
                from honeyos.tools.clarify_gateway import resolve_gateway_clarify
        ...
        async def _on_other(self, interaction: "discord.Interaction") -> None:
            ...
            try:
                from honeyos.tools.clarify_gateway import mark_awaiting_text
...
# is not in this process (e.g. ``honeyos cron`` running standalone) and no live
    ...
    *,
    ...
    try:
        ...
        if thread_id:
            ...
            _channel_type = None
            try:
                from honeyos.gateway.channel_directory import lookup_channel_type
...
def interactive_setup() -> None:
    ...
    """
    from honeyos.runtime.config import get_env_value, remove_env_value, save_env_value
    from honeyos.runtime.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "HONEYOS_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "HONEYOS_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``honeyos.runtime.gateway.get_env_value`` at call time (not via
    ...
    """
    import honeyos.runtime.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the HoneyOS plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `honeyos setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # honeyos.runtime/setup.py::_setup_discord function.  Same shape as Teams.
