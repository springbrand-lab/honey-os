"""
``honeyos photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other HoneyOS
...
# Writable sidecar runtime dir (mirrors to HONEYOS_HOME on immutable
...
# wiring imports this module for `honeyos --help`).
...
# when called later (gateway start, honeyos status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `honeyos photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'HoneyOS')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``honeyos photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "HoneyOS" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.honeyos/.env,
    ...
    try:
        from honeyos.runtime.config import write_platform_config_field
    ...
    print("  Start the gateway:  honeyos gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `honeyos photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`honeyos photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.honeyos/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `honeyos photon telemetry on` / `honeyos photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.honeyos/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  honeyos gateway restart")
    return 0
...
#
# `honeyos gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``honeyos photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `honeyos gateway setup` wizard."""
