...
Credential storage mirrors every other HoneyOS channel:

    * runtime SDK creds  -> ``~/.honeyos/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.honeyos/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a honeyos dependency
...
# registers HoneyOS as its own client_id.
...
# Default name of the project HoneyOS provisions for the operator.
DEFAULT_PROJECT_NAME = "HoneyOS"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of honeyos-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.honeyos/auth.json`` honouring the active HoneyOS profile."""
    try:
        from honeyos.core.constants import get_honeyos_home
        return Path(get_honeyos_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.honeyos")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # honeyos.runtime/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from honeyos.runtime.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.honeyos/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.honeyos/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.honeyos/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: honeyos.runtime.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `honeyos photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `honeyos photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `honeyos photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `honeyos photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `honeyos photon setup`)"
