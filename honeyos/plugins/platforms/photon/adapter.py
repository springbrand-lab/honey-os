"""
Photon Spectrum (iMessage) platform adapter for HoneyOS.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a HoneyOS dep
...
from honeyos.gateway.platforms.helpers import compile_mention_patterns, strip_markdown
...
#
# Out-of-process senders (cron subprocesses, `honeyos send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_honeyos_home() honors profile overrides — never hardcode ~/.honeyos.
    from honeyos.core.constants import get_honeyos_home

    return get_honeyos_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (honeyos status, test collection, plugin discovery).
...
# artifacts so HoneyOS sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `honeyos photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `honeyos photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `honeyos setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, honeyos status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: honeyos photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: honeyos photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `honeyos update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``honeyos photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so HoneyOS does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use HoneyOS defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-HoneyOS-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any HoneyOS checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `honeyos photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `honeyos photon setup`)"
                )
        # A `honeyos update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `honeyos send`) can reach this sidecar
    ...
    async def send_image(
        ...
        try:
            from honeyos.gateway.platforms.base import cache_image_from_url
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`honeyos send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the HoneyOS gateway (which spawns the sidecar and "
                    "records its address under <honeyos-home>/runtime/"
    ...
    headers = {"X-HoneyOS-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the HoneyOS plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `honeyos photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: honeyos photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `honeyos gateway setup` alongside every other
    ...
    # Register CLI subcommands — `honeyos photon ...`
