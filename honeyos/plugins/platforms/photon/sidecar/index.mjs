// HoneyOS — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> HoneyOS): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (HoneyOS -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-HoneyOS-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `honeyos photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so HoneyOS can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "honeyos-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply HoneyOS'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local HoneyOS allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of HoneyOS, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-honeyos-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when HoneyOS supplied them so a known-good
