...
Small Node helper that bridges HoneyOS to Photon's Spectrum SDK
(`spectrum-ts`).  HoneyOS is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-HoneyOS-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and HoneyOS can receive inbound messages
...
The HoneyOS plugin's `honeyos photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. HoneyOS is Python, so replies go through this sidecar
