...
This plugin connects HoneyOS to iMessage (and other Spectrum
...
TypeScript-only, HoneyOS runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other HoneyOS channel,
...
Run `honeyos photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.honeyos/.env` (the same place every other
...
Management metadata lives in `~/.honeyos/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `honeyos photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, HoneyOS
  ...
  `.pluginPayloadAttachment` images immediately after the URL; HoneyOS coalesces
...
- **Native polls are supported.** HoneyOS posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `honeyos send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<honeyos-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `honeyos photon status`, a DM and a group roundtrip,
