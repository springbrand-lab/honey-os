...
under an immutable ``/opt/honeyos`` (read-only for the honeyos user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$HONEYOS_HOME/photon/sidecar`` (the durable data volume,
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".honeyos-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_honeyos_home() / "photon" / "sidecar"
    try:
