...
from honeyos.gateway.platforms.helpers import MessageDeduplicator
...
# identify HoneyOSAgent traffic — matching other HoneyOS outbound surfaces
# that already set ``HoneyOSAgent/<version>`` for platform-partner attribution.
try:
    from honeyos.runtime import __version__ as _HONEYOS_VERSION
except Exception:
    _HONEYOS_VERSION = "unknown"
_HONEYOS_SLACK_USER_AGENT_PREFIX = f"HoneyOSAgent/{_HONEYOS_VERSION}"
...
def check_slack_requirements() -> bool:
    ...
    from honeyos.tools.lazy_deps import ensure_and_bind
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from honeyos.runtime.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `honeyos slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `honeyos gateway setup` "
                "or in the active profile's ~/.honeyos/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `honeyos gateway setup` "
                "or add it to your active profile's ~/.honeyos/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `honeyos gateway setup` "
                "or in the active profile's ~/.honeyos/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `honeyos gateway setup` "
                "or add it to your active profile's ~/.honeyos/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_honeyos_home() / "slack_tokens.json"
        if tokens_file.exists():
            try:
                # Warn if the token file is world- or group-readable — it
                # contains plaintext bot tokens for all saved workspaces.
                from honeyos.utils import warn_if_credential_file_broadly_readable
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_HONEYOS_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # HoneyOS has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by HoneyOS can "
                    "be removed from the Slack app manifest via "
                    "`honeyos slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /honeyos prefix). A single regex
            ...
            # manifest (see `honeyos slack manifest`). In Socket Mode, Slack
            ...
            from honeyos.runtime.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/honeyos$")

            @self._app.command(_slash_pattern)
            async def handle_honeyos_command(ack, command):
            ...
            for _action_id in (
                "honeyos_approve_once",
                "honeyos_approve_session",
                "honeyos_approve_always",
                "honeyos_deny",
            ):
            ...
            for _action_id in (
                "honeyos_confirm_once",
                "honeyos_confirm_always",
                "honeyos_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("honeyos_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^honeyos_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("honeyos_clarify_other")(self._handle_clarify_action)
            ...
            try:
                from honeyos.runtime.plugins import get_plugin_manager
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'honeyos slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: HoneyOS handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own HoneyOS session — matching the per-thread behavior channels already
    ...
    async def send_multiple_images(
        ...
        try:
            ...
            from honeyos.gateway.platforms.base import _ssrf_redirect_guard
    ...
    async def send_image(
        ...
        try:
            async def _ssrf_redirect_guard(response):
                ...
                from honeyos.tools.url_safety import redirect_target_from_response
    ...
    async def _handle_app_context_changed(
        ...
        # HoneyOS owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_honeyos_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_honeyos_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_honeyos_no_thread_response"):
        ...
        force_process = bool(event.get("_honeyos_force_process"))
        ...
        # the shape produced by peer HoneyOS agents in Socket Mode on some
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "honeyos_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "honeyos_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "honeyos_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "honeyos_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "honeyos_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "honeyos_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``honeyos_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``honeyos_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"honeyos_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "honeyos_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "honeyos_confirm_once": "once",
            "honeyos_confirm_always": "always",
            "honeyos_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "honeyos_approve_once": "once",
            "honeyos_approve_session": "session",
            "honeyos_approve_always": "always",
            "honeyos_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "honeyos_clarify_other" or token == "other":
    ...
    async def _format_thread_context(
        ...
        *,
        ...
        from honeyos.gateway.session import neutralize_untrusted_inline_text
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/honeyos <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/honeyos
        ...
        if slash_name in {"honeyos", ""}:
            # Legacy /honeyos <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from honeyos.runtime.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/honeyos   ``).
        ...
        # "Running /cmd…" ack shown by handle_honeyos_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/honeyos <question>" must produce public replies so
    ...
    async def _download_slack_file(
        ...
        import httpx
        from honeyos.gateway.platforms.base import _ssrf_redirect_guard, safe_url_for_log
        from honeyos.tools.url_safety import create_ssrf_safe_async_client, is_safe_url
        ...
        async with create_ssrf_safe_async_client(
            ...
            for attempt in range(3):
                try:
                    ...
                    if audio:
                        from honeyos.gateway.platforms.base import cache_audio_from_bytes

                        return cache_audio_from_bytes(response.content, ext)
                    else:
                        from honeyos.gateway.platforms.base import cache_image_from_bytes
...
# ``honeyos.runtime/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _resolve_slack_user_dm(token: str, user_id: str) -> Optional[str]:
    ...
    try:
        ...
        return None
    try:
        from honeyos.gateway.platforms.base import proxy_kwargs_for_aiohttp
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_honeyos_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``honeyos.runtime/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under HONEYOS_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from honeyos.runtime.slack_cli import _build_full_manifest
            from honeyos.core.constants import get_honeyos_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="HoneyOS",
                bot_description="Your HoneyOS agent on Slack",
            )
            target = Path(get_honeyos_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `honeyos slack manifest --write` anytime to refresh after "
                "HoneyOS adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `honeyos update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``honeyos.runtime.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `honeyos setup` to install Slack support.",
        # Interactive setup wizard — replaces honeyos.runtime/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in honeyos.runtime/gateway.py.
