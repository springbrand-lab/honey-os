...
from honeyos.gateway.platforms.helpers import ThreadParticipationTracker
...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable HoneyOS command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    HoneyOS accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal HoneyOS ``/command`` text."""
...
# Uses get_honeyos_home() so each profile gets its own Matrix store.
from honeyos.core.constants import get_honeyos_dir as _get_honeyos_dir

_STORE_DIR = _get_honeyos_dir("platforms/matrix/store", "matrix/store")
...
def check_matrix_requirements() -> bool:
    ...
    try:
        from honeyos.tools.lazy_deps import feature_missing, ensure_and_bind
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches HoneyOS
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("HONEYOS_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("HONEYOS_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "honeyos"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "HoneyOS" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in honeyos.runtime/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"honeyos_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces honeyos.runtime/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        try:
            from honeyos.tools.lazy_deps import ensure as _lazy_ensure, feature_missing
        ...
        print_info("📬 Home Room: where HoneyOS delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via honeyos.runtime.gateway.get_env_value so setup-status
