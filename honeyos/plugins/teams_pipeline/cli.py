...
from honeyos.plugins.teams_pipeline.meetings import (
    enrich_meeting_with_call_record,
    fetch_preferred_transcript_text,
    list_recording_artifacts,
    resolve_meeting_reference,
)
from honeyos.plugins.teams_pipeline.models import GraphSubscription
from honeyos.plugins.teams_pipeline.pipeline import TeamsMeetingPipeline
from honeyos.plugins.teams_pipeline.store import TeamsPipelineStore, resolve_teams_pipeline_store_path
from honeyos.plugins.teams_pipeline.subscriptions import (
    build_graph_client,
    maintain_graph_subscriptions,
)
from honeyos.tools.microsoft_graph_auth import MicrosoftGraphConfigError, MicrosoftGraphTokenProvider
...
def _graph_setup_hint() -> str:
    return f"""
  Microsoft Graph is not configured. Add these to {display_honeyos_home()}/.env:
