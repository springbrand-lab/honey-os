...
from honeyos.plugins.teams_pipeline.subscriptions import build_graph_client
...
def build_pipeline_runtime_config(gateway_config: Any) -> dict[str, Any]:
    """Build pipeline config from honeyos.gateway platform config.
...
def build_pipeline_runtime(gateway: Any) -> TeamsMeetingPipeline:
    ...
    if teams_config and teams_config.enabled and teams_delivery.get("enabled"):
        try:
            from honeyos.plugins.platforms.teams.adapter import TeamsSummaryWriter
        except ImportError:
            logger.debug(
