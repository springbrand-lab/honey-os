...
session-scoped state file under ``$HONEYOS_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $HONEYOS_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_honeyos_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this honeyos install, leave it first —
    we enforce single-active-meeting semantics.

    Returns a dict summarizing the started bot.
    """
    from honeyos.plugins.google_meet.meet_bot import _is_safe_meet_url, _meeting_id_from_url
    ...
    env = os.environ.copy()
    env["HONEYOS_MEET_URL"] = url
    env["HONEYOS_MEET_OUT_DIR"] = str(out)
    env["HONEYOS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["HONEYOS_MEET_HEADED"] = "1"
    if auth_state:
        env["HONEYOS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["HONEYOS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if HONEYOS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["HONEYOS_MEET_MODE"] = mode
    if realtime_model:
        env["HONEYOS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["HONEYOS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["HONEYOS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    ...
    # meet_bot checks HONEYOS_MEET_REALTIME_KEY before OPENAI_API_KEY.
    if not realtime_api_key:
        try:
            from honeyos.agent.secret_scope import get_secret

            realtime_api_key = (
                get_secret("HONEYOS_MEET_REALTIME_KEY")
                or get_secret("OPENAI_API_KEY")
            )
        except ImportError:  # pragma: no cover — secret_scope is in-repo
            pass
    if realtime_api_key:
        env["HONEYOS_MEET_REALTIME_KEY"] = realtime_api_key
