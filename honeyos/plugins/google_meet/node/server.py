...
Launched by ``honeyos meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$HONEYOS_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``honeyos meet node approve <name> <url> <token>``.
...
from honeyos.plugins.google_meet.node import protocol as _proto


def _default_token_path() -> Path:
    return Path(get_honeyos_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "honeyos-meet-node",
    ) -> None:
        self.host = host
        self.port = port
