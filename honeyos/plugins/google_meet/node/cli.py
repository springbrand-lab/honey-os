"""`honeyos meet node ...` subcommand tree.

Wired into the existing ``honeyos meet`` parser by the plugin's top-level
...
from honeyos.plugins.google_meet.node.server import NodeServer


def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Add ``run / list / approve / remove / status / ping`` subparsers.

    *subparser* is the ``honeyos meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="honeyos-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``honeyos meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             honeyos meet node approve <name> ws://<host>:{args.port} {token}")
        try:
