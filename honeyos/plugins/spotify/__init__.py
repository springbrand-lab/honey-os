...
``_check_spotify_available()`` — when the user has not run ``honeyos auth
spotify``, the tools remain registered (so they appear in ``honeyos
...
The Spotify auth flow (``honeyos auth spotify``), CLI plumbing, and docs
are unchanged. This move is purely structural.
"""

from __future__ import annotations

from honeyos.plugins.spotify.tools import (
