...
def _xai_user_agent() -> str:
    try:
        ...
        return honeyos_xai_user_agent()
    except Exception:
        return "honeyos-agent/video_gen"
...
def _raise_if_blocked_local_input(ref: str) -> None:
    """Refuse to read a local media path that HoneyOS' read deny-list blocks.
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``honeyos.runtime/tools_config.py``) so the picker doesn't blindly
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `honeyos auth add xai-oauth` "
...
async def _submit_xai_video_payload(
    *,
    ...
    try:
        from honeyos.tools.xai_http import (
        ...
        storage_options = build_xai_storage_options(
            "video_gen",
            filename_prefix="honeyos-xai-video",
            extension="mp4",
        )
