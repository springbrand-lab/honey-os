...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      HONEYOS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      HONEYOS_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      HONEYOS_DASHBOARD_BASIC_AUTH_SECRET
      HONEYOS_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
def hash_password(password: str) -> str:
    ...
    ``python -c "from honeyos.plugins.dashboard_auth.basic import hash_password;
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "HONEYOS_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or HONEYOS_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "HONEYOS_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "HONEYOS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "HONEYOS_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "HONEYOS_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "HONEYOS_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # HONEYOS_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "HONEYOS_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
