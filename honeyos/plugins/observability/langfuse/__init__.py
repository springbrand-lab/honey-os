"""langfuse — HoneyOS plugin for Langfuse observability.

Traces HoneyOS conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the HoneyOS plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``honeyos plugins enable
observability/langfuse`` or ``honeyos tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``honeyos tools`` or ~/.honeyos/.env):
  HONEYOS_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  HONEYOS_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  HONEYOS_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  HONEYOS_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  HONEYOS_LANGFUSE_RELEASE     - release/version tag
  HONEYOS_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  HONEYOS_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  HONEYOS_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("HONEYOS_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the HoneyOS plugin system —
    ...
    public_key = _env("HONEYOS_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("HONEYOS_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("HONEYOS_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("HONEYOS_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset HONEYOS_LANGFUSE_PUBLIC_KEY / HONEYOS_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("HONEYOS_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("HONEYOS_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("HONEYOS_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("HONEYOS_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid HONEYOS_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _trace_key(
    ...
    *,
    ...
    Older HoneyOS paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("HONEYOS_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _usage_and_cost(response: Any, *, provider: str, api_mode: str, model: str, base_url: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    try:
        ...
        if cost.amount_usd is not None:
            ...
            try:
                from honeyos.agent.usage_pricing import get_pricing_entry
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="HoneyOS turn",
                tags=["honeyos", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older HoneyOS branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current HoneyOS also has a turn-scoped
    ...
    # messages is a list only for legacy HoneyOS branches that fired
    # pre_llm_call with API messages directly. Current HoneyOS fires
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if getattr(response, "usage", None) is not None:
        ...
        try:
            from honeyos.agent.usage_pricing import CanonicalUsage, estimate_usage_cost, get_pricing_entry
            from decimal import Decimal
            _ONE_M = Decimal("1000000")
            _cu = CanonicalUsage(
                input_tokens=_input,
...
def register(ctx) -> None:
    ...
    # HoneyOS versions.  pre_api_request / post_api_request fire per API
