...
Auto-tracks and cleans up ephemeral files created during HoneyOS
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HONEYOS_HOME` and `/tmp/honeyos-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HONEYOS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under HONEYOS_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `HONEYOS_HOME` or `/tmp/honeyos-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HONEYOS_HOME/disk-cleanup/` is itself excluded
- `$HONEYOS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
