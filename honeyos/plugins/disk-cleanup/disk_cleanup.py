"""disk_cleanup — ephemeral file cleanup for HoneyOS.
...
Rules:
  ...
  - empty dirs    → always delete (under HONEYOS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly HONEYOS_HOME and /tmp/honeyos-*
Never touches: ~/.honeyos/logs/ or any system directory.
"""
...
try:
    ...
    def get_honeyos_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("HONEYOS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".honeyos").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$HONEYOS_HOME/logs/``."""
    return get_honeyos_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$HONEYOS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HONEYOS_HOME or ``/tmp/honeyos-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    honeyos_home = get_honeyos_home()
    try:
        path.resolve().relative_to(honeyos_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/honeyos-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("honeyos-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "honeyos-agent", "backups", "profiles", ".worktrees",
...
# Paths under $HONEYOS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so HONEYOS_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        honeyos_home = get_honeyos_home()
        for parent in ("cron", "cronjobs"):
            base = honeyos_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside HONEYOS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HONEYOS_HOME, but never recurse into known
    # durable state trees.  Some installs place the HoneyOS checkout, venv,
    # and desktop build under HONEYOS_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    honeyos_home = get_honeyos_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in honeyos_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    honeyos_home = get_honeyos_home()
    try:
        rel = path.resolve().relative_to(honeyos_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "honeyos-agent",
        ...
        # Path isn't under HONEYOS_HOME (e.g. /tmp/honeyos-*) — fall through.
        pass
