...
Resolution order for config file:
  1. $HONEYOS_HOME/honcho.json  (instance-local, enables isolated HoneyOS instances)
...
from honeyos.runtime.profiles import _get_default_honeyos_home
from honeyos.plugins.plugin_utils import SingletonSlot
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a HoneyOS profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active HoneyOS profile.

    Resolution order:
      1. HONEYOS_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``honeyos_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"honeyos"`` (default profile)
    """
    explicit = os.environ.get("HONEYOS_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named HoneyOS
    # profile share that same host.  Keep named profiles isolated; only the
    # default HoneyOS profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $HONEYOS_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.honeyos/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_honeyos_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_honeyos_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.honeyos
    ...
    def from_global_config(
        ...
        Resolution: $HONEYOS_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active HoneyOS profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.honeyos block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          1. Gateway session key (stable per-chat identifier from honeyos.gateway platforms)
          2. per-session strategy — HoneyOS session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. HoneyOS session title (from /title command; non-per-session)
...
def _apply_fresh_oauth_token(config: HonchoClientConfig) -> None:
    ...
    """
    try:
        from honeyos.plugins.memory.honcho import oauth
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'honeyos honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            from honeyos.tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `honeyos honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                honeyos_cfg = load_config()
                honcho_cfg = honeyos_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
