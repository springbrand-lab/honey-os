...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/honeyos>
...
```bash
honeyos memory setup honcho   # configure Honcho directly (works on a fresh install)
honeyos memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
honeyos config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.honeyos/.env
```

> `honeyos honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `honeyos memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `honeyos model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HONEYOS_HOME/honcho.json` | Profile-local (isolated HoneyOS instances) |
| 2 | `~/.honeyos/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active HoneyOS profile: `honeyos` (default) or `honeyos_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `honeyos honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | HoneyOS session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`honeyos-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`honeyos`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-honeyos-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening honeyos in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — HoneyOS session ID (timestamp + hex). Every `honeyos` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple HoneyOS profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`honeyos`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active HoneyOS profile: `honeyos` (default) or `honeyos_<profile>` (e.g. `honeyos -p coder` -> host key `honeyos_coder`). Older `honeyos.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HONEYOS_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `honeyos-agent`) |
...
|---------|-------------|
| `honeyos memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `honeyos honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `honeyos memory setup`) |
| `honeyos honcho status` | Show resolved config for active profile |
| `honeyos honcho enable` / `disable` | Toggle Honcho for active profile |
| `honeyos honcho mode <mode>` | Change recall or observation mode |
| `honeyos honcho peer --user <name>` | Update user peer name |
| `honeyos honcho peer --ai <name>` | Update AI peer name |
| `honeyos honcho tokens --context <N>` | Set context token budget |
| `honeyos honcho tokens --dialectic <N>` | Set dialectic max chars |
| `honeyos honcho map <name>` | Map current directory to a session name |
| `honeyos honcho sync` | Create host blocks for all HoneyOS profiles |
