...
Config: Uses the existing Honcho config chain:
  1. $HONEYOS_HOME/honcho.json (profile-scoped)
...
from honeyos.agent.memory_provider import TRIVIAL_PROMPT_RE, MemoryProvider, is_trivial_prompt
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, honeyos_home):
        """Write config to $HONEYOS_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(honeyos_home) / "honcho.json"
        existing = {}
    ...
    def post_setup(self, honeyos_home: str, config: dict) -> None:
        ...
        import types
        from honeyos.plugins.memory.honcho.cli import cmd_setup
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps HoneyOS CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "honeyos-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        from honeyos.plugins.memory.honcho.client import get_honcho_client
        from honeyos.plugins.memory.honcho.session import HonchoSessionManager
        ...
        # Skip under per-session strategy: every HoneyOS run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_honeyos_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "honeyos-default",
...
def register(ctx) -> None:
    ...
    from honeyos.plugins.memory.query_rewrite import rewrite_memory_query
