...
``honeyos://`` handler. Endpoints are env-overridable with local-dev defaults
...
from honeyos.plugins.memory.honcho.client import resolve_active_host, resolve_config_path

logger = logging.getLogger(__name__)

# The loopback redirect registered for the HoneyOS OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "honeyos-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``honeyos-desktop`` / ``honeyos-cli``) so the consent side can attribute
...
def complete_authorization(
    ...
    *,
    ...
    from honeyos.plugins.memory.honcho.client import reset_honcho_client
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to HoneyOS.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local HONEYOS_HOME override can't reach.
