...
- Hosted: API key from [app.supermemory.ai/integrations?connect=honeyos](http://app.supermemory.ai/integrations?connect=honeyos)
...
```bash
honeyos memory setup    # select "supermemory"
```

Or manually:

```bash
honeyos config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.honeyos/.env
```
...
Before running `honeyos memory setup`, add the local endpoint to
`$HONEYOS_HOME/supermemory.json`:
...
Then run `honeyos memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$HONEYOS_HOME/supermemory.json`
...
| `container_tag` | `honeyos` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `honeyos-{identity}` → `honeyos-coder`). |
...
`https://api.supermemory.ai`. HoneyOS resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: honeyos`, and document writes stamp
`metadata.sm_source: honeyos`. This is a **functional routing key, not telemetry**:
it groups HoneyOS-written memories into a dedicated "HoneyOS" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per HoneyOS profile:

```json
{
  "container_tag": "honeyos-{identity}"
}
```

For a profile named `coder`, this resolves to `honeyos-coder`. The default profile resolves to `honeyos-default`. Without `{identity}`, all profiles share the same container.
