...
from honeyos.agent.secret_scope import get_secret, is_multiplex_active
from honeyos.tools.registry import tool_error

logger = logging.getLogger(__name__)

_DEFAULT_CONTAINER_TAG = "honeyos"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=honeyos"
...
def _load_supermemory_config(honeyos_home: str) -> dict:
    config = _default_config()
    config_path = Path(honeyos_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, honeyos_home: str) -> None:
    config_path = Path(honeyos_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "honeyos"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes HoneyOS writes into the "HoneyOS" Space in the Supermemory
        ...
        merged = {"sm_source": "honeyos", **(metadata or {})}
...
def _resolve_container_tag_for_setup(honeyos_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(honeyos_home)
...
def _probe_supermemory_connection(api_key: str, honeyos_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(honeyos_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(honeyos_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `honeyos memory setup`.
        # All other options are documented for $HONEYOS_HOME/supermemory.json
    ...
    def save_config(self, values, honeyos_home):
        ...
        _save_supermemory_config(sanitized, honeyos_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, honeyos_home)
    ...
    def post_setup(self, honeyos_home: str, config: dict) -> None:
        ...
        from honeyos.runtime.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(honeyos_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from honeyos.core.constants import get_honeyos_home
        self._honeyos_home = kwargs.get("honeyos_home") or str(get_honeyos_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._honeyos_home)
