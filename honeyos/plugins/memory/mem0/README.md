...
```bash
honeyos memory setup    # select "mem0"
```

Or manually:
```bash
honeyos config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.honeyos/.env
```

## Config

Behavioral settings live in `$HONEYOS_HOME/mem0.json` (set them via `honeyos memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.honeyos/.env`.
...
| `user_id` | `honeyos-user` | User identifier on Mem0 |
| `agent_id` | `honeyos` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   honeyos memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   honeyos memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.honeyos/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.honeyos/.env
   ```
   or in `$HONEYOS_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh HoneyOS session and call `mem0_search` — it connects to your server.
...
```bash
honeyos memory setup mem0 --mode oss \
...
```bash
honeyos memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$HONEYOS_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.honeyos/mem0_qdrant"}}
  }
}
```
...
```bash
honeyos memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
honeyos memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$HONEYOS_HOME/mem0.json`).
