...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # honeyos.runtime/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(honeyos_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(honeyos_home) / "mem0.json"
    existing = {}
...
def _setup_platform(honeyos_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "honeyos-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "honeyos"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.honeyos/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.honeyos/.env (or unset it) or HoneyOS will keep "
    ...
    from honeyos.plugins.memory.mem0 import Mem0MemoryProvider
    provider = Mem0MemoryProvider()
    provider.save_config(provider_config, honeyos_home)

    if env_writes:
        _write_env(Path(honeyos_home) / ".env", env_writes)
...
def _setup_selfhosted(honeyos_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "honeyos-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "honeyos")
...
def _setup_oss(honeyos_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(honeyos_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "honeyos-user")
    ...
    if env_writes:
        _write_env(Path(honeyos_home) / ".env", env_writes)
    _save_mem0_json(honeyos_home, {"mode": "oss", "user_id": user_id, "agent_id": "honeyos", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, honeyos_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # honeyos.runtime/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "honeyos-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "honeyos"
...
def _setup_oss_interactive(honeyos_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], honeyos_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], honeyos_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'honeyos-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "honeyos-user")

    agent_id = input("  Agent ID [honeyos]: ").strip()
    agent_id = agent_id or "honeyos"
    ...
    if env_writes:
        _write_env(Path(honeyos_home) / ".env", env_writes)
    _save_mem0_json(honeyos_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(honeyos_home: str, config: dict) -> None:
    """Entry point called by honeyos memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(honeyos_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(honeyos_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(honeyos_home, config, flags)
        return
