...
Configuration
-------------
Secret (lives in $HONEYOS_HOME/.env or the environment):
...
Behavioral settings (live in $HONEYOS_HOME/mem0.json, set via `honeyos memory
setup`):
  ...
  agent_id           — Agent identifier (default: honeyos)
...
def _load_config() -> dict:
    """Load config from env vars, with $HONEYOS_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "honeyos"),
        "oss": {},
    }
    ...
    config_path = get_honeyos_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, honeyos_home):
        """Write config to $HONEYOS_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, honeyos_home: str, config: dict) -> None:
        ...
        post_setup(honeyos_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $HONEYOS_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "honeyos")
