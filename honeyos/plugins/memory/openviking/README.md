...
- OpenViking server running and reachable from HoneyOS

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
HoneyOS can identify older servers that expose the legacy status-only health
...
Then configure HoneyOS:

```bash
honeyos memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into HoneyOS, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
honeyos config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.honeyos/.env`; for a named profile use
`~/.honeyos/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=honeyos
```

## Config

OpenViking's server config is separate from HoneyOS:
...
HoneyOS-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `honeyos` | HoneyOS peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, HoneyOS lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
HoneyOS sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
HoneyOS built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| HoneyOS action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because HoneyOS
...
file URIs, such as
`viking://user/peers/honeyos/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/honeyos/memories/preferences/mem_abc123.md`. Files
