...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — HoneyOS peer ID in OpenViking (default: honeyos)
...
from honeyos.agent.skill_commands import extract_user_instruction_from_skill_message
from honeyos.tools.registry import tool_error
from honeyos.utils import atomic_json_write, env_var_enabled
...
_DEFAULT_AGENT = "honeyos"
_AGENT_PROMPT_LABEL = "HoneyOS peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "HONEYOS_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip HoneyOS slash-skill scaffolding before sending content to OpenViking.
...
def _zip_directory(dir_path: Path) -> Path:
    ...
    from honeyos.agent.file_safety import raise_if_read_blocked
...
def _openviking_endpoint_is_always_blocked(candidate: str) -> bool:
    ...
    """
    from honeyos.tools.url_safety import is_always_blocked_url
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach HoneyOS to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; HoneyOS refused the connection."
        ) from exc
...
def _load_honeyos_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("HONEYOS_HOME", "")).expanduser() if os.environ.get("HONEYOS_HOME") else Path.home() / ".honeyos"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. HoneyOS did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; HoneyOS will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new HoneyOS session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in HoneyOS only", "write values only to HoneyOS .env"),
    ...
    _save_honeyos_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to HoneyOS .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See honeyos-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_honeyos_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "HoneyOS peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], honeyos_home: str) -> None:
    ...
    def post_setup(self, honeyos_home: str, config: dict) -> None:
        ...
        from honeyos.runtime.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        honeyos_home_path = Path(honeyos_home)
        env_path = honeyos_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_honeyos_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        honeyos_home = str(kwargs.get("honeyos_home") or "").strip()
        if not honeyos_home:
            try:
                from honeyos.core.constants import get_honeyos_home
                honeyos_home = str(get_honeyos_home())
            except Exception:
                honeyos_home = str(Path.home() / ".honeyos")
        self._honeyos_home = honeyos_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "HoneyOS will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.honeyos/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts honeyos (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; HoneyOS will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._honeyos_home:
            return None
        return Path(self._honeyos_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._honeyos_home:
            return None
        return Path(self._honeyos_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_honeyos_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of HoneyOS' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert HoneyOS canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See honeyos-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/honeyos/.overview.md).
