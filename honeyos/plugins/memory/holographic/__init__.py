"""honeyos-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $HONEYOS_HOME/config.yaml (profile-scoped):
  plugins:
    honeyos-memory-store:
      db_path: $HONEYOS_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "honeyos-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, honeyos_home):
        """Write config to config.yaml under plugins.honeyos-memory-store."""
        from pathlib import Path
        config_path = Path(honeyos_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["honeyos-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_honeyos_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _honeyos_home = str(get_honeyos_home())
        _default_db = _honeyos_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $HONEYOS_HOME in user-supplied paths so config values like
        # "$HONEYOS_HOME/memory_store.db" or "~/.honeyos/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$HONEYOS_HOME", _honeyos_home)
            db_path = db_path.replace("${HONEYOS_HOME}", _honeyos_home)
