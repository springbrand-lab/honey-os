...
```bash
honeyos memory setup    # select "byterover"
```

Or manually:
```bash
honeyos config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.honeyos/.env
```
...
Working directory: `$HONEYOS_HOME/byterover/` (profile-scoped).
