...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with honeyos-agent)
...
Usage:
    from honeyos.plugins.cron_providers import discover_cron_schedulers, load_cron_scheduler
...
_USER_NAMESPACE = "_honeyos_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_honeyos_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_honeyos_user_cron'`` — the same
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$HONEYOS_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    from honeyos.scheduler.scheduler_provider import CronScheduler
