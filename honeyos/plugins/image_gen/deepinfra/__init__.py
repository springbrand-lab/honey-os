...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=honeyos``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`honeyos.runtime.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``honeyos tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from honeyos the next time the catalog is fetched, no patch required.
...
from honeyos.agent.image_gen_provider import (
...
def _live_models() -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        from honeyos.runtime.models import _fetch_deepinfra_models_by_tag
    except Exception as exc:
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `honeyos tools` → Image "
                    "Generation → DeepInfra to configure, or `honeyos setup` "
                    "to add the key."
                ),
        ...
        from honeyos.runtime.models import deepinfra_base_url
