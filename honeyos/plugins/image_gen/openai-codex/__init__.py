...
Output is saved as PNG under ``$HONEYOS_HOME/cache/images/``. Source images for
...
def _read_codex_access_token() -> Optional[str]:
    ...
    """
    try:
        from honeyos.agent.auxiliary_client import _read_codex_access_token as _reader
...
def _sniff_image_mime(raw: bytes) -> Optional[str]:
    ...
    """
    from honeyos.agent.image_routing import _sniff_mime_from_bytes
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `honeyos auth codex` (or `honeyos setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`honeyos auth codex` (or `honeyos setup` → Codex) to sign in."
                ),
