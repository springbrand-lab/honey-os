...
# HoneyOS uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _resolve_managed_krea_gateway():
    ...
    try:
        ...
        from honeyos.tools.tool_backend_helpers import prefers_gateway
    except Exception as exc:  # noqa: BLE001
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `honeyos tools` → Image "
        ...
        headers = {
            ...
            "User-Agent": "HoneyOS-Agent/1.0 (krea-image-gen)",
        }
        if managed is not None:
            ...
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "`honeyos tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
