...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._honeyos_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._honeyos_fts5_probe")
            return True
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `honeyos sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `honeyos db optimize`) must keep its EXISTING
