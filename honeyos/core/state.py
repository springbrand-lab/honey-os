#!/usr/bin/env python3
"""
SQLite State Store for HoneyOS.
...
from honeyos.agent.message_sanitization import _sanitize_surrogates
from honeyos.agent.skill_commands import (
    SKILL_EXCERPT_JOINT,
    SKILL_SCAFFOLD_SQL_LIKE,
    describe_skill_invocation,
)
from honeyos.core.constants import get_honeyos_home
from honeyos.runtime.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from honeyos.core.state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from honeyos.core.state_portability import SessionPortabilityMixin
from honeyos.core.state_schema import SessionSchemaMixin
from honeyos.core.state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``honeyos -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_honeyos_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.honeyos`` even when a test fixture later
    redirects ``HONEYOS_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_honeyos_home()`` fresh so a runtime
       ``HONEYOS_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_honeyos_home() / "state.db"
...
# ~/.honeyos/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects HONEYOS_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without HONEYOS_HOME, a stale worktree
# without the re-pin, or a developer shell that exports HONEYOS_HOME to the
...
#: ``~/.honeyos``).  The test conftest injects the pre-sandbox production
#: root here so custom-``HONEYOS_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default HoneyOS root for the guard.

    Deliberately avoids ``Path.home()`` / ``honeyos.core.constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``honeyos.core.state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "honeyos"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "honeyos"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".honeyos"
        return root.resolve()
    except Exception:
        return None
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real HoneyOS home *root*.
    ...
    under ``~/.honeyos/honeyos-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``HONEYOS_HOME``) paths.
    """
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real HoneyOS root {root}). "
                "Tests must run against a temporary HONEYOS_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "HONEYOS_HOME. If this test genuinely needs the live "
...
# honeyos.runtime/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``honeyos.runtime.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``honeyos update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"HoneyOS-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart HoneyOS"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a HoneyOS-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `honeyos doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see honeyos.runtime/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with honeyos.runtime.config.
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``honeyos.runtime.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the HoneyOS home tree (``get_honeyos_home()``) — the safe repair scope:
      HoneyOS owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``honeyos.runtime.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_honeyos_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. HoneyOS needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (honeyos.core.state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_honeyos.core.state.py:600-632)
        ...
        probe_session_id = f"_honeyos_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.honeyos/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``honeyos sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("HONEYOS_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_honeyos_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("HONEYOS_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship honeyos.core.state without honeyos.runtime). A
    ...
    try:
        ...
        logger.debug(
            "honeyos.runtime.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch honeyos.core.state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``honeyos.runtime.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from honeyos.runtime.backup import is_zeroed_sqlite_file
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `honeyos snapshot list` / "
                "`honeyos snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple honeyos processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling HoneyOS
    ...
    # `honeyos update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `honeyos snapshot list` / "
                    f"`honeyos snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`honeyos db optimize`),
            ...
            # ``honeyos.core.state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `honeyos update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `honeyos sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another HoneyOS process held the "
    ...
    #
    # `optimize_fts_storage()` (the `honeyos sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`honeyos sessions optimize-storage`) ──
    #
    ...
    def get_session_activity(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not row:
            return None
        from honeyos.agent.session_activity import build_activity_snapshot
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``honeyos --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: honeyos.core.state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``honeyos -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``honeyos sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``HONEYOS_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade HoneyOS, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one HoneyOS session.

        A HoneyOS session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a HoneyOS session is already bound to any Telegram DM topic.
