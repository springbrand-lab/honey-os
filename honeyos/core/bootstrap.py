"""Windows UTF-8 bootstrap for HoneyOS entry points.
...
should be imported at the very top of every HoneyOS entry point
(``honeyos``, ``honeyos-agent``, ``honeyos-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``honeyos.runtime._subprocess_compat.suppress_platform_ver_console``
    ...
    ``honeyos.core.bootstrap``, never ``honeyos.runtime.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing HoneyOS modules.

    HoneyOS ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real HoneyOS source root
    ...
    """
    root = src_root or os.environ.get("HONEYOS_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``HONEYOS_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("HONEYOS_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import honeyos.core.bootstrap``
# (or ``from honeyos.core.bootstrap import apply_windows_utf8_bootstrap``) at
