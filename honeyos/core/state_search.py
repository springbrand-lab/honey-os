...
``honeyos.core.state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import honeyos.core.state (cycle) — shared
module-level constants live in honeyos.core.state_common.
"""
...
from honeyos.core.state_common import (
    FTS_CJK_STALE_KEY,
    FTS_SQL,
    FTS_STORAGE_VERSION,
    FTS_TRIGRAM_SQL,
...
# Moved methods logged under the "honeyos.core.state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("honeyos.core.state")
...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: HONEYOS_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("HONEYOS_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another HoneyOS surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``HONEYOS_DISABLE_FTS_TRIGRAM`` or not yet created), so
