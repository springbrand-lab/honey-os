...
_UNSET = object()
_HONEYOS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_HONEYOS_HOME_OVERRIDE", default=_UNSET
)
...
def set_honeyos_home_override(path: str | Path | None) -> Token:
    """Set a context-local HoneyOS home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _HONEYOS_HOME_OVERRIDE.set(value)


def reset_honeyos_home_override(token: Token) -> None:
    """Restore the previous context-local HoneyOS home override."""
    _HONEYOS_HOME_OVERRIDE.reset(token)


def get_honeyos_home_override() -> str | None:
    """Return the active context-local HoneyOS home override, if any."""
    override = _HONEYOS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_honeyos_home() -> Path:
    """Return the platform-native default HoneyOS home path."""
...
def _honeyos_home_from_env() -> Path:
    """Resolve HONEYOS_HOME from the process environment only.

    Reads the ``HONEYOS_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_honeyos_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_honeyos_home`
    and :func:`get_process_honeyos_home` so the two never drift.
    """
    val = os.environ.get("HONEYOS_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_honeyos_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``HONEYOS_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_honeyos_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[HONEYOS_HOME fallback] HONEYOS_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass HONEYOS_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_honeyos_home() -> Path:
    """Return the HoneyOS home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_honeyos_home_override`) → ``HONEYOS_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``HONEYOS_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``HONEYOS_HOME`` explicitly (see the systemd
    template in ``honeyos.runtime/gateway.py`` and the kanban dispatcher in
    ``honeyos.runtime/kanban_db.py``).  See https://github.com/NousResearch/honeyos-agent/issues/18594.
    """
    override = get_honeyos_home_override()
    if override:
        return Path(override)

    if not os.environ.get("HONEYOS_HOME", "").strip():
        _warn_profile_fallback_once()

    return _honeyos_home_from_env()


def get_process_honeyos_home() -> Path:
    """Return the HoneyOS home for the running process, ignoring task overrides.

    Unlike :func:`get_honeyos_home`, this never follows the context-local
    override set by :func:`set_honeyos_home_override`.  It resolves only the
    process ``HONEYOS_HOME`` env var (falling back to the platform default),
...
def get_default_honeyos_root() -> Path:
    """Return the root HoneyOS directory for profile-level operations.

    In standard deployments this is the platform-native HoneyOS home
    (``~/.honeyos`` on POSIX, ``%LOCALAPPDATA%\\honeyos`` on native Windows).

    In Docker or custom deployments where ``HONEYOS_HOME`` points outside
    ``~/.honeyos`` (e.g. ``/opt/data``), returns ``HONEYOS_HOME`` directly
    — that IS the root.

    In profile mode where ``HONEYOS_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.honeyos/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_honeyos_home()
    env_home = os.environ.get("HONEYOS_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # HONEYOS_HOME is under ~/.honeyos (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — HONEYOS_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``HONEYOS_OPTIONAL_SKILLS``.
    """
    override = os.getenv("HONEYOS_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_honeyos_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``HONEYOS_OPTIONAL_MCPS``.
    """
    override = os.getenv("HONEYOS_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_honeyos_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``HONEYOS_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<HONEYOS_HOME>/skills`` last-resort
    """
    override = os.getenv("HONEYOS_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_honeyos_home() / "skills"
...
def get_honeyos_dir(
    ...
    *,
    ...
    """Resolve a HoneyOS subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to HONEYOS_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to HONEYOS_HOME (e.g. ``"image_cache"``).
        home: Optional explicit HoneyOS home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local HONEYOS_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_honeyos_home()
...
def iter_honeyos_node_dirs(home: Path | None = None) -> list[Path]:
    """Return HoneyOS-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\honeyos\\node``. POSIX installs use
    ``$HONEYOS_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_honeyos_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with honeyosManagedNodePathEntries() in
...
_HONEYOS_NODE_TARGET_MAJOR = int(os.environ.get("HONEYOS_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    HoneyOS-managed Node trees live under ``$HONEYOS_HOME/node`` (or a profile's
    ``HONEYOS_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_honeyos_node_executable``
    used to trust file presence alone, so ``honeyos update`` would pick that
...
def honeyos_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any HoneyOS-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_honeyos_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%HONEYOS_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_HONEYOS_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_HONEYOS_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$HONEYOS_HOME/node`` on POSIX.
    ...
    ``HONEYOS_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_honeyos_managed_node() -> str | None:
    """Install a HoneyOS-managed Node tree and return its npm path.
    ...
    HoneyOS never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$HONEYOS_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_honeyos_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_honeyos_node_dirs():
...
def heal_honeyos_managed_node() -> bool:
    """Redownload HoneyOS-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not honeyos_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HONEYOS_HOME": str(get_honeyos_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_honeyos_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_HONEYOS_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_honeyos_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _HONEYOS_NODE_TARGET_MAJOR
    return False


def find_honeyos_node_executable(command: str) -> str | None:
    """Return a HoneyOS-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_HONEYOS_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_honeyos_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for HoneyOS-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy HoneyOS-managed installs.

    This is for HoneyOS-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_honeyos_node_executable(command)
    if managed:
        return managed
    if honeyos_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_honeyos_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with HoneyOS-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_honeyos_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``honeyos update`` — leaving a **dangling symlink** that ``which`` still
...
def display_honeyos_home() -> str:
    """Return a user-friendly display string for the current HONEYOS_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.honeyos``
        profile:  ``~/.honeyos/profiles/coder``
        custom:   ``/opt/honeyos-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.honeyos``.  For code that needs a real ``Path``, use
    :func:`get_honeyos_home` instead.
    """
    home = get_honeyos_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``HONEYOS_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/honeyos-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{HONEYOS_HOME}/home`` when the profile-home directory exists."""
    honeyos_home = get_honeyos_home_override() or (env or {}).get("HONEYOS_HOME") or os.getenv("HONEYOS_HOME")
    if not honeyos_home:
        return None
    profile_home = os.path.join(honeyos_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("HONEYOS_REAL_HOME") or os.getenv("HONEYOS_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding HoneyOS profile HOME.

    ``HONEYOS_HOME`` scopes HoneyOS state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={HONEYOS_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{HONEYOS_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{HONEYOS_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply HoneyOS' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["HONEYOS_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when HoneyOS itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/honeyos-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under HONEYOS_HOME.

    Replaces the ``get_honeyos_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, honeyos.core.logging.py, honeyos.core.time.py, etc.).
    """
    return get_honeyos_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under HONEYOS_HOME."""
    return get_honeyos_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under HONEYOS_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_honeyos_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._honeyos_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``honeyos.runtime`` modules using three different Windows
    ...
    A few sites outside ``honeyos.runtime`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``honeyos.runtime.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of HoneyOS itself. An ImportError
...
# `honeyos.runtime.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with HoneyOS.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("honeyos_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `honeyos update` is exactly
    ...
    return [
        ...
        "    honeyos update",
        "If that also fails, reinstall: https://honeyos-agent.nousresearch.com",
    ]
