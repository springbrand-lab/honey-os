...
Extracted verbatim from honeyos.core.state.py so the SessionDB mixin modules
(honeyos.core.state_search / honeyos.core.state_schema / honeyos.core.state_portability) can
reference them without importing honeyos.core.state (which would be a cycle).
honeyos.core.state re-imports every name here for backward compatibility.
"""
...
# optimized via ``honeyos sessions optimize-storage``. A legacy DB sits at
...
# `honeyos sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `honeyos db optimize`. This is
