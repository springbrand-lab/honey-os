"""Centralized logging setup for HoneyOS.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.honeyos/logs/`` (profile-aware via ``get_honeyos_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``honeyos.runtime.web_server``,
    ``honeyos.runtime.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in HoneyOS (TUI, gateway, ``hy_memory`` server, MCP
...
from honeyos.core.constants import get_config_path, get_honeyos_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_honeyos_session_injector", False):
    ...
    _session_record_factory._honeyos_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``honeyos logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``honeyos logs --component gateway``.
    "gateway": ("gateway", "honeyos_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("honeyos.runtime", "cli"),
    "cron": ("cron",),
    "gui": (
        "honeyos.runtime.web_server",
        "honeyos.runtime.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the HoneyOS logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    honeyos_home
        Override for the HoneyOS home directory.  Falls back to
        ``get_honeyos_home()`` (profile-aware).
    log_level
    ...
    home = honeyos_home or get_honeyos_home()
    log_dir = home / "logs"
    ...
    from honeyos.agent.redact import RedactingFormatter
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_honeyos_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._honeyos_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the honeyos group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from honeyos.runtime.config import is_managed
...
# the module header): when several HoneyOS processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._honeyos_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="honeyos-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_honeyos_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse honeyos.runtime.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when honeyos.runtime.config isn't importable (bare
        # honeyos.core.logging consumers).
        try:
            from honeyos.runtime.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
            from honeyos.utils import fast_safe_load
