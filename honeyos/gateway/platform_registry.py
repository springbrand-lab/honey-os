...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``honeyos setup`` display).
    ...
    # built-ins).  Used by ``honeyos gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `honeyos`
        # invocation -- including plain `honeyos chat`, which never touches any
        ...
        # `honeyos setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``honeyos setup``/``gateway status``, channel
