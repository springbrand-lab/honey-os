...
window before ``honeyos update``'s graceful restart fires), a first-time lazy
...
def _fingerprint() -> str | None:
    ...
    ``honeyos.runtime.main`` is always already imported in a gateway process (it's
    the entry point), so this import is free and avoids duplicating the
    worktree-aware ref resolution.
    """
    try:
        from honeyos.runtime.main import _read_git_revision_fingerprint
