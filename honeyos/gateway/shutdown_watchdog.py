...
2. An event-loop heartbeat file at ``<HONEYOS_HOME>/state/gateway.heartbeat`` so
...
from honeyos.gateway.restart import GATEWAY_SERVICE_RESTART_EXIT_CODE
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        while not stop_event.wait(timeout=interval):
            ...
            try:
                from honeyos.gateway.lifecycle_ledger import mark_exited
...
def _process_honeyos_home() -> Path:
    """HONEYOS_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<HONEYOS_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_honeyos_home()
...
def write_loop_heartbeat(
    *,
    ...
    try:
        from honeyos.gateway.lifecycle_ledger import sample_memory
...
def arm_shutdown_watchdog(
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            from honeyos.gateway.status import remove_pid_file, release_gateway_runtime_lock
            remove_pid_file()
            release_gateway_runtime_lock()
        except Exception:
            pass
        try:
            from honeyos.core.logging import drain_log_queue
