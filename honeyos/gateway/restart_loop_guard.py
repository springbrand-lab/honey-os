...
Defenses 1 and 2 (the ``_HONEYOS_GATEWAY`` guard on ``honeyos gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.honeyos.gateway")``,
...
State lives in ``<HONEYOS_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_honeyos_home() / "gateway" / "restart_loop.json"
