...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{HONEYOS_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``HONEYOS_HOME`` is a **durable** store — on HoneyOS
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``honeyos gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting HONEYOS_HOME."""
    base = home if home is not None else get_honeyos_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable HONEYOS_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (HONEYOS_HOME is a
    durable Fly volume on HoneyOS Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable HONEYOS_HOME volume is
