...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<honeyos_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active HONEYOS_HOME."""
    from honeyos.core.constants import get_honeyos_home

    flush_dir = get_honeyos_home() / "pending_messages"
