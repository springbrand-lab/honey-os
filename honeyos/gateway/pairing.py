...
from honeyos.gateway.whatsapp_identity import (
    expand_whatsapp_aliases,
    normalize_whatsapp_identifier,
)
from honeyos.core.constants import (
    get_default_honeyos_root,
    get_honeyos_dir,
    get_honeyos_home,
)
...
PAIRING_DIR = get_honeyos_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``honeyos.runtime.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from honeyos.runtime.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{HONEYOS_HOME}/pairing`` while newer code/docs may
    write ``{HONEYOS_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own HONEYOS_HOME using the same legacy/consolidated layout rules
    as ``honeyos -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current HONEYOS_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp HONEYOS_HOME
        ...
        if profile:
            ...
            # ``honeyos -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `honeyos` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> honeyos pairing approve ...` as root, "
                    "re-run with `docker exec -u honeyos <container> ...` and "
                    "chown the existing file to the honeyos user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``honeyos
