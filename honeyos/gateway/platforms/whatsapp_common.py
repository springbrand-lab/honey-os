...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *HoneyOS*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to HONEYOS_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/honeyos), this function
    mirrors the bridge source to a writable HONEYOS_HOME location and returns that
    ...
    # Try HONEYOS_HOME location first
    honeyos_home = get_honeyos_home()
    honeyos_home_bridge = honeyos_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to HONEYOS_HOME if needed
    if honeyos_home_bridge.exists():
        return honeyos_home_bridge

    # Mirror the bridge source to HONEYOS_HOME
    try:
        honeyos_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            honeyos_home_bridge,
            dirs_exist_ok=False,
