...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.honeyos to the active profile home."""
    ...
    if raw == "~/.honeyos":
        return honeyos_home
    if raw.startswith("~/.honeyos/"):
        return honeyos_home / raw.removeprefix("~/.honeyos/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return honeyos_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under HONEYOS_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from honeyos.core.constants import get_honeyos_home

    scripts_root = (get_honeyos_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.honeyos" or raw_text.startswith("~/.honeyos/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__honeyos_ignore__") is True
        ):
            return False, None
