...
There are two ways to add a platform to the HoneyOS gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.honeyos/plugins/` (or under `plugins/platforms/`
...
**zero changes to core HoneyOS code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `honeyos gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `honeyos.runtime/config.py` so the setup
...
Checklist for integrating a platform directly into the HoneyOS core.
...
And add it to the `honeyos-gateway` composite:

```python
"honeyos-gateway": {
    "includes": [..., "honeyos-your-platform"]
}
```
...
## 12. Status Display (`honeyos.runtime/status.py`)
...
## 13. Gateway Setup Wizard (`honeyos.runtime/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ honeyos.runtime/ toolsets.py \
  --include="*.py" -l | sort -u
