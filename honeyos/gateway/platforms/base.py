...
from honeyos.utils import normalize_proxy_url

logger = logging.getLogger(__name__)

# Audio file extensions HoneyOS recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to HoneyOS but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through HoneyOS' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. HoneyOS-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_honeyos_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``HONEYOS_SESSION_PLATFORM`` contextvar — that contextvar is
    ...
    """
    from honeyos.tools.tts_tool import OPUS_VOICE_PLATFORMS
...
from honeyos.core.constants import get_default_honeyos_root, get_honeyos_dir, get_honeyos_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.honeyos/.env manually."
)
...
async def _ssrf_redirect_guard(response):
    ...
    """
    from honeyos.tools.url_safety import is_safe_url, redirect_target_from_response
...
IMAGE_CACHE_DIR = get_honeyos_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_honeyos_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_honeyos_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from honeyos.runtime.config import load_config_readonly as _load_config
...
async def cache_image_from_url(url: str, ext: str = ".jpg", retries: int = 2) -> str:
    ...
    """
    from honeyos.tools.url_safety import create_ssrf_safe_async_client, is_safe_url
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; HoneyOSAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_honeyos_dir("cache/audio", "audio_cache")
...
def _sniff_audio_ext(data: bytes, fallback_ext: str) -> str:
    ...
    """
    from honeyos.tools.audio_container import sniff_audio_ext
...
VIDEO_CACHE_DIR = get_honeyos_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_honeyos_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_honeyos_dir("cache/screenshots", "browser_screenshots")
...
_HONEYOS_ROOT = get_default_honeyos_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "HONEYOS_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "HONEYOS_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "HONEYOS_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "HONEYOS_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _HONEYOS_HOME / "image_cache",
    _HONEYOS_HOME / "audio_cache",
    _HONEYOS_HOME / "video_cache",
    _HONEYOS_HOME / "document_cache",
    _HONEYOS_HOME / "browser_screenshots",
    ...
    _HONEYOS_HOME / "cache" / "images",
    _HONEYOS_HOME / "cache" / "audio",
    _HONEYOS_HOME / "cache" / "videos",
    _HONEYOS_HOME / "cache" / "documents",
    _HONEYOS_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared HoneyOS root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* HONEYOS_HOME's cache, so a gateway running at
    the root (e.g. ``HONEYOS_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when HONEYOS_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _HONEYOS_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("HONEYOS_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _HONEYOS_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active HoneyOS profile and shared HoneyOS root both contain control
    ...
    # HONEYOS_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.honeyos
    ...
    for honeyos_root in (_HONEYOS_HOME, _HONEYOS_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(honeyos_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and HoneyOS
    secrets (``~/.honeyos/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``HONEYOS_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    HoneyOS-managed cache, under an operator-allowlisted root
    (``HONEYOS_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the HoneyOS root (~/.honeyos/.env,
    ...
    # ``MEDIA:~/.honeyos/google_token.json``) remain rejected.
...
def cache_media_bytes(
    ...
    *,
    ...
    """
    from honeyos.tools.credential_files import to_agent_visible_cache_path
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?honeyos\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+honeyos[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``HONEYOS_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # HoneyOS commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-HONEYOS_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("HONEYOS_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "HONEYOS_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "HONEYOS_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: HoneyOS sends
    ...
    def render_message_event(self, event: Any, sink: Any) -> None:
        ...
        """
        from honeyos.gateway.stream_events import MessageChunk, MessageStop, Commentary
    ...
    def format_tool_event(self, event: Any, *, mode: str = "all",
        ...
        """
        from honeyos.gateway.stream_events import ToolCallChunk
        if not isinstance(event, ToolCallChunk):
            return None

        from honeyos.agent.display import get_tool_emoji
        ...
        if preview:
            from honeyos.agent.display import prepare_tool_preview
    ...
    def _write_runtime_status_safe(self, context: str, **kwargs) -> None:
        ...
        """
        try:
            from honeyos.gateway.status import write_runtime_status
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-HONEYOS_HOME holder may be replaced only when the runner
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from honeyos.gateway.status import release_scoped_lock
    ...
    def _history_media_paths_for_session(self, session_key: str) -> Optional[set]:
        ...
        from honeyos.gateway.run import _collect_history_media_paths
    ...
    async def send_clarify(
        ...
        if choices:
            ...
            _is_multi = False
            try:
                from honeyos.tools import clarify_gateway as _cg
                with _cg._lock:
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the HoneyOS home layout.
        """
    ...
    def prepare_tts_text(self, text: str) -> str:
        ...
        """
        try:
            from honeyos.tools.tts_text_normalize import prepare_spoken_text
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the HoneyOS home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.honeyos/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from honeyos.runtime.commands import (
            ...
            if not cmd:
                try:
                    from honeyos.tools import clarify_gateway as _clarify_mod
                    _has_text_clarify = (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          HONEYOS_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          HONEYOS_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          HONEYOS_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("HONEYOS_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("HONEYOS_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("HONEYOS_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_honeyos_run_generation", None),
                            event=event,
                        )):
                    try:
                        from honeyos.tools.tts_tool import text_to_speech_tool, check_tts_requirements
                        if check_tts_requirements():
                            ...
                            # HONEYOS_SESSION_PLATFORM contextvar the tool would
                ...
                if text_content and not _tts_caption_delivered:
                    ...
                    if not is_ephemeral_response and not str(
                        ...
                        try:
                            from honeyos.gateway.delivery_ledger import (
            ...
            # not at the top of this task.  _honeyos_run_generation is set on
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        try:
            from honeyos.gateway.shutdown_flush import flush_pending_to_file
    ...
    def build_source(
        ...
        per-profile HONEYOS_HOME isolation.
        """
