...
Connects HoneyOS to WeChat personal accounts via Tencent's iLink Bot API.
...
def _productize_outbound_text(content: str) -> str:
    ...
    if not os.environ.get("HONEYOS_RUNTIME_ID", "").startswith("honeyos-companion-"):
        return content
    branded = content.replace("`honeyos ", "`honeyos ")
    branded = branded.replace("HoneyOS", "HoneyOS")
    branded = branded.replace("HoneyOS", "HoneyOS")
    return branded.replace("HONEYOS", "HoneyOS")
...
from honeyos.gateway.platforms.helpers import MessageDeduplicator, greedy_pack_blocks
...
def _account_dir(honeyos_home: str) -> Path:
    path = Path(honeyos_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(honeyos_home: str, account_id: str) -> Path:
    return _account_dir(honeyos_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(honeyos_home, account_id)
...
def load_weixin_account(honeyos_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, honeyos_home: str):
        self._root = _account_dir(honeyos_home)
...
async def _api_post(
    ...
    *,
    ...
    # invoked via asyncio.run_coroutine_threadsafe() from honeyos.scheduler jobs.
...
def _split_table_row(line: str) -> List[str]:
    ...
    from honeyos.agent.markdown_tables import split_table_row
...
def _sync_buf_path(honeyos_home: str, account_id: str) -> Path:
    return _account_dir(honeyos_home) / f"{account_id}.sync.json"


def _load_sync_buf(honeyos_home: str, account_id: str) -> str:
    path = _sync_buf_path(honeyos_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(honeyos_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(honeyos_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native HoneyOS adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._token_store = ContextTokenStore(honeyos_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(honeyos_home, self._account_id)
            if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        # "Timeout context manager should be used inside a task" errors when
        # send() is invoked via asyncio.run_coroutine_threadsafe() from honeyos.scheduler.
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach HoneyOS regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._honeyos_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._honeyos_home, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"honeyos-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"honeyos-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_honeyos_home()))
