...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-HoneyOS-Session-Id header; opt-in long-term memory scoping via X-HoneyOS-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-HoneyOS-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists honeyos-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible HoneyOS sessions
- POST /api/sessions               — create an empty HoneyOS session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to honeyos-agent
...
from honeyos.gateway.config import Platform, PlatformConfig
from honeyos.gateway.platforms.base import (
...
from honeyos.gateway.readiness import collect_runtime_readiness

from honeyos.agent.secret_scope import UnscopedSecretError as _UnscopedSecretError
from honeyos.agent.secret_scope import get_secret as _scoped_get_secret
...
def _honeyos_version() -> str:
    """Return the canonical HoneyOS version string.

    ``honeyos.runtime.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``honeyos_agent-*.dist-info`` after a source update until the environment is
    reinstalled. Never raises — a version probe must not be able to break the
    health endpoint.
    """
    try:
        from honeyos.runtime import __version__
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from honeyos.runtime.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("HONEYOS_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``honeyos-agent``)
    ...
    ``direct_model_requests`` config value here, while HoneyOS-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously HoneyOS-aware and is
    always honored.
    """
...
def _reap_disconnected_agent_processes(
    ...
    from honeyos.gateway.run import _reap_gateway_turn_processes
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_honeyos_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted HONEYOS_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # honeyos.core.state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same HoneyOS session (and therefore the same Docker container sandbox
...
try:
    from honeyos.scheduler.jobs import (
...
def _notify_cron_provider_jobs_changed() -> None:
    ...
    try:
        from honeyos.scheduler.scheduler import _notify_provider_jobs_changed
...
try:
    from honeyos.tools.cronjob_tools import _scan_cron_prompt as _scan_cron_prompt
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through honeyos-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the HoneyOS-native session-chat
    ...
    def _gateway_is_draining() -> bool:
        ...
        try:
            from honeyos.gateway.run import _gateway_runner_ref
    ...
    def _readiness_work_counts(self) -> tuple[int, int, int]:
        ...
        try:
            ...
            pass
        try:
            from honeyos.tools.async_delegation import active_count
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "honeyos-agent"

        Delegates the tiered fallthrough to
        :func:`honeyos.runtime.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from honeyos.runtime.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "honeyos-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from honeyos.runtime.auth import has_usable_secret
    ...
    def _get_platform_callback_adapter(
        ...
        try:
            from honeyos.gateway.config import Platform as _Platform
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            return None
        try:
            from honeyos.runtime.profiles import profiles_to_serve
    ...
    def _profile_scope(profile: Optional[str]):
        ...
        if not profile:
            try:
                from honeyos.agent.secret_scope import is_multiplex_active

                if is_multiplex_active():
                    from honeyos.gateway.run import _profile_runtime_scope
                    from honeyos.core.constants import get_honeyos_home

                    return _profile_runtime_scope(get_honeyos_home())
            except Exception:
                pass
            return nullcontext()
        from honeyos.gateway.run import _profile_runtime_scope
        from honeyos.runtime.profiles import get_profile_dir
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-HoneyOS-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-HoneyOS-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-HoneyOS-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-HoneyOS-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-HoneyOS-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``honeyos sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_honeyos_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_honeyos_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "honeyos_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "honeyos_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the honeyos-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-HoneyOS-Session-Key``).  Unlike ``session_id``
        ...
        def _resolve_provider_runtime(
            ...
            *,
            ...
            try:
                ...
                try:
                    from honeyos.gateway.run import _resolve_runtime_agent_kwargs_for_provider
        ...
        # honeyos.runtime.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `honeyos auth add openai-codex` without `honeyos model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from honeyos.runtime.models import get_default_model_for_provider
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "honeyos-agent", "version": _honeyos_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        from honeyos.gateway.run import _resolve_gateway_model
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "honeyos-agent",
            "version": _honeyos_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list honeyos-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return HoneyOS provider/model inventory.
        ...
        HoneyOS provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every HoneyOS version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "honeyos.api_server.capabilities",
            "platform": "honeyos-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side HoneyOS AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-HoneyOS-Session-Id",
                "session_key_header": "X-HoneyOS-Session-Key",
    ...
    async def _handle_skills(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from honeyos.tools.skills_tool import _find_all_skills, _sort_skills
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty HoneyOS session row.
        ...
        from honeyos.gateway.session import _is_path_unsafe
        ...
        return web.json_response({"object": "honeyos.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "honeyos.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "honeyos.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "honeyos.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-HoneyOS-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-HoneyOS-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "honeyos.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-HoneyOS-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-HoneyOS-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "honeyos.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-HoneyOS-Session-Key.  This
        # is independent of X-HoneyOS-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-HoneyOS-Session-Id.
        ...
        provided_session_id = request.headers.get("X-HoneyOS-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-HoneyOS-Session-Id rejected: "
            ...
            # conversation map to the same HoneyOS session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``honeyos.tool.progress`` with ``status: running``.
                ...
                from honeyos.agent.display import build_tool_preview, get_tool_emoji
        ...
        response_headers = {
            "X-HoneyOS-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-HoneyOS-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["honeyos"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-HoneyOS-Completed"] = "false"
            response_headers["X-HoneyOS-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + HoneyOS-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-HoneyOS-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-HoneyOS-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-HoneyOS-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-HoneyOS-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: honeyos.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="honeyos.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-HoneyOS-Session-Id header.  Without this,
        ...
        response_headers = {"X-HoneyOS-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-HoneyOS-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_cron_fire(self, request: "web.Request") -> "web.Response":
        ...
        """
        from honeyos.runtime.config import cfg_get, load_config
        from honeyos.plugins.cron_providers.chronos.verify import get_fire_verifier
    ...
    def _bind_api_server_session(
        *,
        ...
        """
        from honeyos.gateway.session_context import set_session_vars
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-HoneyOS-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_honeyos_api_runtime", {}) or {})
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        async def _run_and_close():
            try:
                ...
                try:
                    from honeyos.tools.approval import unregister_gateway_notify
        ...
        response_headers = (
            {"X-HoneyOS-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "honeyos.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local HoneyOS-Relay
            ...
            # that is the exact surface the honeyos-0day campaign abused to write
            # ~/.honeyos/config.yaml and plant persistence. Sandboxing (Docker /
