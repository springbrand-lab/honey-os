...
from honeyos.gateway.platforms.media_cache import ext_for_mime
...
from honeyos.gateway.platforms.qqbot.constants import (
    API_BASE,
    TOKEN_URL,
    GATEWAY_URL_PATH,
    DEFAULT_API_TIMEOUT,
...
from honeyos.gateway.platforms.qqbot.utils import (
    coerce_list as _coerce_list_impl,
    build_user_agent,
)
from honeyos.gateway.platforms.qqbot.chunked_upload import (
    ChunkedUploader,
    UploadDailyLimitExceededError,
    UploadFileTooLargeError,
)
from honeyos.gateway.platforms.qqbot.keyboards import (
    ApprovalRequest,
    InlineKeyboard,
    InteractionEvent,
...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.honeyos/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. HoneyOS upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "honeyos-agent",
                    "$device": "honeyos-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.honeyos/.update_response`` for the
          detached ``honeyos update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``honeyos update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `honeyos setup gateway` / `honeyos gateway`)
    ...
    async def send_approval_request(
        ...
        """
        from honeyos.gateway.platforms.qqbot.keyboards import build_approval_text
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``honeyos update --gateway`` watcher. Button
        ...
        ``~/.honeyos/.update_response`` so the detached update process
        can read it.
        """
