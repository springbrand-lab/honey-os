...
``<HONEYOS_HOME>/state/gateway.lifecycle.json``:
...
def _process_honeyos_home() -> Path:
    """HONEYOS_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<HONEYOS_HOME>/state/gateway.lifecycle.json``."""
...
def detect_unclean_exit(home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        from honeyos.gateway.shutdown_watchdog import get_loop_heartbeat_path
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``honeyos.runtime.container_boot`` to annotate ``container-boot.log``.
    """
    try:
