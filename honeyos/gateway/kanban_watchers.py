...
def _acquire_singleton_lock(lock_path) -> "tuple[Optional[object], str]":
    ...
    """
    try:
        from honeyos.gateway.status import _try_acquire_file_lock  # deferred; same package
    except ImportError:
...
def _release_singleton_lock(handle) -> None:
    ...
    if handle is None:
        return
    try:
        from honeyos.gateway.status import _release_file_lock
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # HONEYOS_KANBAN_DB pins the board path; without this guard
                ...
                for d in deliveries:
                    ...
                    for ev in d["events"]:
                        ...
                        from honeyos.gateway.wake import adapter_supports_push
                        ...
                        from honeyos.gateway.wake import adapter_supports_push as _adapter_push_ok
                        ...
                        if not _is_push_adapter and _wake_kinds and _session_key:
                            ...
                            from honeyos.gateway.wake import deliver_wake
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `honeyos kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours HONEYOS_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        try:
            ...
            return
        env_override = os.environ.get("HONEYOS_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via HONEYOS_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `honeyos kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real HoneyOS profile
            ...
            PATH, missing venv, credential loss for a real HoneyOS profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from honeyos.runtime import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("HONEYOS_KANBAN_BOARD")
                try:
                    os.environ["HONEYOS_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("HONEYOS_KANBAN_BOARD", None)
                    else:
                        os.environ["HONEYOS_KANBAN_BOARD"] = prev_env
            return successes
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`honeyos kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
