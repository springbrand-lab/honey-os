"""Relay/connector support package for the HoneyOS gateway.
...
def relay_url() -> Optional[str]:
    ...
    if url:
        return url.rstrip("/")
    try:
        from honeyos.gateway.run import _load_gateway_config  # late import to avoid cycle
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``honeyos gateway enroll`` writes them to
    ``~/.honeyos/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``HONEYOS_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``honeyos gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from honeyos.runtime.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**HoneyOS:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``honeyos gateway enroll`` CLI. Two modes, in precedence order:
    ...
    if not token_url:
        ...
        from honeyos.runtime.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.honeyos/.env``.
    ...
    sets neither ``HONEYOS_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``honeyos gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'honeyos'}"
...
def send_relay_policy() -> bool:
    ...
    try:
        from honeyos.gateway.relay.auth import make_upgrade_token
...
def register_relay_adapter(force: bool = False, url: Optional[str] = None) -> bool:
    ...
    from honeyos.gateway.platform_registry import PlatformEntry, platform_registry
    from honeyos.gateway.relay.adapter import RelayAdapter
    from honeyos.gateway.relay.descriptor import CONTRACT_VERSION, CapabilityDescriptor
    ...
    def _factory(config):
        ...
        transport = None
        if resolved_url:
            from honeyos.gateway.relay.ws_transport import WebSocketRelayTransport
