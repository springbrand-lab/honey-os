...
from honeyos.gateway.relay.transport import InboundHandler
...
def _normalize_slack_parent_command(
    ...
    """Mirror native Slack ``/honeyos`` routing for authenticated relay text."""
    stripped = text.strip()
    parent_parts = stripped.split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/honeyos":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # The HONEYOS profile this event is routed to (multiplex mode). The
    ...
    if platform_enum == Platform.SLACK:
        ...
        # relay, bypassing HoneyOS' native Slack command callback. Normalize at
        # the wire boundary so adapter-level active-session gates see the real
        # gateway command rather than the legacy `honeyos` parent name.
...
class WebSocketRelayTransport:
    ...
    async def _dial_and_start(self) -> None:
        ...
        for platform, bot_id in self._identities:
            ...
            if platform == "discord":
                try:
                    from honeyos.gateway.relay.command_manifest import build_relay_command_manifest
