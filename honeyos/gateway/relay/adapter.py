...
from honeyos.gateway.platforms.base import BasePlatformAdapter, MessageEvent, SendResult
from honeyos.gateway.relay.descriptor import CapabilityDescriptor
from honeyos.gateway.relay.media import RelayMediaClient
from honeyos.gateway.relay.transport import RelayTransport
...
class RelayAdapter(BasePlatformAdapter):
    ...
    def _relay_slack_extra(self) -> Dict[str, Any]:
        ...
        Enterprise knob shape (HoneyOS-config directed, relay-namespaced):
        ...
        try:
            ...
            if platform == "discord":
                ...
                if event is not None:
                    self._capture_scope(event)
                    # Phase 3: a component press carrying a HoneyOS prompt token
    ...
    def _discord_interaction_to_event(self, forward):
        ...
        from honeyos.gateway.platforms.base import MessageType
        ...
        if itype == 3:
            # Phase 3: a component press whose custom_id is a HoneyOS prompt
    ...
    def _decode_prompt_token(token: str):
        ...
        — and is not — a HoneyOS prompt answer.
        """
        import re
    ...
    def _resolve_reply_to_for_send(
        ...
        Native Slack HoneyOS already suppresses this synthetic DM thread anchor:
    ...
    def _get_media_client(self) -> Optional[RelayMediaClient]:
        ...
        try:
            from honeyos.gateway.relay import relay_connection_auth, relay_url
            from honeyos.gateway.relay.media import media_base_url
    ...
    async def _send_prompt(
        ...
        *,
        ...
        # HoneyOS suppresses this synthetic DM thread anchor; drop it here for the
    ...
    async def _consume_prompt_response(self, event) -> bool:
        ...
        try:
            if kind == "exec_approval":
                ...
                from honeyos.tools import slash_confirm as slash_confirm_mod
                ...
                from honeyos.tools.clarify_gateway import (
                    mark_awaiting_text,
    ...
    async def on_processing_complete(self, event, outcome) -> None:
        ...
        from honeyos.gateway.platforms.base import ProcessingOutcome
