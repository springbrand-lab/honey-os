...
Module-level run.py helpers a handler needs (``_honeyos_home``,
``_load_gateway_config``, ``_resolve_gateway_model``, etc.) are imported lazily
inside the handler body — a deferred ``from honeyos.gateway.run import ...`` resolves at
...
from honeyos.agent.account_usage import fetch_account_usage, render_account_usage_lines
from honeyos.agent.i18n import t
from honeyos.agent.turn_context import extract_api_content_sidecar
from honeyos.gateway.config import HomeChannel, Platform, PlatformConfig, persist_home_channel
from honeyos.gateway.platforms.base import EphemeralReply, MessageEvent, MessageType
from honeyos.gateway.session import (
    AsyncSessionStore,
    SessionSource,
    build_session_key,
    is_shared_multi_user_session,
)
from honeyos.runtime.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from honeyos.utils import (
    atomic_json_write,
...
def _reset_tip_line() -> str:
    ...
    if os.environ.get("HONEYOS_RUNTIME_ID", "").startswith("honeyos-companion-"):
        return ""
    try:
        from honeyos.runtime.tips import get_random_tip
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    """
    from honeyos.gateway.code_skew import detect_code_skew
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: honeyos gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach HoneyOS commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        # best-effort HONEYOS-only seam: generic HoneyOS sessions do no extra I/O,
        # and any read/store failure must never block the reset itself.
        _honeyos_continuity_messages = []
        if (
            old_entry is not None
            and os.environ.get("HONEYOS_RUNTIME_ID", "").startswith("honeyos-companion-")
            and str(getattr(source, "chat_type", "") or "").lower() == "dm"
        ):
            try:
                _honeyos_continuity_messages = await self.async_session_store.load_transcript(
        ...
        # expiring durable session id (delegations dispatched from honeyos.gateway
        # chats are pinned to it via parent_session_id) and by the routing
        # key as a fallback for older records.
        try:
            from honeyos.tools.async_delegation import interrupt_for_session
        ...
        try:
            from honeyos.tools.env_passthrough import clear_env_passthrough
            clear_env_passthrough()
        except Exception:
            pass

        try:
            from honeyos.tools.credential_files import clear_credential_files
        ...
        try:
            from honeyos.runtime.lifecycle import finalize_session
            finalize_session(
                session_id=_old_sid,
        ...
        if old_entry is not None and new_entry is not None and _honeyos_continuity_messages:
            try:
                ...
                record_reset_handoff(
                    ...
                    messages=_honeyos_continuity_messages,
                )
            except Exception:
                logger.debug("HoneyOS continuity handoff save failed", exc_info=True)
            try:
                self._schedule_honeyos_memory_distillation(
                    lane_key=session_key,
                    chat_type=str(getattr(source, "chat_type", "") or ""),
        ...
        try:
            from honeyos.runtime.lifecycle import invoke_hook as _invoke_hook
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        if multiplexed:
            ...
            try:
                from honeyos.gateway.run import _profile_runtime_scope

                profile_home = self._resolve_profile_home_for_source(source)
                with _profile_runtime_scope(profile_home):
                    display = display_honeyos_home()
            except Exception:
                display = display_honeyos_home()
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        """
        from honeyos.gateway.slash_access import policy_for_source as _policy_for_source
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from honeyos.runtime.kanban import run_slash
            ...
            if m:
                ...
                try:
                    ...
                    if platform_str and chat_id:
                        def _sub():
                            from honeyos.runtime import kanban_db as _kb
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.gateway.run import _AGENT_PENDING_SENTINEL, _load_gateway_config, _resolve_gateway_model
    ...
    async def _handle_context_command(self, event: MessageEvent) -> str:
        ...
        """
        from honeyos.gateway.run import _AGENT_PENDING_SENTINEL
        ...
        if history:
            from honeyos.agent.model_metadata import estimate_messages_tokens_rough
    ...
    def _resume_caller_is_admin(self, source: SessionSource) -> bool:
        ...
        """
        try:
            from honeyos.gateway.slash_access import policy_for_source
    ...
    async def _handle_agents_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.gateway.run import _AGENT_PENDING_SENTINEL
        from honeyos.tools.process_registry import format_uptime_short, process_registry
        ...
        try:
            from honeyos.tools.async_delegation import list_async_delegations
            delegations = [
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        """
        from honeyos.gateway.run import _AGENT_PENDING_SENTINEL, _INTERRUPT_REASON_STOP
        source = event.source
        session_entry = await self.async_session_store.get_or_create_session(source)
        session_key = session_entry.session_key
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`honeyos gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from honeyos.gateway.run import _honeyos_home
        ...
        try:
            ...
            atomic_json_write(
                _honeyos_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _honeyos_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running HoneyOS version."""
        from honeyos.runtime.slash_exec import CommandContext, execute_command
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.gateway.run import _telegramize_command_mentions
        from honeyos.runtime.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from honeyos.gateway.run import _honeyos_home, _load_gateway_config
        from honeyos.runtime.model_switch import (
            switch_model as _switch_model, parse_model_switch_args,
            resolve_persist_behavior,
            list_authenticated_providers,
            list_picker_providers,
        )
        from honeyos.runtime.providers import get_label
        ...
        # via the shared single-owner parser (honeyos.runtime.model_switch).
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _honeyos_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from honeyos.runtime.context_switch_guard import (
                        ...
                        from honeyos.runtime.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from honeyos.runtime.route_identity import should_clear_context_pin_async
                                ...
                                from honeyos.runtime.config import save_config
                        ...
                        from honeyos.runtime.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — HoneyOS default runtime
        ...
        from honeyos.runtime import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        config_path = _honeyos_home / 'config.yaml'
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_honeyos_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from honeyos.runtime.goals import draft_contract
            ...
            from honeyos.runtime.goals import parse_contract
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from honeyos.runtime.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `honeyos cron` for durable schedules."
        )
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.gateway.run import _home_target_env_var, _home_thread_env_var
        source = event.source
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.gateway.run import _checkpoint_agent_kwargs, _load_gateway_config
        from honeyos.tools.checkpoint_manager import CheckpointManager, format_checkpoint_list
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        HoneyOS itself changed. ``--stat`` limits output to the summary.
        ...
        from honeyos.tools.working_diff import collect_working_diff
    ...
    async def _gateway_session_diff(self, cwd: str, stat_only: bool) -> str:
        ...
        from honeyos.gateway.run import _checkpoint_agent_kwargs, _load_gateway_config
        from honeyos.tools.checkpoint_manager import CheckpointManager
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from honeyos.gateway.run import _platform_config_key
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from honeyos.gateway.run import _honeyos_home
        from honeyos.runtime.write_approval_commands import handle_pending_subcommand
        from honeyos.tools import write_approval as wa
        from honeyos.tools.memory_tool import load_on_disk_store
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.honeyos/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``honeyos skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from honeyos.gateway.run import _honeyos_home
        from honeyos.runtime.write_approval_commands import handle_pending_subcommand
        from honeyos.tools import write_approval as wa
        ...
        # the real review surface. (Note: `honeyos skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.honeyos/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from honeyos.gateway.run import _load_gateway_config, _resolve_gateway_model
        from honeyos.runtime.models import model_supports_fast_mode
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.gateway.slash_access import policy_for_source
        from honeyos.runtime.approval_mode import run_approval_mode_command
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from honeyos.gateway.run import _honeyos_home, _load_gateway_config, _platform_config_key
        ...
        from honeyos.gateway.display_config import resolve_display_setting
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from honeyos.gateway.run import _honeyos_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
        from honeyos.gateway.runtime_footer import resolve_footer_config
        ...
        if new_state:
            ...
            from honeyos.gateway.runtime_footer import format_runtime_footer
    ...
    async def _handle_compress_command_inner(self, event: MessageEvent) -> str:
        ...
        from honeyos.runtime.partial_compress import (
        ...
        if _preview:
            ...
            from honeyos.agent.model_metadata import estimate_request_tokens_rough
            _pv_msgs = [
        ...
        try:
            ...
            from honeyos.agent.manual_compression_feedback import summarize_manual_compression
            from honeyos.agent.model_metadata import estimate_request_tokens_rough
            ...
            from honeyos.gateway.run import (
            ...
            try:
                ...
                if _lock_skipped is True or isinstance(_lock_skipped, str):
                    from honeyos.agent.manual_compression_feedback import (
    ...
    async def _handle_topic_command(self, event: MessageEvent, args: str = "") -> str:
        ...
        if not self._session_db:
            from honeyos.core.state import format_session_db_unavailable
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from honeyos.runtime.session_listing import (
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        ...
        """
        from honeyos.agent.account_usage import build_credits_view
    ...
    def _context_breakdown_block(self, agent, source, expanded: bool) -> list[str]:
        ...
        """
        try:
            from honeyos.agent.context_breakdown import (
    ...
    def _context_breakdown_lines(self, agent, source) -> list[str]:
        ...
        """
        try:
            from honeyos.agent.context_breakdown import compute_session_context_breakdown
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if wants_reset:
            ...
            from honeyos.agent.account_usage import redeem_codex_reset_credit
        ...
        try:
            from honeyos.agent.account_usage import nous_credits_lines
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            if rl_state and rl_state.has_data:
                from honeyos.agent.rate_limit_tracker import format_rate_limit_compact
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        loop = asyncio.get_running_loop()
        try:
            from honeyos.agent.skill_commands import reload_skills
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `honeyos bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``honeyos debug share`` from the CLI.
        """
        import asyncio
        from honeyos.runtime.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update HoneyOS to the latest version.

        Spawns ``honeyos update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``honeyos update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from honeyos.gateway.run import _honeyos_home, _resolve_honeyos_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from honeyos.runtime.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update HoneyOS')}"
        ...
        honeyos_cmd = _resolve_honeyos_bin()
        if not honeyos_cmd:
            return t("gateway.update.honeyos_cmd_not_found")

        pending_path = _honeyos_home / ".update_pending.json"
        output_path = _honeyos_home / ".update_output.txt"
        exit_code_path = _honeyos_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `honeyos update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `honeyos update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # honeyos_cmd is a list of argv parts we can pass directly
                ...
                honeyos_cmd_str = " ".join(shlex.quote(part) for part in honeyos_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {honeyos_cmd_str} update --gateway"
