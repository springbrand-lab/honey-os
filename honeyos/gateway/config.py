...
from honeyos.agent.secret_scope import current_secret_scope, get_secret as _get_secret
...
# dashboard's pre-write mutation validation (honeyos.runtime/web_server.py) so
...
class PlatformConfig:
    ...
    # "HoneyOS is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_honeyos_home() / "sessions")
    ...
    # gateway behaves exactly as before — single HONEYOS_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``honeyos config set gateway.multiplex_profiles true``.
        ...
        from honeyos.gateway.profile_routing import parse_profile_routes
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.honeyos/config.yaml (primary user-facing config)
    3. ~/.honeyos/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_honeyos_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through honeyos.runtime.config.load_config, so without this a managed
            ...
            # (e.g. via `honeyos config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `honeyos config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``honeyos config set gateway.streaming.*``
            ...
            try:
                ...
                from honeyos.gateway.platform_registry import platform_registry as _pr
            except Exception as e:
