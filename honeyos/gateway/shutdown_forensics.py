...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        honeyos_home_str = os.environ.get("HONEYOS_HOME")
        if honeyos_home_str:
            takeover_path = Path(honeyos_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(honeyos_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded honeyos-agent but never re-ran ``honeyos setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../honeyos-gateway.service"
    ...
    # that's the common case for honeyos.
    ...
    # constant in honeyos.runtime/gateway.py.
    headroom = 30.0
