...
The store is a small JSON file under the active profile's HONEYOS_HOME so each
...
# Error kinds (from honeyos.gateway.platforms.base.classify_send_error) that mean the
...
class DeadTargetRegistry:
    ...
    def __init__(self, path: Optional[Path] = None) -> None:
        ...
        if path is not None:
            ...
            self._path = get_honeyos_home() / "gateway" / "dead_targets.json"
        self._load()
    ...
    def all_dead(self) -> Dict[str, Dict[str, object]]:
        """Snapshot of the current dead set (for diagnostics / `honeyos` CLI)."""
        with self._lock:
