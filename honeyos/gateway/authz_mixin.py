...
(``~/.honeyos/plans/god-file-decomposition.md``, Phase 3 mechanical mixin lifts).
...
that uses it (``from honeyos.gateway.run import logger`` resolves at call time, when
...
class GatewayAuthorizationMixin:
    ...
    def _adapter_group_has_sender_allowlist(
        ...
        *,
        ...
        restricting which senders inside that group can invoke HoneyOS. If such a
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        5. Default: deny
        """
        from honeyos.gateway.run import logger
        ...
        # (honeyos gateway pairing approve / the authenticated dashboard) — an
