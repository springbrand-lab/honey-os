...
Usage:
    from honeyos.toolsets import get_toolset, resolve_toolset, get_all_toolsets
...
_HONEYOS_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on HONEYOS_DESKTOP
    ...
    # Skills
    "skills_list", "skill_view", "skill_manage", "skill_marketplace",
    # Browser automation
    ...
    # spawned as a kanban worker (HONEYOS_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `honeyos tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``honeyos tools`` → Video Generation."
        ),
    ...
    "skills": {
        ...
        "tools": ["skills_list", "skill_view", "skill_manage", "skill_marketplace"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (HONEYOS_KANBAN_TASK env "
    ...
    # Coding posture (base HoneyOS — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in honeyos.runtime/tools_config.py).
        "posture": True,
    },

    # ==========================================================================
    # Full HoneyOS toolsets (CLI + messaging platforms)
    #
    ...
    # the `honeyos send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "honeyos-cli": {
        ...
        "tools": _HONEYOS_CORE_TOOLS,
        "includes": []
    },

    "honeyos-cron": {
        # Mirrors honeyos-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `honeyos tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as honeyos-cli; gated by `honeyos tools`",
        "tools": _HONEYOS_CORE_TOOLS,
        "includes": []
    },
    ...
    "honeyos-email": {
        "description": "Email bot toolset - interact with HoneyOS via email (IMAP/SMTP)",
        "tools": _HONEYOS_CORE_TOOLS,
        "includes": []
    },
    ...
    "honeyos-sms": {
        "description": "SMS bot toolset - interact with HoneyOS via SMS (Twilio)",
        "tools": _HONEYOS_CORE_TOOLS,
        "includes": []
    },
    ...
    "honeyos-webhook": {
        ...
        "tools": _HONEYOS_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "honeyos-gateway": {
        ...
        "tools": [],
        "includes": ["honeyos-telegram", "honeyos-discord", "honeyos-whatsapp", "honeyos-slack", "honeyos-signal", "honeyos-bluebubbles", "honeyos-homeassistant", "honeyos-email", "honeyos-sms", "honeyos-mattermost", "honeyos-matrix", "honeyos-dingtalk", "honeyos-feishu", "honeyos-wecom", "honeyos-wecom-callback", "honeyos-weixin", "honeyos-qqbot", "honeyos-webhook", "honeyos-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``honeyos-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_HONEYOS_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``honeyos-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (honeyos-<name>).
        # Gives them _HONEYOS_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("honeyos-"):
            platform_name = name[len("honeyos-"):]
            try:
                from honeyos.gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_HONEYOS_CORE_TOOLS)
                    try:
                        from honeyos.tools.registry import registry
                        plugin_tools.update(
