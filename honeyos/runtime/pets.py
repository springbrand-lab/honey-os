"""CLI subcommand: ``honeyos pets <subcommand>``.
...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: honeyos pets install boba")
            return 0
    ...
    from honeyos.agent.pet.manifest import ManifestError, fetch_manifest
    ...
    _print("\nInstall one with: honeyos pets install <slug>")
    return 0
...
def _cmd_install(args) -> int:
    ...
    if getattr(args, "select", False) or not _has_active_pet():
        ...
        _print(f"  Make it active with: honeyos pets select {slug}")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: honeyos pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: honeyos pets install {slug}")
        return 1
...
def _cmd_show(args) -> int:
    ...
    from honeyos.agent.pet import store
    from honeyos.agent.pet.constants import DEFAULT_SCALE, LOOP_MS, STATE_ROWS, PetState, resolve_cols
    from honeyos.agent.pet.render import build_renderer

    cfg = _pet_config()
    slug = (getattr(args, "slug", "") or "").strip() or str(cfg.get("slug", "") or "")
    pet = store.resolve_active_pet(slug)
    if pet is None:
        _err("✗ no pet to show — run: honeyos pets install boba")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    from honeyos.agent.pet import store
    from honeyos.agent.pet.render import detect_terminal_graphics, resolve_mode
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: honeyos pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: honeyos pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: honeyos pets select " + active.slug)
...
def set_pet_scale(value: float | str) -> tuple[float, str | None]:
    ...
    """
    from honeyos.agent.pet.constants import clamp_scale
