...
``cli.py``'s ``HoneyOSCLI`` class. ``HoneyOSCLI`` inherits
...
Import discipline mirrors ``honeyos.runtime.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded; honors HONEYOS_DEV_CREDITS_FIXTURE for offline testing.
        """
        from cli import _cprint, _b, _d

        try:
            from honeyos.agent.billing_usage import build_usage_model, format_renews
    ...
    def _show_subscription(self):
        ...
        from honeyos.agent.subscription_view import build_subscription_state, subscription_manage_url
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                print("  Run `honeyos portal` to log in, then /subscription.")
            return
    ...
    def _subscription_overview(self, state, manage_url):
        ...
        from honeyos.agent.billing_usage import format_renews
        try:
            from honeyos.agent.billing_usage import build_usage_model
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``honeyos.runtime.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _subscription_free_catalog(self, state, manage_url):
        ...
        from honeyos.agent.subscription_view import (
            format_tier_row,
            selectable_tiers,
    ...
    def _subscription_pick_tier(self, state):
        ...
        from honeyos.agent.subscription_view import format_tier_row, is_upgrade, selectable_tiers
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from honeyos.agent.subscription_view import subscription_change_preview_from_payload
        from honeyos.runtime.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
        ...
        if effect not in ("charge_now", "scheduled"):
            ...
            from honeyos.agent.subscription_view import is_upgrade, subscription_manage_url
        ...
        if effect == "charge_now":
            ...
            try:
                from honeyos.agent.billing_view import build_billing_state
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from honeyos.runtime.nous_billing import (
            BillingError,
            BillingTransient,
        ...
        key = None
        if kind == "upgrade":
            from honeyos.agent.billing_view import new_idempotency_key
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `honeyos portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from honeyos.runtime.auth import step_up_nous_billing_scope
        ...
        try:
            from honeyos.runtime import nous_billing as _nb
        ...
        from honeyos.agent.subscription_view import build_subscription_state
    ...
    def _show_billing(self, command: str = "/topup"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `honeyos portal` to log in, then /topup.")
            return
    ...
    def _billing_overview(self, state):
        ...
        from honeyos.agent.billing_view import format_money
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's HoneyOS page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's HoneyOS page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_buy_flow(self, state):
        ...
        from honeyos.agent.billing_view import format_money, validate_charge_amount
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from honeyos.agent.billing_view import format_money, new_idempotency_key
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        while _time.time() < deadline:
            ...
            if state_str == "settled":
                ...
                from honeyos.agent.billing_view import parse_money
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `honeyos portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `honeyos portal` to log in again.")
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's HoneyOS page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `honeyos portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's HoneyOS page, then run /topup again.")
        ...
        from honeyos.runtime.nous_billing import BillingError, post_charge
