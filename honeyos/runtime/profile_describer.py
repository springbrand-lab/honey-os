...
Used by ``honeyos profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``honeyos.runtime/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the HoneyOS kanban board.
...
Rules:
  ...
  - Never write "HoneyOS profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_honeyos_home())
        else:
    ...
    try:
        from honeyos.agent.auxiliary_client import call_llm  # type: ignore
    except Exception as exc:
