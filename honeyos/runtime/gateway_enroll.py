"""``honeyos gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.honeyos/auth.json``) — the same path ``honeyos dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.honeyos/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    ...
    """
    from honeyos.gateway.relay import _resolve_relay_identity_token
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `honeyos auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from honeyos.runtime.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `honeyos gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.honeyos/.env (0600 dir) and never logs.
    to_write = {
