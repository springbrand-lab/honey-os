"""``honeyos logs`` — view and filter HoneyOS log files.
...
Usage examples::
    ...
    honeyos logs                    # last 50 lines of agent.log
    honeyos logs -f                 # follow agent.log in real time
    honeyos logs errors             # last 50 lines of errors.log
    honeyos logs gateway -n 100    # last 100 lines of gateway.log
    honeyos logs gui -f            # follow gui.log (dashboard/pty/ws)
    honeyos logs desktop -f        # follow desktop.log (Electron app boot/backend)
    honeyos logs --level WARNING    # only WARNING+ lines
    honeyos logs --session abc123   # filter by session ID substring
    honeyos logs --component tools  # only tool-related lines
    honeyos logs --since 1h         # lines from the last hour
    honeyos logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_honeyos_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when HoneyOS runs — try 'honeyos chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_honeyos_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_honeyos_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_honeyos_home()}/logs/")
        return

    print(f"Log files in {display_honeyos_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'honeyos chat' to generate logs)")
