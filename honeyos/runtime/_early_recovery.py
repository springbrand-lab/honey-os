"""Dependency-light venv recovery that runs BEFORE honeyos.runtime.main's imports.

The ``honeyos`` console entry point is ``honeyos.runtime.main:main``.  Importing
``honeyos.runtime.main`` pulls in third-party packages at module level (``dotenv``
via ``honeyos.runtime.env_loader``, ``yaml`` via ``honeyos.runtime.config``, ...).  In
...
a corrupted venv.  ``honeyos.runtime.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``honeyos.runtime.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``honeyos acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``honeyos.runtime.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``honeyos.runtime.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``honeyos update`` exists AND an import probe
