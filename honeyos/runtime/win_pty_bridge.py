"""Windows ConPTY bridge for the `honeyos dashboard` chat tab.

Drop-in counterpart to ``honeyos.runtime.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``honeyos.runtime.web_server`` consumes: ``spawn``, ``read``,
