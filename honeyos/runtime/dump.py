"""
Dump command for honeyos CLI.

Outputs a compact, plain-text summary of the user's HoneyOS setup
...
from honeyos.runtime.config import get_honeyos_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.honeyos/.env.
    ...
    exports. ``honeyos debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.honeyos_build_sha`` by the Dockerfile's
    ``HONEYOS_GIT_SHA`` build-arg (see ``honeyos.runtime/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``honeyos dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from honeyos.runtime.gateway import get_gateway_runtime_snapshot
...
def _count_skills(honeyos_home: Path) -> int:
...
def _cron_summary(honeyos_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = honeyos_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"honeyos_home:      {display_honeyos_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.honeyos/.env: a managed
        ...
        # A credential added via `honeyos auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `honeyos auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["honeyos-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(honeyos_home)}")
    lines.append(f"  skills:             {_count_skills(honeyos_home)}")
