"""``honeyos.runtime/_scan_venv_blockers.py`` — Standalone venv-process scan for JSON consumption.

Invoked by the Desktop Electron app::

    venv\\Scripts\\python.exe -m honeyos.runtime._scan_venv_blockers
...
def _redact_sensitive_cmdline(cmdline: str) -> str:
    ...
    try:
        from honeyos.agent.redact import redact_sensitive_text  # noqa: PLC0415
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    honeyos.runtime.main gateway run`` and the uv-side interpreter re-running the
    same argv). Reporting those as blockers dead-ends the Desktop update:
    the preflight aborts with ``venv-blocked`` *before* spawning
    ``honeyos-setup``, so the CLI updater's own
    ``_pause_windows_gateways_for_update()`` — which exists precisely to
    stop these processes (and is always active: ``honeyos-setup`` invokes
    ``honeyos update --yes --gateway``) — never gets the chance to run.
    ...
    *value* shadowed the subcommand token. An import failure counts as
    ...
    """
    try:
        from honeyos.gateway.status import looks_like_gateway_command_line  # noqa: PLC0415
    except Exception:
        return False
...
def main() -> None:
    ...
    try:
        from honeyos.runtime.main import _detect_venv_python_processes  # noqa: PLC0415
