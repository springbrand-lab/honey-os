...
Add, remove, or reorder entries here — both `honeyos setup` and
`honeyos` provider-selection will pick up the change automatically.
"""
...
from honeyos.runtime.urllib_security import open_credentialed_url

logger = logging.getLogger(__name__)

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HONEYOS_USER_AGENT = f"honeyos-cli/{_HONEYOS_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `honeyos model`
    flow without maintaining a separate static list.
    """
    from honeyos.runtime.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $HONEYOS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append HoneyOS-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $HONEYOS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. HoneyOS refreshes the cache from
    ...
    """
    try:
        from honeyos.agent.models_dev import _load_disk_cache
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see honeyos.runtime/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older HoneyOS that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older HoneyOS that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_honeyos_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$HONEYOS_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  honeyos model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `honeyos model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `honeyos model` TUI
...
try:
    from honeyos.providers import list_providers as _list_providers_for_canonical
...
#
# Some vendors expose several HoneyOS provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `honeyos model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``honeyos model``, the
...
# In-repo fallback for the model HoneyOS silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from honeyos.runtime.model_catalog import get_default_model_from_cache
...
# ``honeyos model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in honeyos.runtime/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``honeyos auth add openai-codex`` without
    ``honeyos model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``honeyos model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    honeyos-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from honeyos.runtime.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — honeyos-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model HoneyOS lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from honeyos.runtime.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from honeyos.core.constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    """Derive sale chrome from honeyos.gateway ``pricing.original`` when cheaper.
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return honeyos-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; honeyos's picker expects
...
def _fireworks_pricing_from_models_dev(
    *,
    ...
    result: dict[str, dict[str, str]] = {}
    try:
        from honeyos.agent.models_dev import _get_provider_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``honeyos model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from honeyos.runtime.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from honeyos.runtime.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to HoneyOS' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``honeyos.runtime.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether HoneyOS should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``honeyos auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.honeyos/.env``.
    ...
    """
    try:
        from honeyos.runtime.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from honeyos.runtime.model_search import model_alias_canonical
...
def _merge_with_models_dev(provider: str, curated: list[str]) -> list[str]:
    ...
    """
    try:
        from honeyos.agent.models_dev import list_agentic_models
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a HoneyOS release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
        return model_ids(force_refresh=force_refresh)
    if normalized == "openai-codex":
        from honeyos.runtime.codex_models import get_codex_model_ids
        ...
        # a HoneyOS release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from honeyos.runtime.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a HoneyOS release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``honeyos model`` shows.
            from honeyos.runtime.providers import is_official_openai_host
    ...
    if normalized == "bedrock":
        try:
            from honeyos.agent.bedrock_adapter import bedrock_model_ids_or_none
...
#
# Cache strategy:
#   - One JSON file at $HONEYOS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from honeyos.core.constants import get_honeyos_home
    return get_honeyos_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$HONEYOS_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `honeyos config set
...
def _fetch_anthropic_models(
    ...
    *,
    ...
    """
    try:
        from honeyos.agent.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from honeyos.agent.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _HONEYOS_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: HoneyOS' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through HoneyOS' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _HONEYOS_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"honeyos-agent/{_HONEYOS_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=honeyos"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from honeyos.core.constants import get_honeyos_home
    return get_honeyos_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `honeyos moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"HoneyOS will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so HoneyOS cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models HoneyOS actually ships.  (#46850)
            #
    ...
    if normalized == "bedrock":
        try:
            from honeyos.agent.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
