"""
MCP Server Management CLI — ``honeyos mcp`` subcommand.

Implements ``honeyos mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.honeyos/config.yaml under the ``mcp_servers`` key.
"""
...
from honeyos.runtime.config import (
    ...
    get_env_value,
    save_env_value,
    get_honeyos_home,  # noqa: F401 — used by test mocks
)
...
from honeyos.tools.mcp_tool import _ENV_VAR_PATTERN, _env_ref_name
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from honeyos.runtime.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.honeyos/.env`` into ``os.environ`` and recursively interpolate any
    ...
    """
    from honeyos.tools.mcp_tool import _interpolate_env_vars

    from honeyos.agent.secret_scope import current_secret_scope
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``honeyos mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from honeyos.tools.mcp_oauth import HoneyOSTokenStorage
        return HoneyOSTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # honeyos.runtime/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  honeyos mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  honeyos mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  honeyos mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_honeyos_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: honeyos mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_honeyos_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `honeyos mcp test` in the same session) is evicted too.
    try:
        from honeyos.tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  honeyos mcp add <name> --url <endpoint>')
        _info('  honeyos mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``honeyos mcp login`` and
    ``honeyos mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `honeyos mcp remove` + `honeyos mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `honeyos mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (HoneyOS desktop / agent-
    ...
    try:
        from honeyos.tools.mcp_oauth import force_interactive_oauth
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `honeyos mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``honeyos mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``honeyos mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: honeyos mcp reauth <name>   |   honeyos mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── honeyos mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'honeyos mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
    ...
    try:
        from honeyos.tools.mcp_tool import matches_name_filter
...
def mcp_command(args):
    """Main dispatcher for ``honeyos mcp`` subcommands."""
    ...
    if action == "picker":
        from honeyos.runtime.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from honeyos.runtime.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from honeyos.runtime.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `honeyos plugin`.
        from honeyos.runtime.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("honeyos mcp                                    Open the catalog picker (default)")
        _info("honeyos mcp catalog                            List Nous-approved MCPs")
        _info("honeyos mcp install <name>                     Install a catalog MCP")
        _info("honeyos mcp serve                              Run as MCP server")
        _info("honeyos mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("honeyos mcp add <name> --command <cmd>         Add a stdio server")
        _info("honeyos mcp add <name> --preset <preset>       Add from a known preset")
        _info("honeyos mcp remove <name>                      Remove a server")
        _info("honeyos mcp list                               List configured servers")
        _info("honeyos mcp test <name>                        Test connection")
        _info("honeyos mcp configure <name>                   Toggle tools")
        _info("honeyos mcp login <name>                       Re-authenticate OAuth")
        _info("honeyos mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
