"""Pydantic request/response models for the HoneyOS dashboard web server.

Extracted verbatim from ``honeyos.runtime/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from honeyos.runtime.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``honeyos model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `honeyos import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`honeyos -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the HONEYOS_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under HONEYOS_HOME.
    output: str = ""
