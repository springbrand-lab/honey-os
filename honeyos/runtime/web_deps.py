...
Why this exists
---------------
``honeyos.runtime/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``honeyos.runtime/web_routers/`` need those helpers, but
...
thin proxy that resolves ``honeyos.runtime.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``honeyos.runtime.web_server`` module (imported on demand)."""
    mod = sys.modules.get("honeyos.runtime.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import honeyos.runtime.web_server as mod  # type: ignore[no-redef]
    return mod
