...
``CommandDef.execute`` (honeyos.runtime/commands.py) names a key in
...
for a fixed context (enforced by tests/honeyos.runtime/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``honeyos.runtime.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_version(ctx: CommandContext) -> CommandReply:
    ...
    from honeyos.runtime.banner import format_banner_version_label
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_honeyos_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    try:
        from honeyos.agent.skill_bundles import _bundles_dir, list_bundles
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: honeyos bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from honeyos.runtime.commands import gateway_help_lines
