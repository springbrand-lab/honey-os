...
def _root() -> Path:
    ...
    # helper's token, including custom HONEYOS_HOME layouts.
    return get_default_honeyos_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, honeyos_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(honeyos_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the honeyos exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs honeyos.runtime.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "honeyos.runtime.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "honeyos.runtime.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, honeyos_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, honeyos_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves honeyos.runtime's own location and prepends its parent, because the
    ...
    # the honeyos.runtime package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the honeyos.runtime package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["honeyosPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("HoneyOS path must be absolute")
    honeyos_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(honeyos_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("HoneyOS Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('honeyos.runtime.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_honeyos(honeyos_path: str) -> dict[str, Any]:
    path = os.path.abspath(honeyos_path)
    if not os.path.isabs(honeyos_path) or not os.path.isfile(path):
        raise ValueError("HoneyOS path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "honeyosHome": str(get_default_honeyos_root()), "python": sys.executable}
