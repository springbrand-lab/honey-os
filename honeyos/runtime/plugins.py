"""
HoneyOS Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with honeyos-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.honeyos/plugins/<name>/``
3. **Project plugins** – ``./.honeyos/plugins/<name>/`` (opt-in via
   ``HONEYOS_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``honeyos_agent.plugins``
   entry-point group.
...
from honeyos.utils import env_var_enabled, fast_safe_load
from honeyos.runtime.config import cfg_get
from honeyos.runtime.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``HONEYOS_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("HONEYOS_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``HONEYOS_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.honeyos/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("HONEYOS_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When HONEYOS_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other HoneyOS loggers.
    """
    ...
    logger.debug(
        "HONEYOS_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. HoneyOS' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by honeyos.runtime.kanban_db when a task
    ...
    # separate `honeyos -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `honeyos kanban dispatch`),
...
ENTRY_POINTS_GROUP = "honeyos_agent.plugins"

_NS_PARENT = "honeyos_plugins"
...
class PluginManifest:
    ...
    #              in ~/.honeyos/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``honeyos plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from honeyos.agent.plugin_llm import PluginLlm
    ...
    def subagent_lifecycle(self) -> Any:
        ...
        if self._subagent_lifecycle is None:
            from honeyos.agent.subagent_lifecycle import (
    ...
    def profile_name(self) -> str:
        """Return the active HoneyOS profile name (e.g. ``"default"``).

        Derived from ``HONEYOS_HOME`` via
        :func:`honeyos.runtime.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.honeyos/profiles/<name>``, or ``"custom"`` when
        ``HONEYOS_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with HoneyOS core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``honeyos honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``honeyos <subcommand>``
        ...
        try:
            from honeyos.runtime.commands import resolve_command
    ...
    def register_context_engine(self, engine) -> None:
        ...
        from honeyos.agent.context_engine import ContextEngine
    ...
    def register_image_gen_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from honeyos.agent.image_gen_provider import ImageGenProvider
        from honeyos.agent.image_gen_registry import register_provider
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`honeyos.runtime.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_video_gen_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from honeyos.agent.video_gen_provider import VideoGenProvider
        from honeyos.agent.video_gen_registry import register_provider as _register_video_provider
    ...
    def register_web_search_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from honeyos.agent.web_search_provider import WebSearchProvider
        from honeyos.agent.web_search_registry import register_provider as _register_web_provider
    ...
    def register_browser_provider(self, provider) -> None:
        ...
        """
        from honeyos.agent.browser_provider import BrowserProvider
        from honeyos.agent.browser_registry import register_provider as _register_browser_provider
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_honeyos_dotenv()`` startup — after
        ``~/.honeyos/.env`` loads, before HoneyOS reads credentials — when
        ...
        """
        from honeyos.agent.secret_sources.base import SecretSource
        from honeyos.agent.secret_sources.registry import register_source
    ...
    def register_tts_provider(self, provider) -> None:
        ...
        """
        from honeyos.agent.tts_provider import TTSProvider
        from honeyos.agent.tts_registry import register_provider as _register_tts_provider
    ...
    def register_transcription_provider(self, provider) -> None:
        ...
        **but only when**:
        ...
        backends).
        """
        from honeyos.agent.transcription_provider import TranscriptionProvider
        from honeyos.agent.transcription_registry import register_provider as _register_stt_provider
    ...
    def register_platform(
        ...
        **entry_kwargs: Any,
        ...
        """
        from honeyos.gateway.platform_registry import platform_registry, PlatformEntry
    ...
    def register_slack_action_handler(
        ...
        HoneyOS' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``honeyos model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: honeyos.runtime.main imports plugins indirectly
        from honeyos.runtime.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.honeyos/skills/`` tree and is **not** listed in the system
        ...
        """
        from honeyos.agent.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("HONEYOS_SAFE_MODE"):
            logger.info("HONEYOS_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to honeyos.runtime/. Two layouts are
        ...
        # 2. User plugins (~/.honeyos/plugins/)
        user_dir = get_honeyos_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.honeyos/plugins/)
        if _env_enabled("HONEYOS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".honeyos" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set HONEYOS_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with honeyos and must
            ...
            # all ~20 of them added several seconds to every `honeyos`
            # invocation — including plain `honeyos chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform HoneyOS ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `honeyos plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``honeyos plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`honeyos plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        from honeyos.tools.registry import registry as _registry
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `honeyos plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``honeyos_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``honeyos_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def _get_pre_tool_call_directive_details(
    ...
    from honeyos.runtime.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="honeyos-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``honeyos tools`` TUI so plugin-provided toolsets appear
