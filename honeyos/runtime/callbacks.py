...
into prompt_toolkit's event loop. Each function takes the HoneyOSCLI instance
...
from honeyos.runtime.banner import cprint, _DIM, _RST
from honeyos.runtime.config import save_env_value_secure
...
def clarify_callback(cli, question, choices, multi_select=False):
    ...
    from honeyos.tools.clarify_gateway import resolve_clarify_timeout
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.honeyos/.env and never exposed to the model.
    """
