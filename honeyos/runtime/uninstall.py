...
- Keep data: Remove code but keep ~/.honeyos/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove HoneyOS PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing honeyos-agent or honeyos PATH entries
            new_lines = []
            skip_next = False

            for line in content.split('\n'):
                # Skip the "# HoneyOS" comment and following line
                if '# HoneyOS' in line or '# honeyos-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('honeyos' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False

                # Remove any PATH line containing honeyos
                if 'honeyos' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                from honeyos.utils import atomic_write_text

                # This is the user's own shell rc, not a HoneyOS-owned file, and
...
def remove_wrapper_script():
    """Remove the honeyos wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "honeyos",
        Path.home() / ".local" / "bin" / "honeyos-acp",
        Path.home() / ".local" / "bin" / "honeyos-agent",
        Path("/usr/local/bin/honeyos"),
        Path("/usr/local/bin/honeyos-acp"),
        Path("/usr/local/bin/honeyos-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains honeyos.runtime reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'honeyos.runtime' in content or 'honeyos-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(honeyos_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``honeyos`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this HoneyOS home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (honeyos_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``honeyos gateway run`` processes
    ...
    try:
        from honeyos.runtime.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from honeyos.runtime.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``HONEYOS_HOME`` and ``HONEYOS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\honeyos\git\cmd``, ``%LOCALAPPDATA%\honeyos\git\bin``,
#      ``%LOCALAPPDATA%\honeyos\git\usr\bin``, ``%LOCALAPPDATA%\honeyos\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\honeyos\git\`` and Node to
#      ``%LOCALAPPDATA%\honeyos\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``honeyos dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\honeyos\gateway-service\`` and sometimes
...
def _honeyos_path_markers(honeyos_home: Path) -> list[str]:
    """Path-entry substrings that identify HoneyOS-owned User-PATH entries."""
    root = str(honeyos_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare honeyos-agent install dir.
    markers = [root + "\\honeyos-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HONEYOS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "honeyos".  We don't want to catch
    # unrelated entries like "choneyos-foo" or "ephermeral", so we look for
    # backslash-honeyos as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(honeyos_home: Path) -> list[str]:
    """Strip HoneyOS-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _honeyos_path_markers(honeyos_home)
            kept: list[str] = []
...
def remove_honeyos_env_vars_windows() -> list[str]:
    """Delete HONEYOS_HOME and HONEYOS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("HONEYOS_HOME", "HONEYOS_GIT_BASH_PATH"):
                try:
...
def _is_default_honeyos_home(honeyos_home: Path) -> bool:
    """Return True when ``honeyos_home`` points at the default (non-profile) root."""
    try:
        from honeyos.core.constants import get_default_honeyos_root
        return honeyos_home.resolve() == get_default_honeyos_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its HONEYOS_HOME directory.

    We shell out to ``honeyos -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current HONEYOS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m honeyos.runtime.main` so we don't depend on a `honeyos`
    #    wrapper that may be half-removed mid-uninstall.
    honeyos_invocation = [_sys.executable, "-m", "honeyos.runtime.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                honeyos_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's HONEYOS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``honeyos uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$HONEYOS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from honeyos.runtime.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(honeyos_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ HoneyOS Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No HoneyOS Chat GUI installation was found.")
        print(f"  Checked: {honeyos_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The HoneyOS agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(honeyos_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The HoneyOS agent at {honeyos_home / 'honeyos-agent'}")
        print(f"  • Your config, sessions, and secrets under {honeyos_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The HoneyOS agent is still installed. Run 'honeyos' to use the CLI,")
    print("or 'honeyos uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.honeyos/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.honeyos/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie HONEYOS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_honeyos_home(honeyos_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.honeyos data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ HoneyOS Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {honeyos_home / 'config.yaml'}")
    print(f"  Secrets: {honeyos_home / '.env'}")
    print(f"  Data:    {honeyos_home / 'cron/'}, {honeyos_home / 'sessions/'}, {honeyos_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their HONEYOS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL HoneyOS data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the HoneyOS code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, honeyos_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • HoneyOS PATH entries from shell configs / Windows User PATH")
    print("  • HoneyOS wrapper scripts and HoneyOS-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • HoneyOS config/data: {honeyos_home}")
        if _is_default_honeyos_home(honeyos_home):
        ...
        print(f"  • Keep HoneyOS config/data: {honeyos_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``honeyos`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$HONEYOS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in honeyos_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\honeyos\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(honeyos_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No HoneyOS-owned PATH entries in User environment")

        log_info("Removing HONEYOS_HOME / HONEYOS_GIT_BASH_PATH User env vars...")
        removed_env = remove_honeyos_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No HoneyOS-set User env vars to remove")

    # 3. Remove wrapper script
    log_info("Removing honeyos command...")
    ...
    #     (only when they still point into this HoneyOS home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing HoneyOS-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(honeyos_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No HoneyOS-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(honeyos_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE HONEYOS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from honeyos.runtime.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(honeyos_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.honeyos/, just remove the honeyos-agent subdir
            if honeyos_home in project_root.parents or project_root.parent == honeyos_home:
    ...
    #     under HONEYOS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(honeyos_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(honeyos_home)
    ...
    # 5. Optionally remove ~/.honeyos/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile HONEYOS_HOME dirs live under
        ...
        try:
            if honeyos_home.exists():
                shutil.rmtree(honeyos_home)
                log_success(f"Removed {honeyos_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {honeyos_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {honeyos_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {honeyos_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://honeyos-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://honeyos-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using HoneyOS! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m honeyos.runtime.uninstall --mode <gui|lite|full>``.
    ...
    so ``import honeyos.runtime`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``honeyos.core.constants`` + ``honeyos.runtime.colors``
    (and lazily ``honeyos.runtime.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m honeyos.runtime.uninstall")
    parser.add_argument(
        "--mode",
