"""Per-provider model-selection wizard flows for ``honeyos setup`` / ``honeyos model``.

Extracted from ``honeyos.runtime/main.py`` as part of the god-file decomposition
campaign (``~/.honeyos/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from honeyos.runtime.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``honeyos.runtime.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from honeyos.runtime.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``honeyos.runtime.main`` at import time -> no import cycle.
"""
...
from honeyos.runtime.providers import custom_provider_slug
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from honeyos.runtime.auth import _resolve_api_key_provider_secret
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from honeyos.runtime.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from honeyos.runtime.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.honeyos/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `honeyos setup` from scratch. OpenRouter
    ...
    from honeyos.runtime.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.honeyos/.env by hand.
    ...
    from honeyos.runtime.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from honeyos.runtime.auth import _save_model_choice, deactivate_provider
    from honeyos.runtime.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `honeyos moa configure <name>` first.")
        return
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from honeyos.runtime.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from honeyos.runtime.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `honeyos auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``honeyos auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from honeyos.runtime.main import _DEFAULT_QWEN_PORTAL_MODELS
    from honeyos.runtime.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from honeyos.runtime.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from honeyos.runtime.auth import _save_model_choice, deactivate_provider
    from honeyos.runtime.config import (
    ...
    from honeyos.runtime.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `honeyos model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from honeyos.runtime.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from honeyos.runtime.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from honeyos.runtime import azure_detect
    ...
    print("Anthropic-style API endpoints.  HoneyOS will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  HoneyOS will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from honeyos.runtime.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from honeyos.runtime.auth import _save_model_choice, deactivate_provider
    from honeyos.runtime.config import load_config, save_config
    from honeyos.runtime.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from honeyos.runtime.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from honeyos.runtime.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from honeyos.runtime.config import save_env_value, load_config, save_config
    from honeyos.runtime.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from honeyos.runtime.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from honeyos.runtime.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates HoneyOS turns to `copilot --acp`.")
    print("  HoneyOS currently starts its own ACP subprocess for each request.")
    print("  HoneyOS uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set HONEYOS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from honeyos.runtime.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from honeyos.runtime.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from honeyos.runtime.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `honeyos auth`)
    from honeyos.runtime.auth import _resolve_api_key_provider_secret, ProviderConfig
    ...
    if selected:
        ...
        # Save the API key env var name so honeyos knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from honeyos.runtime.config import load_config, save_config, get_env_value
    from honeyos.runtime.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.honeyos/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``honeyos.runtime.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from honeyos.runtime.main import _prompt_provider_choice
    from honeyos.runtime.auth import ZAI_ENDPOINTS
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from honeyos.agent.gemini_native_adapter import probe_gemini_tier
        except Exception:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   HoneyOS typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with HoneyOS, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from honeyos.runtime.main import _run_anthropic_oauth_flow
    from honeyos.runtime.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'honeyos model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
