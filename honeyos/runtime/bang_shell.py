...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("HONEYOS_GATEWAY_SESSION"):
        return False
    if env_var_enabled("HONEYOS_CRON_SESSION"):
        return False
    if (os.getenv("HONEYOS_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def resolve_bang_cwd(session_key: Optional[str] = None) -> Optional[str]:
    ...
    """
    try:
        from honeyos.tools.terminal_tool import _get_env_config, get_session_cwd
...
def check_bang_approval(command: str) -> dict:
    ...
    """
    try:
        from honeyos.tools.terminal_tool import _check_all_guards
    except Exception:
...
def _bang_env() -> dict:
    """Environment for a bang command, with HoneyOS-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the HoneyOS session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
