"""
Unified tool configuration for HoneyOS.

`honeyos tools` and `honeyos setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.honeyos/config.yaml under
...
from honeyos.runtime.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from honeyos.runtime.nous_account import format_nous_portal_entitlement_message
from honeyos.tools.tool_backend_helpers import fal_key_is_configured
from honeyos.utils import base_url_hostname, is_truthy_value
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``honeyos tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`honeyos.runtime._subprocess_compat.windows_hide_flags`) suppresses it
...
from honeyos.runtime.cli_output import (  # noqa: E402 — late import block
...
# They're still in _HONEYOS_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `honeyos tools` → Video Generation, which walks
...
# `honeyos tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `honeyos tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from honeyos.runtime.auth import _read_xai_oauth_tokens
        ...
        pass
    try:
        from honeyos.tools.xai_http import get_env_value as _xai_get_env_value
        ...
        pass
    try:
        from honeyos.agent.secret_scope import get_secret
...
# Platform-scoped toolsets: only appear in the `honeyos tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``honeyos tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from honeyos.runtime.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``honeyos tools`` checklist actually offers
    for ``platform``.
    ...
    so ``honeyos tools`` never claims to add or remove a toolset the user was
...
from honeyos.runtime.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "HoneyOS routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # injected at runtime from honeyos.plugins.browser.<vendor>.provider via
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set HONEYOS_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "HONEYOS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "HONEYOS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "HONEYOS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "HONEYOS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "HONEYOS_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `honeyos model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("HONEYOS_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _resolved_cua_driver_cmd() -> Optional[str]:
    """Resolve cua-driver exactly as the runtime and Desktop status do."""
    from honeyos.tools.computer_use.cua_backend import resolve_cua_driver_cmd

    return resolve_cua_driver_cmd()


def _cua_driver_env() -> dict:
    """cua-driver child env with the HoneyOS telemetry policy applied.
    ...
    """
    try:
        from honeyos.tools.computer_use.cua_backend import cua_driver_child_env
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $HONEYOS_HOME/bin is never on PATH, so a bare which()
    # misses the uv HoneyOS installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``honeyos update`` and
      by ``honeyos computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``honeyos update`` sets
    ...
    a further ~600s wait on Windows). ``honeyos computer-use install
    ...
    ``honeyos update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `honeyos update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `honeyos computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `honeyos update` (require_confirmed_update=True) keeps the
    ...
    # `honeyos computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    confirmed_version = None
    if binary:
        _state = None
        try:
            from honeyos.tools.computer_use.cua_backend import cua_driver_update_check
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: honeyos computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `honeyos update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `honeyos update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.honeyos/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / HoneyOS process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $HONEYOS_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # HoneyOS installed for exactly this toolchain.
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_honeyos_home()}/honeyos-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in honeyos-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/honeyos-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import honeyos.tools.browser_tool as _bt
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.honeyos/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.honeyos/config.yaml")
        ...
        # Run the full `honeyos auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.honeyos/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from honeyos.runtime.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: honeyos auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `honeyos auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: honeyos auth spotify")
        except Exception as exc:
        ...
        try:
            from honeyos.runtime.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: honeyos plugins enable observability/langfuse")
        _print_info("    Restart HoneyOS for tracing to take effect.")
        _print_info("    Verify: honeyos plugins list")
        ...
        try:
            ...
            _print_info("    Run later: honeyos auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `honeyos auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: honeyos auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: honeyos auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``honeyos tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``honeyos tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: honeyos tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``honeyos-discord``, which contains those
...
#:
#: Saving ``honeyos tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[honeyos-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``honeyos tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"honeyos-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (honeyos-acp, honeyos-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    from honeyos.toolsets import resolve_toolset, TOOLSETS
    ...
    # ``honeyos-discord``) is an opt-in to the platform's native default-off
    ...
    # "honeyos-cli" (which include all _HONEYOS_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[honeyos-cli, spotify]`` after enabling Spotify via ``honeyos
        ...
        # (e.g. "honeyos-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``honeyos tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `honeyos tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"honeyos-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("honeyos-")}
    ...
    # shipped as a bundled plugin but user must opt in via `honeyos tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `honeyos tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by honeyos tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `honeyos` instead of
    # `honeyos-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `honeyos update`/`honeyos doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `honeyos tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (honeyos-cli, honeyos-telegram, etc.)
    ...
    # Opening `honeyos tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key == "vision":
        try:
            from honeyos.agent.auxiliary_client import resolve_vision_provider_client
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    try:
        ...
        from honeyos import model_tools # noqa: F401
...
def _plugin_video_gen_providers() -> list[dict]:
    ...
    """
    try:
        from honeyos.agent.video_gen_registry import list_providers
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
def _plugin_web_search_providers() -> list[dict]:
    ...
    """
    try:
        from honeyos.agent.web_search_registry import list_providers as _list_web_providers
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
def web_provider_capabilities(backend: str) -> list:
    ...
    """
    try:
        from honeyos.agent.web_search_registry import get_provider
...
def _plugin_browser_providers() -> list[dict]:
    ...
    """
    try:
        from honeyos.agent.browser_registry import list_providers as _list_browser_providers
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
def _plugin_tts_providers() -> list[dict]:
    ...
    """
    try:
        from honeyos.agent.tts_registry import _BUILTIN_NAMES, list_providers
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `honeyos tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    from honeyos.runtime.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``honeyos tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from honeyos.runtime.nous_subscription import _has_agent_browser
...
def _fal_model_catalog():
    ...
    from honeyos.tools.image_generation_tool import FAL_MODELS, DEFAULT_MODEL
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from honeyos.agent.image_gen_registry import get_provider
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
def _plugin_video_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from honeyos.agent.video_gen_registry import get_provider
        from honeyos.runtime.plugins import _ensure_plugins_discovered
...
# Kept in sync with the dashboard selects (honeyos.runtime/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `honeyos model`).
    if managed_feature:
        from honeyos.runtime.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``honeyos model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``honeyos model`` aux-task
    ...
    """
    try:
        from honeyos.runtime.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `honeyos model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `honeyos model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `honeyos tools` and `honeyos setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ HoneyOS Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://honeyos-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from honeyos.core.constants import display_honeyos_home
    print(color(f"  Tool configuration saved to {display_honeyos_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'honeyos' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    try:
        from honeyos.tools.mcp_tool import probe_mcp_server_tools
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, honeyos mcp
