"""
Cron subcommand for honeyos CLI.
...
# commands at execution time when ``_HONEYOS_GATEWAY=1``.
from honeyos.scheduler.lifecycle_guard import (  # noqa: F401  (re-exported for terminal_tool)
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: honeyos gateway install", Colors.DIM))
    print(color("                    sudo honeyos gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  honeyos cron status", Colors.DIM))


def cron_list(show_all: bool = False):
    """List all scheduled jobs."""
    from honeyos.scheduler.jobs import list_jobs

    jobs = list_jobs(include_disabled=show_all)

    if not jobs:
        print(color("No scheduled jobs.", Colors.DIM))
        print(color("Create one with 'honeyos cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_tick():
    ...
    from honeyos.scheduler.scheduler import tick
    tick(verbose=True)


def cron_runs(job_id: Optional[str] = None, limit: int = 20):
    """Show indexed durable cron execution history."""
    from honeyos.scheduler.executions import list_executions
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: honeyos gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec honeyos "
                        "honeyos cron ...`). Fix ownership to match the "
        ...
        print("    honeyos gateway install    # Install as a user service")
        print("    sudo honeyos gateway install --system  # Linux servers: boot-time system service")
        print("    honeyos gateway            # Or run in foreground")
...
def cron_edit(args):
    from honeyos.scheduler.jobs import AmbiguousJobReference, resolve_job_ref
...
def cron_command(args):
    ...
    print("Usage: honeyos cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
