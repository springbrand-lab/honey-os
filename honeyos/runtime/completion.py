"""Shell completion script generation for honeyos CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_honeyos_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# HoneyOS bash completion
# Add to ~/.bashrc:
#   eval "$(honeyos completion bash)"

_honeyos_profiles() {{
    local profiles_dir="$HOME/.honeyos/profiles"
...
_honeyos_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_honeyos_profiles)" -- "$cur"))
        return
    fi
...
complete -F _honeyos_completion honeyos
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef honeyos
# HoneyOS zsh completion
# Add to ~/.zshrc:
#   eval "$(honeyos completion zsh)"

_honeyos_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.honeyos/profiles" ]]; then
        profiles+=($HOME/.honeyos/profiles/*(N/:t))
    fi
...
_honeyos() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_honeyos_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'honeyos command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# HoneyOS fish completion",
        "# Add to your config:",
        "#   honeyos completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __honeyos_profiles",
        "    echo default",
        "    if test -d $HOME/.honeyos/profiles",
        "        for d in $HOME/.honeyos/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c honeyos -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c honeyos -f -s p -l profile"
        " -d 'Profile name' -xa '(__honeyos_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__honeyos_profiles)' -d 'Profile name'"
                )
