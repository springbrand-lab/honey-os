"""
Profile management for multiple isolated HoneyOS instances.

Each profile is a fully independent HONEYOS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.honeyos/profiles/<name>/`` by default.

The "default" profile is ``~/.honeyos`` itself — backward compatible,
zero migration needed.

Usage::

    honeyos profile create coder          # fresh profile + bundled skills
    honeyos profile create coder --clone  # also copy config, .env, SOUL.md, skills
    honeyos profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    honeyos -p coder chat                 # or via flag
    honeyos profile use coder             # set as sticky default
    honeyos profile delete coder          # remove profile + alias + service
"""
...
from honeyos.agent.skill_utils import is_excluded_skill_path
...
_PROFILE_DIRS = [
    ...
    # See honeyos.core.constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.honeyos``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   honeyos-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `honeyos backup` archives
...
# Marker file written by `honeyos profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `honeyos
...
# `honeyos skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         HoneyOS infrastructure directories that only the default profile
         (``~/.honeyos``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_honeyos_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.honeyos) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "honeyos-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when HONEYOS_HOME equals the
...
# into the export. The set below identifies the *known HoneyOS profile
# artifacts* at the root of HONEYOS_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``honeyos.core.constants``.
...
_RESERVED_NAMES = frozenset({
    "honeyos", "default", "test", "tmp", "root", "sudo",
})

# HoneyOS subcommands that cannot be used as profile names/aliases
_HONEYOS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the honeyos root, NOT to the current HONEYOS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HONEYOS_HOME points outside
    ``~/.honeyos``, profiles live under ``HONEYOS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_honeyos_home() / "profiles"


def _get_default_honeyos_home() -> Path:
    """Return the default (pre-profile) HONEYOS_HOME path.

    In standard deployments this is ``~/.honeyos``.
    In Docker/custom deployments where HONEYOS_HOME is outside ``~/.honeyos``
    (e.g. ``/opt/data``), returns HONEYOS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_honeyos_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the HoneyOS installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HONEYOS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_honeyos_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, honeyos subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HONEYOS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a honeyos subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            honeyos_exe = shutil.which("honeyos") or "honeyos"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(honeyos_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running HoneyOS. If we
    ...
    try:
        ...
        # not be migrated. The next `honeyos doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``honeyos -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``honeyos profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``honeyos -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``honeyos profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``HONEYOS_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing HoneyOS config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_honeyos_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, honeyos_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own HONEYOS_HOME.
    ...
    The returned ``honeyos_home`` is the path to pass to
    ``set_honeyos_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_honeyos_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``honeyos update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.honeyos)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.honeyos/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `honeyos -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this HoneyOS profile.\n"
    ...
    if not soul_path.exists():
        try:
            from honeyos.runtime.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `honeyos update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`honeyos profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `honeyos update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running HoneyOS (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `honeyos profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `honeyos -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HONEYOS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``honeyos profile create
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-c",
             "import json; from honeyos.tools.skills_sync import sync_skills; "
             "r = sync_skills(quiet=True); print(json.dumps(r))"],
            env={**os.environ, "HONEYOS_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_honeyos_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running HoneyOS *backends* bound to this profile.
    ...
    names it, so find it by inspection: a HoneyOS backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``HONEYOS_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `honeyos -p <canon> profile
    ...
    honeyos_markers = ("honeyos.runtime.main", "honeyos-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a HoneyOS process: either an entrypoint marker in argv, or
            # a resolved executable named `honeyos`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_honeyos = (
                any(marker in joined for marker in honeyos_markers)
                or exe_name == "honeyos"
                or exe_name.startswith("honeyos")
            )
            if not is_honeyos:
                continue
            ...
            # ...or by HONEYOS_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("HONEYOS_HOME", "")
...
def _stop_profile_backends(canon: str, profile_dir: Path) -> None:
    ...
    try:
        from honeyos.gateway.status import _pid_exists, terminate_pid as _terminate_pid
    except Exception:
        return
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.honeyos).\n"
            "To remove everything, use: honeyos uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_honeyos_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``HONEYOS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from honeyos.runtime.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set HONEYOS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("HONEYOS_HOME")
    try:
        os.environ["HONEYOS_HOME"] = str(profile_dir)
        from honeyos.runtime.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["HONEYOS_HOME"] = old_home
        elif "HONEYOS_HOME" in os.environ:
            del os.environ["HONEYOS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.honeyos/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: honeyos profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HONEYOS_HOME.

    Returns ``"default"`` if HONEYOS_HOME is not set or points to ``~/.honeyos``.
    Returns the profile name if HONEYOS_HOME points into ``~/.honeyos/profiles/<name>``.
    Returns ``"custom"`` if HONEYOS_HOME is set to an unrecognized path.
    """
    from honeyos.core.constants import get_honeyos_home
    honeyos_home = get_honeyos_home()
    resolved = honeyos_home.resolve()

    default_resolved = _get_default_honeyos_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      HONEYOS_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-HoneyOS file the
      user may have lying alongside HONEYOS_HOME (#58394).
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.honeyos itself — its parent is ~/ and its
        # directory name is ".honeyos", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: honeyos profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.honeyos itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.honeyos). "
            "Specify a different name: honeyos profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="honeyos_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"honeyos_{old_name}"
    legacy_old_host = f"honeyos.{old_name}"
    new_host = f"honeyos_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_honeyos_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("honeyos_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HONEYOS_HOME path string.

    Called early in the CLI entry point, before any honeyos modules
    are imported, to set the HONEYOS_HOME environment variable.
    """
