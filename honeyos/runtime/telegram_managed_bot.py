...
manual BotFather token copy-paste. HoneyOS talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.honeyos-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "HoneyOSSetupBot"

DEFAULT_BOT_NAME = "HoneyOS"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``honeyos_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``honeyos_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"honeyos_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting HoneyOS Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the HoneyOS Telegram onboarding service.")
