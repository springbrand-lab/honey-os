...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``honeyos model`` CLI picker. Adds network calls
    ...
    """
    from honeyos.runtime.model_switch import list_authenticated_providers
    ...
    try:
        from honeyos.runtime.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``honeyos.runtime/moa_cmd.py``.
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    """
    try:
        from honeyos.agent.models_dev import get_model_info
    except Exception:
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from honeyos.runtime.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `honeyos model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for HoneyOS.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `honeyos model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from honeyos.runtime.models import CANONICAL_PROVIDERS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`honeyos.runtime.model_switch.list_picker_providers`) so the
