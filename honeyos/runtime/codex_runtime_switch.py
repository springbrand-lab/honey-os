...
Toggles `model.openai_runtime` between "auto" (= chat_completions, HoneyOS'
...
The actual runtime resolution happens in honeyos.runtime.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "honeyos"}:
        return "auto", []
...
def check_codex_binary_ok() -> tuple[bool, Optional[str]]:
    ...
    try:
        from honeyos.agent.transports.codex_app_server import check_codex_binary
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, honeyos-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate HoneyOS' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into HoneyOS for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from honeyos.runtime.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the honeyos-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "honeyos-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + HoneyOS tool callback are always-on production
            ...
            if "honeyos-tools" in mig_report.migrated:
                msg_lines.append(
                    "HoneyOS tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default HoneyOS runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "HoneyOS tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default HoneyOS runtime.")
