"""Safe HoneyOS Console command engine.

This module backs ``honeyos console`` and is intentionally narrower than the
full HoneyOS CLI. It exposes a curated set of native adapters that can later be
...
from honeyos.tools.ansi_strip import strip_ansi as _strip_ansi
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["HoneyOSConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'honeyos doctor'")
        and last.startswith("Run 'honeyos setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="honeyos", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `honeyos "):
        return ""
    return summary
...
# dashboard opens a fresh HoneyOSConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("honeyos.runtime.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["HoneyOSConsoleEngine", list[str]], str]:
    def handler(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "HoneyOSConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["HoneyOSConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `honeyos {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `honeyos {usage}`?",
        )


class HoneyOSConsoleEngine:
    """Curated line-command executor for HoneyOS Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "honeyos":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "HoneyOS Console does not run shell syntax. Use one supported "
                    "HoneyOS command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show HoneyOS component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent HoneyOS logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update HoneyOS configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for HoneyOS Console."""

        extracted = {
            "version": (
                "honeyos.runtime.subcommands.version",
            ...
            "dump": (
                "honeyos.runtime.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "honeyos.runtime.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "honeyos.runtime.subcommands.prompt_size",
            ...
            "insights": (
                "honeyos.runtime.subcommands.insights",
            ...
            "security": (
                "honeyos.runtime.subcommands.security",
            ...
            "backup": (
                "honeyos.runtime.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "honeyos.runtime.subcommands.import_cmd",
            ...
            "config": (
                "honeyos.runtime.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "honeyos.runtime.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "honeyos.runtime.subcommands.plugins",
            ...
            "skills": (
                "honeyos.runtime.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "mcp": (
                "honeyos.runtime.subcommands.mcp",
                "build_mcp_parser",
                "cmd_mcp",
                [
                    ("list",),
            ...
            "memory": (
                "honeyos.runtime.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "auth": (
                "honeyos.runtime.subcommands.auth",
                "build_auth_parser",
                "cmd_auth",
                [
                    ("list",),
            ...
            "pairing": (
                "honeyos.runtime.subcommands.pairing",
            ...
            "webhook": (
                "honeyos.runtime.subcommands.webhook",
            ...
            "hooks": (
                "honeyos.runtime.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "honeyos.runtime.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "honeyos.runtime.subcommands.profile",
            ...
            "cron": (
                "honeyos.runtime.subcommands.cron",
                "build_cron_parser",
                "cmd_cron",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update HoneyOS configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "honeyos.runtime.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("honeyos.runtime.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "honeyos.runtime.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("honeyos.runtime.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("honeyos.runtime.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported HoneyOS Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in HoneyOS Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`honeyos {first}` is not available in HoneyOS Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in HoneyOS Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in HoneyOS Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in HoneyOS Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in HoneyOS Console.",
            ("skills", "publish"): "`skills publish` is not available in HoneyOS Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in HoneyOS Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in HoneyOS Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in HoneyOS Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in HoneyOS Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in HoneyOS Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in HoneyOS Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in HoneyOS Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in HoneyOS Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in HoneyOS Console.",
            ("kanban", "gc"): "`kanban gc` is not available in HoneyOS Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in HoneyOS Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in HoneyOS Console.")
...
def _status(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in HoneyOS Console.")
    ...
    from honeyos.runtime.logs import list_logs, tail_log
...
def _sessions_list(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from honeyos.runtime.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from honeyos.runtime.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from honeyos.runtime.config import migrate_config
...
def _sessions_export(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from honeyos.core.state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="honeyos.runtime.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from honeyos.runtime.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from honeyos.runtime.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from honeyos.scheduler.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from honeyos console")
...
def _cron_resume(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron resume <job>")
    from honeyos.scheduler.jobs import AmbiguousJobReference, resume_job
...
def _cron_run(_engine: HoneyOSConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron run <job>")
    from honeyos.scheduler.jobs import AmbiguousJobReference, trigger_job
...
def run_console_repl(
    *,
    ...
    """Run the local ``honeyos console`` REPL."""
    ...
    engine = HoneyOSConsoleEngine()
    if interactive:
        print("HoneyOS Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("honeyos> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
