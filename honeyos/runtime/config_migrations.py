...
``honeyos.runtime.config.migrate_config``. Each step is a function
...
``honeyos.runtime.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``honeyos.runtime.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``honeyos.runtime.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``honeyos.runtime.config`` (e.g.
``patch("honeyos.runtime.config.read_raw_config", ...)``) keep working, because
...
#: HONEYOS_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_honeyos_home()}/config.yaml and run `honeyos setup` to "
...
def _cfg():
    """Return the live ``honeyos.runtime.config`` module (lazy, cycle-free)."""
    from honeyos.runtime import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_honeyos_home = _c.get_honeyos_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$HONEYOS_HOME/plugins/`` for currently installed user plugins.
        grandfathered: List[str] = []
        try:
            user_plugins_dir = get_honeyos_home() / "plugins"
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `honeyos plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `honeyos curator status` has no
    ...
    #   3. Creates `~/.honeyos/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_honeyos_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_honeyos_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `honeyos config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `honeyos config set`"
                )
...
# behavior set it to true explicitly via `honeyos config set`.
