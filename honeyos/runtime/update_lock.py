"""Cross-process mutual exclusion for in-flight HoneyOS updates.

Three different surfaces can start an update of the same install tree:

* ``honeyos update`` from a terminal,
* the dashboard's Update button (``POST /api/honeyos/update`` →
  ``_spawn_honeyos_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``honeyos-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``honeyos update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <HONEYOS_HOME>/.honeyos-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``honeyos update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("HoneyOS is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``honeyos-setup`` binary under ``~/.honeyos`` is only refreshed by a full
  installer run (``copy_self_to_honeyos_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("HoneyOS is still running") on every
  GUI update forever, with no HoneyOS process actually running.
"""
...
MARKER_NAME = ".honeyos-update-in-progress"

# Set by an orchestrating updater (the Tauri `honeyos-setup --update` flow) to
# its own pid before spawning `honeyos update` as a child stage. The parent
...
HANDOFF_PID_ENV = "HONEYOS_UPDATE_HANDOFF_PID"
...
# to show "HoneyOS is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* HoneyOS home (never the context-local profile override):
    the Rust updater resolves ``$HONEYOS_HOME`` or the platform default, and the
    ...
    """
    from honeyos.core.constants import get_process_honeyos_home

    return get_process_honeyos_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``honeyos update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``honeyos-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another HoneyOS update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `honeyos update` as a stage): we run under ITS claim
