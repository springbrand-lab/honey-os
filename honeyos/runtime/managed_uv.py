"""HoneyOS-managed uv and Python runtime repair.

HoneyOS owns its own uv binary at ``$HONEYOS_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$HONEYOS_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every HoneyOS
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.honeyos-runtime/python``. A
...
from honeyos.runtime.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".honeyos-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where HoneyOS keeps *its* uv binary.

    ``$HONEYOS_HOME/bin/uv`` on POSIX, ``$HONEYOS_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for HoneyOS-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: HoneyOS keeps databases "
            "out of WAL mode on this SQLite build. The next `honeyos update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``honeyos update`` runs
    the call site from the *old*, already-imported ``honeyos.runtime.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``honeyos update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under HONEYOS_HOME
    caches the last successful self-update time.
    """
    try:
        from honeyos.core.constants import get_honeyos_home

        stamp = get_honeyos_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``honeyos update`` so the managed copy stays current.
...
def _reload_honeyos_constants():
    """Re-execute ``honeyos.core.constants`` from disk and return the fresh module.

    ``honeyos update`` imports ``honeyos.core.constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'honeyos.core.constants'
        (~/.honeyos/honeyos-agent/honeyos.core.constants.py)
    ...
    """
    import honeyos.core.constants

    return importlib.reload(honeyos.core.constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the HoneyOS generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("honeyos.runtime.main")
    ...
    if holders:
        ...
        return True, f"other HoneyOS processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the HoneyOS-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``honeyos update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``honeyos update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$HONEYOS_HOME/bin/`` instead of ``~/.local/bin/``.
    """
