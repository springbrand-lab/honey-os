"""Durable aggregation and local export for HoneyOS shared metrics."""
...
from honeyos.runtime.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "honeyos.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_honeyos_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "honeyos_version": row["honeyos_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across HoneyOS processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "honeyos_version": period_row["honeyos_version"],
