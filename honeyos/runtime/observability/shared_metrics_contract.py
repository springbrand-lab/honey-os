"""Bounded product contract for the first HoneyOS shared-metrics slice."""
...
from honeyos.agent.relay_runtime import (
    LOGICAL_LLM_SCOPE,
...
SCHEMA_KEY = "honeyos.metrics.schema_version"
SCHEMA_VERSION = "honeyos.metrics.event.v2"
MODEL_CALL_SCOPE = "honeyos.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "honeyos.task_run"
TOOL_CALL_SCOPE = "honeyos.tool_call"
CLIENT_ACTIVE_MARK = "honeyos.client.active"
TOOL_APPROVAL_MARK = "honeyos.tool_approval"
SKILL_LIFECYCLE_MARK = "honeyos.skill.lifecycle"
SKILL_LOAD_MARK = "honeyos.skill.load"
SUBSCRIBER_NAME = "honeyos.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "honeyos.client.active"
LEGACY_MODEL_CALL_METRIC = "honeyos.model_call.count"
MODEL_ROUTE_METRIC = "honeyos.model_route.count"
TASK_STARTED_METRIC = "honeyos.task_run.started"
TASK_FINISHED_METRIC = "honeyos.task_run.finished"
TOOL_CALL_METRIC = "honeyos.tool_call.count"
TOOL_APPROVAL_METRIC = "honeyos.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "honeyos.skill.lifecycle.count"
SKILL_LOAD_METRIC = "honeyos.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted HoneyOS installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(honeyos_version or "").strip()
    ...
    return {
        ...
        "honeyos_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("honeyos_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its HoneyOS logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("honeyos.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from honeyos.runtime.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map HoneyOS terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map HoneyOS registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "honeyos-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal HoneyOS tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to HoneyOS."""
