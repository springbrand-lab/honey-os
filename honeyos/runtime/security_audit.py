"""On-demand supply-chain audit for HoneyOS installs.

Scans three surfaces a HoneyOS user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The HoneyOS venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.honeyos/plugins``
...
def _discover_plugins(honeyos_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.honeyos/plugins``.
    ...
    """
    plugins_dir = honeyos_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = honeyos_home or Path(get_honeyos_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `honeyos security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, honeyos_home=home
    )
