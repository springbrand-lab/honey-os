"""honeyos import-agent — import Claude Code / Codex CLI setups into HoneyOS.

Usage:
    honeyos import-agent                       # auto-detect ~/.claude or ~/.codex
    honeyos import-agent claude-code           # import from ~/.claude
    honeyos import-agent codex                 # import from ~/.codex
    honeyos import-agent claude-code --dry-run # preview only, no changes
    honeyos import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``honeyos claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_honeyos.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in HONEYOS_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → HONEYOS_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in HONEYOS_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in HONEYOS_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → HONEYOS_HOME/skills/codex-imports/<name>/
...
deliberately via ``honeyos setup`` or config.yaml.
"""
...
from honeyos.utils import atomic_write_text, atomic_yaml_write

logger = logging.getLogger(__name__)

# Same entry delimiter as the HoneyOS memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `honeyos config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`honeyos config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_honeyos.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → HoneyOS command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a HoneyOS glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct HoneyOS equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in HoneyOS config")
                continue

            honeyos_srv: Dict[str, Any] = {}
            if srv.get("command"):
                honeyos_srv["command"] = srv["command"]
                if srv.get("args"):
                    honeyos_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    honeyos_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    honeyos_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                honeyos_srv["url"] = srv["url"]
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        honeyos_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → HONEYOS_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``honeyos import-agent`` (invoked from honeyos.runtime.main)."""
    from honeyos.runtime.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: honeyos import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: honeyos import-agent claude-code   or   honeyos import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ HoneyOS — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: honeyos import-agent "
    ...
    print_info(f"Target:      {honeyos_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'honeyos setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: honeyos import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'honeyos setup' "
               "to configure providers, or add them to ~/.honeyos/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from honeyos.runtime.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'honeyos setup' or ~/.honeyos/.env.")
        print()
