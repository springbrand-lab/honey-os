"""Dashboard process-hygiene helpers — extracted from ``honeyos.runtime/main.py``.
...
``_detect_concurrent_honeyos_instances``) are lifted verbatim. References to
helpers that STAY in ``honeyos.runtime.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``honeyos.runtime.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``honeyos.runtime.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``honeyos.runtime.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``honeyos dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``honeyos update`` replaces files on disk, the running
    ...
    ``honeyos-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the HoneyOS Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``honeyos serve`` as
    ...
    environment variable ``HONEYOS_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`honeyos serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "honeyos.*dashboard"`) keeps us consistent
            # with `honeyos.runtime.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``honeyos dashboard`` / ``honeyos serve`` processes.

    Called at the end of ``honeyos update`` (default ``reason``) and also
    from ``honeyos dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``honeyos update`` path), a detected
    ``honeyos-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``honeyos-serve.service``) has its owning unit
    ...
    # When the HoneyOS Desktop Electron app spawns this dashboard as a
    # backend child, it sets HONEYOS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("HONEYOS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `honeyos update` path) — `--stop` must
    ...
    #    remote backend (honeyos serve) under Restart=on-failure never comes
...
def _detect_concurrent_honeyos_instances(
    ...
    The HoneyOS Desktop Electron app spawns ``honeyos.EXE`` as a backend child,
    so during ``honeyos update`` the user-invoked process and the desktop's
    ...
    shims (``honeyos.exe`` / ``honeyos-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``honeyos update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``honeyos.exe``) is a separate process from the Python
    ...
    for shim in _m()._honeyos_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated honeyos.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``honeyos update``
    ...
    #      honeyos.exe sitting *under* a non-HoneyOS parent (e.g. a HoneyOS
