#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the HoneyOS Skills Hub.

Powers both:
  - `honeyos skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    """
    from honeyos.tools.skills_hub import unified_search
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `honeyos skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.honeyos/skills/`` that look
    ...
    """
    from honeyos.tools.skills_hub import SKILLS_DIR, _category_skill_dirs
    try:
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.honeyos/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    """
    from honeyos.tools.skills_hub import GitHubAuth, create_source_router, unified_search
    ...
    # into `honeyos skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: honeyos skills inspect <identifier> to preview, "
            "honeyos skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    """
    from honeyos.tools.skills_hub import (
    ...
    # skips the external API sources and serves everything from "honeyos-index".
    ...
    from honeyos.tools.skills_hub import _PROVIDER_FILTER_VALUES, _filter_results_by_provider
    ...
    # The identifier is what you pass to `honeyos skills install`. Browse used
    ...
    c.print("[dim]Tip: 'honeyos skills inspect <identifier>' to preview, "
            "'honeyos skills install <identifier>' to install, "
            "'honeyos skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    from honeyos.tools.skills_guard import scan_skill_cached, should_allow_install, format_scan_report
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]honeyos skills install {url} --name <your-name>[/]\n\n"
    ...
    try:
        ...
        from honeyos.tools.skills_hub import append_audit_log
    ...
    from honeyos.tools.skills_hub import HUB_DIR, source_url_for_bundle
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with honeyos-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_honeyos_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    try:
        ...
        append_audit_log("BLOCKED", bundle.name, bundle.source,
                         bundle.trust_level, "invalid_path", str(exc))
        return
    from honeyos.tools.skills_hub import SKILLS_DIR
    c.print(f"[bold green]Installed:[/] {install_dir.relative_to(SKILLS_DIR)}")
    c.print(f"[dim]Files: {', '.join(bundle.files.keys())}[/]\n")

    # Blueprint detection: if the installed skill declares a
    # metadata.honeyos.blueprint block, it is a runnable automation. Register it as
    ...
    try:
        from honeyos.tools.blueprints import BlueprintError, blueprint_spec_for_installed, register_blueprint_suggestion
        ...
        if spec is not None:
            ...
            if registered is not None:
                ...
                c.print(
                    ...
                    "or via[/] [bold]honeyos cron add[/][dim].[/]\n"
                )
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    from honeyos.tools.skills_hub import GitHubAuth, create_source_router
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="honeyos skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "honeyos-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"honeyos-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying honeyos-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``honeyos -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``HONEYOS_HOME`` at process
    start.  No explicit profile flag needed here.
    """
    from honeyos.tools.skills_hub import HubLockFile, ensure_hub_dirs
    from honeyos.tools.skills_sync import _read_manifest
    from honeyos.tools.skills_tool import _find_all_skills
    from honeyos.agent.skill_utils import get_disabled_skill_names
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from honeyos.tools.skills_hub import check_for_skill_updates
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from honeyos.tools.skills_hub import HubLockFile, check_for_skill_updates
...
def do_audit(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    are unchanged).
    """
    from honeyos.tools.skills_hub import HubLockFile, SKILLS_DIR
    from honeyos.tools.skills_guard import scan_skill, format_scan_report
    ...
    if deep:
        from honeyos.tools.skills_ast_audit import ast_scan_path, format_ast_report
...
def do_uninstall(name: str, console: Optional[Console] = None,
    ...
    from honeyos.tools.skills_hub import uninstall_skill
...
def do_reset(name: str, restore: bool = False,
    ...
    from honeyos.tools.skills_sync import reset_bundled_skill
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `honeyos update` keeps)."""
    from honeyos.tools.skills_sync import list_user_modified_bundled_skills
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `honeyos update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   honeyos skills diff <name>[/]")
    c.print("[dim]Resume updates: honeyos skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: honeyos skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    from honeyos.tools.skills_sync import diff_bundled_skill
    ...
    c.print()
    c.print(f"[dim]Revert with: honeyos skills reset {name} --restore[/]\n")
...
def do_opt_out(remove: bool = False,
    ...
    never touched.
    """
    from honeyos.tools.skills_sync import (
...
def do_repair_official(name: str, restore: bool = False,
    ...
    from honeyos.tools.skills_sync import restore_official_optional_skill
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    from honeyos.tools.skills_hub import TapsManager
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: honeyos skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: honeyos skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
...
def do_publish(skill_path: str, target: str = "github", repo: str = "",
    ...
    from honeyos.tools.skills_hub import GitHubAuth, SKILLS_DIR
    from honeyos.tools.skills_guard import scan_skill, format_scan_report
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: honeyos skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_honeyos_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via HoneyOS Skills Hub.\n\n"
                        f"This skill was scanned by the HoneyOS Skills Guard before submission.",
...
def do_snapshot_export(output_path: str, console: Optional[Console] = None) -> None:
    ...
    from honeyos.tools.skills_hub import HubLockFile, TapsManager
...
def skills_command(args) -> None:
    """Router for `honeyos skills <subcommand>` — called from honeyos.runtime/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: honeyos skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: honeyos skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: honeyos skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'honeyos skills <command> --help' for details.\n")
