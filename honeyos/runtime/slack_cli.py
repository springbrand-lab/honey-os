"""``honeyos slack ...`` CLI subcommands.

Today only ``honeyos slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ honeyos slack manifest > slack-manifest.json
    # or:
    $ honeyos slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of HoneyOS. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a HoneyOS deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps HoneyOS on Slack's older Assistant messaging
    ...
    """
    from honeyos.runtime.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with HoneyOS in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with HoneyOS in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your HoneyOS agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``honeyos.runtime/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$HONEYOS_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "HoneyOS")
    ...
    """
    name = getattr(args, "name", None) or "HoneyOS"
    description = getattr(args, "description", None) or "Your HoneyOS agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "honeyos slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"honeyos slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "honeyos slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "honeyos slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your HoneyOS app\n"
