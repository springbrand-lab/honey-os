"""Implementation of the ``honeyos bundles`` CLI subcommand.

Mirrors the structure of ``honeyos.runtime/skills_hub.py`` but for skill
...
from honeyos.agent.skill_bundles import (
    _bundles_dir,
    delete_bundle,
    get_bundle,
    list_bundles,
...
def _console() -> Console:
    # Bind to stderr so piping `honeyos bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  honeyos bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``honeyos bundles`` argparse tree.

    Called from ``honeyos.runtime/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `honeyos bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``honeyos bundles <subcommand>`` to the right handler."""
