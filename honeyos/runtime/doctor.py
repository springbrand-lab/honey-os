"""
Doctor command for honeyos CLI.

Diagnoses issues with HoneyOS setup.
"""
...
from honeyos.core.constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
HONEYOS_HOME = get_honeyos_home()
_DHH = display_honeyos_home()  # user-facing display path (e.g. ~/.honeyos or ~/.honeyos/profiles/coder)

# Load environment variables from ~/.honeyos/.env so API key checks work
_env_path = get_env_path()
load_honeyos_dotenv(honeyos_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from honeyos.runtime.colors import Colors, color
from honeyos.runtime.models import _HONEYOS_USER_AGENT
from honeyos.runtime.vercel_auth import describe_vercel_auth
from honeyos.core.constants import OPENROUTER_MODELS_URL
from honeyos.utils import base_url_host_matches
...
from honeyos.core.constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all HoneyOS containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `honeyos update`"
    return (
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.honeyos/.env contains provider auth/base URL settings."""
...
def _honcho_is_configured_for_doctor() -> bool:
    ...
    try:
        from honeyos.plugins.memory.honcho.client import HonchoClientConfig
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("HONEYOS_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from honeyos.runtime.auth import get_minimax_oauth_auth_status
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # HONEYOS_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. HONEYOS_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("HONEYOS_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("HONEYOS_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches honeyos.runtime.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``honeyos --version`` reporting a stale version while
    ...
    """
    try:
        from honeyos.runtime import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != honeyos.runtime/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'honeyos update', or set "
            "honeyos.runtime/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-honeyos and dashboard static services are up
    ...
    for static in ("main-honeyos", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `honeyos profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    """
    try:
        from honeyos.agent.ssl_guard import verify_ca_bundle_with_fallback
        from honeyos.agent.errors import SSLConfigurationError
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `honeyos doctor --fix`, or "
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        ...
        check_warn("Gateway service linger", f"(could not import honeyos.gateway helpers: {e})")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        from honeyos.providers.base import ProviderProfile as _PP
        try:
            from honeyos.runtime.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the HONEYOS_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("HONEYOS_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via HONEYOS_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `honeyos`.
    os.environ.setdefault("HONEYOS_INTERACTIVE", "1")

    # Handle `honeyos doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from honeyos.runtime.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.honeyos/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 HoneyOS Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`honeyos doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from honeyos.core.state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: HoneyOS already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and honeyos.runtime/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.honeyos/.env (primary location for user config)
    env_path = HONEYOS_HOME / '.env'
    if env_path.exists():
        ...
        # matches honeyos.runtime.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'honeyos setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'honeyos setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'honeyos setup' to create one")
                issues.append("Run 'honeyos setup' to create .env")

    # Check ~/.honeyos/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = HONEYOS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from honeyos.runtime.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from honeyos.runtime.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'honeyos config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from honeyos.runtime.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.honeyos/.env or run 'honeyos setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'honeyos setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'honeyos config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from honeyos.runtime.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from honeyos.runtime.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'honeyos setup' to migrate config")
                else:
                    issues.append("Run 'honeyos doctor --fix' or 'honeyos setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'honeyos doctor --fix'")
        except Exception:
            pass

        # Detect stale HONEYOS_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives HONEYOS_MAX_ITERATIONS from honeyos.agent.max_turns
        ...
        try:
            from honeyos.runtime.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("HONEYOS_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"HONEYOS_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `honeyos setup` run)",
                )
                if should_fix:
                    if remove_env_value("HONEYOS_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale HONEYOS_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove HONEYOS_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the HONEYOS_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale HONEYOS_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'honeyos doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from honeyos.runtime.config import load_env as _load_env_depr
            from honeyos.runtime.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses HoneyOS' own device-code flow — the Codex CLI is
    ...
    honeyos_home = HONEYOS_HOME
    if honeyos_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        honeyos_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = honeyos_home / subdir_name
    ...
    soul_path = honeyos_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give HoneyOS a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# HoneyOS Persona\n\n"
                "<!-- Edit this file to customize how HoneyOS communicates. -->\n\n"
                "You are HoneyOS, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = honeyos_home / "memories"
    ...
    state_db_path = honeyos_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from honeyos.core.state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'honeyos doctor --fix' "
                        "(or 'honeyos sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from honeyos.core.state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'honeyos doctor --fix' "
                        "(or 'honeyos sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = honeyos_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'honeyos doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "honeyos"
        ...
        _cmd_link = _cmd_link_dir / "honeyos"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(honeyos not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/honeyos → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/honeyos points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/honeyos → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/honeyos — run 'honeyos doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/honeyos exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/honeyos not found",
                    "(honeyos command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/honeyos → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/honeyos symlink — run 'honeyos doctor --fix'")
    ...
    try:
        from honeyos.core.constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'honeyos-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'honeyos-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `honeyos acp --setup-browser` installs agent-browser into the
        # HoneyOS-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_honeyos_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(HONEYOS_HOME / "node" / "bin")
            or _which_in(HONEYOS_HOME / "node")
        )
        _legacy_ab = _which_in(HONEYOS_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `honeyos update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `honeyos doctor` invocation.
                from honeyos.tools.browser_tool import (
                    _chromium_installed,
                    _is_camofox_mode,
                    _get_cloud_provider,
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable HONEYOS_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a HoneyOS one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'honeyos config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from honeyos.agent.auxiliary_client import _to_openai_base_url
            ...
            headers = {
                ...
                "User-Agent": _HONEYOS_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``honeyos doctor``.
    ...
    def _probe_bedrock() -> _ConnectivityResult:
        try:
            from honeyos.agent.bedrock_adapter import (
                has_aws_credentials,
    ...
    # never the legitimate source for `honeyos doctor`.
    ...
    try:
        ...
        from honeyos.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
        ...
        if api_disabled:
            issues.append("Run 'honeyos setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")

    _section("Skills Hub")
    hub_dir = HONEYOS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: honeyos skills list)")
    ...
    try:
        from honeyos.runtime.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = HONEYOS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            from honeyos.plugins.memory.honcho.client import HonchoClientConfig, resolve_config_path
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: honeyos memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: honeyos memory setup",
                    "No Honcho API key — run 'honeyos memory setup'",
                    issues,
                )
            else:
                from honeyos.plugins.memory.honcho.client import get_honcho_client, reset_honcho_client
                reset_honcho_client()
                try:
        ...
        try:
            from honeyos.plugins.memory.mem0 import _load_config as _load_mem0_config
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run honeyos memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: honeyos memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: honeyos memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from honeyos.runtime.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "honeyos -p" in content:
                            _m = _re.search(r"honeyos -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'honeyos doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
