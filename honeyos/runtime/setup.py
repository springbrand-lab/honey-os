"""
Interactive setup wizard for HoneyOS.
...
from honeyos.tools.tool_backend_helpers import managed_nous_tools_enabled
from honeyos.core.constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://honeyos-agent.nousresearch.com/docs"
...
# display_honeyos_home imported lazily at call sites (stale-module safety during honeyos update)
...
from honeyos.runtime.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from honeyos.runtime.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ HoneyOS Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure HoneyOS using environment variables or config commands:")
    print_info("  honeyos config set model.provider custom")
    print_info("  honeyos config set model.base_url http://localhost:8080/v1")
    print_info("  honeyos config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'honeyos setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``HONEYOS_NONINTERACTIVE=1`` (see ``honeyos.runtime/web_server.py``). In that
    ...
    """
    return os.environ.get("HONEYOS_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``HONEYOS_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from honeyos.runtime.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'honeyos setup')")


def _print_setup_summary(config: dict, honeyos_home):
    ...
    try:
        from honeyos.runtime.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — HoneyOS cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    honeyos model            (pick any provider/model)")
        print_info("    honeyos setup --portal   (Nous Portal OAuth, no API key)")
    ...
    try:
        from honeyos.agent.auxiliary_client import get_available_vision_backends
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'honeyos setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        _img_backend = None
        try:
            from honeyos.agent.image_gen_registry import list_providers
            from honeyos.runtime.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `honeyos tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            from honeyos.agent.video_gen_registry import list_providers as _list_video_providers
            from honeyos.runtime.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'honeyos setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'honeyos setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'honeyos tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'honeyos setup terminal'"))
    ...
    # Spotify (OAuth via honeyos auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'honeyos setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {honeyos_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('honeyos setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('honeyos setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('honeyos setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('honeyos setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('honeyos setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('honeyos config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('honeyos config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('honeyos config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('honeyos', Colors.GREEN)}              Start chatting")
    print(f"   {color('honeyos gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('honeyos doctor', Colors.GREEN)}       Check for issues")
    print()
...
def _prompt_vercel_sandbox_settings(config: dict):
    ...
    from honeyos.tools.terminal_tool import _SUPPORTED_VERCEL_RUNTIMES
...
# between `honeyos tools` and `honeyos setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``honeyos model``)
    ...
    provider added to ``honeyos model`` is automatically available here.
    ...
    # Delegate to the shared honeyos model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from honeyos.runtime.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: honeyos model")
    ...
    # on demand via `honeyos auth add`, `honeyos setup` vision, and
    # `honeyos setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from honeyos.runtime.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``honeyos model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from honeyos.runtime.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.honeyos/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the HoneyOS auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via honeyos setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'honeyos setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where HoneyOS runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `honeyos setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `honeyos setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `honeyos egress setup` then `honeyos egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `honeyos egress setup`."
            )
        ...
        from honeyos.tools.managed_tool_gateway import is_managed_tool_gateway_ready
        from honeyos.tools.tool_backend_helpers import normalize_modal_mode
        ...
        print_info("Requires the optional SDK: pip install 'honeyos-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $HONEYOS_HOME/bin is never on PATH, so a bare
            # which() misses the uv HoneyOS installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'honeyos-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into HONEYOS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("HONEYOS_MAX_ITERATIONS")
    ...
    print_info("  Run `honeyos setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HONEYOS_MAX_ITERATIONS from honeyos.agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.honeyos/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from honeyos.runtime.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_honeyos_home(Path(get_honeyos_home()))
    except Exception:
        pass
...
def _profile_name_from_honeyos_home(honeyos_home) -> str | None:
    """Return the active profile name when HONEYOS_HOME is a profile dir."""
    if honeyos_home.parent.name == "profiles":
        return honeyos_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where HoneyOS delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects HoneyOS to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from honeyos.runtime.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  honeyos config edit")
    print_info("   Open config in your editor:  honeyos config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from honeyos.runtime.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with HoneyOS from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'honeyos setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     honeyos config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_honeyos_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: honeyos gateway install --system")
                print_info("  Or run in foreground:  honeyos gateway")
        else:
            from honeyos.core.constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   honeyos gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   honeyos gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `honeyos setup tools` and `honeyos tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this HoneyOS profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``honeyos.runtime.auth`` — lists every supported
    ...
    """
    try:
        from honeyos.runtime.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from honeyos.runtime.config
    ...
    if section_key == "model":
        ...
        from honeyos.runtime.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_honeyos.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_honeyos migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_honeyos", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and HoneyOS.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure HoneyOS to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point HoneyOS at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point HoneyOS at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point HoneyOS at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point HoneyOS at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to HoneyOS equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing HoneyOS config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in HoneyOS.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ HoneyOS's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(honeyos_home: Path) -> bool:
    ...
    print_info("HoneyOS can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: honeyos claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=honeyos_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: honeyos claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing HoneyOS configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing HoneyOS config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in HoneyOS (use honeyos claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``honeyos setup --portal`` and ``honeyos portal``. This is the
    ...
    HoneyOS session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``honeyos setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``honeyos model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``honeyos portal`` always offers a model picker,
    ...
    print(color("│     ⚕ HoneyOS Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `honeyos portal` == quick setup's Nous step.
    try:
        from honeyos.runtime.main import _model_flow_nous
        ...
        print_info("  You can retry later with `honeyos portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `honeyos portal`: %s", exc)
        print()
    ...
    print_info("  Run `honeyos portal info` to inspect routing.")
    print_info("  Run `honeyos` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      honeyos setup           — full or quick (auto-detected)
      honeyos setup model     — just model/provider
      honeyos setup tts       — just text-to-speech
      honeyos setup terminal  — just terminal backend
      honeyos setup gateway   — just messaging platforms
      honeyos setup tools     — just tool configuration
      honeyos setup telemetry — just local shared metrics
      honeyos setup agent     — just agent settings
    """
    from honeyos.runtime.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ HoneyOS Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ HoneyOS Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your HoneyOS installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, honeyos_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have HoneyOS configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'honeyos setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(honeyos_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up HoneyOS?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, honeyos_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, honeyos_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {honeyos_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'honeyos config edit'")
    ...
    # Tune later with `honeyos setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, honeyos_home)


def _run_first_time_quick_setup(config: dict, honeyos_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``honeyos setup <section>``
    or switch providers with ``honeyos model``.
    """
    from honeyos.runtime.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'honeyos setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    honeyos setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  honeyos setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``honeyos tools`` (which
    ...
    try:
        from honeyos.toolsets import TOOLSETS
        from honeyos.runtime.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``honeyos setup agent`` /
    ``honeyos config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, honeyos_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `honeyos update` runs don't re-inject them.
        try:
            from honeyos.tools.skills_sync import set_bundled_skills_opt_out
        ...
        print_info("  Enable tools:        honeyos tools")
        print_info("  Seed skills:         honeyos skills opt-in --sync")
        print_info("  Add MCP servers:     honeyos mcp add")
        print_info("  Enable plugins:      honeyos plugins")
        print_info("  Tune agent settings: honeyos setup agent")
        print()
        _print_setup_summary(config, honeyos_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, honeyos_home)


def _blank_slate_walkthrough(config: dict, honeyos_home):
    ...
    try:
        from honeyos.tools.skills_sync import set_bundled_skills_opt_out, sync_skills
        if seed_skills:
            ...
            print_info("`honeyos update` runs from re-injecting them. Opt back in any")
            print_info("time with `honeyos skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `honeyos tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `honeyos plugins list` / `honeyos plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `honeyos plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `honeyos mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `honeyos mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   honeyos tools")
    print_info("  Seed skills:         honeyos skills opt-in --sync")
    print_info("  Add MCP servers:     honeyos mcp add")
    print_info("  Tune agent settings: honeyos setup agent")
    print()
...
def _run_quick_setup(config: dict, honeyos_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'honeyos setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect HoneyOS to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'honeyos setup gateway'.")
