"""honeyos hooks — inspect and manage shell-script hooks.

Usage::

    honeyos hooks list
    honeyos hooks test <event> [--for-tool X] [--payload-file F]
    honeyos hooks revoke <command>
    honeyos hooks doctor

Consent records live under ``~/.honeyos/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.honeyos/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``honeyos hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: honeyos hooks {list|test|revoke|doctor}")
        print("Run 'honeyos hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    from honeyos.agent import outbound_webhooks, shell_hooks
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.honeyos/config.yaml.")
        print("See `honeyos hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.honeyos/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `honeyos hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `honeyos hooks test` and `honeyos hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (HoneyOS wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `honeyos hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `honeyos hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `honeyos hooks doctor`.")
