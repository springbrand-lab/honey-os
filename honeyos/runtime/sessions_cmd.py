"""``honeyos sessions`` command — extracted from ``honeyos.runtime/main.py``.
...
Helpers that stay in ``honeyos.runtime.main`` (``get_honeyos_home``,
...
``honeyos.runtime.main.<name>`` keep reaching this code path, and so imports stay
...
def get_honeyos_home():
    return _m().get_honeyos_home()
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        from honeyos.core.state import (
            DEFAULT_DB_PATH,
            _db_opens_cleanly,
        ...
        if report.get("repaired"):
            ...
            print(f"    honeyos sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from honeyos.runtime.session_recovery import (
    ...
    if action == "list":
        from honeyos.core.state import workspace_key as _ws_key
        ...
        from honeyos.runtime.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        def _redact(data):
            if not args.redact or data is None:
                return data
            from honeyos.runtime.session_export_md import redact_session_data
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from honeyos.runtime.session_export_html import (
        ...
        if args.format == "trace":
            ...
            from honeyos.agent.trace_upload import (
                TraceRedactionError,
                build_trace_jsonl,
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_honeyos_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from honeyos.runtime.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_honeyos_home() / "session-exports"
            ...
            if args.delete_after_verified:
                for data, exported_path in exported_items:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                sessions_dir = get_honeyos_home() / "sessions"
        ...
        # `honeyos sessions prune`: no time window and no filters at all
        ...
        from honeyos.agent.skill_commands import describe_skill_invocation
        from honeyos.agent.title_generator import generate_title
        ...
        # Launch honeyos --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`honeyos sessions optimize` later to reclaim freed space.)")
