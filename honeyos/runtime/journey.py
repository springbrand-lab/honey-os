"""``honeyos journey`` — what HoneyOS has learned, on a timeline.
...
def _palette() -> dict[str, str]:
    from honeyos.agent.learning_graph_render import derive_palette

    return derive_palette(_primary_hex(), dark=True)


def _fade(base: Optional[str], alpha: float) -> Optional[str]:
    from honeyos.agent.learning_graph_render import hex_to_rgb, mix_rgb, rgb_to_hex
...
def _frame_renderable(payload, *, cols, rows, reveal, color):
    ...
    from honeyos.agent import learning_graph_render as render
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if not payload.get("nodes"):
        console.print(
            "[grey62]No learning yet — use HoneyOS a while and your learned skills and "
...
def _cmd_list(args: argparse.Namespace) -> int:
    from honeyos.agent.learning_graph_render import format_date
...
def _cmd_delete(args: argparse.Namespace) -> int:
    from honeyos.agent.learning_mutations import delete_node, node_detail
...
def _cmd_edit(args: argparse.Namespace) -> int:
    from honeyos.agent.learning_mutations import edit_node, node_detail
...
if __name__ == "__main__":
    _p = argparse.ArgumentParser(prog="honeyos journey")
    register_cli(_p)
    _a = _p.parse_args()
    sys.exit(_a.func(_a))
