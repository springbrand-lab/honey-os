"""HoneyOS middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "honeyos.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "honeyos.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before HoneyOS sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from honeyos.runtime.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from honeyos.runtime.plugins import has_middleware
