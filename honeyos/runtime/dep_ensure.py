...
from honeyos.core.constants import agent_browser_runnable, find_node_executable
from honeyos.tools.environments.local import honeyos_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $HONEYOS_HOME/node is
...
def _has_honeyos_agent_browser() -> bool:
    ...
    # install.sh installs globally into $HONEYOS_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-HoneyOSHome", str(get_honeyos_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = honeyos_subprocess_env(inherit_credentials=False)
