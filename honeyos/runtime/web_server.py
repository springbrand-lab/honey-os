"""
HoneyOS — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m honeyos.runtime.main web          # Start on http://127.0.0.1:9119
    python -m honeyos.runtime.main web --port 8080
"""
...
from honeyos.runtime._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from honeyos.plugins.memory.config_schema import (
    ProviderConfigSchema,
    ProviderField,
    STORAGE_HONCHO_HOST_BLOCK,
    get_provider_config_schema,
)
from honeyos.gateway.status import (
    derive_gateway_busy,
...
from honeyos.utils import env_var_enabled
...
try:
    ...
    # running `honeyos dashboard` needs fastapi+uvicorn; lazy install keeps
    # them out of every other install path. After install, re-import.
    try:
        from honeyos.tools.lazy_deps import ensure as _lazy_ensure
...
WEB_DIST = Path(os.environ["HONEYOS_WEB_DIST"]) if "HONEYOS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``honeyos gateway run`` — but the
    desktop app spawns a ``honeyos dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same HONEYOS_HOME — whichever process grabs the lock
    first wins the tick.
    """
    from honeyos.scheduler.scheduler_provider import resolve_cron_scheduler
...
def _warm_gateway_module() -> None:
    ...
    ``honeyos.runtime.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "honeyos.runtime.auth",
        "honeyos.runtime.copilot_auth",
        "honeyos.runtime.runtime_provider",
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from honeyos.runtime.gateway import _get_restart_drain_timeout
        return _get_restart_drain_timeout()
    except ImportError:
        from honeyos.gateway.restart import DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import honeyos.runtime.gateway eagerly *before* the lifespan yield so the
    ...
    # Desktop-spawned backends (HONEYOS_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `honeyos
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("HONEYOS_DESKTOP") == "1":
...
app = FastAPI(title="HoneyOS", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from honeyos.runtime.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# HONEYOS_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("HONEYOS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-HoneyOS-Session-Token"
...
#
# This list is defined in ``honeyos.runtime.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from honeyos.runtime.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-HoneyOS-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``honeyos-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from honeyos.runtime.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from honeyos.runtime.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from honeyos.runtime.dashboard_auth.token_auth import token_auth_middleware
...
def _memory_provider_options() -> List[str]:
    ...
    options = [""]
    try:
        from honeyos.plugins.memory import list_memory_provider_names
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `honeyos egress setup` "
            "and `honeyos egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates HoneyOS (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during honeyos update. "
...
def _custom_provider_options(
    ...
    *kind* is ``"tts"`` or ``"stt"``. The result keeps the built-in display
    ...
    if kind == "tts":
        from honeyos.tools.tts_tool import BUILTIN_TTS_PROVIDERS as _runtime_builtins
    else:
        from honeyos.tools.transcription_tools import BUILTIN_STT_PROVIDERS as _runtime_builtins
    ...
    try:
        if kind == "tts":
            from honeyos.agent.tts_registry import list_providers as _list_voice_providers
        else:
            from honeyos.agent.transcription_registry import list_providers as _list_voice_providers
...
from honeyos.runtime.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real HoneyOS provider slug — fine.
    ...
    1. Vendor-name → HoneyOS-provider mapping: when the provider string is
       not a known HoneyOS provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from honeyos.runtime.config import get_compatible_custom_providers
    from honeyos.runtime.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from honeyos.runtime.model_normalize import normalize_model_for_provider
    from honeyos.runtime.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known HoneyOS
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from honeyos.runtime.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another HoneyOS process is writing to it.
    """
    from honeyos.core.state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "HONEYOS_DASHBOARD_FILES_ROOT"
...
# HONEYOS_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to HONEYOS_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical HoneyOS credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no HONEYOS_HOME resolution, so it blocks these trees wherever
...
def _default_honeyos_root_is_opt_data() -> bool:
    raw = os.environ.get("HONEYOS_HOME", "").strip()
    if not raw:
        return False
    try:
        from honeyos.core.constants import get_default_honeyos_root

        root = get_default_honeyos_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``honeyos update``.
    ...
    a container — e.g. the honeyos-webui image sharing the HoneyOS source tree),
    the dashboard's ``honeyos update`` button is the correct update path and
    ...
    """
    if _default_honeyos_root_is_opt_data():
        return True
    try:
        from honeyos.core.constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's HoneyOS root is actually /opt/data
    # (the container/hosted layout) or when HONEYOS_DASHBOARD_FILES_ROOT is set.
    if _default_honeyos_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs HoneyOS inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``honeyos:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.honeyos-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``honeyos.runtime.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from honeyos.runtime import web_git as _web_git  # noqa: E402
...
from honeyos.runtime.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from honeyos.runtime.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from honeyos.runtime.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from honeyos.runtime.profiles import _check_gateway_running, profiles_to_serve
        from honeyos.gateway.status import read_runtime_status
...
def _load_configured_gateway_platforms() -> set[str]:
    ...
    """
    from honeyos.gateway.config import load_gateway_config
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_honeyos_home() at call time
    ...
    try:
        ...
        # HONEYOS_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of honeyos.runtime.gateway blocks the
        ...
        # and which providers are registered so ``honeyos status`` and the
        ...
        auth_flows: list[str] = []
        try:
            from honeyos.runtime.dashboard_auth import (
        ...
        try:
            from honeyos.runtime.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_honeyos": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        try:
            from honeyos.gateway.readiness import _probe_state_db

            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_honeyos_home())
            )
        ...
        try:
            from honeyos.core.state import SessionDB as _SDB
            from honeyos.core.constants import get_honeyos_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — HoneyOS Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the honeyos home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_honeyos_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_curator_status():
    try:
        from honeyos.agent import curator
    except Exception as exc:
...
async def run_curator():
    ...
    try:
        proc = _spawn_honeyos_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_learning_graph(profile: Optional[str] = None):
    ...
    """
    try:
        from honeyos.agent.learning_graph import build_learning_graph
...
async def get_learning_node(id: str, profile: Optional[str] = None):
    ...
    from honeyos.agent.learning_mutations import node_detail
...
async def delete_learning_node(body: LearningNodeRef):
    ...
    from honeyos.agent.learning_mutations import delete_node
...
async def update_learning_node(body: LearningNodeEdit):
    ...
    from honeyos.agent.learning_mutations import edit_node
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from honeyos.runtime.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from honeyos.runtime.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_honeyos_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_honeyos_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_honeyos_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from honeyos.runtime.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.honeyos/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_honeyos_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "honeyos-update": "honeyos-update.log",
...
def _spawn_honeyos_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``honeyos <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``honeyos.runtime.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "honeyos.runtime.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _HONEYOS_GATEWAY=1. Inheriting it makes a spawned `honeyos gateway restart`
    ...
    action_env = {**os.environ, "HONEYOS_NONINTERACTIVE": "1"}
    action_env.pop("_HONEYOS_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("HONEYOS_ACTION_ID")
    if action_id:
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["honeyos", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``honeyos gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``honeyos gateway restart`` children race each other on the
    ...
    return _spawn_honeyos_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``honeyos gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``HONEYOS_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
    ...
    """
    from honeyos.gateway.drain_control import (
        clear_drain_request,
        drain_requested,
        write_drain_request,
    )
...
@app.post("/api/honeyos/update")
async def update_honeyos():
    """Kick off ``honeyos update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "HoneyOS updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("honeyos-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "honeyos-update",
    ...
    existing = _ACTION_PROCS.get("honeyos-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("honeyos-update")
        if action_id:
    ...
    try:
        proc = _spawn_honeyos_action(
            ["update"],
            "honeyos-update",
            env_overrides={"HONEYOS_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn honeyos update")
...
@app.get("/api/honeyos/update/check")
async def check_honeyos_update(force: bool = False):
    """Report whether a HoneyOS update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/honeyos/update`` actually runs ``honeyos update``.

    Returns:
        install_method: 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed HoneyOS version string
    ...
    try:
        ...
        if force:
            try:
                (get_honeyos_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def transcribe_audio_upload(
    ...
    try:
        ...
        from honeyos.tools.voice_mode import transcribe_recording
...
async def get_elevenlabs_voices(profile: Optional[str] = None):
    ...
    if not api_key:
        ...
        try:
            from honeyos.agent.secret_scope import UnscopedSecretError, get_secret
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.honeyos/config.yaml`` under ``tts.``.
    """
    ...
    try:
        from honeyos.tools.tts_tool import text_to_speech_tool
...
def _split_text_for_speak_stream(text: str, cap: int) -> list:
    ...
    """
    from honeyos.tools.tts_streaming import SENTENCE_BOUNDARY_RE as _SENTENCE_BOUNDARY_RE
...
async def speak_stream_ws(ws: "WebSocket") -> None:
    ...
    def _resolve():
        from honeyos.tools.tts_streaming import resolve_streaming_provider
        from honeyos.tools.tts_tool import _get_provider, _load_tts_config, _resolve_max_text_length
    ...
    def _produce():
        from honeyos.tools.tts_streaming import SentenceChunker
        from honeyos.tools.tts_tool import _strip_markdown_for_tts
...
from honeyos.runtime.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from honeyos.runtime.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from honeyos.runtime.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from honeyos.runtime.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    HoneyOS supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _coerce_field_value(field: ProviderField, raw: str) -> Any:
    ...
    if kind == "bool":
        from honeyos.utils import is_truthy_value
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_honeyos_home() / provider.name / "config.json"
...
def _honcho_resolvers():
    ...
    from honeyos.plugins.memory.honcho.client import _host_block, resolve_active_host, resolve_config_path
...
def _write_provider_flat(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    from honeyos.utils import atomic_json_write
...
def _write_provider_honcho(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    ...
    from honeyos.plugins.memory.honcho.oauth import ACCESS_TOKEN_PREFIX, _config_refresh_lock
    from honeyos.utils import atomic_json_write
...
def _load_memory_provider(name: str):
    try:
        from honeyos.plugins.memory import load_memory_provider
...
def _memory_provider_manifest(name: str) -> Dict[str, Any]:
    try:
        from honeyos.plugins.memory import find_provider_dir
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/honeyos is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (HONEYOS_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
    ...
    try:
        from honeyos.tools.lazy_deps import install_specs
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    honeyos_home = get_honeyos_home()
    values: Dict[str, Any] = {}

    # Common native provider stores.
    for path in (
        honeyos_home / f"{name}.json",
        honeyos_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            from honeyos.agent.memory_provider import MemoryProvider as _BaseMemoryProvider
        except Exception:
            provider.save_config(values, str(get_honeyos_home()))
            return
...
def _discover_memory_provider_statuses() -> List[Dict[str, Any]]:
    ...
    try:
        from honeyos.plugins.memory import discover_memory_providers
...
async def get_egress_status():
    ...
    from honeyos.runtime.proxy_cli import format_status_text
...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from honeyos.agent.model_metadata import get_model_context_length
        ...
        caps = {}
        try:
            from honeyos.agent.models_dev import get_model_capabilities
...
# in honeyos.runtime/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from honeyos.runtime.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `honeyos model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from honeyos.runtime.auth import get_provider_auth_state
    ...
    try:
        from honeyos.runtime.inventory import build_models_payload, load_picker_context
        from honeyos.runtime.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from honeyos.runtime.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from honeyos.runtime.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.honeyos/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from honeyos.runtime.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (honeyos.runtime/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from honeyos.runtime.nous_subscription import apply_nous_managed_defaults
                from honeyos.runtime.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``honeyos model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from honeyos.runtime.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``honeyos model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from honeyos.runtime.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from honeyos.runtime.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `honeyos model` provider — would be
        ...
        # Vertex — a `honeyos model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `honeyos model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from honeyos.runtime.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from honeyos.runtime.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run HoneyOS from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect HoneyOS to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use HoneyOS from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect HoneyOS to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use HoneyOS in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use HoneyOS through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from HoneyOS via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to HoneyOS through an IMAP/SMTP mailbox.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect HoneyOS to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use HoneyOS inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect HoneyOS to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use HoneyOS through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect HoneyOS to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect HoneyOS to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and HoneyOS.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use HoneyOS from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with HoneyOS over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use HoneyOS through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to HoneyOS over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect HoneyOS to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose HoneyOS as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use HoneyOS via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the HoneyOS Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in honeyos gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in honeyos gateway setup",
...
# honeyos.runtime/setup_hidden_env.py. Shared with the `honeyos setup gateway`
# wizard so the surfaces ask for the same things.
from honeyos.runtime.setup_hidden_env import (  # noqa: E402
...
def _gateway_platform_config(platform_id: str):
    from honeyos.gateway.config import Platform, load_gateway_config
...
def _messaging_platform_payload(
    ...
    # HONEYOS_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from honeyos.core.constants import get_honeyos_dir

    return get_honeyos_dir("platforms/whatsapp/session", "whatsapp/session")
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    from honeyos.utils import env_int
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.honeyos-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"HoneyOSDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "HoneyOS").strip() or "HoneyOS"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, HoneyOS appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the HONEYOS_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    """
    from honeyos.gateway.config import PORT_BINDING_PLATFORM_VALUES, load_gateway_config
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized HONEYOS_HOME)
...
# the canonical ``honeyos auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.honeyos/.anthropic_oauth.json`` — HoneyOS-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_honeyos_dotenv()``, so the same check covers them)
    ...
    try:
        from honeyos.agent.anthropic_adapter import (
            read_honeyos_oauth_credentials,
            _get_honeyos_oauth_file,
        )
    except ImportError:
        read_honeyos_oauth_credentials = None  # type: ignore
        _get_honeyos_oauth_file = None  # type: ignore

    honeyos_creds = None
    if read_honeyos_oauth_credentials:
        try:
            honeyos_creds = read_honeyos_oauth_credentials()
        except Exception:
            honeyos_creds = None
    if honeyos_creds and honeyos_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "honeyos_pkce",
            "source_label": f"HoneyOS PKCE ({_get_honeyos_oauth_file() if _get_honeyos_oauth_file else None})",
            "token_preview": _truncate_token(honeyos_creds.get("accessToken")),
            "expires_at": honeyos_creds.get("expiresAt"),
            "has_refresh_token": bool(honeyos_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from honeyos.runtime.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into HoneyOS even
    when they also have a separate HoneyOS-managed PKCE login.
    """
    try:
        from honeyos.agent.anthropic_adapter import read_claude_code_credentials
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    HoneyOS never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "honeyos auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "honeyos auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "honeyos auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "honeyos auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "honeyos auth add xai-oauth",
        "docs_url": "https://honeyos-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "honeyos auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from honeyos.runtime import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside HoneyOS, so the disconnect
    ...
    user sees exactly what executes, and HoneyOS then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside HoneyOS — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``honeyos model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `honeyos model` universe (zero-edit for new plugins).
    try:
        from honeyos.runtime.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"honeyos auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("honeyos_pkce", "claude_code", ...)
    ...
    sync with the `honeyos model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the HoneyOS-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from honeyos.agent.anthropic_adapter import _get_honeyos_oauth_file
                oauth_file = _get_honeyos_oauth_file()
            ...
            try:
                from honeyos.runtime.auth import clear_provider_auth
        ...
        try:
            from honeyos.runtime.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.honeyos/.anthropic_oauth.json AND credential pool
...
# Guarded so honeyos web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both HoneyOS file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``honeyos auth add anthropic``.
    """
    from honeyos.agent.anthropic_adapter import _get_honeyos_oauth_file
    oauth_file = _get_honeyos_oauth_file()
    payload = {
    ...
    try:
        from honeyos.agent.credential_pool import (
            PooledCredential,
            load_pool,
            AUTH_TYPE_OAUTH,
            SOURCE_MANUAL,
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "honeyos-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("HONEYOS_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from honeyos.runtime.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from honeyos.runtime.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``honeyos auth add minimax-oauth``.
    """
    from honeyos.runtime.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `honeyos auth add xai-oauth`: first credential may become
            ...
            # prior ``honeyos auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``honeyos model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before HoneyOS "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from honeyos.runtime.auth import _save_codex_tokens
...
def _open_session_db_for_profile(profile: Optional[str], *, read_only: bool):
    ...
    from honeyos.core.state import SessionDB, _default_db_path, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``honeyos serve`` — it runs neither the
    ...
    try:
        ...
        from honeyos.runtime.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `honeyos sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_honeyos_home()
...
async def get_logs(
    ...
    from honeyos.runtime.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_honeyos_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from honeyos.core.logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from honeyos.runtime import profiles as profiles_mod
    try:
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (HONEYOS_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.honeyos``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    HONEYOS_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, HONEYOS_HOME)."""
    from honeyos.runtime import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["honeyos_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from honeyos.scheduler import jobs as cron_jobs
    from honeyos.core.constants import (
        reset_honeyos_home_override,
        set_honeyos_home_override,
    )

    token = set_honeyos_home_override(str(home))
    try:
        with cron_jobs.use_cron_store(home):
            result = getattr(cron_jobs, func_name)(*args, **kwargs)
    finally:
        reset_honeyos_home_override(token)
...
from honeyos.runtime.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from honeyos.runtime.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime HoneyOS home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (honeyos.runtime.mcp_config), so
# servers managed here show up under `honeyos mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    from honeyos.runtime.mcp_security import validate_mcp_server_entry
...
from honeyos.runtime.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from honeyos.runtime.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from honeyos.runtime.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _pairing_store(profile: Optional[str] = None):
    ...
    the ``await`` boundary.
    """
    from honeyos.gateway.pairing import PairingStore
...
#
# Wraps the same JSON store the CLI uses (honeyos.runtime.webhook); the webhook
...
async def list_webhooks():
    import honeyos.runtime.webhook as wh
...
# spawn the real `honeyos gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_honeyos_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_honeyos_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    from honeyos.agent.credential_pool import load_pool
    from honeyos.runtime.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `honeyos auth remove`) can seed again.
        # Mirrors the `honeyos auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``honeyos auth remove`` uses: each source cleans up its
    ...
    """
    from honeyos.agent.credential_pool import load_pool
    from honeyos.agent.credential_sources import find_removal_step
    from honeyos.runtime.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_honeyos_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_honeyos_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_honeyos_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"honeyos-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_honeyos_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_honeyos_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from honeyos.runtime.config import load_config as _load_config
    from honeyos.agent import shell_hooks

    try:
        from honeyos.runtime.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <honeyos_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_honeyos_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_honeyos_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``honeyos`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_honeyos_action`` tracks one process/log per name, so a shared
...
from honeyos.runtime.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from honeyos.runtime.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _installed_hub_identifiers(profile: Optional[str] = None) -> dict:
    ...
    """
    try:
        from honeyos.tools.skills_hub import HubLockFile
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_honeyos_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "honeyos setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local HONEYOS_HOME override so the write lands in the target
    ...
    """
    from honeyos.core.constants import set_honeyos_home_override, reset_honeyos_home_override

    token = set_honeyos_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local HONEYOS_HOME override (same mechanism as
    ...
    """
    from honeyos.core.constants import set_honeyos_home_override, reset_honeyos_home_override
    from honeyos.runtime.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the HONEYOS_HOME override. Returns the
    number of skills newly disabled.
    """
    from honeyos.core.constants import set_honeyos_home_override, reset_honeyos_home_override
    from honeyos.runtime.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_honeyos_home()`` at call
       time — the context-local override from ``set_honeyos_home_override``
    ...
    retargeted to the *current* ``get_honeyos_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a HONEYOS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from honeyos.tools import skills_tool as _skills_tool
    from honeyos.tools import skill_manager_tool as _skill_mgr

    token = None
    if not requested or requested.lower() == "current":
        profile_dir = get_honeyos_home()
    else:
        profile_dir = _resolve_profile_dir(requested)
        token = set_honeyos_home_override(str(profile_dir))

    with _SKILLS_PROFILE_LOCK:
        old_home = _skills_tool.HONEYOS_HOME
        old_skills_dir = _skills_tool.SKILLS_DIR
        old_mgr_home = _skill_mgr.HONEYOS_HOME
        old_mgr_skills_dir = _skill_mgr.SKILLS_DIR
        _skills_tool.HONEYOS_HOME = profile_dir
        _skills_tool.SKILLS_DIR = profile_dir / "skills"
        _skill_mgr.HONEYOS_HOME = profile_dir
        _skill_mgr.SKILLS_DIR = profile_dir / "skills"
        try:
            yield profile_dir if token is not None else None
        finally:
            _skills_tool.HONEYOS_HOME = old_home
            _skills_tool.SKILLS_DIR = old_skills_dir
            _skill_mgr.HONEYOS_HOME = old_mgr_home
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_honeyos_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_honeyos_home()`` at call time
...
def _clear_skills_prompt_cache() -> None:
    ...
    """
    try:
        from honeyos.agent.prompt_builder import clear_skills_system_prompt_cache
...
from honeyos.runtime.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from honeyos.runtime.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from honeyos.runtime.tools_config import (
...
def _probe_modal_backend() -> tuple:
    try:
        from honeyos.tools.tool_backend_helpers import has_direct_modal_credentials
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not HoneyOS,
...
#
# The endpoint spawns the same ``honeyos --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from honeyos.runtime.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from honeyos.runtime.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from honeyos.runtime.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_client_reason(ws: "WebSocket") -> Optional[str]:
    ...
    so a "WS keeps closing" report can be diagnosed from honeyos.agent.log without a
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from honeyos.runtime.dashboard_auth.audit import AuditEvent, audit_log
        from honeyos.runtime.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``honeyos --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HONEYOS_TUI_RESUME`` env var —
    matching what ``honeyos.runtime.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``HONEYOS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``HONEYOS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``HONEYOS_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``HONEYOS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_honeyos_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``honeyos -p <name>`` performs. The in-process
    ``HONEYOS_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from honeyos.runtime.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # HoneyOS TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides HONEYOS_HOME afterwards.
    from honeyos.tools.environments.local import build_subprocess_env
    ...
    env.setdefault("HONEYOS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("HONEYOS_TUI_INLINE", "1")
    ...
    env["HONEYOS_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["HONEYOS_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["HONEYOS_TUI_RESUME"] = resume

    if sidecar_url:
        env["HONEYOS_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile HONEYOS_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["HONEYOS_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``HONEYOS_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("HONEYOS_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from honeyos.runtime.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="honeyos-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe HoneyOS Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full HoneyOS CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="honeyos-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from honeyos.runtime.console_engine import HoneyOSConsoleEngine

        engine = HoneyOSConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. HoneyOS Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall HoneyOS inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("HONEYOS_TUI_RESUME") or raw_resume
...
# HONEYOS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`honeyos.runtime.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from honeyos.runtime.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default HoneyOS Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="honeyos-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/honeyos/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /honeyos`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HONEYOS_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `honeyos serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("HONEYOS_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (honeyos serve): web UI disabled — use "
            "`honeyos dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/honeyos``)
        ...
        ``__HONEYOS_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__HONEYOS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HONEYOS_BASE_PATH__="{prefix}";'
                f"window.__HONEYOS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__HONEYOS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__HONEYOS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HONEYOS_BASE_PATH__="{prefix}";'
                f"window.__HONEYOS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``HONEYOS_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default HoneyOS Teal canvas, then
    ...
    # under ``/honeyos`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the HoneyOS backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "HoneyOS Teal",         "description": "Classic dark teal — the canonical HoneyOS look"},
    {"name": "default-large", "label": "HoneyOS Teal (Large)", "description": "HoneyOS Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.honeyos/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.honeyos/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_honeyos_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``HONEYOS_HOME``.
    """
    themes_dir = get_process_honeyos_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.honeyos/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as honeyos.runtime.plugins):
    1. User plugins:    ~/.honeyos/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.honeyos/plugins/  (only if HONEYOS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from honeyos.runtime.plugins import get_bundled_plugins_dir
    ...
    # HONEYOS_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_honeyos_home() / "plugins", "user"),
    ...
    # ``honeyos.runtime/plugins.py`` and the documented user contract.
    if env_var_enabled("HONEYOS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".honeyos" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    # from honeyos.plugins the user has not explicitly activated.  (#46435)
    try:
        from honeyos.runtime.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _schedule_check_fn_probe(fn) -> Optional[threading.Thread]:
    ...
    def _probe():
        try:
            from honeyos.tools.registry import _check_fn_cached
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_honeyos_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        ...
        if provides_tools:
            try:
                from honeyos.tools.registry import get_cached_check_fn_result, registry
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"honeyos auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from honeyos.runtime.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from honeyos.runtime.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from honeyos.runtime.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from honeyos.runtime.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    """Import and mount backend API routes from honeyos.plugins that declare them.
    ...
    Project plugins (``./.honeyos/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.honeyos/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"honeyos_dashboard_plugin_{plugin['name']}"
...
from honeyos.runtime.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``HONEYOS_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("HONEYOS_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``HONEYOS_SERVE_HEADLESS``), so
    ...
    try:
        from honeyos.runtime.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the honeyos-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing HONEYOS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with HoneyOS exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from honeyos.plugins.dashboard_auth import nous as _nous_plugin
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `honeyos dashboard register` (Nous Portal) or "
            ...
            # (#54489).
            try:
                from honeyos.runtime.config import load_config as _load_cfg
                from honeyos.runtime.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "configured.\n"
                        "Remove 'basic' from honeyos.plugins.disabled (or run "
                        "`honeyos plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # HONEYOS_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "HONEYOS_BACKEND_READY" if headless else "HONEYOS_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  HoneyOS backend listening on {host}:{actual_port}")
            else:
                print(f"  HoneyOS Web UI → http://{host}:{actual_port}")
