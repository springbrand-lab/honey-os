"""
Status command for honeyos CLI.

Shows the status of all HoneyOS components.
"""
...
from honeyos.runtime.auth import AuthError, resolve_provider
from honeyos.runtime.colors import Colors, color
from honeyos.runtime.config import get_env_path, get_env_value, get_honeyos_home, load_config
from honeyos.runtime.models import provider_label
...
from honeyos.runtime.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``honeyos config``'s
...
def show_status(args):
    """Show status of all HoneyOS components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ HoneyOS Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `honeyos status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: honeyos portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: honeyos model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: honeyos auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: honeyos auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: honeyos model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from honeyos.runtime.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'honeyos-agent[vercel]')"
    ...
    try:
        from honeyos.runtime.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   honeyos gateway")
    ...
    jobs_file = get_honeyos_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_honeyos_home() / "sessions" / "sessions.json"
    ...
    try:
        from honeyos.runtime.active_sessions import (
    ...
    print(color("  Run 'honeyos doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'honeyos setup' to configure", Colors.DIM))
    print()
