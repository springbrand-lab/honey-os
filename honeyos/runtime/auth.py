"""
Multi-provider authentication system for HoneyOS.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.honeyos/auth.json with cross-process file locking.
...
from honeyos.core.constants import OPENROUTER_BASE_URL, secure_parent_dir
from honeyos.agent.credential_persistence import sanitize_borrowed_credential_payload
from honeyos.utils import atomic_replace, atomic_yaml_write, env_float, is_truthy_value
...
DEFAULT_NOUS_CLIENT_ID = "honeyos-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from honeyos.runtime import __version__ as _HONEYOS_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _HONEYOS_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"honeyos-cli/{_HONEYOS_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://honeyos-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://honeyos-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for HoneyOS' Responses transport.
    """
...
try:
    from honeyos.providers import list_providers as _list_providers_for_registry
...
def get_anthropic_key() -> str:
    ...
    ``~/.honeyos/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from honeyos.runtime.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from honeyos.runtime.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.honeyos/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``honeyos auth``.
    """
    return (
...
def _parse_retry_after_seconds(headers: Any) -> Optional[int]:
    ...
    """
    from honeyos.agent.retry_utils import parse_retry_after_seconds
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `honeyos model` to re-authenticate."
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from honeyos.runtime.nous_account import (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("HONEYOS_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.honeyos/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_honeyos_home() / "auth.json"
    # Seat belt: if pytest is running and HONEYOS_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch HONEYOS_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".honeyos" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set HONEYOS_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom HONEYOS_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_honeyos_root()
    except Exception:
        return None
    profile_home = get_honeyos_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.honeyos/auth.json`` even when HONEYOS_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_honeyos_root()`` for a profile-shaped HONEYOS_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".honeyos" / "auth.json"
            try:
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``honeyos auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``honeyos auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``honeyos auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted HoneyOS
    ...
    # not by the user explicitly configuring anthropic in HoneyOS.
    ...
    if pconfig is None:
        from honeyos.runtime.providers import get_provider
    ...
    # the user initiated inside HoneyOS (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "honeyos_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `honeyos logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from honeyos.runtime.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'honeyos doctor' for full diagnostics:"]
        for ci in issues:
...
def resolve_provider(
    ...
    *,
    ...
    try:
        from honeyos.providers import list_providers as _lp
        for _pp in _lp():
            for _alias in _pp.aliases:
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'honeyos model' for available providers, or run 'honeyos doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `honeyos auth add openrouter`
    ...
    # `honeyos auth list` showing the credential while requests go out with no
    ...
    try:
        from honeyos.agent.credential_pool import load_pool as _load_pool
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.honeyos/.env isn't silently
    ...
    try:
        from honeyos.agent.bedrock_adapter import has_aws_credentials
    ...
    raise AuthError(
        "No inference provider configured. Run 'honeyos model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.honeyos/.env.",
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``HONEYOS_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing HoneyOS at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``HONEYOS_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("HONEYOS_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: honeyos auth add nous",
        provider="nous",
        code=reason,
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("HONEYOS_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `honeyos model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.honeyos/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("HONEYOS_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set HONEYOS_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("HONEYOS_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("HONEYOS_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("HONEYOS_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `honeyos auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `honeyos auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `honeyos auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `honeyos auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.honeyos/.env, and return it.
    ...
    print("       App name:     anything (e.g. honeyos-agent)")
    ...
    # Persist so subsequent `honeyos auth spotify` runs skip the wizard.
    save_env_value("HONEYOS_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("HONEYOS_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved HONEYOS_SPOTIFY_CLIENT_ID to ~/.honeyos/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "HONEYOS_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize HoneyOS:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"HoneyOS is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.honeyos/auth.json (not ~/.codex/)
#
# HoneyOS maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from HoneyOS auth store (~/.honeyos/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `honeyos auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `honeyos auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `honeyos auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `honeyos auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``honeyos setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``honeyos auth add openai-codex``
      ...
      ``honeyos auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to HoneyOS auth store (~/.honeyos/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``honeyos auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into HoneyOS auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating HoneyOS auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run honeyos auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `honeyos auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to HoneyOS auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. HoneyOS keeps its OWN
        ...
        # the Codex CLI (or another HoneyOS process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from HoneyOS's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("HONEYOS_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("HONEYOS_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other HoneyOS processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    HoneyOS persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.honeyos/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `honeyos model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `honeyos model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `honeyos model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `honeyos model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.honeyos/auth.json even when HONEYOS_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``honeyos auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.honeyos/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `honeyos model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``HONEYOS_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `honeyos model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older HoneyOS (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `honeyos model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``honeyos model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("HONEYOS_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("HONEYOS_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("HONEYOS_CA_BUNDLE")
...
# so a new `honeyos --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${HONEYOS_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<honeyos-root>/shared/nous_auth.json`` where ``<honeyos-root>`` is what
# ``get_default_honeyos_root()`` returns — ``~/.honeyos`` on Linux/macOS,
# ``%LOCALAPPDATA%\honeyos`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's HONEYOS_HOME so named profiles (which
# typically live under ``<honeyos-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``HONEYOS_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<honeyos-root>/shared/``, where ``<honeyos-root>`` is what
    :func:`honeyos.core.constants.get_default_honeyos_root` returns — so
    Linux/macOS classic installs land at ``~/.honeyos/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\honeyos\\shared\\``, and
    Docker / custom ``HONEYOS_HOME`` deployments at
    ``<HONEYOS_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("HONEYOS_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from honeyos.core.constants import get_default_honeyos_root
    return get_default_honeyos_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's HoneyOS root, refuse rather than silently corrupt cross-profile
    # state. Tests must set HONEYOS_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from honeyos.core.constants import get_default_honeyos_root
        real_home_shared = (
            get_default_honeyos_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set HONEYOS_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No HONEYOS_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — HoneyOS
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with HoneyOS's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, HoneyOS's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another HoneyOS install sharing "
            "~/.honeyos/auth.json) called POST /api/oauth/token with HoneyOS's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only HoneyOS may call the "
            "refresh endpoint. For health checks, use `honeyos auth status` "
            "instead.\n"
            "Re-authenticate with: honeyos auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip HoneyOS models — they're not reliable for agentic tool-calling
            if "honeyos" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "HoneyOS is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # HONEYOS_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "honeyos-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``honeyos auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``honeyos auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `honeyos auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("HoneyOS is not logged into Nous Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("HONEYOS_PORTAL_BASE_URL")
...
# ~350ms even on the failure path, and read-only UI surfaces (`honeyos tools`, status panels,
# subscription-feature checks) call it many times per render — `honeyos tools` → "All Platforms"
...
# `honeyos auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `honeyos auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `honeyos auth` and
    # `honeyos model` store device_code tokens.
    try:
        from honeyos.agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("HONEYOS_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("HONEYOS_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``honeyos doctor`` runs the live
    ...
    try:
        from honeyos.runtime.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
    ...
    if auth_mode == "entra_id":
        try:
            from honeyos.agent.azure_identity_adapter import (
                EntraIdentityConfig,
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on HoneyOS' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `honeyos doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set HONEYOS_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from honeyos.runtime.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `honeyos logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    try:
        ...
        from honeyos.runtime.model_search import model_search_text
    ...
    from honeyos.runtime.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from honeyos.runtime.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'honeyos model' or 'honeyos setup' instead."""
    print("The 'honeyos login' command has been removed.")
    print("Use 'honeyos auth' to manage credentials,")
    print("'honeyos model' to select a provider, or 'honeyos setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.honeyos/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing HoneyOS-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in HoneyOS auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("HoneyOS will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("HONEYOS_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("HoneyOS will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — HoneyOS gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(HoneyOS creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to HoneyOS auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in HoneyOS auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(HoneyOS creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``honeyos auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``honeyos auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to HoneyOS auth store (~/.honeyos/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting HoneyOS login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `honeyos model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``honeyos status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting HoneyOS login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `honeyos model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``HONEYOS_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older HoneyOS builds still see
        ...
        if not selected_model:
            ...
            print("  Run `honeyos model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("HoneyOS will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `honeyos model` or configure an API key to use HoneyOS.")
        else:
