"""Offline, non-destructive recovery for a damaged HoneyOS session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``honeyos.runtime.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``honeyos.core.state._backup_db_file``.
    """
    from honeyos.runtime.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="honeyos-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every HoneyOS process using this profile and retry."
            )
