...
Used by ``honeyos kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``honeyos.runtime/goals.py`` — same aux
...
HONEYOS_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("HONEYOS_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the HoneyOS board.
...
def _profile_author() -> str:
    """Mirror of ``honeyos.runtime.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("HONEYOS_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=HONEYOS_KANBAN_SPECIFY_MAX_TOKENS,
