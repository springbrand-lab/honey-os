"""
honeyos fallback — manage the fallback provider chain.
...
Subcommands:
  honeyos fallback [list]   Show the current fallback chain (default when no subcommand)
  honeyos fallback add      Pick provider + model via the same picker as `honeyos model`,
                           then append the selection to the chain
  honeyos fallback remove   Pick an entry to delete from the chain
  honeyos fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.honeyos/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from honeyos.runtime.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `honeyos model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  honeyos fallback add")
        print()
        return
    ...
    print("  Docs: https://honeyos-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `honeyos model`, then append the selection to the chain."""
    from honeyos.runtime.main import _require_tty, select_provider_and_model
    ...
    print("  `honeyos model` — select the provider + model you want as a fallback.")
    print()
    ...
    from honeyos.agent.backend_identity import BackendIdentity, same_deployment
    ...
    print()
    print("  Run `honeyos fallback list` to view, or `honeyos fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``honeyos fallback [subcommand]``."""
