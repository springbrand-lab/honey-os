...
This module is imported by ``honeyos.runtime/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no honeyos.runtime.config,
...
``honeyos.runtime/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(honeyos_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect HONEYOS_HOME?"""
    active_profile = os.path.join(honeyos_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    honeyos_home = os.environ.get("HONEYOS_HOME", "").strip()
    if honeyos_home:
        return honeyos_home
    return os.path.join(os.path.expanduser("~"), ".honeyos")
...
def container_mode_may_be_active() -> bool:
    ...
    honeyos_home = os.environ.get("HONEYOS_HOME", "").strip()
    if honeyos_home:
        if os.path.exists(os.path.join(honeyos_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(honeyos_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(honeyos_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".honeyos")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect HONEYOS_HOME.
    """
...
def print_fast_version_info() -> None:
    from honeyos.runtime import __release_date__, __version__

    print(f"HoneyOS v{__version__} ({__release_date__})")
    ...
    print("Run 'honeyos version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``honeyos --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the HONEYOS_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("HONEYOS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
