"""HoneyOS update pipeline — extracted from ``honeyos.runtime/main.py``.
...
change is that references to helpers/constants that STAY in ``honeyos.runtime.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``honeyos.runtime.main`` reference — so existing call sites and test monkeypatches
that target ``honeyos.runtime.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``honeyos.runtime.main``.
...
Imports are one-way: ``honeyos.runtime.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``honeyos.runtime.main`` reference.

    Lets callers keep patching ``honeyos.runtime.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``honeyos.runtime.main`` -> ``honeyos.runtime.update_cmd`` stays one-way
    at import time.
    """
    from honeyos.runtime import main

    return main


_UPDATE_RUNTIME_RELOAD_MODULES = (
    "honeyos.core.constants",
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``honeyos update`` keeps running in the pre-pull Python process. After a
...
# Critical files that HoneyOS must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    ...
    "honeyos.runtime/config.py",
    "honeyos.runtime/__init__.py",
    "honeyos.runtime/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "honeyos.core.constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``honeyos`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="honeyos-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``honeyos update`` can be driven by a
    ...
    """
    from honeyos.core.constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from honeyos.core.constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('honeyos_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``honeyos update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `honeyos update`.
    ...
    print("  Preview now:  honeyos curator run --dry-run")
    print("  Pause it:     honeyos curator pause")
    print(
        "  Docs:         https://honeyos-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `honeyos update`.
    ...
    try:
        ...
        return
    db_path = get_honeyos_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `honeyos sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    honeyos sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  honeyos sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``honeyos update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``honeyos update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: honeyos curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    honeyos dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``honeyos.runtime.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.honeyos-update-staging"
    backup = f"{dst}.honeyos-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `honeyos update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``honeyos.core.constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("HONEYOS_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== honeyos-update completed {action_id} ===")


def _update_via_zip(args):
    """Update HoneyOS by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `honeyos update "
            f"--branch {branch}`, or update against main with `honeyos update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/honeyos-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="honeyos-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"honeyos-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for honeyos-agent itself
        ...
        # GitHub ZIPs extract to honeyos-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"honeyos-agent-{branch}")
        ...
        print(
            "  Re-run `honeyos update` to retry; if the agent won't start, "
            "reinstall from https://honeyos-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from honeyos.runtime.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `honeyos update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: honeyos skills list-modified  "
    ...
    try:
        from honeyos.runtime.model_catalog import seed_cache_from_checkout
    ...
    try:
        from honeyos.runtime.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_honeyos_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_honeyos_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "honeyos-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if HoneyOS behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes honeyos completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but HoneyOS couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but HoneyOS couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if HoneyOS behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but HoneyOS couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but HoneyOS couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/honeyos-agent.git",
    "git@github.com:NousResearch/honeyos-agent.git",
    "https://github.com/NousResearch/honeyos-agent",
    "git@github.com:NousResearch/honeyos-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/honeyos-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_honeyos_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_honeyos_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official HoneyOS repository.")
        print("  This means you may miss updates from NousResearch/honeyos-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/honeyos-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/honeyos-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``honeyos update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from honeyos.core.constants import get_default_honeyos_root

    default_home = get_default_honeyos_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "honeyos.exe"
    lines = ["✗ Another honeyos.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close HoneyOS Desktop, exit any open `honeyos` REPLs, and")
    lines.append("  stop the gateway (`honeyos gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `honeyos update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``honeyos update`` runs ``uv pip install -e .[all]`` which
    ...
    """
    try:
        from honeyos.tools import lazy_deps
    except Exception as exc:
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `honeyos update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `honeyos update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in HoneyOS' editable-install extras or
    ...
    try:
        from honeyos.runtime.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from honeyos.runtime.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$HONEYOS_HOME/bin/uv``, but on Termux the official
    ...
    """
    from honeyos.runtime.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `honeyos update` is exactly the step
...
def _npm_lockfile_changed(honeyos_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `honeyos
    ...
    try:
        ...
        cache_file = honeyos_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(honeyos_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from honeyos.core.constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `honeyos update`.")
            failed = ["repo root"]
            if any(
    ...
    # HoneyOS profile using this checkout. Keep one per-checkout cache under the
    # shared HoneyOS root rather than rerunning npm once per named profile.
    shared_honeyos_root = get_default_honeyos_root()
    if not _m()._npm_lockfile_changed(shared_honeyos_root):
    ...
    # need desktop during `honeyos update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `honeyos update`.")
        return list(labels)
    ...
    nixos_env = with_honeyos_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `honeyos update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_honeyos_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.honeyos/logs/update.log`` only, never the terminal.

    During ``honeyos update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``honeyos update --check``: fetch and report without installing.
    ...
    """
    from honeyos.runtime.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``honeyos update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from honeyos.runtime.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``honeyos update`` without requiring a reinstall.  The
    ...
    ``honeyos`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/honeyos.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v honeyos'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/honeyos, code at /usr/local/lib/honeyos-agent).
    fhs_link = Path("/usr/local/bin/honeyos")
    ...
    path_comment = (
        "# HoneyOS — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``honeyos-acp`` launcher next to the ``honeyos`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``honeyos update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``honeyos-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report HoneyOS as not installed even when it is.

    The shim simply delegates to the sibling ``honeyos`` launcher with the
    ...
    ``honeyos-acp.exe`` already resolves) and wherever a ``honeyos-acp`` is
    already present next to the ``honeyos`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        honeyos_cmd = bin_dir / "honeyos"
        acp_cmd = bin_dir / "honeyos-acp"
        try:
            if not (honeyos_cmd.is_file() or honeyos_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of HONEYOS_HOME under
      ``backups/`` (restorable via ``honeyos import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from honeyos.runtime.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from honeyos.core.constants import
        # get_honeyos_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from honeyos.runtime.config import get_honeyos_home as _get_home
    ...
    try:
        from honeyos.runtime.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_honeyos_home so the user sees ~/.honeyos/...
    try:
        from honeyos.core.constants import get_honeyos_home, display_honeyos_home

        home = get_honeyos_home()
        try:
            display_path = f"{display_honeyos_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  honeyos import {out_path}")
...
def _write_update_planned_stop_marker(profile_path: Path, pid: int) -> bool:
    ...
    try:
        ...
        from honeyos.gateway.status import _get_process_start_time
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``honeyos update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``honeyos update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run honeyos from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.honeyos-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".honeyos-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The honeyos.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m honeyos.runtime.main
    ...
    calling process and its ancestors are always excluded (a CLI ``honeyos
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m honeyos.runtime.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "honeyos.runtime.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other HoneyOS processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← HoneyOS Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the HoneyOS desktop app / other HoneyOS terminals, then re-run:"
    )
    lines.append("    honeyos update")
    lines.append("  (or use `honeyos update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``honeyos update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from honeyos.runtime._scan_venv_blockers import _is_pausable_gateway
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    honeyos.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from honeyos.runtime.gateway import (
    ...
    print("→ Stopping Windows gateway process(es) before updating HoneyOS...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m honeyos.runtime.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: honeyos gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``honeyos gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from honeyos.runtime import gateway_windows
        from honeyos.runtime.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``honeyos-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("honeyos-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``honeyos update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``honeyos update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``honeyos update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating HoneyOS...")
    print()

    # On Windows, abort early if another honeyos.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_honeyos_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `honeyos serve` backend) will keep .pyd
    ...
    # the honeyos.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://honeyos-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all HoneyOS windows/gateways and re-run: honeyos update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running HoneyOS gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in honeyos.runtime/config.py bricked
        # every user who ran ``honeyos update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``honeyos
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``honeyos update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_honeyos_home added to honeyos.core.constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``honeyos`` launch finishes the install
        ...
        # symbols in honeyos.core.constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `honeyos` again "
        ...
        if not import_ok:
            ...
            print("    Run `honeyos update` again — if it persists, reinstall:")
            print("    https://honeyos-agent.nousresearch.com")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``honeyos desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``honeyos desktop`` shouldn't be forced into a full
        # Electron build by ``honeyos update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `honeyos desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "honeyos.runtime.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the HoneyOS-managed Node on PATH:
                # when `honeyos update` runs inside the desktop updater chain
                # (Desktop → honeyos-setup → honeyos update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from honeyos.core.constants import with_honeyos_node_path

                _build_env = with_honeyos_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `honeyos desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_honeyos_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.honeyos/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit HONEYOS_HOME so
        # it is not affected by sync_skills()'s module-level HONEYOS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's HONEYOS_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from honeyos.runtime.profiles import backfill_profile_envs
        ...
        try:
            from honeyos.plugins.memory.honcho.cli import sync_honcho_profiles_quiet
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'honeyos config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: honeyos config migrate")
            else:
                print()
                print("Skipped. Run 'honeyos config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from honeyos.runtime.backup import restore_cron_jobs_if_emptied
        ...
        # check `honeyos curator status`. Self-stamps after printing so it
        ...
        # Self-heal the honeyos-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve HoneyOS on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("honeyos-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``honeyos update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `honeyos update`
                # must stay fast; `honeyos computer-use install --upgrade`
        ...
        # When running as ``honeyos update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_honeyos_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... honeyos-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... honeyos-gateway*)
            ...
            try:
                from honeyos.runtime.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all honeyos-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: honeyos gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `honeyos update` restart it automatically, allow\n"
                        ...
                        # path in `honeyos gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: honeyos gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: honeyos -p <profile> gateway run  for each profile)"
                        )
            ...
            try:
                ...
                if _stuck:
                    ...
                    from honeyos.gateway.status import terminate_pid as _terminate_pid
                    for pid in _stuck:
                        try:
        ...
        # Warn if legacy HoneyOS gateway unit files are still installed.
        # When both honeyos.service (from a pre-rename install) and the
        # current honeyos-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `honeyos update` surfaces the issue until the user migrates.
        try:
            from honeyos.runtime.gateway import (
                has_legacy_honeyos_units,
                _find_legacy_honeyos_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_honeyos_units():
                print()
                print("⚠ Legacy HoneyOS gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_honeyos_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (honeyos.service) fight the current")
                print("  honeyos-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    honeyos gateway migrate-legacy")
                print()
        ...
        print("  honeyos model              # Select provider and model")
