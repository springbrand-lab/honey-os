"""``honeyos sync`` subcommand parser — Skill Sync.

Cloned from ``honeyos.runtime/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    honeyos sync status                 show gate/opt-in/head state
    honeyos sync pull                   pull and materialize opted-in skills
    honeyos sync push                   push opted-in skills
    honeyos sync now                    reconcile: pull then push
    honeyos sync enable <skill>         opt a skill into sync
    honeyos sync disable <skill>        opt a skill out of sync
    honeyos sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    honeyos sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  honeyos sync status            what is synced, and from where\n"
            "  honeyos sync enable my-skill   include a skill in your sync\n"
            "  honeyos sync now               pull, then push\n"
            "  honeyos sync propose my-skill  share a skill with your team\n"
        ),
