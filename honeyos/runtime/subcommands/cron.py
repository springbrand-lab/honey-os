"""``honeyos cron`` subcommand parser.

Extracted verbatim from ``honeyos.runtime/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.honeyos/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.honeyos/scripts/. Pass empty string to clear. "
