"""``honeyos setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure HoneyOS with an interactive wizard. "
        "Run a specific section: "
        "honeyos setup model|tts|terminal|gateway|tools|telemetry|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'honeyos setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
