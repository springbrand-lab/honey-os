"""``honeyos debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for HoneyOS. Use 'honeyos debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    honeyos debug share              Upload debug report (asks for confirmation)
    honeyos debug share --yes        Skip confirmation (for scripts/CI)
    honeyos debug share --lines 500  Include more log lines
    honeyos debug share --expire 30  Keep paste for 30 days
    honeyos debug share --local      Print report locally (no upload)
    honeyos debug share --no-redact  Disable upload-time secret redaction
    honeyos debug share --nous       Upload to Nous-internal storage (private)
    honeyos debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'honeyos debug share'",
    )
