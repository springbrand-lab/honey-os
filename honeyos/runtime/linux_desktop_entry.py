"""Install and remove the Linux desktop entry (``honeyos.desktop``).

``honeyos desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "honeyos.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``honeyos.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``honeyos desktop``.

    Prefer the real ``honeyos`` executable (argv[0] or PATH). When HoneyOS
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from honeyos.runtime.relaunch import resolve_honeyos_bin

    bin_path = resolve_honeyos_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "honeyos.runtime.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=HoneyOS\n"
        "GenericName=HoneyOS Desktop\n"
        "Comment=Launch HoneyOS Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=HoneyOS\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the HoneyOS desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "honeyos"
