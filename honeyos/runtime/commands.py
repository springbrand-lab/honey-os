"""Slash command definitions and autocomplete for the HoneyOS CLI.
...
from honeyos.core.constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``honeyos.runtime.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``honeyos.runtime.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of HoneyOS config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal HoneyOS works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey HoneyOS' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while HoneyOS is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.honeyos/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update HoneyOS to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show HoneyOS version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`honeyos.runtime.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/honeyos`` subcommand mapping, and
    ...
    """
    try:
        from honeyos.runtime.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. HoneyOS ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full HoneyOS
...
def _collect_gateway_skill_entries(
    ...
    try:
        from honeyos.agent.skill_commands import get_skill_commands
        from honeyos.tools.skills_tool import SKILLS_DIR
        from honeyos.agent.skill_utils import get_external_skills_dirs
        ...
        # ``honeyos skills list`` and the agent's ``/skill-name`` dispatch but
        # silently excluded from honeyos.gateway slash menus (#8110).
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``honeyos skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``honeyos skills list`` and
...
# unrelated command landed. These claim their slots right after /honeyos,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /honeyos <command>).
...
# native slot, the alias spelling stays reachable via /honeyos reset).
...
# ``/honeyos <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/honeyos" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /honeyos topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /honeyos moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /honeyos debug on Slack.
#   - egress: Docker-only proxy status; reachable as /honeyos egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /honeyos init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /honeyos version on
...
#   - diff: git working-tree diff; reached via /honeyos diff on Slack so it
...
#     /honeyos update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /honeyos heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /honeyos refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
_SLACK_VIA_HONEYOS_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/honeyos <verb>`` subcommand.
    ...
    can still reach them via ``/honeyos <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/honeyos`` is always reserved as the first entry so the
    legacy ``/honeyos <subcommand>`` form keeps working for anything that
    ...
    # Reserve /honeyos as the catch-all top-level command.
    entries.append(("honeyos", "Talk to HoneyOS or run a subcommand", "[subcommand] [args]"))
    seen.add("honeyos")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_HONEYOS_ONLY:
            # Intentionally Slack-via-/honeyos only (see _SLACK_VIA_HONEYOS_ONLY).
            return
    ...
    # everything except /honeyos, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://honeyos-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /honeyos handler.

    Maps both canonical names and aliases so /honeyos bg do stuff works
    the same as /honeyos background do stuff.

    Plugin-registered slash commands are included so ``/honeyos <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _stacked_skill_completions(self, text: str):
        ...
        """
        try:
            from honeyos.agent.skill_commands import _MAX_STACKED_SKILLS as _cap
        except Exception:
            _cap = 5
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from honeyos.runtime.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
