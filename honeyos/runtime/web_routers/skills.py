...
the late-binding seam in :mod:`honeyos.runtime.web_deps` so tests that
``monkeypatch.setattr(web_server, "_spawn_honeyos_action", ...)`` keep
working.
"""
...
async def list_skills_hub_sources(profile: Optional[str] = None):
    ...
    def _run():
        from honeyos.tools.skills_hub import create_source_router
...
async def search_skills_hub(
    ...
    def _run():
        from honeyos.tools.skills_hub import create_source_router, parallel_search_sources
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from honeyos.runtime.skills_hub import _resolve_source_meta_and_bundle
        from honeyos.tools.skills_hub import create_source_router
...
async def scan_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        ...
        from honeyos.runtime.skills_hub import _resolve_source_meta_and_bundle
        from honeyos.tools.skills_hub import create_source_router, quarantine_bundle
        from honeyos.tools.skills_guard import scan_skill, should_allow_install
...
async def get_skills(profile: Optional[str] = None):
    ...
    from honeyos.runtime.skills_config import get_disabled_skills
    from honeyos.tools.skill_usage import (
...
async def get_skill_content(name: str, profile: Optional[str] = None):
    ...
    from honeyos.tools.skill_manager_tool import _find_skill
...
async def create_skill(body: SkillCreate):
    ...
    """
    from honeyos.tools.skill_manager_tool import _create_skill
...
async def update_skill_content(body: SkillContentUpdate):
    ...
    from honeyos.tools.skill_manager_tool import _edit_skill
