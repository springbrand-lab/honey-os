...
late-binding seam in :mod:`honeyos.runtime.web_deps` (``late`` for callables,
...
from honeyos.runtime.web_deps import late, LateState
from honeyos.runtime.web_models import (
...
_log = logging.getLogger("honeyos.runtime.web_server")
...
_spawn_honeyos_action = late("_spawn_honeyos_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from honeyos.runtime.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``honeyos tools`` picker shows
    ...
    with _profile_scope(profile):
        ...
        if name == "web":
            ...
            try:
                from honeyos.tools.web_tools import _get_extract_backend, _get_search_backend
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `honeyos tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``honeyos tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.honeyos/.env`` via ``save_env_value`` —
    the same store ``honeyos tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``honeyos tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``honeyos -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same HONEYOS_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``honeyos computer-use permissions grant`` as a background action.
