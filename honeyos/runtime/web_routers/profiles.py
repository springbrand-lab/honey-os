...
late-binding seam in :mod:`honeyos.runtime.web_deps` so tests that
...
from honeyos.runtime.web_deps import late
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's HONEYOS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``honeyos profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from HONEYOS_HOME).
    """
    from honeyos.runtime import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``honeyos profile use``).
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (honeyos.runtime.profiles
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the HONEYOS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``honeyos profile describe
    <name> --auto``.
    ...
    try:
        from honeyos.runtime import profile_describer
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps honeyos.runtime.profiles.export_profile /
# import_profile (the same machinery behind `honeyos profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_honeyos_home() / "profile-exports"
        try:
