"""honeyos memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``honeyos memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_honeyos_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as honeyos tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``honeyos update`` heals the active memory provider
    ...
    # data volume (HONEYOS_LAZY_INSTALL_TARGET). install_specs handles the
...
def _get_available_providers() -> list:
    ...
    """
    try:
        from honeyos.plugins.memory import discover_memory_providers, load_memory_provider
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'honeyos memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(honeyos_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.honeyos/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like honeyos-cli).
    ...
    if provider_name:
        ...
        if provider:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.honeyos/plugins/")
