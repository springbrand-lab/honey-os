"""
Backup and import commands for honeyos CLI.

`honeyos backup` creates a zip archive of the entire ~/.honeyos/ directory
(excluding the honeyos-agent repo and transient files).

`honeyos import` restores from a backup zip, overlaying onto the current
HONEYOS_HOME root.
"""
...
from honeyos.core.constants import get_default_honeyos_root, get_honeyos_home, display_honeyos_home
...
# ``honeyos-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/honeyos-agent/``
...
# HONEYOS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "honeyos-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under HONEYOS_HOME) —
...
# File names that ``honeyos import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE HONEYOS_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the HoneyOS backup slot."""
...
@contextmanager
def _backup_operation_lock(honeyos_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = honeyos_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another HoneyOS backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside HONEYOS_HOME, resolved from config only (no network, no init).
    ...
    """
    try:
        from honeyos.plugins.memory import _get_active_memory_provider, load_memory_provider
    except Exception:
        return []
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to honeyos root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``honeyos-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/honeyos-agent/`` — must be preserved.
        if part == "honeyos-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside HONEYOS_HOME.
...
# to the user as a hung `honeyos update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # honeyos.runtime.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the HoneyOS home directory."""
    honeyos_root = get_default_honeyos_root()

    if not honeyos_root.is_dir():
        print(f"Error: HoneyOS home directory not found at {honeyos_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(honeyos_root):
            _run_backup_locked(args, honeyos_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, honeyos_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"honeyos-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"honeyos-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_honeyos_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(honeyos_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(honeyos_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``honeyos-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "honeyos-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(honeyos_root)
    ...
    # outside HONEYOS_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_honeyos_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: honeyos import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a HoneyOS backup.
    ...
    # Look for telltale files that a honeyos home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a HoneyOS backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.honeyos/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a honeyos dir name
        if prefix in {".honeyos", "honeyos"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_honeyos_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (honeyos_root / "config.yaml").exists()
        has_env = (honeyos_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has HoneyOS configuration.")
        ...
        honeyos_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under HONEYOS_HOME.
            ...
            target = honeyos_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(honeyos_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_honeyos_home()}."
            )
        ...
        profiles_dir = honeyos_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # honeyos.runtime.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: honeyos profile list  (after installing honeyos)")

        # Guidance
        print()
        if not (honeyos_root / "honeyos-agent").is_dir():
            print("Note: The honeyos-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: honeyos update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  honeyos -p {pname} gateway install")

        print("Done. Your HoneyOS configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and honeyos backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to HONEYOS_HOME).
...
# — `honeyos update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $HONEYOS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside HONEYOS_HOME and the entry is silently
...
def _quick_snapshot_root(honeyos_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: honeyos snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(honeyos_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside HONEYOS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``honeyos update``.
    ...
    Args:
        ...
        honeyos_home: Override for the HoneyOS home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(honeyos_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for honeyos backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_honeyos_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, honeyos_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(honeyos_root):
            return _write_full_zip_backup_locked(out_path, honeyos_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, honeyos_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``honeyos_root`` to ``out_path``.
...
def _pre_update_backup_dir(honeyos_home: Optional[Path] = None) -> Path:
    home = honeyos_home or get_honeyos_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of HONEYOS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<HONEYOS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``honeyos update``) should continue even if the backup fails.
    """
    honeyos_root = honeyos_home or get_default_honeyos_root()
    if not honeyos_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(honeyos_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, honeyos_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `honeyos claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of HONEYOS_HOME under ``backups/`` before a
    ``honeyos claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``honeyos import <archive>``.  Writes to
    ``<HONEYOS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `honeyos import` and the
