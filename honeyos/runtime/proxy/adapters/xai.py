...
from honeyos.agent.credential_pool import CredentialPool, PooledCredential, load_pool
from honeyos.runtime.auth import DEFAULT_XAI_OAUTH_BASE_URL
from honeyos.runtime.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

logger = logging.getLogger(__name__)

_POOL_PROVIDER = "xai-oauth"

# xAI's public API is OpenAI-compatible for the endpoints HoneyOS commonly
# uses. The Responses endpoint is included because HoneyOS' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via HoneyOS-managed OAuth credentials."""

    auth_hint = "honeyos auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`honeyos auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`honeyos auth reset xai-oauth` or re-authenticate with "
                    "`honeyos auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `honeyos auth add xai-oauth --type oauth`."
            )
