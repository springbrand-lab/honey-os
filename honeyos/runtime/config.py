"""
Configuration management for HoneyOS.

Config files are stored in ~/.honeyos/ for easy access:
- ~/.honeyos/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.honeyos/.env         - API keys and secrets

This module provides:
- honeyos config          - Show current configuration
- honeyos config edit     - Open config in editor
- honeyos config get      - Print a resolved configuration value
- honeyos config set      - Set a specific value
- honeyos config unset    - Remove a user configuration value
- honeyos config wizard   - Re-run setup wizard
"""
...
from honeyos.runtime.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``honeyos config set`` (which rewrites
    ...
    ``config.yaml`` in place: honeyos never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.honeyos/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``honeyos logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  honeyos config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` HoneyOS makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. HoneyOS itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``honeyos update``, the TUI build.
...
#   on every plugin install / ``honeyos update``.
...
# * ``HONEYOS_HOME`` / ``HONEYOS_PROFILE`` / ``HONEYOS_CONFIG`` /
#   ``HONEYOS_ENV`` — HoneyOS runtime location flags. Writing these into
...
#
# IMPORTANT: ``HONEYOS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (HONEYOS_LANGFUSE_PUBLIC_KEY,
# HONEYOS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # HoneyOS runtime location — never via dashboard env writer.
    # NOT a HONEYOS_* blanket: integration credentials (HONEYOS_GEMINI_*,
    # HONEYOS_LANGFUSE_*, HONEYOS_SPOTIFY_*, ...) ARE allowed.
    "HONEYOS_HOME", "HONEYOS_PROFILE", "HONEYOS_CONFIG", "HONEYOS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or HoneyOS runtime location "
            "(HONEYOS_HOME, HONEYOS_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.honeyos/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # HONEYOS_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `honeyos plugins enable
    # observability/langfuse` or `honeyos tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "HONEYOS_LANGFUSE_ENV",
    "HONEYOS_LANGFUSE_RELEASE",
    "HONEYOS_LANGFUSE_SAMPLE_RATE",
    "HONEYOS_LANGFUSE_MAX_CHARS",
    "HONEYOS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "HONEYOS_ACP_AUTH_METHOD",
    "HONEYOS_ACP_AUTO_APPROVE",
    "HONEYOS_COPILOT_ACP_COMMAND",
    "HONEYOS_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from honeyos.runtime.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set HONEYOS_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("HONEYOS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_honeyos_home() / ".managed"
...
def is_managed() -> bool:
    """Check if HoneyOS is running in package-manager-managed mode.

    Two signals: the HONEYOS_MANAGED env var (set by the systemd service),
    or a .managed marker file in HONEYOS_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update HoneyOS through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``honeyos.runtime/`` — i.e. the git checkout for source
    installs, ``/opt/honeyos`` inside the published image. It is a property of
    the running interpreter, NOT of ``$HONEYOS_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how HoneyOS was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$HONEYOS_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. HONEYOS_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.honeyos``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$HONEYOS_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.honeyos:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$HONEYOS_HOME``, a
    ...
    ``docker`` and ``honeyos update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/honeyos-agent`` image bakes a ``docker``
        stamp into ``/opt/honeyos`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $HONEYOS_HOME.
    try:
    ...
    #    host install whose shared $HONEYOS_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``honeyos update``.
    try:
        method = (
            (get_honeyos_home() / ".install_method")
    ...
    # detect Nix installs that don't set HONEYOS_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``honeyos.core.constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HONEYOS_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/honeyos`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/honeyos-agent:latest"
    return "honeyos update"
...
# Long-form text for ``honeyos update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# honeyos.runtime/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side HoneyOS, it doesn't update
...
✗ ``honeyos update`` doesn't apply inside the Docker container.

HoneyOS runs as a published image (nousresearch/honeyos-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/honeyos-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate honeyos-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/honeyos-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/honeyos-agent/tags
  • Your config and session history live under ``$HONEYOS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``honeyos update`` inside Docker.
...
def format_managed_message(action: str = "modify this HoneyOS installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("HONEYOS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this HoneyOS installation is managed by NixOS "
            f"(HONEYOS_MANAGED={env_hint}).\n"
            "Edit services.honeyos-agent.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this HoneyOS installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall HoneyOS."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HONEYOS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, honeyos_bin
    or None if container mode is not active, we're already inside the
    container, or HONEYOS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("HONEYOS_DEV") == "1":
        return None
    ...
    container_mode_file = get_honeyos_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "honeyos-agent")
    exec_user = info.get("exec_user", "honeyos")
    honeyos_bin = info.get("honeyos_bin", "/data/current-package/bin/honeyos")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "honeyos_bin": honeyos_bin,
    }
...
# Re-export from honeyos.core.constants — canonical definition lives there.
from honeyos.core.constants import get_honeyos_home, get_process_honeyos_home  # noqa: F811,E402
from honeyos.utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_honeyos_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_honeyos_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_honeyos_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the HONEYOS_UID / HONEYOS_GID env vars set by Docker deployments.

    Docker containers running HoneyOS commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level HONEYOS_HOME once, but
    subdirectories created at runtime by ``ensure_honeyos_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("HONEYOS_UID", "").strip()
    gid_str = os.environ.get("HONEYOS_GID", "").strip()
    try:
...
def _chown_to_honeyos_uid(path) -> None:
    """Chown ``path`` to ``HONEYOS_UID:HONEYOS_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_honeyos_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_honeyos_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the honeyos group can
    share state with the gateway service.

    The mode can be overridden via the HONEYOS_HOME_MODE environment variable
    (e.g. HONEYOS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HONEYOS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``HONEYOS_UID``/``HONEYOS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HONEYOS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_honeyos_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When HoneyOS runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("HONEYOS_CONTAINER") or os.environ.get("HONEYOS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set HONEYOS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HONEYOS_HOME, upgrading legacy empty templates.
...
# ensure_honeyos_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_HONEYOS_HOME_ENSURED: set = set()


def ensure_honeyos_home():
    """Ensure ~/.honeyos directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``HONEYOS_HOME`` we only re-run
    ...
    ``get_honeyos_home()`` and therefore re-run for the new path.
    """
    home = get_honeyos_home()
    key = str(home)

    if key in _HONEYOS_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``honeyos profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old HONEYOS_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_honeyos_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _HONEYOS_HOME_ENSURED.add(key)


def _ensure_honeyos_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HONEYOS_HOME {home} does not exist. "
...
from honeyos.runtime.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `honeyos config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.honeyos.config`` entries, then checks
    which ones are absent or empty under ``skills.config.<key>`` in the user's
    config.yaml.  Returns a list of dicts suitable for prompting.
    """
    try:
        from honeyos.agent.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    try:
        ...
        # should never break `honeyos update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but HoneyOS' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older HoneyOS versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``honeyos.runtime.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (honeyos.runtime/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (honeyos.runtime/setup.py)
    "platform_toolsets",     # written by the setup wizard (honeyos.runtime/setup.py)
    "known_plugin_toolsets", # written/read by honeyos.runtime/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'honeyos setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — HoneyOS won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, honeyos send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'honeyos doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_honeyos_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "honeyos update / honeyos -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from honeyos.runtime.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ honeyos config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in honeyos.runtime.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from honeyos.runtime.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        from honeyos.toolsets import validate_toolset
        from honeyos.runtime.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: honeyos config set <key> <value>")
    ...
    # `honeyos update`, but only the version bump is persisted.
    ...
    # metadata.honeyos.config in their SKILL.md frontmatter.
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = read_raw_config()
            try:
                from honeyos.agent.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.honeyos/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_honeyos_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``honeyos config set``
    ...
    backends) — while display paths (``honeyos status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _honeyos_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.honeyos/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_honeyos_home``, tui profile override, multi-profile probes).
    """
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    """
    from honeyos.utils import atomic_yaml_write
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.honeyos/config.yaml.
    ...
    (which change ``HONEYOS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/honeyos/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_honeyos_dotenv()
...
#   openai-codex (OAuth — honeyos auth) — OpenAI Codex
#   nous         (OAuth — honeyos auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.honeyos/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset HoneyOS writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.honeyos/.env.
    ...
    interactive menu render (`honeyos tools`, `honeyos setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `honeyos tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.honeyos/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.honeyos/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``HONEYOS_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"HONEYOS_CUSTOM_{slug}_API_KEY" if slug else "HONEYOS_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.honeyos/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.honeyos/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to HoneyOS — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known HoneyOS vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.honeyos/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.honeyos/.env`` over ``os.environ``.

    Used for HoneyOS-managed credentials where a deliberate edit to ``.env``
...
def redact_key(key: str) -> str:
    ...
    """
    from honeyos.agent.redact import mask_secret
...
def show_config():
    ...
    print(color("│              ⚕ HoneyOS Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from honeyos.runtime.auth import get_anthropic_key
    ...
    # Warn on stale HONEYOS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("HONEYOS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale HONEYOS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'honeyos doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    # Skill config
    try:
        from honeyos.agent.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
    ...
    print(color("  honeyos config edit     # Edit config file", Colors.DIM))
    print(color("  honeyos config set <key> <value>", Colors.DIM))
    print(color("  honeyos setup           # Run setup wizard", Colors.DIM))
    print()
...
def _load_cron_jobs_for_config_warning() -> List[Dict[str, Any]]:
    ...
    """
    try:
        from honeyos.scheduler.jobs import load_jobs
        return load_jobs()
    except Exception:
        return []
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `honeyos cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``honeyos config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``honeyos config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``honeyos config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (honeyos config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``honeyos config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``honeyos config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    honeyos config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    honeyos config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `honeyos config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_honeyos_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `honeyos config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `honeyos config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: honeyos config get <key> [--json]")
            print()
            print("Examples:")
            print("  honeyos config get model")
            print("  honeyos config get terminal.backend")
            print("  honeyos config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: honeyos config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  honeyos config set model anthropic/claude-sonnet-4")
            print("  honeyos config set terminal.backend docker")
            print("  honeyos config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: honeyos config unset <key>")
            print()
            print("Examples:")
            print("  honeyos config unset model")
            print("  honeyos config unset terminal.backend")
            print("  honeyos config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'honeyos config migrate' to add them")
        ...
        print("  honeyos config           Show current configuration")
        print("  honeyos config edit      Open config in editor")
        print("  honeyos config get <key>          Print a resolved config value")
        print("  honeyos config set <key> <value>   Set a config value")
        print("  honeyos config unset <key>        Remove a config value")
        print("  honeyos config check     Check for missing/outdated config")
        print("  honeyos config migrate   Update config with new options")
        print("  honeyos config path      Show config file path")
        print("  honeyos config env-path  Show .env file path")
        sys.exit(1)
...
def _inject_profile_env_vars() -> None:
    ...
    try:
        from honeyos.providers import list_providers
...
# ``_inject_profile_env_vars`` surfaces them in ``honeyos config`` UI so users
