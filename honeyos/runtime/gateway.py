"""
Gateway subcommand for honeyos CLI.

Handles: honeyos gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from honeyos.gateway.config import coerce_systemd_watchdog_seconds, load_gateway_config
from honeyos.gateway.status import terminate_pid
from honeyos.gateway.restart import (
...
# display_honeyos_home is imported lazily at call sites to avoid ImportError
# when honeyos.core.constants is cached from a pre-update version during `honeyos update`.
...
from honeyos.runtime.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "honeyos CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``honeyos gateway status`` from falsely counting the ``honeyos`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``honeyos gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_honeyos_home().resolve())
    # Forward slashes on both sides of the HONEYOS_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"honeyos_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. HONEYOS_HOME
        ...
        # HONEYOS_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "honeyos_home=" in command_lc
            and f"honeyos_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            from honeyos.runtime._subprocess_compat import windows_hide_flags
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``honeyos update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            HoneyOS profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to HoneyOS profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from honeyos.gateway.status import get_running_pid
        from honeyos.runtime.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "honeyos.runtime.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m honeyos.runtime.main gateway
    ...
    try:
        from honeyos.gateway.status import looks_like_gateway_command_line
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``honeyos update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting HoneyOS's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose HONEYOS_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``honeyos update`` users
    # with no running gateway until they re-invoke ``honeyos gateway``
    ...
    from honeyos.runtime._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (HONEYOS_HOME,
    ...
    if sys.platform == "win32":
        try:
            from honeyos.runtime.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / HONEYOS_HOME) the windowless
        # base interpreter needs to import honeyos.runtime.  Empty on POSIX, where
...
def _honeyos_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``HONEYOS_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("HONEYOS_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_honeyos_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HONEYOS_HOME``.

    Under ``sudo``, ``HONEYOS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_honeyos_home` falls back to ``/root/.honeyos`` — the wrong
    profile. The unit file pins ``HONEYOS_HOME`` for the actual gateway
    ...
    unit_home = (_honeyos_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("HONEYOS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("HONEYOS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["HONEYOS_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}honeyos gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}honeyos gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from honeyos.runtime.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: honeyos gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `honeyos gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``honeyos gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from honeyos.runtime.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (honeyos.runtime/gateway.py restart
    ...
    from honeyos.gateway.status import _pid_exists, write_planned_stop_marker
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (HONEYOS_HOME-scoped).
    ...
    """
    try:
        from honeyos.gateway.status import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from honeyos.gateway.status import write_planned_stop_marker
...
from honeyos.core.constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our HoneyOS Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``honeyos gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``HONEYOS_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("HONEYOS_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "honeyos-gateway"
SERVICE_DESCRIPTION = "HoneyOS Gateway - Messaging Platform Integration"


def _is_honeyos_runtime() -> bool:
    """Return whether the embedded gateway is running for HONEYOS."""
...
def _service_product_name() -> str:
    if _is_honeyos_runtime():
        return os.environ.get("HONEYOS_PRODUCT_NAME", "Honey OS").strip() or "Honey OS"
    return "HoneyOS"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HONEYOS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (HONEYOS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from honeyos.core.constants import get_default_honeyos_root

    home = get_honeyos_home().resolve()
    default = get_default_honeyos_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary HONEYOS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(honeyos_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when HONEYOS_HOME is a named profile.

    For ``~/.honeyos/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        honeyos_home: Optional explicit HONEYOS_HOME path. Defaults to the current
            ``get_honeyos_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional HoneyOS root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_honeyos_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from honeyos.core.constants import get_default_honeyos_root

    home = Path(honeyos_home or str(get_honeyos_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_honeyos_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(honeyos_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".honeyos"
    try:
        Path(honeyos_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(honeyos_home, default_root=target_root)
    except ValueError:
        return _profile_arg(honeyos_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this HONEYOS_HOME.

    Default ``~/.honeyos`` returns ``honeyos-gateway`` (backward compatible).
    Profile ``~/.honeyos/profiles/coder`` returns ``honeyos-gateway-coder``.
    Any other HONEYOS_HOME appends a short hash for uniqueness.
    """
    if _is_honeyos_runtime():
        return "honeyos-gateway"
...
# Legacy service names from older HoneyOS installs that predate the
# honeyos-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (honeyos-gateway-*.service) and unrelated third-party
# "honeyos" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("honeyos.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "honeyos.runtime.main gateway",
    "honeyos.runtime/main.py gateway",
    "gateway/run.py",
    " honeyos gateway ",
    "/honeyos gateway ",
)
...
def _find_legacy_honeyos_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy HoneyOS gateway units.

    Detects unit files installed by older HoneyOS versions that used a
    different service name (e.g. ``honeyos.service`` before the rename to
    ``honeyos-gateway.service``). When both a legacy unit and the current
    ``honeyos-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``honeyos-gateway-coder.service`` and unrelated third-party
      ``honeyos-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``honeyos.service`` running an unrelated
...
def has_legacy_honeyos_units() -> bool:
    """Return True when any legacy HoneyOS gateway unit files exist."""
    return bool(_find_legacy_honeyos_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy HoneyOS gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_honeyos_units()
    if not legacy:
        return
    print_warning("Legacy HoneyOS gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current honeyos-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    honeyos gateway migrate-legacy")


def remove_legacy_honeyos_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy HoneyOS gateway unit files.

    Iterates over whatever ``_find_legacy_honeyos_units()`` returns — which is
    ...
    legacy = _find_legacy_honeyos_units()
    if not legacy:
        print("No legacy HoneyOS gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy HoneyOS gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: honeyos gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo honeyos gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    honeyos gateway uninstall")
    print_info("    sudo honeyos gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: honeyos gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode HoneyOS often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.honeyos`` → ``ai.honeyos.gateway.plist`` (backward compatible).
    Profile ``~/.honeyos/profiles/coder`` → ``ai.honeyos.gateway-coder.plist``.
    """
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # honeyos.core.constants cached from before this symbol existed. See
            # _reload_honeyos_constants() in honeyos.runtime/managed_uv.py.
            from honeyos.runtime.managed_uv import _reload_honeyos_constants

            venv_python_path = _reload_honeyos_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.honeyos/honeyos-agent  -> /home/alice/.honeyos/honeyos-agent
      /opt/honeyos                 -> /opt/honeyos  (kept as-is)
...
def _honeyos_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HONEYOS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_honeyos_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.honeyos                    → /home/alice/.honeyos
      /root/.honeyos/profiles/coder     → /home/alice/.honeyos/profiles/coder
      /opt/custom-honeyos               → /opt/custom-honeyos  (kept as-is)
    """
    current_honeyos_raw = os.environ.get("HONEYOS_HOME", "").strip()
    current_honeyos = (
        Path(current_honeyos_raw).expanduser()
        if current_honeyos_raw
        else get_honeyos_home()
    )
    ...
    # different HONEYOS_HOME into the generated service unit.
    current_default = Path.home() / ".honeyos"
    target_default = Path(target_home_dir) / ".honeyos"

    # Default ~/.honeyos → remap to target user's default
    if current_honeyos == current_default:
        return str(target_default)

    # Profile or subdir of ~/.honeyos → preserve the relative structure
    try:
        relative = current_honeyos.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.honeyos) — keep as-is
        return str(current_honeyos)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    honeyos_node = honeyos_home / "node" / "bin"
    if _is_dir(honeyos_node):
        candidates.append(str(honeyos_node))
    honeyos_nm = honeyos_home / "node_modules" / ".bin"
    if _is_dir(honeyos_nm):
        candidates.append(str(honeyos_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m honeyos.runtime.main``, so module
    ...
    dir, or a clone that ``honeyos update`` later relocates/removes — the path
    ...
    ``HONEYOS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if HONEYOS_HOME
...
def _systemd_watchdog_seconds(honeyos_home: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = None
    reset_home_override = None
    if honeyos_home is not None:
        from honeyos.core.constants import (
            reset_honeyos_home_override,
            set_honeyos_home_override,
        )

        override_token = set_honeyos_home_override(honeyos_home)
        reset_home_override = reset_honeyos_home_override
    try:
...
def _systemd_watchdog_service_fields(
    honeyos_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(honeyos_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], honeyos_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The HoneyOS-managed Node under ``$HONEYOS_HOME/node`` goes first when it
    ...
    *honeyos_root* is the HoneyOS home the unit will run against. System units
    ...
    """
    from honeyos.core.constants import iter_honeyos_node_dirs

    for directory in iter_honeyos_node_dirs(honeyos_root):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's HoneyOS home is known — probing here would stat the calling
    ...
    if system:
        ...
        honeyos_home = _honeyos_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            honeyos_home
        )
        profile_arg = _profile_arg_for_target_user(honeyos_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's HONEYOS_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(honeyos_home) if honeyos_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped honeyos_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(honeyos_home) if honeyos_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m honeyos.runtime.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="HONEYOS_HOME={honeyos_home}"
{f'Environment="HONEYOS_HOME={honeyos_home}"' if _is_honeyos_runtime() else ''}
{f'Environment="HONEYOS_RUNTIME_ID={os.environ.get("HONEYOS_RUNTIME_ID", "honeyos-companion-v0.1")}"' if _is_honeyos_runtime() else ''}
{f'Environment="HONEYOS_PRODUCT_NAME={_service_product_name()}"' if _is_honeyos_runtime() else ''}
Restart=always
RestartSec=5
...
"""
    ...
    honeyos_home = str(get_honeyos_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        honeyos_home
    )
    profile_arg = _profile_arg(honeyos_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m honeyos.runtime.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="HONEYOS_HOME={honeyos_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned HONEYOS_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo honeyos gateway … --system``, HONEYOS_HOME is often stripped
    # and falls back to ``/root/.honeyos``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / HONEYOS_HOME comparisons
    ...
    _sync_honeyos_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir HONEYOS_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved HONEYOS_HOME
    ...
    exported a throwaway ``HONEYOS_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``honeyos gateway restart``
    with ``HONEYOS_HOME=/tmp/honeyos-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="HONEYOS_HOME=..."`` lines and launchd
    ``<key>HONEYOS_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'HONEYOS_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>HONEYOS_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp HONEYOS_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: HONEYOS_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported HONEYOS_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single HONEYOS_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only HONEYOS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp HONEYOS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../honeyos_test`` path into
    # ``Environment="HONEYOS_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/honeyos_test"' in new_unit
        or "/honeyos_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # HONEYOS_HOME into the unit (manual E2E homes like /tmp/honeyos-e2e-NNN
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo honeyos gateway uninstall --system")
    print_info("         honeyos gateway install")
    print_info("         honeyos gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("HONEYOS_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("HONEYOS_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (honeyos.service from pre-rename installs)
    # before installing the new honeyos-gateway.service. If both remain, they
    ...
    if has_legacy_honeyos_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_honeyos_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin HONEYOS_HOME; adopt it before any
    ...
    # ``sudo honeyos gateway install --system --force`` would bake /root/.honeyos
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}honeyos gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}honeyos gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}honeyos gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # HONEYOS_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    _sync_honeyos_home_from_systemd_unit(system=system)
    try:
        from honeyos.gateway.status import get_running_pid, write_planned_stop_marker
    ...
    try:
        ...
        print(
            ...
            "check `honeyos gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `honeyos gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}honeyos gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}honeyos gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}honeyos gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}honeyos gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    if _is_honeyos_runtime():
        return "ai.honeyos.honeyos"
    suffix = _profile_suffix()
    return f"ai.honeyos.gateway-{suffix}" if suffix else "ai.honeyos.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``honeyos gateway start/stop/status`` call).
...
#      background process (the `nohup honeyos gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_honeyos_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_honeyos_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m honeyos.runtime.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active HONEYOS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "honeyos.runtime.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup honeyos gateway run --replace`
    ...
    """
    from honeyos.runtime._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_honeyos_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from honeyos.core.constants import display_honeyos_home as _dhh
    ...
    print(
        f"  Try manually: nohup honeyos gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>honeyos.runtime.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>HONEYOS_HOME</key>
        <string>{honeyos_home}</string>
        {f'''<key>HONEYOS_HOME</key>
        <string>{honeyos_home}</string>
        <key>HONEYOS_RUNTIME_ID</key>
        <string>{os.environ.get("HONEYOS_RUNTIME_ID", "honeyos-companion-v0.1")}</string>
        <key>HONEYOS_PRODUCT_NAME</key>
        <string>{_service_product_name()}</string>''' if _is_honeyos_runtime() else ''}
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.honeyos/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See honeyos-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_honeyos_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current "
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    status_command = "honeyos status" if _is_honeyos_runtime() else "honeyos gateway status"
    print(f"  {status_command:<33} # Check status")
    from honeyos.core.constants import display_honeyos_home as _dhh
...
def launchd_stop():
    ...
    # `honeyos gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # HoneyOS PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the HoneyOS PID file).
    ...
    if launchd_plist_is_current():
        ...
        command = "honey-os start" if _is_honeyos_runtime() else "honeyos gateway start"
        print(f"  Run: {command}")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                stop_command = (
                    "honey-os stop" if _is_honeyos_runtime() else "honeyos gateway stop"
                )
    ...
    if deep:
        log_file = get_honeyos_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``HONEYOS_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``honeyos gateway run`` is always a misconfiguration, so we
    ...
    try:
        from honeyos.core.constants import get_default_honeyos_root
        default_root = get_default_honeyos_root()
        # (b) Is the default-profile gateway running?
        from honeyos.gateway.status import get_running_pid as _default_running_pid  # noqa
    except Exception:
        return

    try:
        import yaml as _yaml
        from honeyos.gateway.status import _read_pid_record  # type: ignore
        ...
        if not rec:
            return
        from honeyos.gateway.status import _pid_exists, _pid_from_record
        ...
        # to config.yaml.
        from honeyos.gateway.config import _env_multiplex_profiles_override
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``honeyos gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``honeyos gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other HONEYOS_HOME roots.
    """
    ...
    print("  Use 'honeyos gateway restart' to replace it,")
    print("  or 'honeyos gateway stop' first.")
    print("  Or use 'honeyos gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("HONEYOS_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the HoneyOS gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'honeyos' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/honeyos/docker/entrypoint.sh before the HoneyOS command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$HONEYOS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set HONEYOS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `honeyos gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set HONEYOS_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `honeyos gateway restart` which already
    ...
    # `honeyos gateway start/restart` — leaving the gateway vulnerable to
    ...
    from honeyos.gateway.run import start_gateway

    print("┌─────────────────────────────────────────────────────────┐")
    product_name = _service_product_name()
    title = f"⚕ {product_name} Gateway Starting..."
    print(f"│{title:^57}│")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # HONEYOS_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HONEYOS_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``HONEYOS_GATEWAY_MAX_STARTS`` /
    # ``HONEYOS_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    # Set max_starts <= 0 to disable. Best-effort: a bookkeeping failure must
    # never block startup.
    try:
        import time as _time

        from honeyos.gateway.status import record_start_and_check_storm
        ...
        try:
            _env_starts = os.getenv("HONEYOS_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("HONEYOS_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``honeyos gateway run`` enters through this CLI wrapper, not through
        ...
        from honeyos.gateway.run import _exit_after_graceful_shutdown
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. honeyos) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where HoneyOS delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. HoneyOS connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: honeyos pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and HoneyOS will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``honeyos setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run honeyos under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.honeyos/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_honeyos_home() / "whatsapp" / "session" / "creds.json"
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        from honeyos.gateway.status import parse_active_agents
...
def _setup_standard_platform(platform: dict):
    ...
    from honeyos.runtime.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from honeyos.runtime.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'honeyos pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: honeyos pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'honeyos gateway setup'"
                    )
            continue
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from honeyos.runtime import gateway_windows
...
def _setup_weixin():
    ...
    honeyos_mode = bool(os.environ.get("HONEYOS_RUNTIME_ID"))
    product_name = _service_product_name()
    data_home = str(get_honeyos_home()) if honeyos_mode else "~/.honeyos"
    retry_command = "honey-os channel setup weixin" if honeyos_mode else "honeyos gateway setup"
    pairing_command = "honey-os pairing approve" if honeyos_mode else "honeyos pairing approve"
    print()
    print(color("  ─── 💬 Weixin / WeChat Setup ───", Colors.CYAN))
    print()
    print_info(f"  1. {product_name} will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        f"  3. {product_name} will store the returned account_id/token in {data_home}/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info(f"  Install them, then rerun `{retry_command}`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_honeyos_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            f"  Unknown DM users can request access and you approve them with `{pairing_command}`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from honeyos.gateway.platforms.qqbot import qr_register
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `honeyos pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "HoneyOSAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``honeyos.runtime.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from honeyos.runtime import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.honeyos/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.honeyos/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_honeyos_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: honeyos gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: honeyos gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: honeyos gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo honeyos gateway install --system"
                        )
                    print_info("  Or run in foreground:  honeyos gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: honeyos gateway run")
                print_info(
                    "  For persistence:   tmux new -s honeyos 'honeyos gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: honeyos gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup honeyos gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'honeyos gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`honeyos.runtime.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from honeyos.runtime.service_manager import (
    ...
    if profile is None:
        ...
        # HONEYOS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``honeyos gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``honeyos gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``honeyos gateway run`` are unaffected.
      2. ``HONEYOS_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``honeyos gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``HONEYOS_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("HONEYOS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("HONEYOS_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${HONEYOS_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if HONEYOS_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or HONEYOS_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-honeyos service's
    # pattern in docker/s6-rc.d/main-honeyos/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: honeyos gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: honeyos gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s honeyos 'honeyos gateway run'"
                )
                print()
            ...
            print()
            print(
                "  honeyos gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s honeyos 'honeyos gateway run'         # persistent via tmux"
            )
            print(
                "  nohup honeyos gateway run > ~/.honeyos/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from honeyos.runtime.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  honeyos profile create <name>     # creates the s6 service slot")
                print("  honeyos -p <name> gateway start   # bring it up via s6")
                print("  honeyos status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: honeyos gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: honeyos gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: honeyos gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  honeyos profile delete <name>     # tears down the s6 service slot")
                print("  honeyos -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `honeyos -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: honeyos gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HONEYOS_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `honeyos gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HONEYOS_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `honeyos gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: honeyos gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  honeyos gateway install")
                else:
                    print("To install as a service:")
                    print("  honeyos gateway install")
                    print("  sudo honeyos gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  honeyos gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup honeyos gateway run > ~/.honeyos/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s honeyos 'honeyos gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  honeyos gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  honeyos gateway install  # Install as user service")
                    print(
                        "  sudo honeyos gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy HoneyOS gateway unit files from
        # pre-rename installs (e.g. honeyos.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_honeyos_units(interactive=not yes, dry_run=dry_run)
