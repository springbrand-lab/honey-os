"""
Single source of truth for provider identity in HoneyOS.
...
2. **HoneyOS overlays** — transport type, auth patterns, aggregator flags,
...
# -- HoneyOS overlay ----------------------------------------------------------
# HoneyOS-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HoneyOSOverlay:
    """HoneyOS-specific provider metadata layered on top of models.dev."""
...
HONEYOS_OVERLAYS: Dict[str, HoneyOSOverlay] = {
    "moa": HoneyOSOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": HoneyOSOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": HoneyOSOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": HoneyOSOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": HoneyOSOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": HoneyOSOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HONEYOS_QWEN_BASE_URL",
    ),
    "lmstudio": HoneyOSOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": HoneyOSOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": HoneyOSOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": HoneyOSOverlay(
    ...
    "kimi-for-coding": HoneyOSOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": HoneyOSOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": HoneyOSOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": HoneyOSOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": HoneyOSOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": HoneyOSOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": HoneyOSOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": HoneyOSOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": HoneyOSOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": HoneyOSOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": HoneyOSOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": HoneyOSOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": HoneyOSOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": HoneyOSOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": HoneyOSOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": HoneyOSOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": HoneyOSOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": HoneyOSOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": HoneyOSOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "honeyos", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. HoneyOS overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + HoneyOS overlay
    ...
    try:
        from honeyos.agent.models_dev import get_provider_info as _mdev_provider
    ...
    overlay = HONEYOS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + honeyos extra
    ...
    if overlay is not None:
        # HoneyOS-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The HoneyOS overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in HONEYOS_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact HoneyOS provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from honeyos.runtime.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
