...
A system-level directory (default ``/etc/honeyos``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.honeyos/config.yaml`` and ``~/.honeyos/.env`` on a per-leaf-key basis.

This is DISTINCT from ``honeyos.runtime.config.is_managed()`` / ``HONEYOS_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/honeyos")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/honeyos`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``HONEYOS_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$HONEYOS_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/honeyos`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("HONEYOS_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through honeyos.runtime.config.load_config) to honor managed
    scope. Mirrors honeyos.runtime.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from honeyos.runtime.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
