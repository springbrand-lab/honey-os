"""`honeyos checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.honeyos/checkpoints/``.  Actions:

    honeyos checkpoints               # same as `status`
    honeyos checkpoints status        # total size, project count, breakdown
    honeyos checkpoints list          # per-project checkpoint counts + workdir
    honeyos checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    honeyos checkpoints clear [-f]    # nuke the entire base (asks first)
    honeyos checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    honeyos checkpoints
    honeyos checkpoints prune --retention-days 3 --max-size-mb 200
    honeyos checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    from honeyos.tools.checkpoint_manager import store_status
    ...
    if legacy:
        ...
        print()
        print("Clear with: honeyos checkpoints clear-legacy")
    return 0
...
def cmd_prune(args: argparse.Namespace) -> int:
    from honeyos.tools.checkpoint_manager import prune_checkpoints, store_status
...
def cmd_clear(args: argparse.Namespace) -> int:
    from honeyos.tools.checkpoint_manager import CHECKPOINT_BASE, clear_all, store_status
...
def cmd_clear_legacy(args: argparse.Namespace) -> int:
    from honeyos.tools.checkpoint_manager import clear_legacy, store_status
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``honeyos checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `honeyos checkpoints` → status
