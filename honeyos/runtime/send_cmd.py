"""CLI subcommand: ``honeyos send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"honeyos send: {file_path} is not a text file. --file reads the "
                ...
                f'  honeyos send --to telegram "MEDIA:{file_path}"\n'
                f'  honeyos send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  honeyos send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"honeyos send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"honeyos send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    """
    try:
        from honeyos.gateway.channel_directory import (
            format_directory_for_display,
            load_directory,
        )
    except Exception as exc:
        print(f"honeyos send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"honeyos send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `honeyos send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"honeyos send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `honeyos gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.honeyos/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_honeyos_env() -> None:
    """Populate ``os.environ`` from ``~/.honeyos/.env`` AND bridge top-level
    ...
    startup that ``honeyos send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.honeyos/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.honeyos/config.yaml`` into
       ...
       via ``honeyos config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``honeyos send``
    ...
    try:
        from honeyos.runtime.config import _expand_env_vars
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.honeyos/.env and ~/.honeyos/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "honeyos send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  honeyos send --to telegram \"hello\"\n"
            "  honeyos send --to discord:#ops --file report.md\n"
            "  honeyos send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "honeyos send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `honeyos send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform HoneyOS "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.honeyos/.env + ~/.honeyos/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  honeyos send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | honeyos send --to telegram:-1001234567890\n"
            "  honeyos send --to discord:#ops --file /tmp/report.md\n"
            "  honeyos send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  honeyos send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  honeyos send --list                  # all platforms\n"
            "  honeyos send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `honeyos send --list telegram`.",
    )
