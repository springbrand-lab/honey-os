"""
HoneyOS Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``honeyos desktop`` / ``honeyos gui``)
     Built inside the agent checkout under ``$HONEYOS_HOME/honeyos-agent/``:
       ...
       - ``$HONEYOS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/HoneyOS.app`` or ``~/Applications/HoneyOS.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\HoneyOS`` (NSIS per-user)
...
the app name ("HoneyOS"), separate from ``$HONEYOS_HOME``:
  - macOS:   ``~/Library/Application Support/HoneyOS``
  - Windows: ``%APPDATA%\\HoneyOS``
  - Linux:   ``$XDG_CONFIG_HOME/HoneyOS`` (default ``~/.config/HoneyOS``)
...
``honeyos uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(honeyos_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return honeyos_home / "honeyos-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "HoneyOS"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "HoneyOS"
...
def source_built_gui_artifacts(honeyos_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``honeyos desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``honeyos-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(honeyos_home)
    ...
    return [
        ...
        honeyos_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "HoneyOS" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/HoneyOS.app"),
            home / "Applications" / "HoneyOS.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\HoneyOS).
            local_base / "Programs" / "HoneyOS",
            # Older / alternate layout some builds used.
            local_base / "honeyos-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "HoneyOS")
    else:
        ...
        # ``uninstall_gui``.
        from honeyos.runtime.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `honeyos desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "HoneyOS.desktop",
        ]
    return paths


def agent_is_installed(honeyos_home: Path) -> bool:
    """Return True when a usable Python agent install exists under HONEYOS_HOME.
    ...
    if (agent_root / "honeyos.runtime").is_dir():
        return True
...
def gui_is_installed(honeyos_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(honeyos_home):
        if p.exists():
            return True
...
def gui_install_summary(honeyos_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = honeyos_home if honeyos_home is not None else get_honeyos_home()
...
def uninstall_gui(honeyos_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``honeyos-agent/honeyos.runtime`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$HONEYOS_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so HoneyOS disappears from the launcher.
        try:
            from honeyos.runtime.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove honeyos' or "
            "'sudo dnf remove honeyos'). AppImage builds are a single file you "
