...
from honeyos.runtime.config import get_env_value, load_config
...
# coverage category (honeyos.runtime.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `honeyos tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    from honeyos.core.constants import agent_browser_runnable, with_honeyos_node_path
    ...
    # HoneyOS-managed Node dirs (Windows installer / POSIX $HONEYOS_HOME/node)
    ...
    managed_path = with_honeyos_node_path().get("PATH", "")
    if managed_path:
...
def _local_browser_runnable() -> bool:
    ...
    if not _has_agent_browser():
        return False
    try:
        from honeyos.tools.browser_tool import _chromium_installed, _using_lightpanda_engine
    except Exception:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("HONEYOS_LOCAL_STT_COMMAND"):
        return True
    try:
        from honeyos.tools.transcription_tools import _HAS_FASTER_WHISPER
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `honeyos model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("HONEYOS_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("HONEYOS_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from honeyos.runtime.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`honeyos tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``honeyos tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``honeyos model``'s Nous
    login, this:
