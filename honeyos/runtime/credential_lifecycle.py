"""Unified provider-credential lifecycle across every store HoneyOS reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.honeyos/.env``                     — the canonical secret store
    2. ``~/.honeyos/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.honeyos/config.yaml``              — inline mirrors written by the
...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    if not old_value:
        return []
    from honeyos.utils import atomic_yaml_write, fast_safe_load
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `honeyos auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``honeyos auth add``.
    """
    from honeyos.runtime.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from honeyos.runtime.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from honeyos.runtime.config import load_env, remove_env_value
