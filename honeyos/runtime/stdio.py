...
on Japanese locales, etc.).  HoneyOS's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``honeyos.runtime/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. HoneyOS is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``HONEYOS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("HONEYOS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the HoneyOS-managed Git install directories so
    ...
    # already-running shells don't see the change — which means honeyos
    ...
    # install without HoneyOS-managed PortableGit).
    ...
    # stdin is re-configured for completeness; HoneyOS's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and HoneyOS's
    ``honeyos config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching HoneyOS (User env var in Windows Settings, or
    export in a PowerShell profile) and HoneyOS picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known HoneyOS-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\honeyos\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``honeyos`` from right after install) retain their old PATH.

    Any subprocess HoneyOS spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known HoneyOS-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "honeyos", "git", "cmd"),
        os.path.join(local_appdata, "honeyos", "git", "bin"),
        os.path.join(local_appdata, "honeyos", "git", "usr", "bin"),
        # HoneyOS venv Scripts directory — host of the honeyos.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes honeyos, but harmless to include.
        os.path.join(local_appdata, "honeyos", "honeyos-agent", "venv", "Scripts"),
