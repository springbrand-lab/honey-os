...
the existing module-level functions in honeyos.runtime.gateway and
honeyos.runtime.gateway_windows directly. This protocol is a thin facade
...
``honeyos gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``honeyos gateway start/stop/restart``).
    """
    ...
    # `honeyos gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged honeyos
    ...
    detection always fails. Since every HoneyOS runtime call inside the
    container drops to honeyos via ``s6-setuidgid``, that silent failure
...
# in ``honeyos.runtime.gateway`` (systemd/launchd) and ``honeyos.runtime.gateway_windows``
...
# active (set via the ``honeyos -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in honeyos.runtime.gateway.
    ...
    def start(self, name: str) -> None:
        from honeyos.runtime.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from honeyos.runtime.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from honeyos.runtime.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from honeyos.runtime.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in honeyos.runtime.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from honeyos.runtime.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from honeyos.runtime.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from honeyos.runtime.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from honeyos.runtime.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``honeyos.runtime.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `honeyos profile create`
# runs inside the container (Phase 4). Static services (main-honeyos, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's HONEYOS_HOME blindly;
    derive the shared profile root from the current HONEYOS_HOME and map the
    ...
    honeyos_home = Path(os.environ.get("HONEYOS_HOME", "/opt/data"))
    if honeyos_home.parent.name == "profiles":
        root = honeyos_home.parent.parent
    else:
        root = honeyos_home
...
# UID/GID of the in-image ``honeyos`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the honeyos user.
    ...
    (the ``honeyos`` user — UID 10000 — running every HoneyOS runtime
    ...
    is dispatched as the honeyos user.
    ...
    with honeyos ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           honeyos:honeyos, 0755 (parent must already exist)
    ``svc_dir/event/``                     honeyos:honeyos, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 honeyos:honeyos, 0755
    ``svc_dir/supervise/event/``           honeyos:honeyos, 03730
    ``svc_dir/supervise/control``          honeyos:honeyos, 0660    (FIFO)
    ...
    slot's supervise/ was honeyos-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _HONEYOS_UID, _HONEYOS_GID)
        except PermissionError:
            # Running as the honeyos user already — directory is honeyos-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _HONEYOS_UID, _HONEYOS_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is honeyos-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _HONEYOS_UID, _HONEYOS_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``honeyos -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`honeyos profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-honeyos, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HONEYOS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HONEYOS_HOME=/data/honeyos`` is honored at run
          ...
          4. Drops to the honeyos user and exec's
             ``honeyos -p <profile> gateway run`` (or just ``honeyos
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``honeyos gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root HONEYOS_HOME profile" (the implicit profile that exists at
        the top of $HONEYOS_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``honeyos.runtime.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$HONEYOS_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``HONEYOS_HOME``,
        ...
        allocator in ``honeyos.runtime.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export HONEYOS_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's HONEYOS_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``honeyos gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-HONEYOS_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The HONEYOS_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own HONEYOS_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "honeyos gateway run --replace"
        else:
            gateway_cmd = f"honeyos -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid honeyos {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_finish_script() -> str:
        ...
        normally.  See #51228.
        """
        from honeyos.gateway.restart import GATEWAY_FATAL_CONFIG_EXIT_CODE
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${HONEYOS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the HONEYOS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e HONEYOS_HOME=/data/honeyos``
        gets its logs under /data/honeyos/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``honeyos logs`` reads and what
        ...
        return (
            ...
            f': "${{HONEYOS_HOME:=/opt/data}}"\n'
            f'log_dir="$HONEYOS_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as honeyos when
            # this script starts as root. Never chown or unlink honeyos-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is honeyos-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded honeyos-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid honeyos mkdir -p "$log_dir"\n'
            f'  s6-setuidgid honeyos rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid honeyos s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``honeyos gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``honeyos -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with honeyos ownership
            ...
            # the honeyos user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is honeyos-owned (see ``_seed_supervise_skeleton``), and on
