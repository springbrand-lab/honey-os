...
Profile-aware location: ``$HONEYOS_HOME/logs/dashboard-auth.log``.
...
from ``honeyos.core.constants`` or other honeyos.runtime modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$HONEYOS_HOME/logs/dashboard-auth.log``.

    Uses ``honeyos.core.constants.get_honeyos_home()`` (a leaf module — no import
    ...
    return get_honeyos_home() / "logs" / "dashboard-auth.log"
