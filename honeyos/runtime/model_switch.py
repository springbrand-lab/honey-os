...
- ``honeyos.runtime.providers``        -- canonical provider identity + overlays
- ``honeyos.runtime.model_normalize``  -- per-provider name formatting
...
from honeyos.runtime.model_normalize import (
    normalize_model_for_provider,
)
from honeyos.agent.models_dev import (
    ModelCapabilities,
    ModelInfo,
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``honeyos model`` wizard — not a
    ...
    while ``honeyos model`` still live-probed the full ``/v1/models`` list.
...
_HONEYOS_MODEL_WARNING = (
    "Nous Research HoneyOS 3 & 4 models are NOT agentic and are not designed "
    "for use with HoneyOS. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research HoneyOS 3 / HoneyOS 4 chat families.
# The previous substring check (`"honeyos" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``honeyos-brain:qwen3-14b-ctx16k`` that just
# happen to carry "honeyos" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/HoneyOS-3-Llama-3.1-70B, honeyos-4-405b, openrouter/honeyos3:70b
# Negative examples it must NOT match:
#   honeyos-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HONEYOS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])honeyos[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_honeyos_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous HoneyOS 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_HONEYOS_NON_AGENTIC_RE.search(model_name))


def _check_honeyos_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous HoneyOS 3/4 chat model."""
    if is_nous_honeyos_non_agentic(model_name):
        return _HONEYOS_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``honeyos config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from honeyos.runtime.model_switch import
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from honeyos.runtime.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``honeyos.runtime.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'honeyos model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'honeyos doctor' — config issues detected:"
        ...
        from honeyos.runtime.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from honeyos.runtime.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from honeyos.runtime.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from honeyos.runtime.config import load_config
            from honeyos.runtime.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from honeyos.runtime.providers import resolve_user_provider as _ruser
    ...
    # honeyos.runtime.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from honeyos.runtime.models import normalize_opencode_base_url
    ...
    honeyos_warn = _check_honeyos_model_warning(new_model)
    if honeyos_warn:
        warnings.append(honeyos_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from honeyos.runtime.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from honeyos.runtime.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from honeyos.runtime/models.py (OPENROUTER_MODELS,
    ...
    # Compared against honeyos_id / mdev_id (section 1), pid / honeyos_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from honeyos.runtime.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by honeyos provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://honeyos-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a HoneyOS release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from honeyos.runtime.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from honeyos.runtime.models import fetch_lmstudio_models
    ...
    # --- 1. Check HoneyOS-mapped providers ---
    from honeyos.runtime.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from honeyos.runtime.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for honeyos_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(honeyos_id)
        if (
            _alias_target
            and _alias_target != honeyos_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip honeyos_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose honeyos_id matches the canonical profile name so
        # distinct profiles (e.g. kimi-coding, kimi-coding-cn) each get
        # their own picker row.
        _canonical = honeyos_id
        try:
            from honeyos.providers import get_provider_profile as _gpp
            _prof = _gpp(honeyos_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != honeyos_id:
            continue
        ...
        # same honeyos_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if honeyos_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(honeyos_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HONEYOS_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers HoneyOS may not route yet.
        ...
        from honeyos.runtime.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(honeyos_id):
            continue
        ...
        if not has_creds:
            try:
                from honeyos.runtime.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(honeyos_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        honeyos_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `honeyos model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(honeyos_id)
        if not model_ids:
            model_ids = curated.get(honeyos_id, [])
            if honeyos_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(honeyos_id, model_ids)
        ...
        results.append({
            ...
            "is_current": (
                ...
                or honeyos_id == current_provider
    ...
    # --- 2. Check HoneyOS-only providers (nous, openai-codex, copilot, opencode-go) ---
    from honeyos.runtime.providers import HONEYOS_OVERLAYS
    from honeyos.runtime.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → HoneyOS provider ID.
    # HONEYOS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use HoneyOS IDs ("copilot").
    _mdev_to_honeyos = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HONEYOS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve HoneyOS slug — e.g. "github-copilot" → "copilot"
        honeyos_slug = _mdev_to_honeyos.get(pid, pid)
        if honeyos_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or honeyos_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(honeyos_slug)
            ...
            # fully configured.
            try:
                from honeyos.agent.vertex_adapter import has_vertex_credentials
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or honeyos_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(honeyos_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from honeyos.agent.credential_pool import load_pool
                        _pool = load_pool(honeyos_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", honeyos_slug, exc)
        ...
        # configured.
        if not has_creds and honeyos_slug == "anthropic":
            try:
                ...
                if (honeyos_creds and honeyos_creds.get("accessToken")) or \
        ...
        if honeyos_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(honeyos_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(honeyos_slug)
                model_ids = _ids if _ids else (curated.get(honeyos_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(honeyos_slug, []) or curated.get(pid, [])
        elif honeyos_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `honeyos model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from honeyos.runtime.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(honeyos_slug, []) or curated.get(pid, [])
                if honeyos_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(honeyos_slug, model_ids)
        total = len(model_ids)
        if honeyos_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": honeyos_slug,
            "name": get_label(honeyos_slug),
            "is_current": honeyos_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "honeyos",
        ...
        seen_slugs.add(honeyos_slug.lower())
        _record_builtin_endpoint(honeyos_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HONEYOS_OVERLAYS (keeps /model in sync
    # with `honeyos model`).
    try:
        from honeyos.runtime.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # HoneyOS writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # HoneyOS's own writer uses (mirrors section-4 grouping).
            ...
            # ``honeyos model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``honeyos model``. Pin with
            ...
            if should_probe:
                try:
                    from honeyos.runtime.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. HoneyOS's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, honeyos.runtime/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``honeyos model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from honeyos.runtime.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`honeyos.runtime.models.fetch_openrouter_models`, which filters the
    ...
    """
    from honeyos.runtime.models import fetch_openrouter_models
