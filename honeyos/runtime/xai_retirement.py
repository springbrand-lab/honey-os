...
Pure logic: walks a HoneyOS config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `honeyos doctor` and a future `honeyos migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a HoneyOS config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a HoneyOS config and return retirement issues.
...
def apply_migration(
    ...
    from honeyos.runtime.config import require_readable_config_before_write
    ...
    # None``, so the next `honeyos migrate xai` reports nothing to migrate
