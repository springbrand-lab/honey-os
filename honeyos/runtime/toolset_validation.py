...
the tool registry or launching HoneyOS (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``honeyos-cli`` to the non-existent ``honeyos``. ``resolve_toolset('honeyos')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``honeyos-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``honeyos`` instead of ``honeyos-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `honeyos tools` to reconfigure."
        )
    return warnings
