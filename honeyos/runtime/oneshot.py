...
configured for "cli" in `honeyos tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HONEYOS_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `honeyos chat`:
...
from honeyos.gateway.session_context import declare_stateless_channel
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"honeyos -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "honeyos -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from honeyos.runtime.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"honeyos -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "honeyos -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "honeyos -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to HONEYOS_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("HONEYOS_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "honeyos -z: --provider requires --model (or HONEYOS_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"honeyos -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("honeyos -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``honeyos -z`` / oneshot mode.

    Oneshot bypasses ``HoneyOSCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when honeyos is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from honeyos.runtime.config import load_config
    from honeyos.runtime.models import detect_provider_for_model
    ...
    env_model = os.getenv("HONEYOS_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from honeyos.runtime import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("HONEYOS_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # HoneyOSCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from honeyos.runtime.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                HONEYOS_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via HONEYOS_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via HONEYOS_YOLO_MODE=1
