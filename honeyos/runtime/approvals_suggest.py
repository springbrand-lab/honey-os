"""``honeyos approvals suggest`` — mine approval history into allowlist proposals.

HoneyOS has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.honeyos/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, honeyos self-management, …) are eligible.
...
def scan_approval_history(
    ...
    """
    from honeyos.tools.approval import detect_dangerous_command, detect_hardline_command
...
def normalize_command(command: str) -> str:
    """Fold user/honeyos home prefixes and collapse whitespace.
    ...
    from honeyos.tools.approval import (
        _rewrite_resolved_honeyos_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_honeyos_home(command))
...
def derive_glob(normalized: str) -> Optional[str]:
    ...
    unsafe root binary.
    """
    from honeyos.tools.approval import _has_allowlist_shell_operator
...
def apply_proposals(proposals: list[Proposal], indices: list[int]) -> set:
    ...
    import honeyos.tools.approval as approval_module
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  honeyos approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.honeyos/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``honeyos approvals suggest``."""
