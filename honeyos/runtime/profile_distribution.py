"""Profile distributions — shareable, packaged HoneyOS profiles via git.

A distribution is a HoneyOS profile published as a git repository (or
...
* ``honeyos profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``honeyos skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``honeyos profile``, not a parallel tree):

    honeyos profile install <source> [--name N] [--alias] [--force] [--yes]
    honeyos profile update  <name>  [--force-config] [--yes]
    honeyos profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "honeyos.core.state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".honeyos_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "honeyos-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            honeyos_requires=str(data.get("honeyos_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.honeyos_requires:
            out["honeyos_requires"] = self.honeyos_requires
        if self.author:
...
def check_honeyos_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires HoneyOS {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this HoneyOS distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``honeyos profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a HoneyOS profile distribution."
            )
...
def plan_install(
    ...
    from honeyos.runtime import __version__ as honeyos_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a HoneyOS distribution."
        )

    # Version check up-front so we fail fast
    check_honeyos_requires(manifest.honeyos_requires, honeyos_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.honeyos).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="honeyos_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `honeyos profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`honeyos profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="honeyos_dist_update_") as tmp:
        plan = plan_install(
