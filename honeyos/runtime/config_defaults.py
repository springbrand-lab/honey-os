"""Default configuration data for HoneyOS.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from honeyos.runtime/config.py. Must not import from honeyos.runtime.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["honeyos-cli"],
    # SQLite journal mode used by every HoneyOS database opener. WAL is the
    ...
    "agent": {
        ...
        # the HoneyOS-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps HoneyOS
        ...
        # identity slot (SOUL.md). Empty by default. The HONEYOS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, HoneyOS adds a coding operating brief
        ...
        # ``HONEYOS_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             HONEYOS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force HONEYOS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, HoneyOS auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), HoneyOS sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when HoneyOS
        ...
        #  "/home/user/.honeyos/cache/documents:/output"]
        ...
        # bundled HoneyOS image, which drops to the `honeyos` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, HoneyOS sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and HoneyOS should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``honeyos chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.honeyos/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: honeyos sweeps the checkpoint base at startup
        ...
        # ``honeyos checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .honeyos.md, or .cursorrules before HoneyOS applies
    ...
    # Single-query (``honeyos -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, HoneyOS truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so HoneyOS' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); honeyos =
                                      # HoneyOS' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``honeyos kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``honeyos kanban decompose`` and the kanban
        ...
        # ``honeyos profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `honeyos model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `honeyos` (and `honeyos chat`) launches by default:
        ...
        # REPL and `--tui` (or HONEYOS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `honeyos --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `honeyos -c` muscle memory.  Default off so existing
        # users aren't surprised.  HONEYOS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `honeyos --tui` drops a one-time hint
        ...
        # model — see honeyos.runtime/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to HONEYOS_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/honeyos`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `honeyos pets`. Disabled until
        ...
        "pet": {
            ...
            # get_honeyos_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``HONEYOS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``HONEYOS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``HONEYOS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``password_hash`` with
        # ``python -c "from honeyos.plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"``.
        "basic_auth": {
        ...
        # ``HONEYOS_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``HONEYOS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/honeyos``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; HoneyOS appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.honeyos/cache/piper-voices/
    ...
    # "Hey HoneyOS" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey honeyos",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_honeyos" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.honeyos/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.honeyos/cache/delegation/
    ...
    # response. If not, HoneyOS feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before HoneyOS auto-pauses the goal and
    ...
    "moa": {
        ...
        # <honeyos_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.honeyos/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${HONEYOS_SKILL_DIR} and ${HONEYOS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `honeyos curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `honeyos curator run --consolidate` overrides
        ...
        # normally restored on every `honeyos update`, so pruning them only
        ...
        # skipped), snapshot ~/.honeyos/skills/ into
        # ~/.honeyos/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `honeyos curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for honeyos-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *HoneyOS*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (HONEYOS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace HoneyOS' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.honeyos/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or HONEYOS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `honeyos doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``honeyos.runtime/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow HoneyOS to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $HONEYOS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via HONEYOS_CRON_MAX_PARALLEL env var.
        ...
        # HONEYOS_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `honeyos -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `honeyos` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `honeyos kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     honeyos-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core HoneyOS tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.honeyos/logs/.
    ...
    # update model picker lists without shipping a honeyos-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://honeyos-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `honeyos model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal HONEYOS_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.honeyos/sessions/sessions.json entirely.
        ...
        # the HONEYOS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.honeyos.gateway` that defenses 1-2 don't
        ...
        # ``HONEYOS_GATEWAY_MAX_STARTS`` / ``HONEYOS_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.honeyos/.env,
        ...
        # behavior: files must live under the HoneyOS cache, under
        ...
        # user. Bridged to HONEYOS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the HoneyOS
        # cache (~/.honeyos/cache/{documents,images,audio,video,screenshots})
        ...
        # to HONEYOS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # HONEYOS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to HONEYOS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.honeyos/state.db.
    ...
    "sessions": {
        ...
        # the default of ``honeyos sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.honeyos/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `honeyos sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `honeyos update` prints a one-line notice with
        ...
        # ~/.honeyos/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to HONEYOS_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # HONEYOS_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <HONEYOS_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``honeyos backup``-style zip
        #     of HONEYOS_HOME into <HONEYOS_HOME>/backups/, restorable with
        #     ``honeyos import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `honeyos update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<HONEYOS_HOME>/lsp/bin/`` on first use.
    ...
    # `honeyos tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.honeyos/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.honeyos/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.honeyos/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, HoneyOS may use this encrypted
            ...
            # ~/.honeyos/bin/ on first use.  When False you must install
            ...
            # `honeyos secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.honeyos/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. HoneyOS disables it for our users unless they
        # explicitly opt in here. When false (default), HoneyOS sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; HoneyOS also calls
    ...
    #
    # Configure with `honeyos egress setup`.  Disabled by default — the rest of
    # HoneyOS works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.honeyos/bin/ on
    ...
    # HoneyOS Desktop (Electron app) launch options. These only affect
    # `honeyos desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the HONEYOS_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials HoneyOS mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'honeyos model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the HoneyOS auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey HoneyOS' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use HoneyOS, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @honeyos:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HONEYOS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #honeyos)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: honeyos-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'honeyos-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote HoneyOS API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote HoneyOS API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote HoneyOS API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # HONEYOS_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to HONEYOS_TOOL_PROGRESS_MODE for backward
    ...
    "HONEYOS_EPHEMERAL_SYSTEM_PROMPT": {
