...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local HONEYOS_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from honeyos.core.constants import set_honeyos_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from honeyos.core.constants import reset_honeyos_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``honeyos -z "..."`` / ``-q``), the larger
    ...
    try:
        from honeyos.runtime.config import load_config, DEFAULT_CONFIG
...
def _discover_mcp_tools_without_interactive_oauth() -> None:
    ...
    try:
        from honeyos.tools.mcp_oauth import suppress_interactive_oauth
    except Exception:
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``honeyos dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``honeyos -z``) can construct
