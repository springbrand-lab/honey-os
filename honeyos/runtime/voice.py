...
# lookup, and honeyos-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and honeyos-ink rejects them; collapse
...
from honeyos.tools.voice_mode import (
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when HONEYOS_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("HONEYOS_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def _play_beep(frequency: int, count: int = 1) -> None:
    ...
    if not _beeps_enabled():
        return
    try:
        from honeyos.tools.voice_mode import play_beep
...
def _speak_text_streaming(text: str, stop_event: Optional[threading.Event] = None) -> bool:
    ...
    from honeyos.tools.tts_tool import stream_tts_to_speaker
...
def speak_text(text: str, stop_event: Optional[threading.Event] = None) -> None:
    ...
    try:
        ...
        try:
            ...
            from honeyos.tools.tts_tool import _load_tts_config
        ...
        os.makedirs(os.path.join(tempfile.gettempdir(), "honeyos_voice"), exist_ok=True)
