...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from honeyos.agent.credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        from honeyos.agent import anthropic_adapter as anthropic_mod

        creds = anthropic_mod.run_honeyos_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:honeyos_pkce",
    ...
    if provider == "nous":
        ...
        # <honeyos-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `honeyos --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``honeyos auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``honeyos auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`honeyos auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source HoneyOS reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `honeyos auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `honeyos auth` is called bare."""
    ...
    try:
        from honeyos.agent.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region
