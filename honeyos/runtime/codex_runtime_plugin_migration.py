"""Migrate HoneyOS' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads HoneyOS' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  HoneyOS mcp_servers.<n>.command/args/env  → codex stdio transport
  HoneyOS mcp_servers.<n>.url/headers       → codex streamable_http transport
  HoneyOS mcp_servers.<n>.timeout           → codex tool_timeout_sec
  HoneyOS mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  HoneyOS-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. HoneyOS' own
...
MIGRATION_MARKER = (
    "# managed by honeyos-agent — `honeyos codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end honeyos-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in HoneyOS config.")
...
# HoneyOS keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # HoneyOS' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, honeyos_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one HoneyOS MCP server config to the codex inline-table dict
    ...
    if not isinstance(honeyos_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(honeyos_cfg.get("command"))
    has_url = bool(honeyos_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (HONEYOS_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by HoneyOS)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert HoneyOS' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before HoneyOS' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior HoneyOS
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    """
    try:
        from honeyos.agent.transports.codex_app_server import CodexAppServerClient
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="honeyos-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a HONEYOS_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed honeyos-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.honeyos`` symlink that happens to live under ``/private/var/folders``
...
def _build_honeyos_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches HoneyOS' own
    ...
    so a honeyos installed under /opt/, /usr/local/, or a venv all work.
    HONEYOS_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # HONEYOS_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_honeyos_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever HONEYOS_HOME its launcher sets
    ...
    # — that would override the launcher's HONEYOS_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("HONEYOS_HOME", tmp_path) would
    ...
    honeyos_home = os.environ.get("HONEYOS_HOME") or ""
    if honeyos_home and _looks_like_test_tempdir(honeyos_home):
        honeyos_home = ""
    if honeyos_home:
        env["HONEYOS_HOME"] = honeyos_home
    # PYTHONPATH passes through so a worktree-launched honeyos finds the
    ...
    env["HONEYOS_QUIET"] = "1"
    env["HONEYOS_REDACT_SECRETS"] = env.get("HONEYOS_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.honeyos_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_honeyos_tools: bool = True,
) -> MigrationReport:
    """Translate HoneyOS mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        honeyos_config: full ~/.honeyos/config.yaml dict
        ...
        expose_honeyos_tools: when True (default), register HoneyOS' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into HoneyOS for tools
    ...
    honeyos_servers = (honeyos_config or {}).get("mcp_servers") or {}
    if not isinstance(honeyos_servers, dict):
        report.errors.append(
            "mcp_servers in HoneyOS config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in honeyos_servers.items():
    ...
    # Inject HoneyOS' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into HoneyOS for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/honeyos_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_honeyos_tools:
        translated["honeyos-tools"] = _build_honeyos_tools_mcp_entry()
        if "honeyos-tools" not in report.migrated:
            report.migrated.append("honeyos-tools")
