"""Helpers for loading HoneyOS .env files consistently across entrypoints."""
...
# load_honeyos_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_honeyos_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_honeyos_dotenv().
# Used by setup / `honeyos model` flows to label detected credentials so
...
# HONEYOS_HOME paths we've already pulled external secrets for during this
# process.  ``load_honeyos_dotenv()`` is called at module-import time from
# several hot modules (cli.py, honeyos.runtime/main.py, run_agent.py,
...
def _known_honeyos_env_keys() -> set[str]:
    """Return the combined set of known HoneyOS env-var keys.
    ...
    ``load_honeyos_dotenv()`` calls.
    """
    from honeyos.runtime.config import _EXTRA_ENV_KEYS
    from honeyos.runtime.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent HoneyOS process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed HoneyOS keys absent from ``.env``.
    ...
    parent HoneyOS process injects and that silently change *which provider
    ...
    ``tests/honeyos.runtime/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_honeyos_dotenv()`` call.  Returns
...
def get_secret_source_values(
    honeyos_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``honeyos_home``."""
    home_key = str(Path(honeyos_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(honeyos_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        from honeyos.agent.secret_scope import _is_global_env, load_env_file
        from honeyos.agent.secret_sources.registry import apply_all
        ...
        # Mirror load_honeyos_dotenv()'s .op.env bootstrap: the 1Password
        ...
        local_env["HONEYOS_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which HONEYOS_HOME paths have already had external secrets applied.
...
def format_secret_source_suffix(env_var: str) -> str:
    ...
    try:
        from honeyos.agent.secret_sources.registry import get_source
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `honeyos setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``honeyos.runtime.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from honeyos.runtime.config import _sanitize_env_lines
    except ImportError:
...
def load_honeyos_dotenv(
    *,
    honeyos_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load HoneyOS environment files with user config taking precedence.

    Behavior:
    - `~/.honeyos/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(honeyos_home or os.getenv("HONEYOS_HOME", Path.home() / ".honeyos"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited HoneyOS keys
    ...
    #   EnvironmentFile=-/path/to/.honeyos/.op.env
    ...
    # TERMINAL_ENV=docker left in ~/.honeyos/.env (e.g. written by an older
    # `honeyos setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_honeyos_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``honeyos.runtime.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process HONEYOS_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_honeyos_dotenv(honeyos_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_honeyos_home().resolve():
            return
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of HONEYOS_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of HoneyOS reads
    ...
    owns the once-per-HONEYOS_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_honeyos_dotenv()`` runs at import
    time from several hot modules (cli.py, honeyos.runtime/main.py,
    ...
    if not cfg:
        ...
        # on its next load_honeyos_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_honeyos_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `honeyos model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `honeyos` startup, so populating the
    # shared cache here lets main.py's early bridge and honeyos.core.logging reuse
    ...
    if home_path == _process_honeyos_home():
        try:
...
def _process_honeyos_home() -> Path:
    """The HONEYOS_HOME the shared config cache is keyed to."""
    try:
