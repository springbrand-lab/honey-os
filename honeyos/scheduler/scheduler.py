...
Uses a file-based lock (~/.honeyos/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `honeyos update` reloads
# the module) fail with ModuleNotFoundError for honeyos.core.time et al.
...
from honeyos.core.time import now as _honeyos_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy honeyos.runtime import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from honeyos.runtime.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``honeyos tools`` config for the ``cron`` platform.
    ...
    try:
        from honeyos.runtime.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
from honeyos.scheduler.jobs import get_due_jobs, mark_job_run, save_job_output, advance_next_runs, claim_dispatch, heartbeat_run_claim
from honeyos.scheduler.executions import create_execution, finish_execution, mark_execution_running
...
def _is_cron_silence_response(text: str) -> bool:
    ...
    """
    from honeyos.gateway.response_filters import is_autonomous_silence_response
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``honeyos update`` / ``honeyos gateway stop`` / systemd restart, or an
...
_honeyos_home: Path | None = None


def _get_honeyos_home() -> Path:
    """Resolve HoneyOS home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active HONEYOS_HOME at call time
    ...
    """
    return _honeyos_home or get_honeyos_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    honeyos_home = _get_honeyos_home()
    lock_dir = honeyos_home / "cron"
...
def _maybe_mirror_cron_delivery(
    ...
    *,
    ...
    if not text:
        return
    try:
        from honeyos.gateway.mirror import mirror_to_session
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"HoneyOS — {task_name}"
    try:
        from honeyos.agent.async_utils import safe_schedule_threadsafe
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        from honeyos.tools.send_message_tool import _parse_target_ref
        ...
        try:
            from honeyos.gateway.channel_directory import resolve_channel_name
...
def _send_media_via_adapter(
    ...
    from honeyos.gateway.platforms.base import BasePlatformAdapter, should_send_media_as_audio
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    from honeyos.tools.send_message_tool import _send_to_platform
    from honeyos.gateway.config import load_gateway_config, Platform
    ...
    from honeyos.gateway.platforms.base import BasePlatformAdapter
    ...
    for target in targets:
        ...
        from honeyos.gateway.delivery import resolve_delivery_transport
        ...
        if live_adapter_ready:
            ...
            from honeyos.gateway.delivery import (
                DeliveryRouter,
                DeliveryTarget,
                _looks_like_int,
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("HONEYOS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid HONEYOS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within HONEYOS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other HoneyOS-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against HONEYOS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_honeyos_home() / "scripts"
    ...
    # escape — scripts MUST reside within HONEYOS_HOME/scripts/.
    try:
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    if context_from:
        from honeyos.scheduler.jobs import get_cron_output_dir
    ...
    from honeyos.tools.skills_tool import skill_view
    from honeyos.tools.skill_usage import bump_use
    from honeyos.agent.skill_bundles import build_bundle_invocation_message, resolve_bundle_command_key
    from honeyos.agent.skill_utils import normalize_skill_lookup_name
...
def _scan_assembled_cron_prompt(
    ...
    *,
    ...
    """
    from honeyos.tools.cronjob_tools import _scan_cron_prompt, _scan_cron_skill_assembled
...
def _guard_job_credential_exfil(job: dict) -> None:
    ...
    """
    try:
        from honeyos.tools.cronjob_tools import _validate_cron_base_url
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _honeyos_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    #
    # Bounded with its own timeout (separate from HONEYOS_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("HONEYOS_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid HONEYOS_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_honeyos_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_honeyos_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    from honeyos.gateway.session_context import set_session_vars, clear_session_vars, _VAR_MAP

    # Cron execution is an internal scheduler context, not a live inbound
    # gateway message. Do not seed HONEYOS_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads HONEYOS_SESSION_PLATFORM
    #     and HONEYOS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via HONEYOS_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     HONEYOS_SESSION_PLATFORM == "telegram".
    ...
    #     HONEYOS_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read HONEYOS_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the HONEYOS_CRON_AUTO_DELIVER_* vars set
    # below, so clearing HONEYOS_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # HONEYOS_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ HONEYOS_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "HONEYOS_CRON_AUTO_DELIVER_PLATFORM",
        "HONEYOS_CRON_AUTO_DELIVER_CHAT_ID",
        "HONEYOS_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["HONEYOS_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `honeyos chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with HONEYOS_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $HONEYOS_KANBAN_TASK -- letting
        ...
        # load_honeyos_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this HONEYOS_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_honeyos_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from honeyos.runtime.env_loader import (
            load_honeyos_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_honeyos_dotenv(honeyos_home=_get_honeyos_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["HONEYOS_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["HONEYOS_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["HONEYOS_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > HONEYOS_MODEL env > config.yaml ``model:``
        ...
        model = job.get("model") or os.getenv("HONEYOS_MODEL") or ""
        ...
        try:
            ...
            _cfg_path = str(_get_honeyos_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"HONEYOS_MODEL={os.getenv('HONEYOS_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `honeyos model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from honeyos.core.constants import apply_ipv4_preference
        ...
        from honeyos.core.constants import resolve_reasoning_config
        ...
        prefill_file = (
            os.getenv("HONEYOS_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_honeyos_home() / pfpath
            if pfpath.exists():
                try:
        ...
        from honeyos.runtime.runtime_provider import (
        ...
        try:
            # Do not inject HONEYOS_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # HONEYOS_HOME. When a workdir is configured, also inject project
        ...
        # override via HONEYOS_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("HONEYOS_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid HONEYOS_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_honeyos_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_honeyos_now().strftime('%b %d %H:%M')}"
...
def _teardown_cron_agent(agent, job_id: str) -> None:
    ...
    try:
        from honeyos.agent.auxiliary_client import cleanup_stale_async_clients
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_honeyos_home())
        )
    ...
    *,
    ...
    try:
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _honeyos_now().strftime('%H:%M:%S'))
            try:
                from honeyos.tools.mcp_tool import _kill_orphaned_mcp_children
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _honeyos_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set HONEYOS_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("HONEYOS_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid HONEYOS_CRON_MAX_PARALLEL value; defaulting to unbounded")
