...
Jobs are stored in ~/.honeyos/cron/jobs.json
Output is saved to ~/.honeyos/cron/output/{job_id}/{timestamp}.md
"""
...
from honeyos.utils import atomic_replace, atomic_write_text
...
# store under its own HONEYOS_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same HONEYOS_HOME — so a job authored in profile
# `coder` lives in `~/.honeyos/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_honeyos_home()` (the active profile home), NOT `get_default_honeyos_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever HONEYOS_HOME the
...
# `_get_honeyos_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
HONEYOS_DIR = get_honeyos_home().resolve()
...
CRON_DIR = HONEYOS_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``honeyos cron status`` process share it
...
# threshold in `honeyos cron status` (honeyos.runtime/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_honeyos_home()
       (context-local override, then the HONEYOS_HOME env var) — so a test
       or embedder that re-points HONEYOS_HOME after this module was
...
# the cron inactivity timeout is disabled (HONEYOS_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. HONEYOS_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``HONEYOS_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("HONEYOS_CRON_TIMEOUT", "").strip()
...
def _job_running_in_this_process(job_id: str) -> bool:
    ...
    """
    try:
        from honeyos.scheduler.scheduler import get_running_job_ids
...
def _jobs_lock():
    ...
    and standalone ``honeyos`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec honeyos honeyos cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED HoneyOS timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `honeyos.core.time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                honeyos_tz = _honeyos_now().tzinfo
                dt = dt.replace(tzinfo=honeyos_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _honeyos_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in HoneyOS configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured HoneyOS timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _honeyos_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If HoneyOS/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall honeyos-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `honeyos cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``honeyos cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `honeyos cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``honeyos cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``honeyos cron
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _honeyos_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    """
    try:
        from honeyos.runtime.config import _expand_env_vars, read_user_config_raw

        cfg_path = get_honeyos_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.honeyos/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `honeyos cron create`.
    from honeyos.scheduler.lifecycle_guard import check_gateway_lifecycle
...
def list_jobs(include_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        from honeyos.scheduler.executions import latest_executions
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _honeyos_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _honeyos_now().isoformat(),
        },
    )
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_honeyos_now().isoformat()}\n\n"
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_owner:
                return False
            claim["at"] = _honeyos_now().isoformat()
            save_jobs(jobs)
            return True
    return False
...
def _machine_id() -> str:
    ...
    Uses ``HONEYOS_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("HONEYOS_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from HONEYOS_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one HONEYOS_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from HONEYOS_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`honeyos.runtime.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``honeyos.runtime.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _honeyos_now().strftime("%Y-%m-%d_%H-%M-%S")
