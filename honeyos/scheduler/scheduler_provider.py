...
class CronScheduler(ABC):
    ...
    def recover_interrupted(self) -> int:
        ...
        from honeyos.scheduler.executions import recover_interrupted_executions
    ...
    def fire_due(self, job_id: str, *, adapters: Any = None, loop: Any = None) -> bool:
        ...
        """
        from honeyos.scheduler.jobs import claim_job_for_fire, get_job
        from honeyos.scheduler.executions import create_execution
...
def resolve_cron_scheduler() -> "CronScheduler":
    ...
    try:
        from honeyos.plugins.cron_providers import load_cron_scheduler
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        import logging
        from honeyos.scheduler.scheduler import tick as cron_tick
        ...
        # only the process-global HONEYOS_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `honeyos cron
        ...
        # Heartbeat once before the first sleep so `honeyos cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `honeyos cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_honeyos_home_override()`` + ``use_cron_store()``
        ...
        for entry in profile_homes:
            ...
            home_token = set_honeyos_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `honeyos cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
