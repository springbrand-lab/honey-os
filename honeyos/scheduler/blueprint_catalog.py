...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``honeyos://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``honeyos://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"honeyos://blueprint/{quote(blueprint.key)}{qs}"
