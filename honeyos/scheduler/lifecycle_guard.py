...
``honeyos gateway restart`` (or ``launchctl kickstart ai.honeyos.gateway``
or ``systemctl restart honeyos-gateway``).  When the cron fires, the
...
``honeyos cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``honeyos gateway``, ``launchctl ... honeyos-gateway``,
``systemctl ... honeyos-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_HONEYOS_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``honeyos gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `honeyos gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:honeyos\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a honeyos-gateway label. macOS launchd
    # labels look like `ai.honeyos.gateway` / `honeyos-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated honeyos services (e.g.
    # `launchctl unload ai.honeyos.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.honeyos.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a honeyos-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bhoneyos[.\-]?gateway)"
    # Branch D: pkill / kill targeting the honeyos gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.honeyos.gateway-... \` / `  -- ...`,
...
# for `systemctl restart honeyos-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart honeyos-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.honeyos.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart honeyos-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        grep -c 'systemctl restart honeyos-gateway' /var/log/syslog
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart honeyos-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".honeyos"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<HONEYOS_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.honeyos/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_honeyos_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_honeyos_home() falls back to Path.home(), which raises when
        # neither HONEYOS_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "honeyos gateway restart"
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `honeyos gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `honeyos gateway restart` from a shell outside "
