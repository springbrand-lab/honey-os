...
from honeyos.gateway.config import GatewayConfig, Platform, PlatformConfig
from honeyos.gateway.platforms.base import MessageEvent
from honeyos.gateway.session import SessionEntry, SessionSource, build_session_key
...
@pytest.mark.asyncio
@patch("honeyos.companion.continuity.record_reset_handoff")
async def test_new_records_old_to_new_handoff_without_blocking_reset(
    ...
    runner = _runner()
    runner._schedule_honeyos_memory_distillation = MagicMock(return_value=True)
    ...
    runner._schedule_honeyos_memory_distillation.assert_called_once()
    distill_call = runner._schedule_honeyos_memory_distillation.call_args.kwargs
...
async def test_gateway_tracks_distillation_as_non_blocking_background_work(
    ...
    monkeypatch.setattr(
        "honeyos.companion.distillation.active_honeyos_distiller", lambda: FakeDistiller()
    )
    runner = _runner()

    scheduled = runner._schedule_honeyos_memory_distillation(
