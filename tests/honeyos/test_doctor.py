...
def test_doctor_reports_companion_contract_without_secrets(tmp_path):
    ...
    assert report.by_name("sandbox-backend").ok
...
def test_doctor_fails_open_host_terminal_backend(tmp_path):
    home = _home(tmp_path)
    ...
    assert report.by_name("sandbox-backend").ok is False
    assert "local" in report.by_name("sandbox-backend").detail
